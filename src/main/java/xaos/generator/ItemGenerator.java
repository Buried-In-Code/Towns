...
public class ItemGenerator extends Generator {
  ...
  public static final String XML_FILE = "gen_items.xml"; // $NON-NLS-1$
  ...
  public static void generateItems(Cell[][][] cells, String sCampaignID, String sMissionID) {
    ...
    for (int i = 0; i < generator.getList().size(); i++) {
      ...
      if (item.getName() == ItemGeneratorItem.ITEM_ADD) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemGenerator.0")
                + item.getName()
                + Messages.getString("ItemGenerator.1")
                + XML_FILE
                + "]",
            "ItemGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      }
    }
  }
  ...
  private static void addItem(GeneratorItem item, Cell[][][] cells) {
    ...
    for (int i = 0; i < item.getList().size(); i++) {
      if (item.getList().get(i).getName().equalsIgnoreCase(ItemGeneratorItem.ITEM_ADD_ITEM)) {
        entity = item.getList().get(i).getValue();
          ...
          .get(i)
          .getName()
          .equalsIgnoreCase(ItemGeneratorItem.ITEM_ADD_QTTY)) {
        qtty = Utils.launchDice(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(ItemGeneratorItem.ITEM_ADD_POINTX)) {
        pointx = item.getList().get(i).getValue();
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(ItemGeneratorItem.ITEM_ADD_POINTY)) {
        pointy = item.getList().get(i).getValue();
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(ItemGeneratorItem.ITEM_ADD_LEVEL)) {
    ...
    if (entity == null || entity.isEmpty()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("ItemGenerator.4"),
          "ItemGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    if (imi == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("ItemGenerator.6") + entity + "]",
          "ItemGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      return;
    }
    ...
    while (qtty > 0) {
      ...
      if (x < 0 || x >= World.MAP_WIDTH) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemGenerator.2") + imi.getIniHeader() + "]",
            "ItemGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        tryes = 200;
        qtty--;
        continue;
      ...
      if (y < 0 || y >= World.MAP_WIDTH) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemGenerator.7") + imi.getIniHeader() + "]",
            "ItemGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        tryes = 200;
        qtty--;
        continue;
      ...
      if (z < 0 || z >= World.MAP_DEPTH) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemGenerator.10") + imi.getIniHeader() + "]",
            "ItemGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        tryes = 200;
        qtty--;
        continue;
