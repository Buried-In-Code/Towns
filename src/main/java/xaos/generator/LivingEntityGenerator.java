...
public class LivingEntityGenerator extends Generator {
  ...
  public static final String XML_FILE = "gen_livingentities.xml"; // $NON-NLS-1$

  /** Lee del gen_livingentities.xml */
  public static void generateLivingEntities(
      Cell[][][] cells,
      String sCampaignID,
      String sMissionID) {
    ...
    for (int i = 0; i < generator.getList().size(); i++) {
      ...
      if (item.getName() == LivingEntityGeneratorItem.ITEM_ADD) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("LivingEntityGenerator.0")
                + item.getName()
                + Messages.getString("ItemGenerator.1")
                + XML_FILE
                + "]",
            "LivingEntityGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      }
    }
  }
  ...
  private static void addLivingEntity(GeneratorItem item, Cell[][][] cells) {
    ...
    for (int i = 0; i < item.getList().size(); i++) {
      if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(LivingEntityGeneratorItem.ITEM_ADD_ENTITY)) {
        entity = item.getList().get(i).getValue();
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(LivingEntityGeneratorItem.ITEM_ADD_QTTY)) {
        qtty = Utils.launchDice(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(LivingEntityGeneratorItem.ITEM_ADD_POINTX)) {
        pointx = item.getList().get(i).getValue();
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(LivingEntityGeneratorItem.ITEM_ADD_POINTY)) {
        pointy = item.getList().get(i).getValue();
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(LivingEntityGeneratorItem.ITEM_ADD_LEVEL)) {
    ...
    if (entity == null || entity.isEmpty()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("LivingEntityGenerator.4"),
          "LivingEntityGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    if (lemi == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("LivingEntityGenerator.5") + " [" + entity + "]",
          "LivingEntityGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      return;
    }
    ...
    while (qtty > 0) {
      ...
      if (x < 0
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("LivingEntityGenerator.2") + lemi.getIniHeader() + "]",
            "LivingEntityGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        tryes = 200;
        qtty--;
        continue;
      }
      ...
      if (bOK
          && (LivingEntity.isCellAllowed(x, y, z) || cells[x][y][z].isCave())
          && checkHabitatHeight(lemi, x, y, z)
          && (lemi.getHabitat() == null
              || z == (World.MAP_DEPTH - 1)
              || lemi.getHabitat().size() == 0
              || lemi.getHabitat().contains(cells[x][y][z + 1].getTerrain().getTerrainID()))) {
        World.addNewLiving(
            lemi.getIniHeader(),
            lemi.getType(),
            cells[x][y][z].isDiscovered(),
            x,
            y,
            z);
