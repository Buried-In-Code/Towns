...
public class MapGenerator extends Generator {
  ...
  public static final String XML_FILE = "gen_map.xml"; // $NON-NLS-1$
  ...
  public static Cell[][][] generateMap(String sCampaignID, String sMissionID) {
    ...
    // Leemos el gen_map.xml (si estß en una misi¾n se carga de otro sitio)
    ...
    if (item == null || !item.getName().equalsIgnoreCase(MapGeneratorItem.ITEM_INIT)) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.2"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      Game.exit();
    }
    ...
    for (int i = 0; i < item.getList().size(); i++) {
      if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_NUM_LEVELES_UNDERGROUND)) {
        iNumLevelsUnderground = (short) Utils.launchDice(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_NUM_LEVELS_OUTSIDE)) {
        iNumLevelsOutside = (short) Utils.launchDice(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_MAINTERAIN)) {
        MAINTERRAIN = item.getList().get(i).getValue();
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_NUM_CITIZENS)) {
        NUM_CITIZENS = Utils.launchDice(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_STARTING_POINT)) {
        String sX = null;
        String sY = null;
        ...
        if (sX != null && sY != null) {
          ...
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_STARTING_LEVEL)) {
        STARTING_LEVEL = (short) Utils.launchDice(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_ALLOW_BURY)) {
        ALLOW_BURY = Boolean.parseBoolean(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_STARTING_EVENTS)) {
        STARTING_EVENTS = Utils.getArray(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_STARTING_EVENTS_PCT)) {
        alStartingEventsPCT = Utils.getArray(item.getList().get(i).getValue());
      } else if (item.getList()
          .get(i)
          .getName()
          .equalsIgnoreCase(MapGeneratorItem.ITEM_INIT_NUM_GODS)) {
    ...
    if (iNumLevelsUnderground <= 0) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.8"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      Game.exit();
    }
    if (iNumLevelsOutside <= 0) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.11"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      Game.exit();
    }
    if (MAINTERRAIN == null || MAINTERRAIN.trim().isEmpty()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.4"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      Game.exit();
    }
    if (NUM_CITIZENS < 1) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.12") + NUM_CITIZENS + "]",
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
    ...
    if (STARTING_EVENTS != null && STARTING_EVENTS.size() > 0) {
      ...
      if (alStartingEventsPCT == null || alStartingEventsPCT.size() != STARTING_EVENTS.size()) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("MapGenerator.14"),
            "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
        Game.exit();
      }
      ...
      for (int i = 0; i < STARTING_EVENTS.size(); i++) {
        if (EventManager.getItem(STARTING_EVENTS.get(i)) == null) {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("MapGenerator.15") + " [" + STARTING_EVENTS.get(i) + "]",
              "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          Game.exit();
        }
      }
    ...
    if (World.MAP_DEPTH > World.MAX_DEPTH) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.9") + World.MAX_DEPTH + "]",
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
    ...
    if (STARTING_LEVEL < 0 || STARTING_LEVEL >= World.MAP_DEPTH) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.16") + STARTING_LEVEL + "]",
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
    ...
    if (alBaseTerrains.size() <= 0) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.6") + MAINTERRAIN + "]",
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
    ...
    // Mapa temporal, cada celda contiene una lista, el primer elemento es el terreno, los
    // siguientes son tipos especiales (ej: WATER, LAVA, ...)
    ...
    for (int i = 0; i < generator.getList().size(); i++) {
      ...
      if (item.getName() == MapGeneratorItem.ITEM_SEED) {
        ...
        Game.getPanelMainMenu()
            .setLoadingText(Messages.getString("MapGenerator.44")); // $NON-NLS-1$
        ...
        Game.getPanelMainMenu()
            .setLoadingText(Messages.getString("MapGenerator.45")); // $NON-NLS-1$
        ...
        Game.getPanelMainMenu()
            .setLoadingText(Messages.getString("MapGenerator.46")); // $NON-NLS-1$
        ...
        Game.getPanelMainMenu()
            .setLoadingText(Messages.getString("MapGenerator.47")); // $NON-NLS-1$
      ...
      for (int x = 0; x < World.MAP_WIDTH; x++) {
        for (int y = 0; y < World.MAP_HEIGHT; y++) {
          ...
          if (tmi == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("MapGenerator.5") + asMap[x][y][z].getTerrainID() + "]",
                "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
          if (!asMap[x][y][z].hasSpecial()) {
            ...
            cells[x][y][z] = new Cell(
                TerrainManager.getItemByID(TerrainManagerItem.TERRAIN_AIR_ID).getTerrain());
            // Tipos especiales
            if (asMap[x][y][z].hasSpecial()) {
              if (asMap[x][y][z].getSpecial() == MapGeneratorItem.SPECIAL_INT_WATER_1) {
                ...
                Log.log(
                    Log.LEVEL_ERROR,
                    Messages.getString("MapGenerator.10") + asMap[x][y][z].getSpecial() + "]",
                    "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
              }
    ...
    for (int x = 0; x < World.MAP_WIDTH; x++) {
      for (int y = 0; y < World.MAP_HEIGHT; y++) {
        ...
        cells[x][y][0]
            .getTerrain()
            .setTerrainTileID(
                TerrainManagerItem.TERRAIN_AIR_ID * TerrainManager.SLOPES_INIHEADER.length);
    ...
    // Cambiamos los grßficos (al final pq usa el ismined de Cell)
    // Terrain.changeSlopes (cells);
    if (TownsProperties.DEBUG_MODE) {
      Log.log(
          Log.LEVEL_DEBUG,
          Messages.getString("MapGenerator.51"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
    }
  ...
  private static boolean airAround(Cell[][][] cells, int x, int y, int z) {
    ...
    // Arriba
    if (z > 0
        && cells[x][y][z - 1].getTerrain().getTerrainID() == TerrainManagerItem.TERRAIN_AIR_ID) {
      return true;
    }
        ...
        if (i != x || j != y) {
          if ((i != 0 && j == 0) || (i == 0 && j != 0)) {
            if (Utils.isInsideMap(i + x, j + y, z)) {
              if (cells[i + x][j + y][z].getTerrain()
                  .getTerrainID() == TerrainManagerItem.TERRAIN_AIR_ID) {
                return true;
              }
            }
          }
   ...
   * @return
   */
  private static boolean checkNeighbors(
      MapGeneratorItem[][][] asMap,
      int x,
      int y,
      int z,
      int iTerrainID,
   ...
   * @return
   */
  private static boolean checkNeighborsSpecial(
      MapGeneratorItem[][][] asMap,
      int x,
      int y,
      int z,
      ...
      for (int j = -radius; j <= radius; j++) {
        for (int n = -radius; n <= radius; n++) {
          if (i != 0 || j != 0 || n != 0) {
            if (Utils.isInsideMap(x + i, y + j, n + z)) {
              // Miramos si tiene el terrain
              if (asMap[x + i][y + j][n + z].hasSpecial()
                  && asMap[x + i][y + j][n + z].getSpecial() == iSpecialTerrain) {
                return true;
              }
            }
          }
  ...
  private static void generateBezier(MapGeneratorItem[][][] asMap, GeneratorItem item) {
    ...
    if (bd.type == null || bd.type.trim().length() == 0) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.18"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    if (bd.level == -1) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.20"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    if (bd.point1xmin == -1
        ...
        || bd.point2ymin == -1
        || bd.point2ymax == -1
        || bd.controlpoint1xmin == -1
        || bd.controlpoint1xmax == -1
        || bd.controlpoint1ymin == -1
        || bd.controlpoint1ymax == -1
        || bd.controlpoint2xmin == -1
        || bd.controlpoint2xmax == -1
        || bd.controlpoint2ymin == -1
        || bd.controlpoint2ymax == -1) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.26"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    while (!bBezierPintado) {
      Point pointPuntoInicial = new Point(
          Utils.getRandomBetween(bd.point1xmin, bd.point1xmax),
          Utils.getRandomBetween(bd.point1ymin, bd.point1ymax));
      Point pointPuntoControlA = new Point(
          Utils.getRandomBetween(bd.controlpoint1xmin, bd.controlpoint1xmax),
          Utils.getRandomBetween(bd.controlpoint1ymin, bd.controlpoint1ymax));
      Point pointPuntoControlB = new Point(
          Utils.getRandomBetween(bd.controlpoint2xmin, bd.controlpoint2xmax),
          Utils.getRandomBetween(bd.controlpoint2ymin, bd.controlpoint2ymax));
      Point pointPuntoFinal = new Point(
          Utils.getRandomBetween(bd.point2xmin, bd.point2xmax),
          Utils.getRandomBetween(bd.point2ymin, bd.point2ymax));
      ...
      for (int i = 0; i < ((World.MAP_WIDTH + World.MAP_HEIGHT) * 10); i++) {
        pointPuntoBezier = Utils.getBezierPoint(
            pointPuntoInicial,
            pointPuntoFinal,
            pointPuntoControlA,
            pointPuntoControlB,
            (((double) i) / (double) ((World.MAP_WIDTH + World.MAP_HEIGHT) * 10)));
        for (int x = 0; x < iGrosor; x++) {
          for (int y = 0; y < iGrosor; y++) {
            if (Utils.isValidPoint(
                pointPuntoBezier.x + x,
                pointPuntoBezier.y + y,
                World.MAP_WIDTH,
                World.MAP_HEIGHT)) {
              // Metemos el tipo (o special type)
              if (iSpecialType != MapGeneratorItem.SPECIAL_INT_NONE) {
                for (int d = 0; d < bd.depth; d++) {
                  asMap[pointPuntoBezier.x + x][pointPuntoBezier.y + y][bd.level + d].setSpecial(
                      iSpecialType);
                }
              } else {
                for (int d = 0; d < bd.depth; d++) {
                  asMap[pointPuntoBezier.x + x][pointPuntoBezier.y + y][bd.level + d].setTerrainID(
                      iTerrainID);
                  asMap[pointPuntoBezier.x + x][pointPuntoBezier.y + y][bd.level + d].setSpecial(
                      MapGeneratorItem.SPECIAL_INT_NONE);
                }
              }
              bBezierPintado = true;
            }
  ...
  private static void generateChange(MapGeneratorItem[][][] asMap, GeneratorItem item) {
    ...
    if (cd.destination == null
        || cd.destination.length() == 0
        || (cd.iHeightMin > cd.iHeightMax && cd.iHeightMax != -1)) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.3"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    if (cd.source != null) {
      ...
      if (tmi != null) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("MapGenerator.5") + cd.source + "]",
            "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        return;
      }
    }
    ...
    if (iSpecialType == MapGeneratorItem.SPECIAL_INT_NONE && cd.terrain != null) {
      ...
      if (tmi == null) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("MapGenerator.5") + cd.terrain + "]",
            "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
    ...
    for (int z = 0; z < World.MAP_DEPTH; z++) {
      // Miramos heights
      if ((z >= cd.iHeightMin || cd.iHeightMin == -1)
          && (z <= cd.iHeightMax || cd.iHeightMax == -1)) {
        for (int x = 0; x < World.MAP_WIDTH; x++) {
          for (int y = 0; y < World.MAP_HEIGHT; y++) {
            if (cd.source == null
                || (asMap[x][y][z].getTerrainID() == iSourceTerrainID
                    && !asMap[x][y][z].hasSpecial())) {
              if (iSourceTerrainID != TerrainManagerItem.TERRAIN_AIR_ID) {
                ...
                if (Utils.getRandomBetween(1, 100) <= cd.pct) {
                  ...
                  if (iSpecialType == MapGeneratorItem.SPECIAL_INT_NONE) {
                    if (cd.terrain == null
                        || checkNeighbors(asMap, x, y, z, iTerrainTerrainID, cd.radius)) {
                      abMap[x][y][z] = true;
                    }
                  } else {
          ...
          if (abMap[x][y]) {
            ...
            for (int z = 0; z <= World.MAP_NUM_LEVELS_OUTSIDE; z++) {
              if (asMap[x][y][z].getTerrainID() == TerrainManagerItem.TERRAIN_AIR_ID) {
                ...
                // Tenemos un no-air
                if (bHayAIRs) {
                  // Tiene espacio por encima, asÝ que simplemente cambiamos el terrain por el de
                  // abajo
  ...
  private static void generateSeed(MapGeneratorItem[][][] asMap, GeneratorItem item) {
    ...
    if (sd.type == null || sd.type.trim().length() == 0) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MapGenerator.13"),
          "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
      return;
    }
    ...
    if (iSpecialType == MapGeneratorItem.SPECIAL_INT_NONE) {
      ...
      if (tmi == null) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("MapGenerator.5") + sd.type + "]",
            "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        return;
      }
