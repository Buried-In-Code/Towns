...
public class Generator {
  ...
  public static Generator read(String sFile, Generator generator, boolean bFirst) {
    ...
    try {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Generator.0") + sFile + "]",
          "Generator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
          ...
          if (iIndexExists != -1) {
            if (bFirst) {
              Log.log(
                  ...
                  "Generator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            }
  ...
  private static ArrayList<GeneratorNode> getNodes(Node node) {
    ...
    for (int i = 0, n = node.getChildNodes().getLength(); i < n; i++) {
      ...
      if (child.getNodeType() == Node.ELEMENT_NODE
          ...
          && child.getFirstChild().getNodeValue().trim().length() > 0) {
        alReturn.add(
            new GeneratorNode(child.getNodeName(), child.getChildNodes().item(0).getNodeValue()));
      }
    }
