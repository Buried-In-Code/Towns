...
public class EffectManager {
  ...
  private static void loadItems() {
    ...
    // Cargar de fichero
    loadXMLEffects(
        Towns.getPropertiesString("DATA_FOLDER") + "effects.xml",
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "effects.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
    ...
    // Miramos que los afterEffects, onHitEffects, onRangedHitEffects, effectsImmune,
    // effectsPrerequisite y castEffects existan
      ...
      if (emi.getAfterEffects() != null && emi.getAfterEffects().size() > 0) {
        for (int e = 0; e < emi.getAfterEffects().size(); e++) {
          if (!effectList.containsKey(emi.getAfterEffects().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EffectManager.2") + emi.getAfterEffects().get(e) + "]",
                "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
        }
      }
      ...
      if (emi.getOnHitEffects() != null && emi.getOnHitEffects().size() > 0) {
        for (int e = 0; e < emi.getOnHitEffects().size(); e++) {
          if (!effectList.containsKey(emi.getOnHitEffects().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EffectManager.3") + emi.getOnHitEffects().get(e) + "]",
                "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
        }
      ...
      if (emi.getOnRangedHitEffects() != null && emi.getOnRangedHitEffects().size() > 0) {
        for (int e = 0; e < emi.getOnRangedHitEffects().size(); e++) {
          if (!effectList.containsKey(emi.getOnRangedHitEffects().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EffectManager.4") + emi.getOnRangedHitEffects().get(e) + "]",
                "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
        }
      ...
      if (emi.getEffectsImmune() != null && emi.getEffectsImmune().size() > 0) {
        for (int e = 0; e < emi.getEffectsImmune().size(); e++) {
          if (!effectList.containsKey(emi.getEffectsImmune().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EffectManager.1") + emi.getEffectsImmune().get(e) + "]",
                "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
        }
      ...
      if (emi.getEffectsPrerequisite() != null && emi.getEffectsPrerequisite().size() > 0) {
        for (int e = 0; e < emi.getEffectsPrerequisite().size(); e++) {
          if (!effectList.containsKey(emi.getEffectsPrerequisite().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EffectManager.7")
                    + " ["
                    + emi.getEffectsPrerequisite().get(e)
                    + "]",
                "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      }
      ...
      if (emi.getCastEffects() != null && emi.getCastEffects().size() > 0) {
        for (int e = 0; e < emi.getCastEffects().size(); e++) {
          if (!effectList.containsKey(emi.getCastEffects().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EffectManager.5") + emi.getCastEffects().get(e) + "]",
                "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
        }
  ...
  private static void loadXMLEffects(String sXMLName, boolean bLoadingMain) {
    ...
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo las skills (SkillManagerItem) a la hash
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            effectData.setDamagePCT(
                UtilsXML.getChildValue(node.getChildNodes(), "damagePCT")); // $NON-NLS-1$
            effectData.setDefensePCT(
                UtilsXML.getChildValue(node.getChildNodes(), "defensePCT")); // $NON-NLS-1$
            effectData.setAttackPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "attackPCT")); // $NON-NLS-1$
            effectData.setAttackSpeedPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "attackSpeedPCT")); // $NON-NLS-1$
            effectData.setDOT(UtilsXML.getChildValue(node.getChildNodes(), "DOT")); // $NON-NLS-1$
            effectData.setOnHitPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "onHitPCT")); // $NON-NLS-1$
            effectData.setOnHitEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "onHitEffects")); // $NON-NLS-1$
            effectData.setOnRangedHitPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "onRangedHitPCT")); // $NON-NLS-1$
            effectData.setOnRangedHitEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "onRangedHitEffects")); // $NON-NLS-1$
            effectData.setHealthPointsPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "healthPointsPCT")); // $NON-NLS-1$
            effectData.setLOSPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "LOSPCT")); // $NON-NLS-1$
            effectData.setSpeedPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "speedPCT")); // $NON-NLS-1$
            effectData.setLasts(
                UtilsXML.getChildValue(node.getChildNodes(), "lasts")); // $NON-NLS-1$
            effectData.setAttackAllies(
                UtilsXML.getChildValue(node.getChildNodes(), "attackAllies")); // $NON-NLS-1$
            effectData.setRemoveTarget(
                UtilsXML.getChildValue(node.getChildNodes(), "removeTarget")); // $NON-NLS-1$
            effectData.setFlee(UtilsXML.getChildValue(node.getChildNodes(), "flee")); // $NON-NLS-1$
            effectData.setGraphicChange(
                UtilsXML.getChildValue(node.getChildNodes(), "graphicChange")); // $NON-NLS-1$
            effectData.setRaiseDead(
                UtilsXML.getChildValue(node.getChildNodes(), "raiseDead")); // $NON-NLS-1$
            effectData.setMaxRaised(
                UtilsXML.getChildValue(node.getChildNodes(), "maxRaised")); // $NON-NLS-1$

            // Aftereffects list
            effectData.setAfterEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "afterEffects")); // $NON-NLS-1$

            // Casts
            effectData.setCastEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "castEffects")); // $NON-NLS-1$
            effectData.setCastCooldown(
                UtilsXML.getChildValue(node.getChildNodes(), "castCooldown")); // $NON-NLS-1$
            effectData.setCastTrigger(
                UtilsXML.getChildValue(node.getChildNodes(), "castTrigger")); // $NON-NLS-1$
            effectData.setCastTargets(
                UtilsXML.getChildValue(node.getChildNodes(), "castTargets")); // $NON-NLS-1$

            // Immunity
            effectData.setEffectsImmune(
                UtilsXML.getChildValue(node.getChildNodes(), "effectsImmune")); // $NON-NLS-1$

            // Prerequisite
            effectData.setEffectsPrerequisite(
                UtilsXML.getChildValue(node.getChildNodes(), "effectsPrerequisite")); // $NON-NLS-1$

            // Messaging
            effectData.setMessageWhenGain(
                UtilsXML.getChildValue(node.getChildNodes(), "messageWhenGain")); // $NON-NLS-1$
            effectData.setMessageWhenVanish(
                UtilsXML.getChildValue(node.getChildNodes(), "messageWhenVanish")); // $NON-NLS-1$

            // Happy
            effectData.setHappy(
                UtilsXML.getChildValue(node.getChildNodes(), "happy")); // $NON-NLS-1$
          }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("EffectManager.0") + e.toString() + "]",
          "EffectManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
