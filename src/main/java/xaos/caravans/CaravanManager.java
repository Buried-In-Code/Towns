...
public class CaravanManager {
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "caravans.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
  ...
  private static void loadXMLEffects(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo las caravans a la hash
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            caravanData.setComePCT(
                UtilsXML.getChildValue(node.getChildNodes(), "comePCT")); // $NON-NLS-1$
          } else {
            caravanData = new CaravanManagerItem();
            caravanData.setId(sID);
            caravanData.setZone(
                UtilsXML.getChildValue(node.getChildNodes(), "zone")); // $NON-NLS-1$
            caravanData.setPricePCT(
                UtilsXML.getChildValue(node.getChildNodes(), "pricePCT")); // $NON-NLS-1$
            caravanData.setCoins(
                UtilsXML.getChildValue(node.getChildNodes(), "coins")); // $NON-NLS-1$
            caravanData.setBuysString(
                UtilsXML.getChildValue(node.getChildNodes(), "buys")); // $NON-NLS-1$
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("CaravanManager.0") + e.toString() + "]",
          "CaravanManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
}
