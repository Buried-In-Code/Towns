...
public class CaravanManagerItem {
        ...
        for (int q = 0; q < iQtty; q++) {
          ...
          if (!(item instanceof MilitaryItem)) {
            ...
            for (int current = 0; current < alItems.size(); current++) {
              if (iPrice == alItems.get(current).getPrice()
                  && alItems.get(current).getItem().getIniHeader().equals(itemID)) {
                // Bingo!
  ...
  public void setComePCT(String sComePCT) throws Exception {
    if (sComePCT == null || sComePCT.trim().isEmpty()) {
      ...
      if (bError) {
        throw new Exception(
            Messages.getString("CaravanManagerItem.0")
                + sComePCT
                + "]"); //$NON-NLS-1$ //$NON-NLS-2$
      }
    }
  ...
  public void setZone(String zone) throws Exception {
    ...
    if (ZoneManager.getItem(zone) == null) {
      throw new Exception(
          Messages.getString("CaravanManagerItem.2") + zone + "]"); // $NON-NLS-1$ //$NON-NLS-2$
    }
