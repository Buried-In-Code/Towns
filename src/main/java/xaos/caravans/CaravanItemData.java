...
public class CaravanItemData {
  ...
  public void setId(String id) throws Exception {
    if (id != null) {
      if (ItemManager.getItem(id) == null) {
        throw new Exception(
            Messages.getString("CaravanItemData.1") + id + "]"); // $NON-NLS-1$ //$NON-NLS-2$
      }
    }
  ...
  public void setPCT(String sPCT) throws Exception {
    try {
      ...
      throw new Exception(
          Messages.getString("CaravanItemData.0") + sPCT + "]"); // $NON-NLS-1$ //$NON-NLS-2$
    }
  }
  ...
  public void setType(String type) throws Exception {
    if (type != null) {
      if (ItemManager.getRandomItemByType(type) == null) {
        throw new Exception(
            Messages.getString("CaravanItemData.2") + type + "]"); // $NON-NLS-1$ //$NON-NLS-2$
      }
    }
    this.type = type;
  }
