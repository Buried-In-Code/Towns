...
public class TerrainManager {
  ...
  public static final String[] SLOPES_INIHEADER = {
      "",
      "_N",
      "_S",
      "_NS",
    ...
    // //$NON-NLS-19$ //$NON-NLS-20$
    // public final static String[] SLOPES_INIHEADER = { "", "_N", "_S", "_NS", "_E", "_NE", "_SE",
    // "_W", "_NW", "_SW", "_EW", "_NSE", "_NSW", "_NEW", "_SEW", "_NSEW"}; //$NON-NLS-1$
    // //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    // //$NON-NLS-8$ //$NON-NLS-9$ //$NON-NLS-10$ //$NON-NLS-11$ //$NON-NLS-12$ //$NON-NLS-13$
    // //$NON-NLS-14$ //$NON-NLS-15$ //$NON-NLS-16$
  ...
  private static void loadItems() {
    ...
    loadXMLTerrain(
        Towns.getPropertiesString("DATA_FOLDER") + "terrain.xml",
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "terrain.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
    ...
    for (int i = 0; i < terrainListByID.size(); i++) {
      ...
      if (tmi.getLadderItem() != null) {
        ...
        if (ItemManager.getItem(tmi.getLadderItem()) == null) {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("TerrainManager.1") + tmi.getLadderItem() + "]",
              "TerrainManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          Game.exit();
        }
      }
    }
  ...
  private static void loadXMLTerrain(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los terrenos (TerrainManagerItem) a la hash
      ...
      if (!terrainList.containsKey("_AIR")) {
        TerrainManagerItem tmiAir = new TerrainManagerItem(
            TerrainManagerItem.TERRAIN_AIR_INIHEADER,
            Messages.getString("TerrainManager.2")); // $NON-NLS-1$
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            item.setMineTurns(
                UtilsXML.getChildValue(node.getChildNodes(), "mineTurns")); // $NON-NLS-1$
            ...
            item.setLadderItem(
                UtilsXML.getChildValue(node.getChildNodes(), "ladderItem")); // $NON-NLS-1$
            item.setActions(UtilsXML.getChildValues(node.getChildNodes(), "action")); // $NON-NLS-1$
            item.setCanBeFilled(
                UtilsXML.getChildValue(node.getChildNodes(), "canBeFilled")); // $NON-NLS-1$
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("TerrainManager.0") + e.toString() + "]",
          "TerrainManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
