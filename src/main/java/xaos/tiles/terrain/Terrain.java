...
public class Terrain implements Externalizable {
  ...
  public static boolean canGoDown(int x, int y, int z) {
    ...
    // Podrá ir hacia abajo si aquí hay una escalera operativa, está digado y abajo está minado
    // (ESTO YA NO APLICA)
    // También puede ir hacia abajo si abajo hay una escalera operativa esta minado (se presupone) y
    // aquí está digado
    // Cell
    ...
    if (!cell
        .isDiscovered()) { // || !cell.getTerrain ().isMined ()) { --- no hace falta mirar mined pq
      // se ha mirado "digged" arriba
      return false;
    }

    // Se cumplen los prerequisitos, sólo nos falta mirar que aquí o abajo haya escalera (SÓLO
    // ABAJO)
  ...
  public static boolean canGoUp(int x, int y, int z) {
    ...
    // (ESTO YA NO APLICA) También puede ir hacia arriba si arriba hay una escalera operativa esta
    // digado y aquí está minado
    // Up
    ...
    if (!cell.isDiscovered()) {
      /* || !cell.getTerrain ().isMined ()) { --- No hace falta mirar el mined pq ya se ha mirado digged arriba */

      return false;
    }

    // Se cumplen los prerequisitos, sólo nos falta mirar que aquí o arriba haya escalera (SÓLO
    // AQUÍ, ARRIBA YA NO APLICA)
    Item item = cell.getItem();
    if (item != null && item.isOperative() && item.isLocked()) {
  ...
  /**
   * Cambia los gráficos de todas las celdas según las adyacentes (slopes outside)
   *
   * @param cells
   */
      ...
      if (buffer != null) {
        ...
        if (iIndex != 0) {
          cellCurrent
              .getTerrain()
              .setTerrainTileID(
                  cellCurrent.getTerrain().getTerrainID() * TerrainManager.SLOPES_INIHEADER.length
                      + iIndex);
  ...
  public static SmartMenu createMenuEvents(SmartMenu smParent) {
    ...
    while (itAllEvents.hasNext()) {
      ...
      menuAddEvent.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              emi.getName() + " (" + sIniHeader + ")",
              null,
              CommandPanel.COMMAND_ADD_EVENT,
              sIniHeader,
              null,
  ...
  public static SmartMenu createMenuItems(Cell cell, SmartMenu smParent) {
    ...
    while (itAllItems.hasNext()) {
      ...
      menuAddItem.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              imi.getName() + " (" + sIniHeader + ")",
              null,
              CommandPanel.COMMAND_ADD_ITEM,
              sIniHeader,
              null,
              cell.getCoordinates().toPoint3D())); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public static SmartMenu createMenuLivings(Cell cell, SmartMenu smParent) {
    ...
    while (itAllLivings.hasNext()) {
      ...
      menuAddLiving.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              lemi.getName() + " (" + sIniHeader + ")",
              null,
              CommandPanel.COMMAND_ADD_LIVING,
              sIniHeader,
              null,
              cell.getCoordinates().toPoint3D())); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  // SmartMenu menuChangeGodStatus = new SmartMenu (SmartMenu.TYPE_MENU, "Change god status",
  // smParent, null, null); //$NON-NLS-1$
  // ArrayList<GodData> alGods = Game.getWorld ().getGods ();
  //
  // if (alGods != null) {
  // for (int i = 0; i < alGods.size (); i++) {
  // SmartMenu smGod = new SmartMenu (SmartMenu.TYPE_MENU, alGods.get (i).getFullName (),
  // menuChangeGodStatus, null, null);
  // smGod.addItem (new SmartMenu (SmartMenu.TYPE_ITEM, "-5 status", null,
  // CommandPanel.COMMAND_GOD_STATUS_LOWER_5, alGods.get (i).getGodID (), null, null));
  // //$NON-NLS-1$ //$NON-NLS-2$
  // smGod.addItem (new SmartMenu (SmartMenu.TYPE_ITEM, "+5 status", null,
  // CommandPanel.COMMAND_GOD_STATUS_RAISE_5, alGods.get (i).getGodID (), null, null));
  // //$NON-NLS-1$ //$NON-NLS-2$
  // smGod.addItem (new SmartMenu (SmartMenu.TYPE_TEXT, null, null, null, null));
  // smGod.addItem (new SmartMenu (SmartMenu.TYPE_ITEM, Messages.getString ("Terrain.8"), null,
  ...
  // menuChangeGodStatus.addItem (new SmartMenu (SmartMenu.TYPE_ITEM, Messages.getString
  // ("Terrain.8"), smParent, CommandPanel.COMMAND_BACK, null)); //$NON-NLS-1$
  //
  ...
  public static void fillMenu(Cell cell, SmartMenu sm) {
    // Menu add fluids
    SmartMenu menuAddFluids = new SmartMenu(
        SmartMenu.TYPE_MENU, Messages.getString("Terrain.4"), sm, null, null); // $NON-NLS-1$
    if (cell.getTerrain().getFluidCount() != 0) {
      menuAddFluids.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              Messages.getString("Terrain.5"),
              null,
              CommandPanel.COMMAND_TERRAIN_REMOVE_FLUID,
              null,
              null,
              cell.getCoordinates().toPoint3D())); // $NON-NLS-1$
    ...
    menuAddFluids.addItem(
        new SmartMenu(
            SmartMenu.TYPE_ITEM,
            Messages.getString("Terrain.6"),
            null,
            CommandPanel.COMMAND_TERRAIN_ADD_FLUID,
            Integer.toString(FLUIDS_WATER),
            Integer.toString(FLUIDS_COUNT_MAX),
            cell.getCoordinates().toPoint3D())); // $NON-NLS-1$
    if (TownsProperties.DEBUG_MODE) {
      menuAddFluids.addItem(
          new SmartMenu(
              ...
              CommandPanel.COMMAND_TERRAIN_ADD_FLUID,
              Integer.toString(FLUIDS_WATER),
              Integer.toString(FLUIDS_COUNT_INFINITE),
              cell.getCoordinates().toPoint3D())); // $NON-NLS-1$
    }
    menuAddFluids.addItem(
        new SmartMenu(
            SmartMenu.TYPE_ITEM,
            Messages.getString("Terrain.7"),
            null,
            CommandPanel.COMMAND_TERRAIN_ADD_FLUID,
            Integer.toString(FLUIDS_LAVA),
            Integer.toString(FLUIDS_COUNT_MAX),
            cell.getCoordinates().toPoint3D())); // $NON-NLS-1$
    if (TownsProperties.DEBUG_MODE) {
      menuAddFluids.addItem(
    ...
    if (TownsProperties.DEBUG_MODE) {
      ...
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_TEXT, "Digged " + cell.isDigged(), null, null, null)); // $NON-NLS-1$
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_TEXT, "Mined " + cell.isMined(), null, null, null)); // $NON-NLS-1$

      // Menu change terrain
      SmartMenu menuChangeTerrain = new SmartMenu(
          SmartMenu.TYPE_MENU,
          "Change terrain ["
              + TerrainManager.getItemByID(cell.getTerrain().getTerrainID()).getIniHeader()
              + "]",
          sm,
          null,
          null); // $NON-NLS-1$ //$NON-NLS-2$
      ...
      for (int i = 0; i < asTerrains.length; i++) {
        ...
        if (tmi != null && tmi.getTerrainID() != cell.getTerrain().getTerrainID()) {
          menuChangeTerrain.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  tmi.getName() + " (" + tmi.getIniHeader() + ")",
                  null,
      ...
      menuChangeTerrain.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM, "Back", sm, CommandPanel.COMMAND_BACK, null)); // $NON-NLS-1$
      ...
      if (cell.getTerrain().getFluidCount() != 0) {
        ...
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_TEXT,
                "Fluid [" + sFluidType + "] Strenght [" + sStrenght + "]",
                null,
                null,
    ...
    if (!cell.getTerrain().hasFluids()) {
      ...
      if (alPatrolSoldiers.size() > 0 || alPatrolGroups.size() > 0) {
        if (cell.getAstarZoneID() != -1) {
          ...
          SmartMenu menuPatrols = new SmartMenu(
              SmartMenu.TYPE_MENU,
              Messages.getString("Terrain.9"),
              sm,
              null,
              null); // $NON-NLS-1$
          ...
          for (int i = 0; i < alPatrolSoldiers.size(); i++) {
            menuPatrols.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    alPatrolSoldiers.get(i).getCitizenData().getFullName(),
                    null,
                    CommandPanel.COMMAND_ADD_PATROL_POINT,
                    Integer.toString(alPatrolSoldiers.get(i).getID()),
                    null,
          ...
          for (int i = 0; i < alPatrolGroups.size(); i++) {
            menuPatrols.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    Game.getWorld().getSoldierGroups().getGroup(alPatrolGroups.get(i)).getName(),
                    null,
                    CommandPanel.COMMAND_ADD_PATROL_POINT_GROUP,
                    Integer.toString(alPatrolGroups.get(i)),
                    null,
                    cell.getCoordinates().toPoint3D()));
          }
      ...
      if (cell.isFlagPatrol()) {
        // Miramos si hay más de 1 aldeano con ese punto, para crear un menú lista (también miramos
        // los grupos)
        SmartMenu menuPatrols = new SmartMenu(
            SmartMenu.TYPE_MENU,
            Messages.getString("Terrain.10"),
            sm,
            null,
            null); // $NON-NLS-1$
        ...
        sm.addItem(
            new SmartMenu(
                ...
                new Point3D(
                    ...
                    cell.getCoordinates().z + 1))); // $NON-NLS-1$
        TerrainManagerItem tmiUnder = TerrainManager.getItemByID(
            World.getCell(
                cell.getCoordinates().x,
                cell.getCoordinates().y,
      ...
      // sm.addItem (new SmartMenu (SmartMenu.TYPE_ITEM, ami.getName () + " " + tmi.getName
      // ().toLowerCase (), null, CommandPanel.COMMAND_CUSTOM_ACTION, ami.getId (), null,
      // cell.getCoordinates ())); //$NON-NLS-1$
      // }
      // }
        ...
        if (tmiUnder.hasActions()) {
          ...
          for (int i = 0; i < tmiUnder.getActions().size(); i++) {
            ...
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    ami.getName() + " " + tmiUnder.getName().toLowerCase(),
                    null,
                    CommandPanel.COMMAND_CUSTOM_ACTION,
                    ami.getId(),
                    null,
                    cell.getCoordinates().toPoint3D())); // $NON-NLS-1$
          }
  ...
  public void mine(short x, short y, short z, boolean bMineLadder) {
    if (getMineTurns() > 0) {
      ...
      if (getMineTurns() <= 0) {
        ...
        // Ańadimos el item de la celda de arriba (si tiene) a la lista de items posibles a caer,
        // tambien los de los lados (por si tienen glue)
        Item item;
        if (z > 0) {
        ...
        if (bMineLadder) {
          // Antes de minar miro si es un mine a ladder y pillo el ItemManagerItem, ya que luego
          // quizá la celda se ha convertido en AIR debido a otras tareas o lo que sea
          ItemManagerItem imiLadder = null;
          // Bingo, creamos el item si se puede
          String sLadderItem = TerrainManager.getItemByID(currentCell.getTerrain().getTerrainID())
              .getLadderItem();
          if (sLadderItem != null) {
        ...
        // Cambiamos el tipo a AIR (al final por si el mine suelta un drop o por si metemos un
        // ladder)
        ...
        // ShouldPaintUnders 2 (potser pot anar a dalt, per si acŕs no li poso :D )
        if (z < (World.MAP_DEPTH - 1)) {
