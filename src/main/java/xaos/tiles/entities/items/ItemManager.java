...
public class ItemManager {
  ...
  /**
   * Devuelve una lista con todos los items que se pueden construir en el edificio pasado
   *
   * @param buildingIniHeader
   *            ID del edificio
   * @return
   */
  ...
  /**
   * Devuelve una lista con todos los items con el type pasado, nunca devuelve null
   *
   * @return una lista con todos los items con el type pasado, nunca devuelve null
   */
  ...
  /**
   * Devuelve un item a random a partir de un nivel mínimo y máximo o null si no encuentra Items de level 0 no se
   * devuelven
   *
   * @param level
   *            Nivel
  ...
  public static void loadItems() {
    if (itemList == null) {
      ...
      // Cargar de fichero
      loadXMLItems(
          Towns.getPropertiesString("DATA_FOLDER") + "items.xml",
      ...
      if (alMods != null && alMods.size() > 0) {
        for (int i = 0; i < alMods.size(); i++) {
          String sModActionsPath = fUserFolder.getAbsolutePath()
              ...
              + "items.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          // //$NON-NLS-5$
      ...
      // Y DE PASO ańadimos los types
      // Y DE PASO ańadimos los maxAgeTerrain
      ...
      while (itItems.hasNext()) {
        ...
        if (imi.getMaxAgeItem() != null && imi.getMaxAgeItem().length() > 0) {
          if (itemList.get(imi.getMaxAgeItem()) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("ItemManager.1") + imi.getMaxAgeItem() + "]",
                "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
          }
        }
        ...
        if (imi.getBuryItem() != null && imi.getBuryItem().size() > 0) {
          for (int i = 0; i < imi.getBuryItem().size(); i++) {
            if (itemList.get(imi.getBuryItem().get(i)) == null) {
              Log.log(
                  Log.LEVEL_ERROR,
                  Messages.getString("ItemManager.3") + " [" + imi.getBuryItem().get(i) + "]",
                  "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
              Game.exit();
            }
          }
        }
        ...
        if (Towns.getPropertiesInt(
            PropertyFile.PROPERTY_FILE_GRAPHICS,
            "[" + sMiniItem + "]TILE_X",
            -666) != -666) { // $NON-NLS-1$ //$NON-NLS-2$
        ...
        if (imi.getHabitatAsString() != null) {
          for (int i = 0; i < imi.getHabitatAsString().size(); i++) {
            if (!habitat.contains(imi.getHabitatAsString().get(i))) {
              habitat.add(
                  new Integer(
                      TerrainManager.getItem(imi.getHabitatAsString().get(i)).getTerrainID()));
        ...
        if (imi.getMaxAgeTerrain() != null && imi.getMaxAgeTerrain().length() > 0) {
          if (TerrainManager.getItem(imi.getMaxAgeTerrain()) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("ItemManager.2") + " [" + imi.getMaxAgeTerrain() + "]",
                "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
  ...
  private static void loadXMLItems(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los items (ItemManagerItem) a la hash
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (sIniHeader.equalsIgnoreCase("DELETE")) { // $NON-NLS-1$
            ...
            if (node.getAttributes() != null
                && node.getAttributes().getNamedItem("id") != null) { // $NON-NLS-1$
          ...
          if (bLoadingMain && bExists) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("ItemManager.0") + sIniHeader + "]",
                "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setDescriptions(
                UtilsXML.getChildValues(node.getChildNodes(), "description")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setBuilding(
                UtilsXML.getChildValue(node.getChildNodes(), "building")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setPrerequisites(
                UtilsXML.getChildValues(node.getChildNodes(), "prerequisite")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setBuildingTime(
                UtilsXML.getChildValue(node.getChildNodes(), "buildingTime")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setFloorWalkSpeed(
                UtilsXML.getChildValue(node.getChildNodes(), "floorWalkSpeed")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setHabitatAsString(
                UtilsXML.getChildValues(node.getChildNodes(), "habitat")); // $NON-NLS-1$
            item.setHabitatGroup(
                UtilsXML.getChildValue(node.getChildNodes(), "habitatGroup")); // $NON-NLS-1$
            item.setHabitatHeightMin(
                UtilsXML.getChildValue(node.getChildNodes(), "habitatHeightMin")); // $NON-NLS-1$
            item.setHabitatHeightMax(
                UtilsXML.getChildValue(node.getChildNodes(), "habitatHeightMax")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            sAux = UtilsXML.getChildValue(
                node.getChildNodes(),
                "maxAgeNeedsWaterRadius"); // $NON-NLS-1$
            if (sAux != null) {
            ...
            sAux = UtilsXML.getChildValue(
                node.getChildNodes(),
                "maxAgeNeedsItemsRadius"); // $NON-NLS-1$
            if (sAux != null) {
            ...
            item.setMaxAgeItem(
                UtilsXML.getChildValue(node.getChildNodes(), "maxAgeItem")); // $NON-NLS-1$
            item.setMaxAgeTerrain(
                UtilsXML.getChildValue(node.getChildNodes(), "maxAgeTerrain")); // $NON-NLS-1$
            item.setMaxAgeNeedsWater(
                UtilsXML.getChildValue(node.getChildNodes(), "maxAgeNeedsWater")); // $NON-NLS-1$
            item.setMaxAgeNeedsWaterRadius(
                UtilsXML.getChildValue(
                    node.getChildNodes(),
                    "maxAgeNeedsWaterRadius")); // $NON-NLS-1$
            item.setMaxAgeNeedsItems(
                UtilsXML.getChildValue(node.getChildNodes(), "maxAgeNeedsItems")); // $NON-NLS-1$
            item.setMaxAgeNeedsItemsRadius(
                UtilsXML.getChildValue(
                    node.getChildNodes(),
                    "maxAgeNeedsItemsRadius")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setSpawnMaxItems(
                UtilsXML.getChildValue(node.getChildNodes(), "spawnMaxItems")); // $NON-NLS-1$
            item.setSpawnTurns(
                UtilsXML.getChildValue(node.getChildNodes(), "spawnTurns")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setWallConnector(
                UtilsXML.getChildValue(node.getChildNodes(), "wallConnector")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setCanBeEaten(
                UtilsXML.getChildValue(node.getChildNodes(), "canBeEaten")); // $NON-NLS-1$
            item.setFoodValue(
                UtilsXML.getChildValue(node.getChildNodes(), "foodValue")); // $NON-NLS-1$
            item.setFoodFillPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "foodFillPCT")); // $NON-NLS-1$
            item.setFoodEatTime(
                UtilsXML.getChildValue(node.getChildNodes(), "foodEatTime")); // $NON-NLS-1$
            item.setFoodEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "foodEffects")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setHappiness(
                UtilsXML.getChildValue(node.getChildNodes(), "happiness")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setCanBeUsedToSleep(
                UtilsXML.getChildValue(node.getChildNodes(), "usedToSleep")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setCanBeUsedToSit(
                UtilsXML.getChildValue(node.getChildNodes(), "usedToSit")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setZoneMergerUp(
                UtilsXML.getChildValue(node.getChildNodes(), "zoneMergerUp")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setCanBeBuiltOnFloor(
                UtilsXML.getChildValue(node.getChildNodes(), "canBeBuiltOnFloor")); // $NON-NLS-1$
            item.setCanBeBuiltOnHoles(
                UtilsXML.getChildValue(node.getChildNodes(), "canBeBuiltOnHoles")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setCanBeUnlocked(
                UtilsXML.getChildValue(node.getChildNodes(), "canBeUnlocked")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setLightRadius(
                UtilsXML.getChildValue(node.getChildNodes(), "lightRadius")); // $NON-NLS-1$
            item.setLightRed(
                UtilsXML.getChildValue(node.getChildNodes(), "lightRed")); // $NON-NLS-1$
            item.setLightGreen(
                UtilsXML.getChildValue(node.getChildNodes(), "lightGreen")); // $NON-NLS-1$
            item.setLightBlue(
                UtilsXML.getChildValue(node.getChildNodes(), "lightBlue")); // $NON-NLS-1$
            item.setTranslucent(
                UtilsXML.getChildValue(node.getChildNodes(), "translucent")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setAlwaysOperative(
                UtilsXML.getChildValue(node.getChildNodes(), "alwaysOperative")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setContainer(
                UtilsXML.getChildValue(node.getChildNodes(), "container")); // $NON-NLS-1$
            item.setContainerSize(
                UtilsXML.getChildValue(node.getChildNodes(), "containerSize")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setStackable(
                UtilsXML.getChildValue(node.getChildNodes(), "stackable")); // $NON-NLS-1$
            item.setStackableSize(
                UtilsXML.getChildValue(node.getChildNodes(), "stackableSize")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setSpeedUpPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "speedUpPCT")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setLocation(
                UtilsXML.getChildValue(node.getChildNodes(), "location")); // $NON-NLS-1$
            item.setAttackModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "attack")); // $NON-NLS-1$
            item.setAttackSpeedModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "attackSpeed")); // $NON-NLS-1$
            item.setDefenseModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "defense")); // $NON-NLS-1$
            item.setHealthModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "health")); // $NON-NLS-1$
            item.setDamageModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "damage")); // $NON-NLS-1$
            item.setLOSModifier(UtilsXML.getChildValue(node.getChildNodes(), "LOS")); // $NON-NLS-1$
            item.setMovePCTModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "movePCT")); // $NON-NLS-1$
            item.setWalkSpeedModifier(
                UtilsXML.getChildValue(node.getChildNodes(), "walkSpeed")); // $NON-NLS-1$
            item.setRanged(UtilsXML.getChildValue(node.getChildNodes(), "ranged")); // $NON-NLS-1$
            item.setRangedAmmo(
                UtilsXML.getChildValue(node.getChildNodes(), "rangedAmmo")); // $NON-NLS-1$
            item.setRangedOneShoot(
                UtilsXML.getChildValue(node.getChildNodes(), "rangedOneShoot")); // $NON-NLS-1$
            item.setVerb(UtilsXML.getChildValue(node.getChildNodes(), "verb")); // $NON-NLS-1$
            item.setVerbInfinitive(
                UtilsXML.getChildValue(node.getChildNodes(), "verbInfinitive")); // $NON-NLS-1$

            item.setWearEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "wearEffects")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setTrapEffects(
                UtilsXML.getChildValue(node.getChildNodes(), "trapEffects")); // $NON-NLS-1$
            item.setTrapCooldown(
                UtilsXML.getChildValue(node.getChildNodes(), "trapCooldown")); // $NON-NLS-1$
            item.setTrapOnIcon(
                UtilsXML.getChildValue(node.getChildNodes(), "trapOnIcon")); // $NON-NLS-1$
            item.setTrapTargets(
                UtilsXML.getChildValue(node.getChildNodes(), "trapTargets")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setBlockFluids(
                UtilsXML.getChildValue(node.getChildNodes(), "blockFluids")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setFluidsElevator(
                UtilsXML.getChildValue(node.getChildNodes(), "fluidsElevator")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setAllowFluids(
                UtilsXML.getChildValue(node.getChildNodes(), "allowFluids")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setBuryLocked(
                UtilsXML.getChildValue(node.getChildNodes(), "buryLocked")); // $NON-NLS-1$
            item.setBuryItem(
                UtilsXML.getChildValue(node.getChildNodes(), "buryItem")); // $NON-NLS-1$
            item.setBuryItemPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "buryItemPCT")); // $NON-NLS-1$
            item.setBuryDestroyItem(
                UtilsXML.getChildValue(node.getChildNodes(), "buryDestroyItem")); // $NON-NLS-1$
            item.setBuryLivings(
                UtilsXML.getChildValue(node.getChildNodes(), "buryLivings")); // $NON-NLS-1$
            item.setBuryLivingsPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "buryLivingsPCT")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setCanBeRotated(
                UtilsXML.getChildValue(node.getChildNodes(), "canBeRotated")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setBuildAction(
                UtilsXML.getChildValue(node.getChildNodes(), "buildAction")); // $NON-NLS-1$
          }

          // Lo ańadimos a la hash
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("ItemManager.6")
              + sXMLName
              + Messages.getString("ItemManager.7")
              + e.toString()
              + "]",
          "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
