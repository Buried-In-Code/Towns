...
public class PrefixSuffixManagerItem {
  ...
  public void setType(String type) throws Exception {
    if (!type.equalsIgnoreCase(PrefixSuffixData.TYPE_PREFIX)
        && !type.equalsIgnoreCase(PrefixSuffixData.TYPE_SUFFIX)) {
      throw new Exception(
          Messages.getString("PrefixSuffixData.2") + type + "]"); // $NON-NLS-1$ //$NON-NLS-2$
    }
    this.type = type;
  }
