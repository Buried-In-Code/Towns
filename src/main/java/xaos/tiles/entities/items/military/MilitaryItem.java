...
public class MilitaryItem extends Item implements Externalizable {
  ...
  public static final int LOCATION_LEGS = 3;
  public static final int LOCATION_FEET = 4;
  public static final int LOCATION_WEAPON = 5;
  ...
  public String getExtendedTilename() {
    ...
    if (getAttackModifier() != 0) {
      sb.append(
          Messages.getString("MilitaryItem.0")
              + getAttackModifier()
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (getDefenseModifier() != 0) {
      sb.append(
          Messages.getString("MilitaryItem.2")
              + getDefenseModifier()
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (getHealthModifier() != 0) {
      sb.append(
          Messages.getString("MilitaryItem.4")
              + getHealthModifier()
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (getDamageModifier() != 0) {
      sb.append(
          Messages.getString("MilitaryItem.6")
              + getDamageModifier()
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (getLOSModifier() != 0) {
      sb.append(
          Messages.getString("MilitaryItem.7")
              + getLOSModifier()
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
  ...
  /**
   * Devuelve el color para pintar el item en los menues, se basa en si tiene prefijo y/o sufijo
   *
   * @return el color para pintar el item en los menues, se basa en si tiene prefijo y/o sufijo
   */
