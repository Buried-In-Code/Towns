...
public class PrefixSuffixManager {
      ...
      if (alMods != null && alMods.size() > 0) {
        for (int i = 0; i < alMods.size(); i++) {
          String sModActionsPath = fUserFolder.getAbsolutePath()
              ...
              + "prefixsuffix.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          // //$NON-NLS-5$
  ...
  private static void loadXMLData(String sXMLPath) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los prefijos/sufijos en el array que toque
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("PrefixSuffixManager.0") + e.toString() + "]",
          "PrefixSuffixManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
}
