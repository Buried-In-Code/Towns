...
public class Item extends Entity implements Externalizable {
  ...
  // Estos 3 se usan para buscar un item, indicando para cada cosa (locked, operative) si tiene que
  // estar a true a false o que no importa
  public static final int SEARCH_DOESNTMATTER = 0;
  public static final int SEARCH_FALSE = 1;
  public static final int SEARCH_TRUE = 2;
  ...
  private static final byte FLAG_OPERATIVE = 1; // 00000001 (Indica si estß operativo)
  private static final byte FLAG_LOCKED = 2; // 00000010 (Indica si estß bloqueado en una celda (vamos, que no se
  // puede move para
  // llevarlo a una stockpile por ejemplo))
  public static final byte FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_CLOSED = 4;
  public static final byte FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_OPENED = 8;
  public static final byte FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED = 16;
  public static final byte FLAG_FACING_BIT_0 = 32;
  public static final byte FLAG_FACING_BIT_1 = 64; // 00 - west, 01 - east, 10 - north, 11 - south
  ...
  /**
   * Son los prerequisitos para construirse, irß desapareciendo durante la construcci¾n Cuando lleguen a 0 el item
  ...
  private int maxAge; // Edad mßxima del item

  // HPs
  private int hp; // Puntos de vida, si llega a 0 palma
  private int maxHp; // Puntos de vida mßximos (se usa para sacar un mensaje a la primera hostia que le
  // meten)
    ...
    if (Game.getWorld() != null && Game.getWorld().getEvents() != null) {
      for (int i = 0; i < Game.getWorld().getEvents().size(); i++) {
        ...
        if (emi != null
            && emi.getItems() != null
            && emi.getItems().size() > 0
            && emi.getItemsMaxAgePCT() != null
            && emi.getItems().size() == emi.getItemsMaxAgePCT().size()) {
  ...
  public static int addItem(Item item) {
    return addItem(
        UtilsIniHeaders.getIntIniHeader(item.getIniHeader()),
        item.getID(),
        item.isLocked());
  }
   ...
   *            Item ID
   * @param locked
   *            Indica si es la lista de lockeds o no
   */
  ...
  public static Item createItem(ItemManagerItem imi) {
    ...
    if (item != null) {
      ...
      if (item instanceof MilitaryItem) {
        ...
        ((MilitaryItem) item)
            .setAttackSpeedModifier(Utils.launchDice(imi.getAttackSpeedModifier()));
    ...
    if (item != null) {
      if (TownsProperties.DEBUG_MODE) {
        ...
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_TEXT,
                "ID: " + item.getID() + " (" + item.getIniHeader() + ")",
                null,
                null,
        ...
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_TEXT,
                "Num in world: " + Item.getNumItemsTotal(item.getIniHeader(), World.MAP_DEPTH - 1),
                null,
                null,
                null)); // $NON-NLS-1$
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_TEXT,
                "Num in world over restriction ("
                    + Game.getWorld().getRestrictHaulEquippingLevel()
                    + "): "
                    + Item.getNumItemsTotal(
                        item.getIniHeader(),
                        Game.getWorld().getRestrictHaulEquippingLevel()),
                null,
                null,
        ...
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_TEXT,
                "age/maxAge: " + item.getAge() + " / " + item.getMaxAge(),
                null,
                null,
                null)); // $NON-NLS-1$ //$NON-NLS-2$
      ...
      if (imi.isDoor() && item.isOperative() && item.isLocked()) {
        if (item.isDoorStatus(FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_OPENED)) {
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.5") + imi.getName(),
                  null,
                  CommandPanel.COMMAND_UNLOCK_CLOSE,
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.6") + imi.getName(),
                  null,
                  CommandPanel.COMMAND_LOCK,
                  Integer.toString(item.getID()),
                  null,
                  null,
                  Color.GREEN)); // $NON-NLS-1$
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.7") + imi.getName(),
                  null,
                  CommandPanel.COMMAND_UNLOCK_OPEN,
                  Integer.toString(item.getID()),
                  null,
                  null,
                  Color.GREEN)); // $NON-NLS-1$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.3") + imi.getName(),
                  null,
                  CommandPanel.COMMAND_LOCK,
                  Integer.toString(item.getID()),
                  null,
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.4") + imi.getName(),
                  null,
                  CommandPanel.COMMAND_UNLOCK_CLOSE,
                  Integer.toString(item.getID()),
                  null,
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.8") + imi.getName(),
                  null,
                  CommandPanel.COMMAND_UNLOCK_OPEN,
                  Integer.toString(item.getID()),
                  null,
      ...
      if (imi.isText()) {
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("Item.11"),
                null,
                CommandPanel.COMMAND_ITEM_TEXT_ADD,
                Integer.toString(item.getID()),
                null,
                null)); // $NON-NLS-1$
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("Item.12"),
                null,
                CommandPanel.COMMAND_ITEM_TEXT_DELETE,
                Integer.toString(item.getID()),
                null,
                null)); // $NON-NLS-1$
        sm.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
      ...
      if (imi.hasActions()) {
        ...
        for (int i = 0; i < imi.getActions().size(); i++) {
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  ami.getName() + " " + imi.getName().toLowerCase(),
                  null,
                  CommandPanel.COMMAND_CUSTOM_ACTION_DIRECT_ITEM,
                  ami.getId(),
                  Integer.toString(item.getID()))); // $NON-NLS-1$
        }
        sm.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
      }
      ...
      if (imi.getBuildAction() != null) {
        ...
        if (ami != null) {
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.19") + imi.getName(),
                  null,
      ...
      if (item.isLocked() && imi.canBeUnlocked()) {
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("Item.18"),
                null,
                CommandPanel.COMMAND_UNLOCK,
                Integer.toString(item.getID()),
                null)); // $NON-NLS-1$
        sm.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
      }
      ...
      if (imi.isCanBeRotated()) {
        SmartMenu menuRotate = new SmartMenu(
            SmartMenu.TYPE_MENU, Messages.getString("Item.13"), sm, null, null); // $NON-NLS-1$

        if (item.isFacingWest()) {
          menuRotate.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.14"),
                  null,
                  CommandPanel.COMMAND_ITEM_ROTATE,
                  Integer.toString(item.getID()),
                  Integer.toString(FACE_NORTH))); // $NON-NLS-1$
          menuRotate.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.15"),
                  null,
                  CommandPanel.COMMAND_ITEM_ROTATE,
                  Integer.toString(item.getID()),
                  Integer.toString(FACE_SOUTH))); // $NON-NLS-1$
          menuRotate.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  Messages.getString("Item.16"),
                  null,
                  CommandPanel.COMMAND_ITEM_ROTATE,
                  Integer.toString(item.getID()),
                  Integer.toString(FACE_EAST))); // $NON-NLS-1$
          menuRotate.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  Messages.getString("Item.17"),
                  null,
                  null,
                  null,
                  null,
          ...
          menuRotate.addItem(
              new SmartMenu(
                  ...
                  CommandPanel.COMMAND_ITEM_ROTATE,
                  Integer.toString(item.getID()),
                  Integer.toString(FACE_WEST))); // $NON-NLS-1$
      ...
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              Messages.getString("Item.0") + imi.getName(),
              null,
              CommandPanel.COMMAND_DESTROY_ENTITY,
              null,
              null,
              cell.getCoordinates().toPoint3D(),
  ...
  public static Item getItemByID(int iItemID, boolean bSearchInContainers) {
    ...
    if (bSearchInContainers) {
      // Item no existe o estß en containers, lo buscamos
   ...
   *            IniHeader del item
   * @param locked
   *            Indica si devuelve el n·mero de items locked o los no lockeds
   * @param iMaxLevelToCheck
   *            Max level to check the number of items
  ...
  public static int getNumItemsTotal(String sIniHeader, int iMaxLevelToCheck) {
    ...
    return getNumItems(iIniHeader, true, iMaxLevelToCheck)
        + getNumItems(iIniHeader, false, iMaxLevelToCheck);
  }
   ...
   */
  public static boolean isCellAvailableForItem(
      ItemManagerItem imi,
      Point3DShort point,
      boolean checkZones,
      boolean checkDiscovered) {
  ...
  public static boolean isCellAvailableForItem(
      ...
      boolean checkZones,
      boolean checkDiscovered,
      boolean checkHabitat) {
    return isCellAvailableForItem(
        imi,
        point.x,
        point.y,
        point.z,
  ...
  public static boolean isCellAvailableForItem(
    ...
    // No minada, no vacÝa, con fluidos
    ...
    if (checkHabitat && z >= World.MAP_NUM_LEVELS_OUTSIDE) {
      if (imi.getHabitatHeightMin() != -1
          && z <= (World.MAP_NUM_LEVELS_OUTSIDE - imi.getHabitatHeightMin())) {
        return false;
      }
      if (imi.getHabitatHeightMax() != -1
          && z >= (World.MAP_NUM_LEVELS_OUTSIDE - imi.getHabitatHeightMin())) {
        return false;
      }
      // POPO
      /*
       * if (imi.getHabitatHeightMin () != -1) { if ((World.getMapMaxHeight (x, y, z) /
       * MapGenerator.PIXELS_BETWEEN_HEIGHTS) < imi.getHabitatHeightMin ()) { return false; } } if
       * (imi.getHabitatHeightMax () != -1) { if ((World.getMapMaxHeight (x, y, z) /
       * MapGenerator.PIXELS_BETWEEN_HEIGHTS) > imi.getHabitatHeightMax ()) { return false; } }
       */
    }
  ...
  /**
   * Devuelve la coordenada de un item DE COMIDA Busca, de los que tengan valores mßs altos el mßs cercano Primero
   * containers y pilas, en otro caso cualquier item del mundo
   *
   * <p>
   * Evita comidas que tienen "lockeada" otros aldeanos con tarea de EAT
   *
   * @param p3dCurrentPoint
   *            Punto origen
   * @param citID
   *            ID del aldeano que busca la comida
  ...
  public static Point3DShort searchFood(Point3DShort p3dCurrentPoint, int citID) {
    ...
    boolean bContainerChecked = false; // Para ver si el contenedor estß en su sitio
      ...
      for (int j = 0; j < alContainerItems.size(); j++) {
        ...
        if (foodItem != null) {
          ...
          if (imi.canBeEaten() && imi.getFoodValue() >= iMaxFoodValue) {
            // Item bueno para comer, miramos si estß en la zona y ning·n otro aldeano va a por el
            p3d = foodItem.getCoordinates();
            // Miramos que el container estÚ ahÝ (caso robbery siege o alg·n aldeano moviÚndolo)
            ...
            if (bContainerChecked
                && iCurrentPointASZID == World.getCell(foodItem.getCoordinates()).getAstarZoneID()
                && !Citizen.isCitizenWalkingToFood(foodItem.getCoordinates(), citID)) {
              // Estß en la zona y el item no es de nadie, si el valor de comida es igual al max
              // simplemente lo metemos en la lista
        ...
        for (int c = 0; c < pile.getPoints().size(); c++) {
          ...
          if (cell.hasItem()) {
            ...
            if (foodItem != null) {
              imi = ItemManager.getItem(foodItem.getIniHeader());
              if (imi.canBeEaten() && imi.getFoodValue() >= iMaxFoodValue) {
                // Item bueno para comer, miramos si estß en la zona y ning·n otro aldeano va a por
                // el
                p3d = foodItem.getCoordinates();
                if (iCurrentPointASZID == World.getCell(foodItem.getCoordinates()).getAstarZoneID()
                    && !Citizen.isCitizenWalkingToFood(foodItem.getCoordinates(), citID)) {
                  // Estß en la zona y el item no es de nadie, si el valor de comida es igual al max
                  // simplemente lo metemos en la lista
                  // En otro caso borramos la lista y metemos el nuevo item
    ...
    for (int i = 0; i < aItems.length; i++) {
      ...
      if (foodItem != null) {
        ...
        if (imi.canBeEaten() && imi.getFoodValue() >= iMaxFoodValue) {
          // Item bueno para comer, miramos si estß en la zona y ning·n otro aldeano va a por el
          p3d = foodItem.getCoordinates();
          if (iCurrentPointASZID == World.getCell(p3d).getAstarZoneID()
              && !Citizen.isCitizenWalkingToFood(p3d, citID)) {
            // Estß en la zona y el item no es de nadie, si el valor de comida es igual al max
            // simplemente lo metemos en la lista
            // En otro caso borramos la lista y metemos el nuevo item
            if (imi.getFoodValue() > iMaxFoodValue) {
  ...
  /**
   * Devuelve la coordenada de un item que cumpla alguno de los prerequisitos. Intenta devolver el item mßs cercano al
   * punto de origen. Primero mira en la casilla de origen. Si no lo encuentra mira en containers. Si no lo encuentra
   * mira en stockpiles. Si no lo encuentra mira en todos los materiales del mundo
   *
   * @param p3dCurrentPoint
   *            Punto origen
   * @param aiIniHeaders
   *            Prerequisitos
   * @param near
   *            Indica si hay que buscar el item mßs cercano
   * @param locked
   *            Indica si el item tiene que estar locked
   * @param operative
   *            Indica si el item tiene que estar operative
   * @param alItemsToAvoid
   *            Lista de IDs de items que no hay que devolver
   * @param searchItemsInOrder
   *            Hace que, de la lista pasada de items primero busca el primero, si no lo encuentra buscarß el 2o, ...
   ...
   * @return la coordenada de un item que cumpla alguno de los prerequisitos o null si no existe el item
   */
  public static Point3DShort searchItem(
      boolean bJustKnowIfExists,
      Point3DShort p3dCurrentPoint,
      int[] aiIniHeaders,
      boolean near,
      int locked,
      int operative,
      ArrayList<Integer> alItemsToAvoid,
      boolean searchItemsInOrder,
      boolean bCheckContainers,
      int iMaxLevelToCheck) {
    ...
    for (int i = 0; i < aiIniHeaders.length; i++) {
      if (locked == SEARCH_DOESNTMATTER) {
        iNum = Item.getNumItemsTotal(
            UtilsIniHeaders.getStringIniHeader(aiIniHeaders[i]),
            iMaxLevelToCheck);
    ...
    if (!bHayAlguno) {
      ...
      if (bJustKnowIfExists) {
        // Miramos que no estÚ en uso
        if (alItemsToAvoid == null || alItemsToAvoid.isEmpty()) {
          return Point3DShort.getPoolInstance(0, 0, 0);
          // } else {
          // Hay que hacer la b·squeda entera, para saber si los items encontrados estarßn en uso o
          // no
        }
      }
    }

    // Si llega aquÝ es que en el mundo existen items, vamos a ver si en la casilla que estamos hay
    ...
    // Si llegamos aquÝ es que no hay material en nuestra casilla, vamos a buscar uno en containers
    ...
    if (bCheckContainers) {
      ...
      for (int i = 0; i < containers.size(); i++) {
        ...
        if (containerItem != null
            && World.getCell(containerItem.getCoordinates()).getAstarZoneID() == iCurrentASZID
            && containerItem.getCoordinates().z <= iMaxLevelToCheck) {
    ...
    // Si llegamos aquÝ es que no hay material en nuestra casilla ni containers, vamos a buscar uno
    // en stockpiles
    Stockpile stockPile;
    int distanciaMin = Utils.MAX_DISTANCE;
    boolean bAllowed;
    stockpiles1 : for (int i = 0; i < Game.getWorld().getStockpiles().size(); i++) {
      ...
      if (stockPile.isEmpty()
          || stockPile.getPoints().size() == 0
          || stockPile.getPoints().get(0).z > iMaxLevelToCheck) {
        continue;
      }
      ...
      for (int j = 0; j < stockPile.getPoints().size(); j++) {
        ...
        // Punto de stockpile, si estß en la zona A* del aldeano y luego miramos si hay item
    ...
    // Si llega aquÝ es que en stockpiles no hay nada, miramos en todos los materiales del mundo
        ...
        if (alItems != null) {
          for (int i = 0; i < alItems.size(); i++) {
            ...
            if (itemAux != null && itemAux.getCoordinates().z <= iMaxLevelToCheck) {
              if (checkLockedOperativeAndItems(
                  World.getCell(itemAux.getCoordinates()).getEntity(),
                  locked,
                  operative,
                  alItemsToAvoid)) {
                if (World.getCell(itemAux.getCoordinates()).getAstarZoneID() == World
                    .getCell(p3dCurrentPoint).getAstarZoneID()) {
    ...
    //
    // if (itemAux != null) {
    // if (checkLockedOperativeAndItems (World.getCell (itemAux.getCoordinates ()).getEntity (),
    // locked, operative, alItemsToAvoid)) {
    // boolean bItemInCellOK = false;
    // for (int m = 0; m < aiIniHeaders.length; m++) {
    ...
    // }
    // if (bItemInCellOK) {
    // if (World.getCell (itemAux.getCoordinates ()).getAstarZoneID () == World.getCell
    // (p3dCurrentPoint).getAstarZoneID ()) {
    // p3d = itemAux.getCoordinates ();
    // if (near) {
    // if (p3dMin == null) {
    // p3dMin = p3d;
    ...
    // Si llega aquÝ es que en el mundo hay items pero ninguno nos sirve, salimos de la tarea
    return null;
  }
  ...
  /**
   * Devuelve una lista de items en uso por aldeanos que NO sea el aldeano pasado como parßmetro
   *
   * @param iCitID
      ...
      if (iCitID == -1 || citizen.getID() != iCitID) {
        if (citizen.getCurrentCustomAction() != null
          ...
          if (citizen.getCurrentCustomAction().getQueueData().getItemIDCurrentPlace() != -1) {
            alItemsInUse.add(
                new Integer(
                    citizen.getCurrentCustomAction().getQueueData().getItemIDCurrentPlace()));
          }
          // Pick
          if (citizen.getCurrentCustomAction().getQueueData().getItemIDPick() != -1) {
            alItemsInUse.add(
                new Integer(citizen.getCurrentCustomAction().getQueueData().getItemIDPick()));
          }
        }
      }
    }
  ...
  public static boolean shouldFall(Cell[][][] cells, ItemManagerItem imi, Point3DShort p3dItem) {
    ...
    if (cell.isDigged()) {
      Cell cellDown = cells[p3dItem.x][p3dItem.y][p3dItem.z + 1]; // Si la de arriba se puede diggar siempre habrß
      // un z+1
      // Toca caer
      boolean bFall = true;
  ...
  /**
   * Comprueba que un item estÚ locked
   *
   * @param entity
   * @return
   */
  ...
  /**
   * Comprueba que un item estÚ operativo, locked y que no estÚ en la lista de items a evitar
   *
   * @param entity
   * @param locked
   * @param operative
   * @param alItemsToAvoid
   * @return true si un item estß operativo, locked y que no estß en la lista de items a evitar
   */
  private static boolean checkLockedOperativeAndItems(
      Entity entity,
      int locked,
      int operative,
      ArrayList<Integer> alItemsToAvoid) {
  ...
  /**
   * Comprueba que un item estÚ operative
   *
   * @param entity
   * @return
   */
  ...
  public void checkFall() {
    ...
    if (shouldFall(World.getCells(), imi, getCoordinates())) {
      ...
      // Si abajo hay un item, buscamos una celda adyacente, si no hay celda adyacente el item se
      // pierde
      if (itemDown != null) {
        ArrayList<Cell> alNeighbours = new ArrayList<Cell>();
        Cell cellTmp;
        for (short i = (getCoordinates().x > 0) ? (short) -1 : (short) 0,
            iMax = (getCoordinates().x < (World.MAP_WIDTH - 1)) ? (short) 1 : (short) 0; i <= iMax; i++) {
          for (short j = (getCoordinates().y > 0) ? (short) -1 : (short) 0,
              jMax = (getCoordinates().y < (World.MAP_HEIGHT - 1))
                  ? (short) 1
                  : (short) 0; j <= jMax; j++) {
            cellTmp = World.getCell(
                cellDown.getCoordinates().x + i,
                cellDown.getCoordinates().y + j,
                cellDown.getCoordinates().z);
            if (cellTmp.isEmpty()) {
              if (cellTmp.isDigged()
                  || Item.isCellAvailableForItem(
                      imi,
                      (short) (cellDown.getCoordinates().x + i),
                      (short) (cellDown.getCoordinates().y + j),
                      cellDown.getCoordinates().z,
                      false,
                      true,
                      false,
        ...
        if (alNeighbours.size() > 0) {
          ...
          init(
              cellRandom.getCoordinates().x,
              cellRandom.getCoordinates().y,
              cellRandom.getCoordinates().z);
          cellRandom.setEntity(this);
          // } else {
          // No cabe, se destruye
        }
      } else {
        if (Item.isCellAvailableForItem(
            imi,
            getCoordinates().x,
            getCoordinates().y,
            (short) (getCoordinates().z + 1),
            true,
            true)) {
  ...
  public void delete() {
    ...
    if (getAge() >= getMaxAge()) {
      ...
      if (imi.getMaxAgeItem() != null) {
        // Miramos si puede ir ahÝ
        if (isCellAvailableForItem(
            ItemManager.getItem(imi.getMaxAgeItem()),
            getX(),
            getY(),
            getZ(),
            true,
    ...
    // Si era un zoneMerger, zoneMergerUpDown, muro (splitter) o connector locked (splitter)
    // reasignamos los A*Zone ID
    checkConnectors(imi);

    // A±adimos el item de arriba de la celda (si tiene) a la lista de items posibles a caer
    // TambiÚn los items de los lados por si tienen glue
    ...
    if (imi.isBlockFluids()) {
      Game.getWorld()
          .addFluidCellToProcess(getCoordinates().x, getCoordinates().y, getCoordinates().z, true);
    }
  ...
  public void doHit(LivingEntity livingEntity, int damage) {
    ...
    if (bFirstHit || getHp() < hpToShowMessage) {
      MessagesPanel.addMessage(
          ...
          livingEntity.getLivingEntityData().getName()
              + Messages.getString("Item.1")
              + Messages.getString("LivingEntity.5")
              + ItemManager.getItem(getIniHeader()).getName(),
          ColorGL.RED,
          getCoordinates()); // $NON-NLS-1$//$NON-NLS-2$
    }
  ...
  public void init(int x, int y, int z) {
    ...
    if (imi.isContainer()) {
      Game.getWorld()
          .addContainer(
              getID()); // A±adimos los datos del container al mundo (si es que no existe ya)
    }
  ...
  public boolean nextTurn() {
    ...
    if (getMaxAge() > 0) {
      ...
      if (getAge() >= getMaxAge()) {
        // En teorÝa muere, pero vamos a mirar el maxAgeNeedsWater
            ...
            for (int y = -iRadius; y <= iRadius; y++) {
              for (int z = 0; z <= 1; z++) {
                if (Utils.isInsideMap(x + getX(), y + getY(), getZ() + z)) {
                  ...
                  if (terrain.getFluidType() == Terrain.FLUIDS_WATER
                      && terrain.getFluidCount() > 0) {
                    bWaterNear = true;
                    break forWater;
                  }
                }
        ...
        if (imi.getSpawn() == null) {
          ...
          if (!bItem) {
            if (LivingEntityManager.getItem(imi.getSpawn()) == null) {
              ...
              Log.log(
                  Log.LEVEL_ERROR,
                  Messages.getString("Item.9") + imi.getSpawn() + "]",
                  getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$
              return false;
            }
          }

          // Miramos que no tenga el mßximo al lado (s¾lo aplica con items)
          ...
          if (bItem && imi.getSpawnMaxItems() > 0) {
            ...
            // Mßximo de items (performance issue)
    ...
    if (getTrapCooldown() != -1) {
      if (isLocked() && isOperative()) {
        if (getTrapCooldown() > 0) {
          ...
          // Si llega a 0 le ponemos el grßfico de ready
          if (getTrapCooldown() == 0) {
              ...
              if (!LivingEntityManager.getItem(alLivings.get(i).getIniHeader()).isEvadeTraps()) {
                for (int e = 0; e < imi.getTrapEffects().size(); e++) {
                  if (hateData.isHate(alLivings.get(i))) {
                    alLivings
                        .get(i)
                        .addEffect(EffectManager.getItem(imi.getTrapEffects().get(e)), true);
                    salta = true;
                  }
                }
              }
    ...
    /*
     * // Fluids elevator if (getZ () > 0 && imi.isFluidsElevator ()) { Cell cell = World.getCell (getCoordinates
     * ()); if (cell.getTerrain ().getFluidCount () > 3) { // Fuerza 3 o mßs, sube 2 fluidos arriba Cell upperCell =
     * World.getCell (getX (), getY (), getZ () - 1); if (upperCell.getTerrain ().getFluidCount () <
     * Terrain.FLUIDS_COUNT_MAX) { World.moveSingleFluid (cell, upperCell, false, Point3DShort.getPoolInstance
     * (cell.getCoordinates ()), Point3DShort.getPoolInstance (upperCell.getCoordinates ()), new
     * ArrayList<Point3DShort> ());
     *
     * if (upperCell.getTerrain ().getFluidCount () < Terrain.FLUIDS_COUNT_MAX) { World.moveSingleFluid (cell,
     * upperCell, false, Point3DShort.getPoolInstance (cell.getCoordinates ()), Point3DShort.getPoolInstance
     * (upperCell.getCoordinates ()), new ArrayList<Point3DShort> ());
     *
     * if (upperCell.getTerrain ().getFluidCount () < Terrain.FLUIDS_COUNT_MAX) { World.moveSingleFluid (cell,
     * upperCell, false, Point3DShort.getPoolInstance (cell.getCoordinates ()), Point3DShort.getPoolInstance
     * (upperCell.getCoordinates ()), new ArrayList<Point3DShort> ()); }
  ...
  public void refreshDoorTile() {
    // Grßfico (por defecto es cerrada, en caso de opened cambiamos el grßfico)
  ...
  public void setLocked(boolean locked) {
    // Cambia de estado, cambiamos el numerito en las listas (primero a±adimos y luego sacamos, para
    // que no coincida con un chequeo del APS)
  ...
  public void setWallConnectorStatus(byte doorStatus, boolean setZoneIDs) {
    ...
    if (setZoneIDs) {
      if (doorStatus == FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED) {
        ...
        if (((iAnterior & FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED) == 0)
            && (doorStatus != FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED)) {
    ...
    if (getCoordinates().x != -1) {
      // Block fluids
      if (isDoorStatus(FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_OPENED)) {
        // Si se ha abierto miramos si es un item block y en ese caso seteamos para que fluya el
        // agua
        ItemManagerItem imi = ItemManager.getItem(getIniHeader());
        if (imi != null && imi.isBlockFluids()) {
          Game.getWorld()
              .addFluidCellToProcess(
                  getCoordinates().x,
                  getCoordinates().y,
                  getCoordinates().z,
                  true);
      ...
      // Lights
      Cell.generateLightsItemRemovedCellMined(
          getCoordinates().x,
          getCoordinates().y,
          getCoordinates().z,
          ItemManagerItem.MAX_LIGHT_RADIUS);
    }
  }
  ...
  private void checkConnectors(ItemManagerItem imi) {
    if (isOperative()
        ...
        && (imi.isZoneMergerUp()
            || imi.isWall()
            || (imi.isDoor() && isDoorStatus(FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED)))) {
      World.setRecheckASZID(
          true); // Ojo! No nos vale con un simple merge (caso escaleras que tienen un dig arriba
      // con -1)
    }
  }
        ...
        while (itLiving.hasNext()) {
          ...
          if (LivingEntityManager.getItem(le.getIniHeader()).getType() == LivingEntity.TYPE_ENEMY) {
            ...
            if (enemy.getCarryingData() != null
                && enemy.getCarryingData().getCarrying() != null
                && enemy.getCarryingData().getCarrying().getID() == getID()) {
              // Bingo
              bDeleteContainer = false;
              break;
            }
  ...
  /**
   * Crea un nuevo hijo en la coordenada pasada, comprueba que estÚ vacÝa
   *
   * @param x
   * @param y
   * @param z
