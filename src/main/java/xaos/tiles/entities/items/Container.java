...
public class Container implements Externalizable {
  ...
  public static SmartMenu createContainerMenu(int iContainerID) {
    ...
    SmartMenu smContainerMenu = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("Container.2"),
        null,
        null,
        sContainerID); // $NON-NLS-1$

    // A±adimos el enable todo/disable todo
    SmartMenu smAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("Container.3"),
        null,
        CommandPanel.COMMAND_CONTAINER_ENABLE_ALL,
        Integer.toString(iContainerID),
        null,
        null,
    ...
    smAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("Container.4"),
        null,
    ...
    for (int i = 0; i < fullType.getElements().size(); i++) {
      ...
      if (iIndex != -1) {
        ...
        if (iIndexSubtype != -1) {
          ...
          alElementsWithSubtype
              .get(iIndexSubtype)
              .addElement(
                  fullType.getElements().get(i),
                  container.getType().contains(fullType.getElements().get(i)));
        } else {
        ...
        // No tiene
        elementsWithoutSubtype.addElement(
            fullType.getElements().get(i),
            container.getType().contains(fullType.getElements().get(i)));
      }
    }
    ...
    for (int i = 0; i < alElementsWithSubtypeName.size(); i++) {
      SmartMenu smSubMenu = new SmartMenu(
          SmartMenu.TYPE_MENU,
          Type.getTypeName(alElementsWithSubtypeName.get(i)),
          smContainerMenu,
          null,
          sContainerID);
      ...
      smAux = new SmartMenu(
          ...
          CommandPanel.COMMAND_CONTAINER_ENABLE_ALL,
          Integer.toString(iContainerID),
          alElementsWithSubtypeName.get(i),
          null,
          Color.GREEN); // $NON-NLS-1$
      smAux.setIcon("iconenableall"); // $NON-NLS-1$
      ...
      if (imi != null && !imi.isContainer() && imi.isStackable()) {
        ...
        if (elementsWithoutSubtype.getAlElementsStatus().get(j).booleanValue()) {
          smAux = new SmartMenu(
              ...
              CommandPanel.COMMAND_CONTAINER_DISABLE_ITEM,
              Integer.toString(iContainerID),
              elementsWithoutSubtype.getAlElements().get(j),
              null,
              Color.GREEN);
          smAux.setIcon(imi.getIniHeader());
          smContainerMenu.addItem(smAux);
  ...
  private static void regenerateContainerPanelMenu(Container container, SmartMenu menuPile) {
    ...
    for (int i = 0; i < menuPile.getItems().size(); i++) {
      ...
      if (smAux.getType() == SmartMenu.TYPE_MENU) {
        ...
        // Item
        if (smAux.getCommand().equals(CommandPanel.COMMAND_CONTAINER_ENABLE_ITEM)
          ...
          if (container.getType().contains(smAux.getParameter2())) {
            // Existe, puÚs lo ponemos para disable
      ...
      for (int t = 0; t < globalType.getElements().size(); t++) {
        ...
        if (imi.getType() != null && imi.getType().equals(sSubType)) {
          if (!getType().contains(globalType.getElements().get(t))) {
            getType()
                .addElement(globalType.getElements().get(t), globalType.getElementNames().get(t));
          }
        }
      }
  ...
  public void fillMenu(SmartMenu sm, String sItemName) {
    sm.addItem(
        new SmartMenu(
            SmartMenu.TYPE_ITEM,
            Messages.getString("Container.2") + sItemName,
            null,
            CommandPanel.COMMAND_CONTAINER_MANAGE,
            Integer.toString(getItemID()),
            null,
            null,
            Color.GREEN)); // $NON-NLS-1$
    ...
    if (isLockedToCopy()) {
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_TEXT,
              "(" + Messages.getString("Stockpile.2") + ")",
              null,
              null,
      ...
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_TEXT,
              "(" + Messages.getString("Stockpile.9") + ")",
              null,
              null,
              null)); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
  ...
  /**
   * Indica si el item pasado cabe en el container. Chequea que el item NO sea un container, que el item sea stackable
   * y que los tipos coincidan
   *
   * @param item
   * @return
   */
  public boolean itemAllowed(Item item) {
    ...
    // Si llega aquÝ es que no es container y es stackable, miramos si cabe
  ...
  public boolean refreshTransients() {
    ...
    if (!itemOnWorld) {
      // Item no estß en el mundo, miramos el carrying de los citizens
      Citizen citizen;
    ...
    for (int i = 0; i < itemsInside.size(); i++) {
      ...
      Item.addItem(
          itemsInside.get(
              i)); // Para que sume 1 al n·mero de items, ya que el item no existe realmente
  ...
  public Item removeBadItem() {
    ...
    for (int i = 0; i < itemsInside.size(); i++) {
      ...
      if (!getType().contains(imi.getIniHeader())) {
        ...
        Item.removeItem(
            item); // Para que reste 1 al n·mero de items, ya que el item no existe realmente
        World.setTownValue(World.getTownValue() - PricesManager.getPrice(item));
        setWrongItemsInside(hasWrongItems());
  ...
  /**
   * Elimina un item de comida del container y lo devuelve. Pillamos el de mßs value
   *
   * @param iID
   *            ID del item a devolver
  ...
  /**
   * Elimina un item del container y lo devuelve basßndose en un iniheader
   *
   * @param iID
   *            ID del item a devolver
   * @return item del container o null si no lo encuentra
  ...
  /**
   * Elimina un item del container y lo devuelve basßndose en una lista de iniheaders
   *
   * @param iID
   *            ID del item a devolver
   * @return item del container o null si no lo encuentra
  ...
  /**
   * Indica si hay items que no deben ir ahÝ
   *
   * @return
   */
