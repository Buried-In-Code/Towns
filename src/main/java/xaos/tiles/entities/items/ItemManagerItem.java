...
public class ItemManagerItem {
  ...
  public static final String CHARGES_INFINITE = "INFINITE"; // $NON-NLS-1$

  public static final String LIGHT_NONE = "NONE"; // $NON-NLS-1$
  public static final String LIGHT_FULL = "FULL"; // $NON-NLS-1$
  public static final String LIGHT_HALF = "HALF"; // $NON-NLS-1$

  public static final int LIGHT_I_NONE = 0;
  public static final int LIGHT_I_FULL = 1;
  public static final int LIGHT_I_HALF = 2;
  ...
  // Habitat
  private ArrayList<Integer> habitat; // Lista de terrenos donde puede crecer (o aparecer), se usa en los child (ver
  // más
  // abajo). (Ej: grass, sand, ...)
  private ArrayList<String> habitatString; // Lista de terrenos donde puede crecer (o aparecer), se usa en los child
  // (ver
  // más abajo). (Ej: grass, sand, ...)
  ...
  private int spawnMaxItems; // Máximo de hijos alrrededor. Sólo aplica con items. (ej: Si ya tiene 3
  // arbustos alrededor no "suelta" otro)
  ...
  // AlwaysOperative
  private boolean alwaysOperative; // Indica si cuando se construye ya está operativo o tienen que ponerse en
  // algún sitio para que lo esté
  ...
  private String damageModifier; // Modificador de dańo
      ...
      if (habitats != null) {
        for (int i = 0; i < habitats.size(); i++) {
          if (!this.habitat.contains(habitats.get(i))) {
            // getHabitat ().add (new Integer (TerrainManager.getItem (habitats.get
            // (i)).getTerrainID ()));
  ...
  public int getLightIntValue(String sLight) throws Exception {
    if (sLight == null || sLight.trim().isEmpty()) {
      ...
      if (sLight.trim().equalsIgnoreCase(LIGHT_FULL)) {
        ...
        throw new Exception(
            Messages.getString("ItemManagerItem.15")
                + " ["
                + sLight
  ...
  /**
   * @param sFloorWalkSpeed
   *            the floorWalkSpeed to set (dice)
   */
  ...
  /**
   * @param floorWalkSpeed
   *            the floorWalkSpeed to set
   */
    ...
    if (foodEffects != null) {
      ...
      for (int i = 0; i < foodEffects.size(); i++) {
        if (EffectManager.getItem(foodEffects.get(i)) == null) {
          throw new Exception(
              Messages.getString("ItemManagerItem.11")
                  + foodEffects.get(i)
                  + "]"); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
  ...
  public void setHappiness(String sHappiness) {
    if (sHappiness == null || sHappiness.trim().isEmpty()) {
      ...
      try {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemManagerItem.0") + sHappiness + "]",
  ...
  public void setLightRadius(String sLightRadius) throws Exception {
    if (sLightRadius == null || sLightRadius.trim().isEmpty()) {
      ...
      try {
        ...
        throw new Exception(
            Messages.getString("ItemManagerItem.14")
                + " ["
                + sLightRadius
                + "]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      }
  ...
  public void setMaxAgeNeedsItemsRadius(String sMaxAgeNeedsItemsRadius) {
    if (sMaxAgeNeedsItemsRadius == null || sMaxAgeNeedsItemsRadius.isEmpty()) {
      ...
      try {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemManagerItem.18") + " [" + sMaxAgeNeedsItemsRadius + "]",
  ...
  public void setMaxAgeNeedsWaterRadius(String sMaxAgeNeedsWaterRadius) {
    if (sMaxAgeNeedsWaterRadius == null || sMaxAgeNeedsWaterRadius.isEmpty()) {
      ...
      try {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemManagerItem.12") + sMaxAgeNeedsWaterRadius + "]",
            getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public void setRangedAmmo(String rangedAmmo) throws Exception {
    if (ranged) {
      if (rangedAmmo == null || rangedAmmo.trim().isEmpty()) {
        throw new Exception(
            Messages.getString("ItemManagerItem.2")
                + getIniHeader()
                + "]"); //$NON-NLS-1$ //$NON-NLS-2$
      }
    }
  ...
  public void setSpeedUpPCT(String sSpeedUpPCT) {
    if (sSpeedUpPCT == null || sSpeedUpPCT.trim().isEmpty()) {
      ...
      try {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ItemManagerItem.1")
                + sSpeedUpPCT
                + Messages.getString("ItemManagerItem.3"),
            getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$
        setSpeedUpPCT(100);
      }
    }
