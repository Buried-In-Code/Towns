...
public class BuildingManager {
  ...
  public static void loadItems() {
    ...
    // Cargar de fichero
    loadXMLBuildings(
        Towns.getPropertiesString("DATA_FOLDER") + "buildings.xml",
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "buildings.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
  ...
  private static void loadXMLBuildings(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los buildings (BuildingManagerItem) a la hash
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            iWidth = Short.parseShort(
                UtilsXML.getChildValue(node.getChildNodes(), "width")); // $NON-NLS-1$
            iheight = Short.parseShort(
                UtilsXML.getChildValue(node.getChildNodes(), "height")); // $NON-NLS-1$
          ...
          if (bModChangingValues) {
            ...
            item.setGroundData(
                UtilsXML.getChildValue(node.getChildNodes(), "groundData")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            String sAux = UtilsXML.getChildValue(
                node.getChildNodes(),
                "canBeBuiltUnderground"); // $NON-NLS-1$
            if (sAux != null) {
              item.setCanBeBuiltUnderground(sAux);
            }
          } else {
            item.setCanBeBuiltUnderground(
          ...
          if (bModChangingValues) {
            ...
            item.setMustBeBuiltOver(
                UtilsXML.getChildValues(node.getChildNodes(), "mustBeBuiltOver")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setMineTerrain(
                UtilsXML.getChildValue(node.getChildNodes(), "mineTerrain")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            ArrayList<String> alAux2 = UtilsXML.getChildValues(
                node.getChildNodes(),
                "prerequisiteFriendly"); // $NON-NLS-1$
            ...
            item.setPrerequisitesFriendly(
                UtilsXML.getChildValues(
                    node.getChildNodes(),
                    "prerequisiteFriendly")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setAutomatic(
                UtilsXML.getChildValue(node.getChildNodes(), "automatic")); // $NON-NLS-1$
          }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("BuildingManager.5")
              + sXMLName
              + Messages.getString("BuildingManager.6")
              + e.toString()
              + "]",
          "BuildingManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
}
