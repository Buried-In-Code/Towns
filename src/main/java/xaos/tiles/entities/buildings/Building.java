...
public class Building extends Entity implements Externalizable {
  ...
  /**
   * Son los prerequisitos para construirse, a medida que los aldeanos los traigan irá desapareciendo Cuando lleguen a
   * 0 el edificio pasa a estar operativo
   */
   ...
   *
   * @param item
   *            Objeto con las características del edificio
    ...
    if (cell.hasBuilding()) {
      ...
      if (building != null) {
        if (!building.isOperative()
            && (building.getPrerequisites().size() > 0
                || building.getPrerequisitesLiving().size() > 0)) {
          // Mostramos los prerequisitos si aún no se ha construido
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  Messages.getString("Building.0"),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          if (building.getPrerequisites() != null) {
            for (int i = 0; i < building.getPrerequisites().size(); i++) {
              ...
              for (int j = 0; j < building.getPrerequisites().get(i).length; j++) {
                iNumInWorld += Item.getNumItems(
                    building.getPrerequisites().get(i)[j],
                    false,
                    Game.getWorld().getRestrictHaulEquippingLevel());

                if (j == 0) {
                  sMats = ItemManager.getItem(
                      UtilsIniHeaders.getStringIniHeader(
                          building.getPrerequisites().get(i)[j]))
                      .getName();
                } else {
                  sMats += Messages.getString("SmartMenu.3")
                      + ItemManager.getItem(
                          UtilsIniHeaders.getStringIniHeader(
                              building.getPrerequisites().get(i)[j]))
                          .getName();
              ...
              if (sMats != null) {
                if (iNumInWorld > 0) {
                  sm.addItem(
                      new SmartMenu(
                          SmartMenu.TYPE_TEXT,
                          Messages.getString("Building.1") + sMats,
                          null,
                          null,
                          null,
                          null,
          ...
          if (building.getPrerequisitesLiving() != null) {
            for (int i = 0; i < building.getPrerequisitesLiving().size(); i++) {
              ...
              for (int j = 0; j < building.getPrerequisitesLiving().get(i).length; j++) {
                iNumInWorld += LivingEntity.getNumLivings(
                    UtilsIniHeaders.getStringIniHeader(
                        building.getPrerequisitesLiving().get(i)[j]),
                    true);

                if (j == 0) {
                  sMats = LivingEntityManager.getItem(
                      UtilsIniHeaders.getStringIniHeader(
                          building.getPrerequisitesLiving().get(i)[j]))
                      .getName();
                } else {
        ...
        if (building.isOperative()) {
          ...
          if (building.isNonStop()) {
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    Messages.getString("Building.9"),
                    null,
                    CommandPanel.COMMAND_TURN_OFF_NONSTOP,
                    null,
                    null,
                    cell.getBuildingCoordinates().toPoint3D())); // $NON-NLS-1$
          } else {
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    Messages.getString("Building.10"),
                    null,
        ...
        if (building.isOperative() && !bmi.isAutomatic()) {
          ...
          if (alIMI.size() > 0) {
            // Ańadir objetos para crear
            SmartMenu smBuildingAdd = new SmartMenu(
                SmartMenu.TYPE_MENU,
                Messages.getString("Building.2"),
                sm,
                null,
                null); // $NON-NLS-1$
            ...
            for (int i = 0; i < alIMI.size(); i++) {
              ...
              smBuildingAdd.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_ITEM,
                      Messages.getString("Building.3") + sName,
                      null,
                      CommandPanel.COMMAND_CREATE_IN_A_BUILDING,
                      alIMI.get(i).getIniHeader(),
                      null,
                      cell.getBuildingCoordinates().toPoint3D())); // $NON-NLS-1$
            }
          ...
          // Si tiene cosas en cola ańadimos el menú de quitarlas
          if (building.hasItemsInQueue()) {
            // Remove tasks
            SmartMenu smBuildingRemove = new SmartMenu(
                SmartMenu.TYPE_MENU,
                Messages.getString("Building.5"),
                sm,
                null,
                null); // $NON-NLS-1$
            ItemManagerItem imi;
            for (int i = 0; i < building.getItemQueue().size(); i++) {
              ...
              smBuildingRemove.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_ITEM,
                      Messages.getString("Building.6")
                          + imi.getName()
                          + Messages.getString("Building.7"),
                      null,
                      CommandPanel.COMMAND_REMOVE_BUILDING_TASK,
                      building.getItemQueue().get(i).getIniHeader(),
                      Integer.toString(i),
                      cell.getBuildingCoordinates().toPoint3D())); // $NON-NLS-1$ //$NON-NLS-2$
            }
        ...
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("Building.8") + bmi.getName(),
                null,
                CommandPanel.COMMAND_DESTROY_BUILDING,
                null,
                null,
                p3dEntrance.toPoint3D(),
  ...
  /** Recorre todas las celdas del mundo y lanza un init por cada building encontrado */
  ...
  public void delete() {
    ...
    for (int i = 0; i < World.getBuildings().size(); i++) {
      ...
      if (building
          .getCoordinates()
          .equals(World.getCell(getCoordinates()).getBuildingCoordinates())) {
        iIndex = i;
        break;
      }
    }
      ...
      for (int x = building.getX(); x < (building.getX() + bmi.getWidth()); x++) {
        for (int y = building.getY(); y < (building.getY() + bmi.getHeight()); y++) {
          char groundDataChar = bmi.getGroundData()
              .charAt((y - building.getY()) * bmi.getWidth() + (x - building.getX()));
  ...
  public boolean nextTurn() {
    ...
    if (bSpawn) {
      ...
      World.addNewLiving(
          lemi.getIniHeader(),
          lemi.getType(),
          World.getCell(p3dEntrance).isDiscovered(),
          p3dEntrance.x,
          p3dEntrance.y,
          p3dEntrance.z,
          true);
