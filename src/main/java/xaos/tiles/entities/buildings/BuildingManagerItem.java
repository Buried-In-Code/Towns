...
public class BuildingManagerItem {
  ...
  public void setGroundData(String groundData) {
    if (groundData == null
        || groundData.isEmpty()
        || groundData.length() != (getWidth() * getHeight())
        || groundData.indexOf(Building.GROUND_ENTRANCE) == -1) {
      if (groundData != null && groundData.length() > 0) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("BuildingManagerItem.0")
                + getIniHeader()
                + Messages.getString("BuildingManagerItem.1"),
    ...
    for (int i = 0; i < prerequisites.size(); i++) {
      ...
      if (aInts != null) {
        for (int items = 0; items < aInts.length; items++) {
          ...
          if (imi == null) {
            throw new Exception(
                Messages.getString("BuildingManagerItem.2")
                    + sHeader
                    + "]"); //$NON-NLS-1$ //$NON-NLS-2$
          }
        }
    ...
    for (int i = 0; i < prerequisitesFriendly.size(); i++) {
      ...
      if (aInts != null) {
        for (int items = 0; items < aInts.length; items++) {
          ...
          if (lemi == null) {
            throw new Exception(
                Messages.getString("BuildingManagerItem.4")
                    + sHeader
                    + "]"); //$NON-NLS-1$ //$NON-NLS-2$
          }
        }
