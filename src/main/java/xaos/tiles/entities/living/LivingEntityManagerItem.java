...
public class LivingEntityManagerItem {
  ...
  public LivingEntityData getRandom() {
    // Se ponen aquí las cosas que hay que lanzar dados, las cosas fijas no hace falta, siempre se
    // pueden consultar de un LivingEntityManagerItem
  ...
  public void setCaravan(String sCaravan) throws Exception {
    if (sCaravan != null) {
      if (CaravanManager.getItem(sCaravan) == null) {
        throw new Exception(
            Messages.getString("LivingEntityManagerItem.8")
                + sCaravan
                + "]"); //$NON-NLS-1$ //$NON-NLS-2$
      }
    }
    ...
    if (effects != null) {
      for (int i = 0; i < effects.size(); i++) {
        if (EffectManager.getItem(effects.get(i)) == null) {
          throw new Exception(
              Messages.getString("LivingEntityManagerItem.2")
                  + effects.get(i)
                  + "]"); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
  ...
  public void setFoodNeededDieTurns(String sFoodNeededDieTurns) throws Exception {
    if (sFoodNeededDieTurns == null || sFoodNeededDieTurns.isEmpty()) {
      ...
      try {
        ...
        throw new Exception(
            Messages.getString("LivingEntityManagerItem.13")
                + "["
                + sFoodNeededDieTurns
  ...
  public void setFxDead(String fxDead) throws Exception {
    if (fxDead == null || fxDead.trim().isEmpty()) {
      ...
      if (!UtilsAL.exists(this.fxDead)) {
        throw new Exception(
            Messages.getString("LivingEntityManagerItem.12")
                + " ["
                + this.fxDead
                + "]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      }
  ...
  public void setHabitatAsString(ArrayList<String> habitat) throws Exception {
    if (habitat != null) {
      ...
      for (int i = 0; i < habitat.size(); i++) {
        ...
        if (tmi == null) {
          throw new Exception(
              Messages.getString("LivingEntityManagerItem.9")
                  + habitat.get(i)
                  + "]"); //$NON-NLS-1$ //$NON-NLS-2$
        }
  ...
  public void setMaxHungryTurns(String maxHungryTurns) throws Exception {
    ...
    if (Utils.launchDice(maxHungryTurns) == 0) {
      throw new Exception(
          Messages.getString("LivingEntityManagerItem.1")
              + maxHungryTurns
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
  ...
  public void setMaxSleepTurns(String maxSleepTurns) throws Exception {
    ...
    if (Utils.launchDice(maxSleepTurns) == 0) {
      throw new Exception(
          Messages.getString("LivingEntityManagerItem.5")
              + maxSleepTurns
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    if (steal != null) {
      ...
      for (int i = 0; i < alItems.size(); i++) {
        if (ItemManager.getItem(alItems.get(i)) == null) {
          throw new Exception(
              Messages.getString("LivingEntityManagerItem.11")
                  + " ["
                  + alItems.get(i)
                  + "]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
  ...
  public void setType(String type) throws Exception {
    if (type == null) {
      ...
      if (type.equalsIgnoreCase(TYPE_CITIZEN)) {
        ...
        throw new Exception(
            Messages.getString("LivingEntityManagerItem.10")
                + type
                + "]"); //$NON-NLS-1$ //$NON-NLS-2$
      }
    }
