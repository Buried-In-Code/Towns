...
public class Citizen extends LivingEntity implements Externalizable {
  ...
  public Citizen(String iniHeader) {
    ...
    getCitizenData()
        .setHungry(
            getCitizenData().getMaxHungry()
                * 6); // Al crearse los aldeanos llegan con fuerzas, por eso el multiplicador
          ...
          if (lemi.getType() == TYPE_CITIZEN) {
            ...
            if (TownsProperties.DEBUG_MODE) {
              sm.addItem(
                  new SmartMenu(
                      ...
                      "Hungry "
                          ...
                          + ", HE "
                          + citizen.getCitizenData().getHungryEating(),
                      null,
                      null,
              ...
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Sleep " + citizen.getCitizenData().getSleep(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Happy " + citizen.getCitizenData().getHappiness(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Happy work counter " + citizen.getCitizenData().getHappinessWorkCounter(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Happy idle counter " + citizen.getCitizenData().getHappinessIdleCounter(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "LOS "
                          + citizen.getLivingEntityData().getLOSCurrent()
                          + " / "
                          + citizen.getLivingEntityData().getLOSBase(),
                      null,
                      null,
                      null)); // $NON-NLS-1$ //$NON-NLS-2$
              if (citizen.getCurrentTask() != null) {
                sm.addItem(
                    new SmartMenu(
                        SmartMenu.TYPE_TEXT,
                        "Task ID " + citizen.getCurrentTask().getTask(),
                        null,
                        null,
                        null)); // $NON-NLS-1$
              ...
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Waiting " + citizen.isWaitingForPath(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
            }
            ...
            if (citizen.getSoldierData().isSoldier()) {
              if (citizen.getSoldierData().isSoldier()
                ...
                sm.addItem(
                    new SmartMenu(
                        SmartMenu.TYPE_TEXT,
                        citizen.getCitizenData().getFullName()
                            + Messages.getString("Citizen.6")
                            + " ("
                            + Game.getWorld()
                                .getSoldierGroups()
                                .getGroup(citizen.getSoldierData().getGroup())
                                .getName()
                            + ")",
                        null,
                        null,
                        ...
                        null,
                        p3d.toPoint3D(),
                        Color.YELLOW)); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
              } else {
                sm.addItem(
                    new SmartMenu(
                        SmartMenu.TYPE_TEXT,
                        ...
                        null,
                        p3d.toPoint3D(),
                        Color.YELLOW)); // $NON-NLS-1$
              }
            } else {
              if (citizen.getCitizenData().getGroupID() != -1
                  && Game.getWorld()
                      .getCitizenGroups()
                ...
                sm.addItem(
                    new SmartMenu(
                        ...
                        citizen.getCitizenData().getFullName()
                            ...
                            + Game.getWorld()
                                .getCitizenGroups()
                                .getGroup(citizen.getCitizenData().getGroupID())
                                .getName()
                            + ")",
                        null,
                        null,
                        ...
                        null,
                        p3d.toPoint3D(),
                        Color.YELLOW)); // $NON-NLS-1$ //$NON-NLS-2$
              } else {
                sm.addItem(
                    new SmartMenu(
                        SmartMenu.TYPE_TEXT,
            ...
            if (citizen.getCurrentTask() != null) {
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      Messages.getString("Citizen.7") + citizen.getCurrentTask(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
            }
            ...
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_TEXT,
                    citizen.getLivingEntityData().toString(),
                    null,
                    null,
                    null));
            ...
            if (bMilitaryItems) {
              ...
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_ITEM,
                      Messages.getString("Citizen.31") + citizen.getCitizenData().getFullName(),
                      sm,
              ...
              // Equip
              SmartMenu smEquip = new SmartMenu(
                  SmartMenu.TYPE_MENU,
                  Messages.getString("Citizen.13") + citizen.getCitizenData().getFullName(),
                  sm,
                  null,
                  null); // $NON-NLS-1$
              SmartMenu smAux = createEquipMenu(
                  MilitaryItem.LOCATION_HEAD,
                  Messages.getString("Citizen.14"),
                  alHead,
                  smEquip,
                  citizen); // $NON-NLS-1$
              if (smAux != null) {
              ...
              smAux = createEquipMenu(
                  MilitaryItem.LOCATION_BODY,
                  Messages.getString("Citizen.15"),
                  alBody,
                  smEquip,
                  citizen); // $NON-NLS-1$
              if (smAux != null) {
              ...
              smAux = createEquipMenu(
                  MilitaryItem.LOCATION_LEGS,
                  Messages.getString("Citizen.16"),
                  alLegs,
                  smEquip,
                  citizen); // $NON-NLS-1$
              if (smAux != null) {
              ...
              smAux = createEquipMenu(
                  MilitaryItem.LOCATION_FEET,
                  Messages.getString("Citizen.17"),
                  alFeet,
                  smEquip,
                  citizen); // $NON-NLS-1$
              if (smAux != null) {
              ...
              smAux = createEquipMenu(
                  MilitaryItem.LOCATION_WEAPON,
                  Messages.getString("Citizen.18"),
                  alWeapon,
                  smEquip,
                  citizen); // $NON-NLS-1$
              if (smAux != null) {
            ...
            // Desequipar
            itemMetido = false;
            SmartMenu smUnEquip = new SmartMenu(
                SmartMenu.TYPE_MENU,
                Messages.getString("Citizen.20") + citizen.getCitizenData().getFullName(),
                sm,
                null,
                null); // $NON-NLS-1$
            if (citizen.getEquippedData().isWearing(MilitaryItem.LOCATION_HEAD)) {
              smUnEquip.addItem(
                  new SmartMenu(
                      ...
                      citizen.getEquippedData().getHead().getItemTextColor())); // $NON-NLS-1$
              itemMetido = true;
            }
            if (citizen.getEquippedData().isWearing(MilitaryItem.LOCATION_BODY)) {
              smUnEquip.addItem(
                  new SmartMenu(
                      ...
                      citizen.getEquippedData().getBody().getItemTextColor())); // $NON-NLS-1$
              itemMetido = true;
            }
            if (citizen.getEquippedData().isWearing(MilitaryItem.LOCATION_LEGS)) {
              smUnEquip.addItem(
                  new SmartMenu(
                      ...
                      citizen.getEquippedData().getLegs().getItemTextColor())); // $NON-NLS-1$
              itemMetido = true;
            }
            if (citizen.getEquippedData().isWearing(MilitaryItem.LOCATION_FEET)) {
              smUnEquip.addItem(
                  new SmartMenu(
                      ...
                      citizen.getEquippedData().getFeet().getItemTextColor())); // $NON-NLS-1$
              itemMetido = true;
            }
            if (citizen.getEquippedData().isWearing(MilitaryItem.LOCATION_WEAPON)) {
              smUnEquip.addItem(
                  new SmartMenu(
                      ...
                      citizen.getEquippedData().getWeapon().getItemTextColor())); // $NON-NLS-1$
              itemMetido = true;
            }
            ...
            if (citizen.getSoldierData().isSoldier()) {
              ...
              // Change state
              SmartMenu smChangeState = new SmartMenu(
                  SmartMenu.TYPE_MENU,
                  Messages.getString("Citizen.12"),
                  sm,
                  null,
                  null); // $NON-NLS-1$
              ...
              for (int j = 0; j < lemi.getActions().size(); j++) {
                ...
                sm.addItem(
                    new SmartMenu(
                        ...
                        ami.getId(),
                        Integer.toString(citizen.getID()))); // $NON-NLS-1$
              }
              sm.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
            }
          }
  ...
  /**
   * Comprueba que una comida no esté "pillada" por algún aldeano (pillada quiero decir que no haya un aldeano con
   * tarea de EAT caminando hacia ella)
   *
   * @param p3dItem
   *            Coordenada de la comida
   * @param citID
   *            Aldeano que hace la comprobación
   * @return
   */
  ...
  private static SmartMenu createEquipMenu(
      int location,
      String sLocation,
      ArrayList<MilitaryItem> alMI,
      SmartMenu parent,
      Citizen citizen) {
    ItemManagerItem imi;
    ...
    if (alMISorted.size() > 0) {
      ...
      for (int it = 0; it < alMISorted.size(); it++) {
        smEM.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                alMISorted.get(it).getExtendedTilename(),
                parent,
                CommandPanel.COMMAND_WEAR,
                Integer.toString(citizen.getID()),
                Integer.toString(alMISorted.get(it).getID()),
                alMISorted.get(it).getCoordinates().toPoint3D(),
    ...
    if (citizen.getCurrentTask() != null && citizen.getCurrentTask().getTask() == Task.TASK_EAT) {
      ...
      if (p3dFull != null) {
        ...
        if (p3d.equals(p3dItem)) {
          // Misma coordenada, quizá es un container y tiene más comida, así que miraremos que este
          // aldeano no tenga ya comida en el carrying
  ...
  public void deleteJobGroupsStuff() {
    ...
    Game.getWorld()
        .getCitizenGroups()
        .removeCitizenFromGroup(getID(), getCitizenData().getGroupID());
  }
  ...
  /**
   * Retorna el % de velocidad a aplicar a la velocidad actual basado en el hambre y la felicidad
   *
   * @return el % de velocidad a aplicar a la velocidad actual basado en el hambre y la felicidad
   */
  public int getMalusSpeedPCT() {
    ...
    if (getCitizenData().getHungry() == 0 && getCitizenData().getHungryEating() < 0) {
      if (getCitizenData().getHungryEating() < -(World.TIME_MODIFIER_DAY)) {
        ...
        int iHE = World.TIME_MODIFIER_DAY
            + getCitizenData()
                .getHungryEating(); // Es una suma pero el hungryEating es negativo
   ...
   *            El atacante
   * @param bHitted
   *            Indica si le ha pegado o sólo lo ha intentado
   */
    ...
    for (int i = 0; i < World.getSoldierIDs().size(); i++) {
      ...
      if (cit.getSoldierData().getState() == SoldierData.STATE_GUARD) {
        if (cit.getFocusData().getEntityID() == -1) {
          ...
          cit.getFocusData()
              .setEntityType(LivingEntityManager.getItem(le.getIniHeader()).getType());
          setFighting(true);
        }
  ...
  public void resetTaskIndexes() {
    if (getCurrentTask() != null && getCurrentTask().getTask() == Task.TASK_CUSTOM_ACTION) {
      ...
      // Custom action, si no está terminada la ponemos en la lista de tareas para que lo haga otro
      // aldeano
  ...
  public void setCurrentCustomAction(Action currentCustomAction) {
    ...
    if (this.currentCustomAction != null
        && getCurrentTask() != null
        && getCurrentTask().getParameter() != null) {
      ...
      if (ami != null) {
        ...
        // Primero de todo vamos a bloquear el primer <move> para que no vayan 5 aldeanos a hacer
        // una tarea si sólo hay 2 benches (por ejemplo)
        ...
        if (getCurrentCustomAction().getQueueData().getItemIDCurrentPlace() == -1) {
          ...
          if (firstMove != null) {
            ...
            Point3DShort p3dMoveCoordenadas = Item.searchItem(
                false,
                getCoordinates(),
                aiMoves,
                true,
                Item.SEARCH_DOESNTMATTER,
                Item.SEARCH_TRUE,
                alItemsInUse,
                true,
                false,
  ...
  protected void checkEatSleep() {
    ...
    if (getCurrentTask() == null
        || getCurrentTask().getTask() == Task.TASK_BUILD
        || getCurrentTask().getTask() == Task.TASK_HEAL
        || getCurrentTask().getTask() == Task.TASK_HAUL
      ...
      if (getCitizenData().getSleep() == 0) {
        // Tiene sueńo, que deje lo que tiene encima y lo sacamos de la tarea actual
        if (getCarrying() != null) {
          dropCarryingItem();
          return;
        }
  ...
  protected void checkHealthPoints() {
    if (getLivingEntityData().getHealthPoints() <= getLivingEntityData().getHealthPointsMAXCurrent() / 3) { // <=
      ...
      // getCurrentTask ().getTask () != Task.TASK_SLEEP && getCurrentTask ().getTask () !=
      // Task.TASK_HEAL && getCurrentTask ().getTask () != Task.TASK_FIGHT)) {
      if (getCurrentTask() == null) {
        // Antes de sacarlo de la tarea y meterla la nueva miramos si existe un hospital con
        // casillas libres
        ...
        for (int i = 0; i < alZones.size(); i++) {
          ...
          if (zmi != null
              && zmi.getType() == ZoneManagerItem.TYPE_HOSPITAL
              && alZones.get(i).isOperative()) {
            // Hospital encontrado, miramos si hay celda posible
            if (Zone.getFreeCellAtRandom(
                alZones.get(i),
  ...
  protected void doTasksStuff() {
    if (getCurrentTask() != null) {
      ...
      // Quitamos los índices de tarea, no fuera que la tarea la haya terminado otro aldeano y éste
      // aún esté intentando hacerla
      resetTaskIndexes();
  ...
  /**
   * Resta los contadores de comida y sueńo, si muere de hambre retorna true, también resta happiness (cada hora) en
   * caso de hambre
   */
      ...
      if (getCurrentTask() == null || (getCurrentTask().getTask() != Task.TASK_EAT)) {
        ...
        if (getCitizenData().getHungryEating() < -(2 * World.TIME_MODIFIER_DAY)) { // 2 dias sin comer, muere
          MessagesPanel.addMessage(
              ...
              getCitizenData().getFullName() + Messages.getString("Citizen.8"),
              ColorGL.RED,
              getCoordinates(),
              getID()); // $NON-NLS-1$
          return true;
  ...
  private boolean doAutoEquipInternal(ArrayList<MilitaryItem> alList, int location) {
    ...
    for (int i = 0; i < alList.size(); i++) {
      ...
      if (cell.getAstarZoneID() == citizenCell.getAstarZoneID()) {
        // Item bueno, lo ańadimos a la lista para luego hacer un random
        alIndexes.add(i);
      }
    }
    ...
    if (alIndexes.size() > 0) {
      ...
      getCurrentTask()
          .setParameter2(Integer.toString(alList.get(alIndexes.get(i).intValue()).getID()));
  ...
  private void doAutoEquipTask() {
    ...
    // Pillaremos los de mayor nivel para cada zona (cabeza, cuerpo, ...) (si el aldeano puede
    // llegar al area, claro)
    Integer[] aItems = World.getItems().keySet().toArray(new Integer[0]);
    ArrayList<MilitaryItem> alHead = new ArrayList<MilitaryItem>(); // Head
    ArrayList<MilitaryItem> alBody = new ArrayList<MilitaryItem>(); // Body
    ArrayList<MilitaryItem> alLegs = new ArrayList<MilitaryItem>(); // Legs
  ...
  /**
   * Hace las cosas necesarias para ejecutar una tarea de construcción (buscar materiales, llevarlos, ...)
   *
   * @param task
   */
  private void doBuildingTask() {
    ...
    if (building == null) {
      ...
      Log.log(
          ...
          Messages.getString("Citizen.4")
              ...
              + hp.getHotPoint().z
              + "]",
          getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      getCurrentTask().setFinished(true);
      Game.getWorld().getTaskManager().removeCitizen(this);
      return;
    }
    ...
    if (alPrerequisites != null && alPrerequisites.size() > 0) {
      // Primero de todo miramos si llevamos un material bueno encima y estamos en la casilla del
      // edificio
      if (getCitizenData().getCarryingData().getCarrying() != null) {
        // Llevamos algo encima, miramos si va bien para la construcción

        int iIndex = UtilsIniHeaders.contains(
            alPrerequisites,
      ...
      if (bHayItems) {
        ...
        for (int i = 0; i < alPrerequisites.size(); i++) {
          p3dItem = Item.searchItem(
              false,
              getCoordinates(),
              alPrerequisites.get(i),
              true,
              Item.SEARCH_FALSE,
              Item.SEARCH_DOESNTMATTER,
              null,
      ...
      if (alPrerequisitesLiving != null && alPrerequisitesLiving.size() > 0) {
        // Primero de todo miramos si llevamos un living bueno encima y estamos en la casilla del
        // edificio
        if (getCitizenData().getCarryingData().getCarryingLiving() != null) {
          // Llevamos algo encima, miramos si va bien para la construcción
          int iIndex = UtilsIniHeaders.contains(
              alPrerequisitesLiving,
          ...
          if (iIndex != -1) {
            ...
            if (x == hp.getHotPoint().x && y == hp.getHotPoint().y && z == hp.getHotPoint().z) {
              ...
              LivingEntityManagerItem lemi = LivingEntityManager.getItem(
                  getCitizenData().getCarryingData().getCarryingLiving().getIniHeader());
        ...
        if (x != hp.getHotPoint().x || y != hp.getHotPoint().y || z != hp.getHotPoint().z) {
          ...
          // Nos esperamos unos turnos en el caso de que haya algun citizen encima para que no muera
          // aplastado
          building.setOperative(
              true,
              false); // temporalmente operativo para llamar al Citizen.iscellallowed
          ...
          if (bAldeanoEncima) {
            ...
            Game.updateTutorialFlow(
                TutorialTrigger.TYPE_INT_BUILD,
                building.getNumericIniHeader(),
                null);
          }
        }
      }
  ...
  // }
  // }
  /** Hace lo necesario para que el aldeano construya un item y lo lleve a su sitio */
  private void doCreateAndPlaceTask() {
    ...
    if (getCitizenData().getCarryingData().getCarrying() != null) {
      ...
      if (sItemAux.equals(
          getCitizenData()
              .getCarryingData()
              .getCarrying()
              .getIniHeader())) { // Es el item adecuado, procedemos
        ...
        if (getCitizenData().getCarryingData().getCarrying() instanceof Item
            && ItemManager.getItem(
                ((Item) getCitizenData().getCarryingData().getCarrying()).getIniHeader())
                .canBeBuiltOnHoles()) {
          ArrayList<Point3DShort> alPoints = Task.getAccesingPointsMatchingASZI(
              ...
              World.getCell(x, y, z).getAstarZoneID(),
              task.getTask());
        ...
        if (bEnDestino) {
          ...
          if (cell.isEmpty() && Item.isCellAvailableForItem(imi, x, y, z, true, false)) {
            ...
            getCarrying()
                .init(
                    getCitizenData().getCarryingData().getCarrying().getX(),
        ...
        // Lleva un item que no es el que toca, puede ser un material para construirlo o cualquier
        // otra cosa, no hacemos nada de momento
      }
    }
    ...
    Point3DShort p3dItem = Item.searchItem(
        false,
        getCoordinates(),
        UtilsIniHeaders.getIntsArray(alPrerequisites),
        true,
        Item.SEARCH_FALSE,
        Item.SEARCH_DOESNTMATTER,
        null,
  ...
  private void doCustomActionTask() {
    ...
    if (qi.getType() == QueueItem.TYPE_CREATE_ITEM) {
      ...
      if (alItems != null && alItems.size() > 0) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("TaskManager.11") + " [" + qi.getValue() + "]",
      ...
      if (destPoint != null) {
        ...
        if (getCarrying() != null && getCarrying().getIniHeader().equals(qi.getValue())) {
          ...
          // No lelvamos item o no es el que toca ż?ż? ya estamos
          World.getCell(destPoint).setFlagOrders(false);
          getCurrentCustomAction().getQueue().remove(0);
        }
      } else {
      ...
      if (getCurrentCustomAction().getQueueData().getWaitCounter() < 0) {
        ...
        try {
          ...
          // turns = (turns * 105) / 100;
          // }
          // Nueva fórmula de happiness (de 0 a 50 la velocidad se ve afectada, de 50% a 100% más
          // lento)
          ...
          // Miramos si hay un wall (iOver = 1) o un terrain encima (iOver = 2), lo primero que
          // encontremos
          int iOver = 0;
          int iZOver = getZ();
          Cell cellOVer;
          ...
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("Citizen.33") + qi.getValue() + "]",
      ...
      ArrayList<Integer> alItemsInUse = Item.searchItemInUse(getID());
      p3dCoordenadas = Item.searchItem(
          false,
          getCoordinates(),
          UtilsIniHeaders.getIntsArray(alPicks),
          true,
          Item.SEARCH_FALSE,
          Item.SEARCH_DOESNTMATTER,
          alItemsInUse,
      ...
      // (para dar coherencia a la construcción, que si no pueden construir un item en distintas
      // mesas de carpintero por ejemplo)
      if (getCurrentCustomAction().getQueueData().getItemIDCurrentPlace() != -1) {
      ...
      if (p3dCoordenadas == null) {
        ...
        if (qi.isUseSource() && getCurrentCustomAction().getEntityID() != -1) {
          ...
          // Obtenemos una lista de items "en uso" para que al buscarlos no nos de esos
          ArrayList<Integer> alItemsInUse = Item.searchItemInUse(getID());
          // p3dCoordenadas = Item.searchItem (getCoordinates (), alPicks, true, Item.SEARCH_TRUE,
          // Item.SEARCH_TRUE, alItemsInUse, true);
          p3dCoordenadas = Item.searchItem(
              false,
              getCoordinates(),
              UtilsIniHeaders.getIntsArray(alMoves),
              true,
              Item.SEARCH_DOESNTMATTER,
              Item.SEARCH_TRUE,
              alItemsInUse,
      ...
      // // BINGO
      // Game.getWorld ().getGods ().get (g).setStatus (Game.getWorld ().getGods ().get
      // (g).getStatus () + qi.getGodStatus ());
      // break;
      // }
      // }
      // }
  ...
  /** Lleva le material que está cargando a alguna celda libre y suelta una living si la lleva */
    ...
    if (getCarrying() == null) {
      ...
      if (p3dItem == null) {
        ...
        if (p3dItem.equals(getCoordinates())) {
          ...
          if (cell.getEntity() != null && cell.getEntity() instanceof Item) {
            if (ItemManager.getItem(cell.getEntity().getIniHeader()).isContainer()) {
              ...
              if (container != null) {
                ...
                // Algo ha pasado, en la casilla no hay nada que nos sirva (no debería pasar, quizá
                // están moviendo un container)
                resetTaskIndexes();
              }
            } else {
          ...
          getCurrentTask()
              .setPointIni(
                  p3dItem); // Ponemos esto para que otro aldeano pueda detectar que vamos a por la
          // comida
          setDestination(p3dItem);
        }
      }
  ...
  private void doFoodNeeded() {
    ...
    if (itemCarrying == null) {
      ...
      // Si llega aquí es que hemos encontrado el item, vamos a ver si ya estamos donde toca para
      // pillarlo
      if (p3dItem.equals(getCoordinates())) {
        // Item en la casilla actual
        Cell cell = World.getCell(getCoordinates());
  ...
  private void doGenericTask() {
    ...
    if (bFirstTry) {
      // Pondremos la place más cercana en el primer punto (esto evitará movimientos raros como ir a
      // minar un sitio dando toda la vuelta por un pasillo)
    ...
    if (getPlacesIndex() < hp.getPlaces().size()) {
      ...
      if (x != p3d.x || y != p3d.y || z != p3d.z) {
        ...
        if (!bFirstTry) { // Ya ha buscado camino y NO lo ha encontrado
          ...
          // Si ya no hay más places es que este aldeano no puede llegar a destino, lo desligamos de
          // la tarea y terminamos el turno
          if (getPlacesIndex() >= hp.getPlaces().size()) {
            Game.getWorld().getTaskManager().removeCitizen(this);
            return;
          }
   ...
   *
   * @param lockItem
   *            Indica si hay que poner operativo y bloquear el item al llevarlo a destino
   */
  private void doHaulingTask(
      boolean operativeAndLock,
      boolean bPutInContainer,
      boolean bCheckSourceNoLocked) {
    int x = getX();
    int y = getY();
    int z = getZ();
    Task task = getCurrentTask();
      ...
      if (getCitizenData()
        ...
        if (!bEnDestino) {
          ...
          if (cell.isEmpty()) {
            ...
            // Si llega aquí es que no ha podido meterlo o no es tarea de container (raro), soltamos
            // el item y fin de tarea
            cell.setFlagOrders(false);
            getCurrentTask().setFinished(true);
            dropCarryingItem();
            return;
  ...
  /** Hace lo necesario para que el aldeano llegue hasta una cama (o suelo) y duerma */
  private void doSleepTask() {
    executeDropLivingTask();

    // Si tiene zona personal vamos a ella
    // Antes de mirar nada miraremos si está en una cama, en ese caso no lo movemos pq ya está
    // durmiendo
    ...
    if (!bEstaEnCama) {
      ...
      if (getSoldierData().isSoldier()
          && getSoldierData().getState() == SoldierData.STATE_IN_A_GROUP) {
    ...
    if (getCitizenData().getSleepSleeping() >= (((iTechoCave == 1) ? 5 : 6)
        * World.TIME_MODIFIER_HOUR)) { // Con techo duerme 5 horas, sin techo duerme 6
      getCitizenData().setSleepSleeping(0);
      resetTaskIndexes();
      if (iTechoCave == 1) { // techo
        // Con techo aguantará un 25% más de tiempo sin dormir
        getCitizenData()
            .setSleep(getCitizenData().getMaxSleep() + (getCitizenData().getMaxSleep() / 4));
      } else if (iTechoCave == 2) { // cave
        // Con cave aguantará un 12.5% más de tiempo sin dormir
        getCitizenData()
            .setSleep(getCitizenData().getMaxSleep() + (getCitizenData().getMaxSleep() / 8));
      } else {
      ...
      // Happiness counters
      getCitizenData()
          .setHappinessWorkCounter(
              getCitizenData().calculateWorkCounter(getCitizenData().getSleep()));
      getCitizenData()
          .setHappinessIdleCounter(
              getCitizenData().calculateIdleCounter(getCitizenData().getSleep()));
  ...
  /** Hace las cosas de soldado. Al llamarse no tiene nada en carrying ni carrying living */
    ...
    if (iSoldierState == SoldierData.STATE_PATROL) {
      ...
      if (alPatrolPoints.size() == 0) {
        ...
        if (getSoldierData().getState() == SoldierData.STATE_IN_A_GROUP) {
          alPatrolPoints = Game.getWorld()
              .getSoldierGroups()
              .getGroup(getSoldierData().getGroup())
              .getPatrolPoints();
        }
      }
      if (alPatrolPoints.size() == 0) {
      ...
      if (getSoldierData().getTargetID() != -1) {
        ...
        if (citTarget != null) {
          ...
          boolean bTargetClose = Math.abs(citTarget.getZ() - getZ()) <= 1
              && Math.abs(citTarget.getY() - getY()) <= 1
              && Math.abs(citTarget.getX() - getX()) <= 1;
          ...
          if (bTargetClose) {
            ...
            if (!citTarget.getSoldierData().isSoldier()) {
              citTarget
                  .getCitizenData()
                  .setHappiness(citTarget.getCitizenData().getHappiness() - 1);
          ...
          if (ItemManager.getItem(cellItem.getEntity().getIniHeader()).isContainer()) {
            ...
            if (container != null) {
              ...
              if (item != null) {
                ...
                MessagesPanel.addMessage(
                    MessagesPanel.TYPE_ANNOUNCEMENT,
                    getCitizenData().getFullName() + Messages.getString("Citizen.1"),
                    ColorGL.ORANGE,
                    getCoordinates(),
                    getID()); // $NON-NLS-1$
                getCurrentTask().setFinished(true);
            ...
            MessagesPanel.addMessage(
                MessagesPanel.TYPE_ANNOUNCEMENT,
                getCitizenData().getFullName() + Messages.getString("Citizen.0"),
                ColorGL.ORANGE,
                getCoordinates(),
                getID()); // $NON-NLS-1$
            getCurrentTask().setFinished(true);
   ...
   *
   * @param bSearch
   *            Indica si tiene que buscar un sitio adecuado para el item
   */
  ...
  /**
   * Lleva le material que está cargando a alguna celda libre, devuelve true si lo consigue. No saca al aldeano de
   * ninguna tarea.
   *
  ...
  private boolean executeDropItemTask() {
    ...
    while (distancia < distanciaMAX) {
      ...
      for (int x = (getX() - distancia); x <= (getX() + distancia); x++) {
        for (int y = (getY() - distancia); y <= (getY() + distancia); y++) {
          if (Math.abs((getX() - x)) == distancia
              || Math.abs((getY() - y)) == distancia) { // Para que sólo mire puntos exteriores del radio
    ...
    // En todo el nivel no hay ni un hueco, haremos el truqui de buscar en un radio de 8x8 (por
    // ejemplo) a partir
      ...
      if (!cell.isMined()) {
        cell.getTerrain()
            .mine(
                ...
                hotPoint3D.y,
                hotPoint3D.z,
                task.getTask() == Task.TASK_MINE_LADDER);
      }
  ...
  /**
   * Indica si la celda es buena para un item en concreto. Por buena se entiende que haya una pila adecuada o un
   * container.
   *
   * @param imi
   *            ItemManagerItem del objeto
   * @param p3ds
