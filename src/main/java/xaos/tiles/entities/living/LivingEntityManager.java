...
public class LivingEntityManager {
  ...
  /**
   * Devuelve un enemigo del nivel indicado. En caso de no existir va disminuyendo de 1 en 1 hasta encontrar algo
   */
   ...
   */
  public static ArrayList<LivingEntityManagerItem> getItemByLevelList(
      int iLevelMin,
      int iLevelMax,
      int iType,
      boolean bSteal) {
    ...
    while (iterator.hasNext()) {
      ...
      if (lemi.getLevel() >= iLevelMin && lemi.getLevel() <= iLevelMax && lemi.getType() == iType) {
        if (bSteal
            && ((lemi.getSteal() != null && lemi.getSteal().length > 0)
                || (lemi.getStealLivings() != null && lemi.getStealLivings().length > 0))) {
          alLemi.add(lemi);
        } else if (!bSteal
            && ((lemi.getSteal() == null || lemi.getSteal().length == 0)
                && (lemi.getStealLivings() == null || lemi.getStealLivings().length == 0))) {
  ...
  /**
   * Devuelve una lista con todos los living que pueden spawnear en el edificio pasado
   *
   ...
   */
  private static LivingEntityManagerItem getItemByLevel(
      int iLevelMin,
      int iLevelMax,
      int iType,
      boolean bSteal) {
  ...
  private static ArrayList<DropData> loadDropData(NodeList nodeList) throws Exception {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE
          && node.getNodeName().equals("drop")) { // $NON-NLS-1$
        ...
        dropData.setLevelMin(
            UtilsXML.getChildValue(node.getChildNodes(), "levelMin")); // $NON-NLS-1$
        dropData.setLevelMax(
            UtilsXML.getChildValue(node.getChildNodes(), "levelMax")); // $NON-NLS-1$
  ...
  private static void loadLivingEntities() {
    if (hmLivingEntities == null) {
      ...
      // Cargar de fichero
      loadXML(
          Towns.getPropertiesString("DATA_FOLDER") + "livingentities.xml",
      ...
      if (alMods != null && alMods.size() > 0) {
        for (int i = 0; i < alMods.size(); i++) {
          String sModActionsPath = fUserFolder.getAbsolutePath()
              ...
              + "livingentities.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          // //$NON-NLS-5$
      ...
      while (iterator.hasNext()) {
        ...
        if (lemi.getStealLivings() != null && lemi.getStealLivings().length > 0) {
          for (int i = 0; i < lemi.getStealLivings().length; i++) {
            if (hmLivingEntities
                .get(UtilsIniHeaders.getStringIniHeader(lemi.getStealLivings()[i])) == null) {
              Log.log(
                  Log.LEVEL_ERROR,
                  "Wrong stealLivings"
                      + " ["
                      + UtilsIniHeaders.getStringIniHeader(lemi.getStealLivings()[i])
                      + "]",
                  "LivingEntityManager");
              Game.exit();
            }
  ...
  private static void loadXML(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los livingEntities a la hash
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (sIniHeader != null && sIniHeader.equalsIgnoreCase("DELETE")) {
            ...
            if (node.getAttributes() != null && node.getAttributes().getNamedItem("id") != null) {
              ...
              if (sIDToDelete != null) {
                ...
                if (lemi != null
                    && lemi.getAltGraphics() != null
                    && lemi.getAltGraphics().size() > 0) {
                  String altG;
          ...
          if (bModChangingValues) {
            ...
            lemi.setNamePoolTag(
                UtilsXML.getChildValue(node.getChildNodes(), "namePoolTag")); // $NON-NLS-1$
            lemi.setSurnamePoolTag(
                UtilsXML.getChildValue(node.getChildNodes(), "surnamePoolTag")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setHealthPoints(
                UtilsXML.getChildValue(node.getChildNodes(), "healthPoints")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setAttackVerb(
                UtilsXML.getChildValue(node.getChildNodes(), "attackVerb")); // $NON-NLS-1$
            lemi.setAttackVerbInfinitive(
                UtilsXML.getChildValue(
                    node.getChildNodes(),
                    "attackVerbInfinitive")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setGrouping(
                UtilsXML.getChildValue(node.getChildNodes(), "grouping")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setHPRegeneration(
                UtilsXML.getChildValue(node.getChildNodes(), "HPRegeneration")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setMaxAgeLiving(
                UtilsXML.getChildValue(node.getChildNodes(), "maxAgeLiving")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setEquipAllowed(
                UtilsXML.getChildValue(node.getChildNodes(), "equipAllowed")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setAltGraphics(
                UtilsXML.getChildValues(node.getChildNodes(), "altGraphics")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setFacingDirections(
                UtilsXML.getChildValue(node.getChildNodes(), "facingDirections")); // $NON-NLS-1$
          }

          // Hungry & sleep
          if (lemi.getType() == LivingEntity.TYPE_CITIZEN
              || lemi.getType() == LivingEntity.TYPE_HERO) {
            if (bModChangingValues) {
              ...
              lemi.setMaxHungryTurns(
                  UtilsXML.getChildValue(node.getChildNodes(), "maxHungryTurns")); // $NON-NLS-1$
              lemi.setMaxSleepTurns(
                  UtilsXML.getChildValue(node.getChildNodes(), "maxSleepTurns")); // $NON-NLS-1$
            }
          }
          ...
          if (lemi.getType() == LivingEntity.TYPE_HERO) {
            if (bModChangingValues) {
              String sAux = UtilsXML.getChildValue(
                  node.getChildNodes(),
                  "heroComePrerequisite"); // $NON-NLS-1$
              if (sAux != null) {
                if (HeroManager.getComePrerequisites(sAux) != null) {
                  lemi.setHeroComePrerequisite(sAux);
                } else {
                  throw new Exception(
                      Messages.getString("LivingEntityManager.0")
                          + sAux
                          + "]"); //$NON-NLS-1$ //$NON-NLS-2$
                }
              }
              sAux = UtilsXML.getChildValue(
                  node.getChildNodes(),
                  "heroStayPrerequisite"); // $NON-NLS-1$
              if (sAux != null) {
              ...
              if (sAux != null) {
                if (HeroManager.getBehaviour(sAux) != null) {
                  ...
                  throw new Exception(
                      Messages.getString("LivingEntityManager.1")
                          + sAux
                          + "]"); //$NON-NLS-1$ //$NON-NLS-2$
                }
              }
              ...
              String sHeroComePrerequisite = UtilsXML.getChildValue(
                  node.getChildNodes(),
                  "heroComePrerequisite"); // $NON-NLS-1$
              if (sHeroComePrerequisite == null
                  || HeroManager.getComePrerequisites(sHeroComePrerequisite) == null) {
                throw new Exception(
                    Messages.getString("LivingEntityManager.0")
                        + sHeroComePrerequisite
                        + "]"); //$NON-NLS-1$ //$NON-NLS-2$
              }
              lemi.setHeroComePrerequisite(sHeroComePrerequisite);

              String sHeroStayPrerequisite = UtilsXML.getChildValue(
                  node.getChildNodes(),
                  "heroStayPrerequisite"); // $NON-NLS-1$
              if (sHeroStayPrerequisite == null
                  || HeroManager.getStayPrerequisites(sHeroStayPrerequisite) == null) {
                throw new Exception(
                    Messages.getString("LivingEntityManager.0")
                        + sHeroStayPrerequisite
                        + "]"); //$NON-NLS-1$ //$NON-NLS-2$
              }
              ...
              lemi.setHeroSkills(
                  UtilsXML.getChildValue(node.getChildNodes(), "heroSkills")); // $NON-NLS-1$
            }
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setFollowEntity(
                UtilsXML.getChildValue(node.getChildNodes(), "followEntity")); // $NON-NLS-1$
            lemi.setFollowTurns(
                UtilsXML.getChildValue(node.getChildNodes(), "followTurns")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setEffectsImmune(
                UtilsXML.getChildValue(node.getChildNodes(), "effectsImmune")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setEvadeTraps(
                UtilsXML.getChildValue(node.getChildNodes(), "evadeTraps")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setStealLivings(
                UtilsXML.getChildValue(node.getChildNodes(), "stealLivings")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setFoodNeeded(
                UtilsXML.getChildValue(node.getChildNodes(), "foodNeeded")); // $NON-NLS-1$
            lemi.setFoodNeededTurns(
                UtilsXML.getChildValue(node.getChildNodes(), "foodNeededTurns")); // $NON-NLS-1$
            lemi.setFoodNeededDieTurns(
                UtilsXML.getChildValue(node.getChildNodes(), "foodNeededDieTurns")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setAnimatedWhenIdle(
                UtilsXML.getChildValue(node.getChildNodes(), "animatedWhenIdle")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            lemi.setWorkCounterPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "workCounterPCT")); // $NON-NLS-1$
            lemi.setIdleCounterPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "idleCounterPCT")); // $NON-NLS-1$
          }

          // Lo ańadimos a la hash
          hmLivingEntities.put(sIniHeader, lemi);

          // System.out.println (lemi.getIniHeader () + "," + lemi.getName () + "," + lemi.getAttack
          // () + "," + lemi.getAttackSpeed () + "," + lemi.getDamage () + "," + lemi.getDefense ()
          // + "," + lemi.getHealthPoints () + "," + lemi.getLevel ());
          // Miramos si tiene gráficos alternativos, en ese caso creamos un "lemi" igual con
          // distinto ID
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("LivingEntityManager.11")
              + sXMLName
              + Messages.getString("LivingEntityManager.12")
              + e.toString()
              + "]",
          "LivingEntityManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
}
