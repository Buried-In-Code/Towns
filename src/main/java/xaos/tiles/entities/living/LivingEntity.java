...
public abstract class LivingEntity extends Entity implements Externalizable {
  ...
  public static final int TYPE_CITIZEN = 1;
  public static final int TYPE_ENEMY = 2;
  public static final int TYPE_FRIENDLY = 3;
  public static final int TYPE_HERO = 4;
  public static final int TYPE_ALLY = 5;

  public static final byte FACING_DIRECTION_NORTH = 1;
  public static final byte FACING_DIRECTION_SOUTH = 2;
  public static final byte FACING_DIRECTION_EAST = 3;
  public static final byte FACING_DIRECTION_WEST = 4;
  public static final byte FACING_DIRECTION_NORTH_EAST = 5;
  public static final byte FACING_DIRECTION_NORTH_WEST = 6;
  public static final byte FACING_DIRECTION_SOUTH_EAST = 7;
  public static final byte FACING_DIRECTION_SOUTH_WEST = 8;

  public static final int DAMAGE_ANIMATION_FPS = 8;
  public static final int ATTACK_ANIMATION_MAX_COUNTER = 8;
  ...
  // Flags
  private static final byte FLAG_WAITING_FOR_PATH = 1; // 00000001 (Indica si el ser viviente estß esperando camino)

  private static final byte FLAG_GROUPING = 2; // 00000010 (Indica si llama a sus amigos cuando le atacan)
  private static final byte FLAG_FIGHTING = 4; // 00000100 (Indica si estß luchando)
  // Lista de puntos hasta destino (nunca serß nulo)
  ...
  private int maxAge; // Edad mßxima del friendly

  private byte facingDirection;

  private FocusData focusData; // Guarda el ID + type de un entity al que estß atacando

  private int attackSpeedCounter; // Cuando attackTurn > que el atackTurnCurrent es que ya puede atacar,
  // sino suma 1 y espera
  private int walkSpeedCounter; // Cuando walk > que el walkTurnCurrent es que ya puede caminar, sino suma 1
  // y espera

  private Point2D.Float positionOffset = new Point2D.Float(
      0,
      0); // Se usa en el dibujado, para ir moviendo poco a poco a la living (en vez de celda a
  // celda)
  private Point2D.Float positionOffsetConstants = new Point2D.Float(
      0, 0); // Se usa en el dibujado, es lo que hay que a±adir al offset a cada frame
  ...
  public LivingEntity(String iniHeader) {
    ...
    LivingEntityManagerItem lemi = LivingEntityManager.getItem(
        getIniHeader()); // Ojo, aquÝ hay que usar el getIniHeader, pq puede estar cambiado
    ...
    if (lemi.getEquip() != null) {
      for (int i = 0; i < lemi.getEquip().size(); i++) {
        ...
        if (item instanceof MilitaryItem) {
          ...
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("LivingEntity.6") + lemi.getEquip().get(i) + "]",
  ...
  /**
   * Indica si el personaje estß listo para lanzar otro ataque. Actualiza el contador de turnos
   *
   * @param attacker
   * @return
   */
  ...
  /**
   * Indica si el personaje estß listo para caminar. Actualiza el contador de turnos
   *
   * @param living
   * @return
   */
  ...
  /**
   * Intenta dar un golpe de atacante a vÝctima. Si lo consigue resta puntos de vida a la vÝctima
   *
   * @param attacker
   *            Atacante
   * @param victim
   *            VÝctima
   * @return true si el golpe se da con Úxito
   */
  public static boolean doHit(
      LivingEntity attacker,
      LivingEntity victim,
      boolean bCheckAttackTurn) {
   ...
   * @param victim
   *            VÝctima
   * @param bCheckAttackTurn
   *            Indica si hay que comprobar si el atacante estß listo para el siguiente ataque
   * @param rangedWeapon
   *            Indica si el ataque es ranged. En otro caso valdrß null. Se usa para obtener los verbos
   * @param iRangedDamage
   *            Da±o fijo, se usa para los ataques ranged, ya que el da±o se calcula en el momento de lanzar el
   *            proyectil. AquÝ se aplicarß el % en base a la distancia del enemigo
   * @param iRangedMaxDistance
   *            Distancia mßxima en el momento del disparo.
   * @param bRangedHit
   *            Indica si el ammo ha tocado al enemigo o no (no quiere decir que acierte)
   * @return true si el golpe se da con Úxito
   */
  public static boolean doHit(
      LivingEntity attacker,
      LivingEntity victim,
      boolean bCheckAttackTurn,
      ItemManagerItem rangedWeapon,
      int iRangedDamage,
      int iRangedMaxDistance,
      boolean bRangedHit) {
    ...
    // Calculamos el porcentaje de ataque en base a la defensa de la vÝctima
    // Hay un mÝnimo de 5% de atacar y un mßximo de 95%
    int iPCT;
    ...
    if (Utils.getRandomBetween(1, 100) <= iPCT) {
      // Hit
      // Si es ranged miramos la distßncia
      int iDanyo;
      if (rangedWeapon != null) {
        // Calculamos el % de da±o que harß seg·n la distancia (de 100% a 20%)
        int iDistancia = Math.max(
            Math.abs(attacker.getX() - victim.getX()),
            Math.abs(attacker.getX() - victim.getX()));
      ...
      victim
          .getLivingEntityData()
          .setHealthPoints(victim.getLivingEntityData().getHealthPoints() - iDanyo);
      ...
      MessagesPanel.addMessage(
          iMessageType,
          attacker.getLivingEntityData().getName()
              + " "
              + verb
              + Messages.getString("LivingEntity.5")
              + victim.getLivingEntityData().getName()
              + " ("
              + iDanyo
              + Messages.getString("LivingEntity.11"),
          (lemiAttacker.getType() == TYPE_CITIZEN
              || lemiAttacker.getType() == TYPE_ALLY
              || (lemiAttacker.getType() == TYPE_HERO && lemiVictim.getType() != TYPE_CITIZEN))
                  ? ColorGL.GREEN
                  : ColorGL.RED,
          attacker.getCoordinates(),
          attacker.getID()); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$

      // Notificamos a la vÝctima
      victim.hitted(attacker, true, iDanyo);

      // Si el atacante es hÚroe o soldado le sumamos experiencia
      // Si es hÚroe o citizen sumamos 1 a los enemigos de ese tipo matados
      if (lemiAttacker.getType() == TYPE_HERO) {
        ...
        if (hero.getHeroData().checkAndUpdateLevelUp(hero, true)) {
          ...
          MessagesPanel.addMessage(
              iMessageType,
              hero.getCitizenData().getFullName() + Messages.getString("LivingEntity.1"),
              ColorGL.GREEN,
              hero.getCoordinates(),
              hero.getID()); // $NON-NLS-1$
        }
      ...
      // La vÝctima ha muerto de este golpe, sumamos 1 al medallero en caso de HERO o CITIZEN
      ...
      // Si el atacante tiene un effect con attackDOT, le metemos un efecto de DOT a la vÝctima
      ...
      for (int i = 0; i < effects.size(); i++) {
        ...
        if (rangedWeapon == null) {
          // Melee
          if (effectData.getOnHitPCT() > 0
              && Utils.getRandomBetween(1, 100) <= effectData.getOnHitPCT()) {
          ...
          // Ranged
          if (effectData.getOnRangedHitPCT() > 0
              && Utils.getRandomBetween(1, 100) <= effectData.getOnRangedHitPCT()) {
            emi = EffectManager.getItem(effectData.getEffectID());
      ...
      if (rangedWeapon != null) {
        ...
        if (attacker.getEquippedData().isWearing(MilitaryItem.LOCATION_WEAPON)) {
          verb = ItemManager.getItem(attacker.getEquippedData().getWeapon().getIniHeader())
              .getVerbInfinitive();
        } else {
      ...
      MessagesPanel.addMessage(
          iMessageType,
          attacker.getLivingEntityData().getName()
              + Messages.getString("LivingEntity.2")
              + verb
              + Messages.getString("LivingEntity.5")
              + victim.getLivingEntityData().getName()
              + Messages.getString("LivingEntity.4"),
          ColorGL.WHITE,
          attacker.getCoordinates(),
          attacker.getID()); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  /**
   * Pone en juego un item munici¾n (ej: flecha, bola de fuego, ...) Tiene en cuenta las cargas del arma y controla si
   * se acaban y se rompe
   *
   * @param attacker
   * @param victim
   */
  public static void doRangeAttack(
      LivingEntity attacker,
      LivingEntity victim,
      boolean bCheckAttackTurn) {
    if (bCheckAttackTurn && !canLaunchNextAttack(attacker)) {
    ...
    int damage = attacker
        .getLivingEntityData()
        .getDamageCurrent(); // Se pone aquÝ ppq mßs abajo el arma puede estar destruida
    int maxDistance = attacker
        .getLivingEntityData()
        .getLOSCurrent(); // Se pone aquÝ pq mßs abajo el arma puede estar destruida
    ...
    if (attacker.getCoordinates().z == victim.getCoordinates().z) {
      ArrayList<Point3DShort> path = Utils.bresenhamLine(
          attacker.getCoordinates(),
          victim.getCoordinates(),
          attacker.getZ()); // , attackerType);
      if (path != null) {
      ...
      for (int liv = 0; liv < alLivings.size(); liv++) {
        ...
        if (TownsProperties.DEBUG_MODE) {
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "path pos. offset: " + le.getPositionOffset().x + "," + le.getPositionOffset().y,
                  null,
                  null,
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "path pos. offset constant: "
                      + le.getPositionOffsetConstants().x
                      + ","
                      + le.getPositionOffsetConstants().y,
                  null,
                  null,
                  null)); // $NON-NLS-1$ //$NON-NLS-2$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "Focus: " + le.getFocusData().getEntityID(),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "LOS: " + le.getLivingEntityData().getLOSCurrent(),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "HPs "
                      + le.getLivingEntityData().getHealthPoints()
                      + "/"
                      + le.getLivingEntityData().getHealthPointsMAXCurrent(),
                  null,
                  null,
                  null)); // $NON-NLS-1$ //$NON-NLS-2$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "Defense " + le.getLivingEntityData().getDefenseCurrent(),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "Damage " + le.getLivingEntityData().getDamageCurrent(),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "attackSpeed "
                      + le.getAttackSpeedCounter()
                      + "/"
                      + le.getLivingEntityData().getAttackSpeedCurrent(),
                  null,
                  null,
                  null)); // $NON-NLS-1$ //$NON-NLS-2$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "walkSpeed"
                      + le.getWalkSpeedCounter()
                      + "/"
                      + le.getLivingEntityData().getWalkSpeedCurrent(),
                  null,
                  null,
                  null)); // $NON-NLS-1$ //$NON-NLS-2$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "movePCT "
                      + le.getLivingEntityData().getMovePCTCurrent()
                      + "/"
                      + le.getLivingEntityData().getMovePCTBase(),
                  null,
                  null,
                  null)); // $NON-NLS-1$ //$NON-NLS-2$
          if (le.getFoodNeededTurns() != 0) {
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_TEXT,
                    "foodNeededTurns " + le.getFoodNeededTurns(),
                    null,
                    null,
                    null)); // $NON-NLS-1$
          }
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "Num in world (discovered): " + getNumLivings(le.getIniHeader(), true),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  "Num in world (undiscovered): " + getNumLivings(le.getIniHeader(), false),
                  null,
                  null,
                  null)); // $NON-NLS-1$
          if (lemi.getType() == TYPE_ENEMY) {
            ...
            if (enemy.getSiegeData() != null) {
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "siegeType " + enemy.getSiegeData().getType(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "siegecount " + enemy.getSiegeData().getCount(),
                      null,
                      null,
                      null)); // $NON-NLS-1$
            }
        ...
        if (le.getLivingEntityData().getEffects().size() > 0) {
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT,
                  Messages.getString("LivingEntity.7") + sBuffer.toString(),
                  null,
                  null,
                  null,
                  null,
        ...
        if (lemi.hasActions()) {
          ...
          for (int j = 0; j < lemi.getActions().size(); j++) {
            ...
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    ami.getName() + " " + lemi.getName().toLowerCase(),
                    null,
                    CommandPanel.COMMAND_CUSTOM_ACTION_DIRECT_LIVING,
                    ami.getId(),
                    Integer.toString(le.getID()))); // $NON-NLS-1$
          }
        ...
        if (lemi.getCaravan() != null && Game.getWorld().getCurrentCaravanData() != null) {
          ...
          if (Game.getWorld().getCurrentCaravanData().getStatus() == CaravanData.STATUS_IN_PLACE) {
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    Messages.getString("LivingEntity.9"),
                    null,
                    CommandPanel.COMMAND_TRADE,
                    null)); // $NON-NLS-1$
            sm.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
          }
        }
  ...
  public static String getAltGraphics(String sIniHeader) {
    ...
    if (lemi.getAltGraphics() != null && lemi.getAltGraphics().size() > 0) {
      ...
      // Si el random == altGraphics.size() usamos el grßfico original (por lo tanto no hacemos
      // nada)
   ...
   */
  public static LivingEntity getLivingByCoordinatesAndIniHeader(
      Point3DShort p3d,
      String sIniHeader) {
  ...
  /**
   * Devuelve una living entity hate de la celda y living pasada Se le pasa un focus, si tiene alguien ahÝ, eso tiene
   * preferencia
   *
   * @param sIniHeader
   * @param p3d
   * @return
   */
  public static LivingEntity getLivingEntityHate(
      String sIniHeader,
      int x,
      int y,
      int z,
      FocusData focusData,
      boolean bAttackAllies) {
    ...
    if (focusData != null && focusData.getEntityID() != -1) {
      ...
      if (leFocus != null
          && leFocus.getCoordinates().x == x
          && leFocus.getCoordinates().y == y
          && leFocus.getCoordinates().z == z) {
        if (!bAttackAllies) {
          return leFocus;
        } else {
          // Attack allies
          if (LivingEntityManager.getItem(leFocus.getIniHeader()).getType() == LivingEntityManager
              .getItem(sIniHeader).getType()) {
            return leFocus;
          }
        }
      }
  ...
  /**
   * Indica si hay alg·n enemigo de la livingentity pasada en LOS del punto pasado, en ese caso devuelve el FocusData
   * del mismo
   *
   * @return el FocusData de enemigo en LOS o null
   */
  public static FocusData hasEnemyInLOS(
      String sLivingSource,
      Point3DShort p3d,
      int iRadius,
      boolean bAttackAllies) {
      ...
      if (iTmp >= 0) {
        for (short i = (short) (x - radio); i <= (x + radio); i++) {
          if (i >= 0 && i < World.MAP_WIDTH) {
            ...
            if (fdReturn != null) {
              // En el caso de heroes, miramos si tiene que lanzar una skill de tipo
              // use=ENEMIES_IN_LOS
              return fdReturn;
            }
          }
  ...
  public static boolean isCellAllowed(Cell cell) {
      ...
      if (!Terrain.canGoDown(cell.getCoordinates())) {
        ...
        if (!bItemHere) {
          ...
          if (cell.getCoordinates().z < (World.MAP_DEPTH - 1)) {
            Cell cellDown = World.getCell(
                cell.getCoordinates().x,
                cell.getCoordinates().y,
                cell.getCoordinates().z + 1);
    ...
    if (cell.hasBuilding()) {
      ...
      char groundDataChar = bmi.getGroundData()
          .charAt(
              (cell.getCoordinates().y - cell.getBuildingCoordinates().y) * bmi.getWidth()
                  + (cell.getCoordinates().x - cell.getBuildingCoordinates().x));
      if (groundDataChar == Building.GROUND_TRANSITABLE
          || groundDataChar == Building.GROUND_ENTRANCE) {
        // Celda transitable, puede pasar siempre
        return true;
      } else {
        // Celda NO principal, s¾lo se puede pasar si el edificio no estß a·n operativo
   ...
   *
   * @param p3dIni
   *            Punto inicial de b·squeda
   * @param alHeaders
   *            Lista de numeric Iniheaders de los livings
   * @param near
   *            Indica si hay que buscar la mßs cercana o cualquiera
   * @param alLivingsToAvoid
   *            Lista de IDs de las livings a evitar
   * @return
   */
  public static LivingEntity searchLiving(
      Point3DShort p3dIni,
      int[] alHeaders,
      boolean near,
      ArrayList<Integer> alLivingsToAvoid) {
        ...
        if (UtilsIniHeaders.contains(alHeaders, le.getNumericIniHeader())) {
          if (alLivingsToAvoid == null || !alLivingsToAvoid.contains(Integer.valueOf(le.getID()))) {
            if (!near) {
              ...
              // Usaremos el iNum (n·mero de livings en el mundo) para saber si hemos acabado
              // (performance trick)
              iNum--;
              if (iNum == 0) {
                break;
   ...
   * @return
   */
  public static LivingEntity searchLivingForcedASZID(
      Point3DShort p3dIni,
      int[] alHeaders,
      boolean near,
      ArrayList<Integer> alLivingsToAvoid,
  ...
  /**
   * Devuelve una lista de livings en uso por aldeanos que NO sea el aldeano pasado como parßmetro
   *
   * @param iCitID
   *            ID del aldeano a excluir o -1 en caso de no excluir a nadie
  ...
  public static ArrayList<Integer> searchLivingsInUse(int iCitID) {
    ...
    for (int i = 0; i < World.getCitizenIDs().size(); i++) {
      ...
      if (iCitID == -1 || citizen.getID() != iCitID) {
        if (citizen.getCurrentCustomAction() != null
            && citizen.getCurrentCustomAction().getQueueData() != null) {
          // Pick_living
          if (citizen.getCurrentCustomAction().getQueueData().getLivingIDPick() != -1) {
            alItemsInUse.add(
                new Integer(citizen.getCurrentCustomAction().getQueueData().getLivingIDPick()));
          }
        }
      }
    }
   ...
   */
  private static FocusData hasEnemyInLOSAtCell(
      String sLivingSource,
      short sourceX,
      short sourceY,
      short sourceZ,
    ...
    if (World.getCell(sourceX, sourceY, sourceZ).getAstarZoneID() == World.getCell(pointX, pointY, sourceZ)
        .getAstarZoneID()) {
      // Misma zona
      // int sourceType = LivingEntityManager.getItem (sLivingSource).getType ();
      // Miramos que haya vista "directa", teniendo en cuenta muros y demßs panochadas
      // if (Utils.bresenhamLineExists (sourceX, sourceY, pointX, pointY, sourceZ, sourceType) ||
      // Utils.bresenhamLineExists (pointX, pointY, sourceX, sourceY, sourceZ, sourceType)) {
      if (Utils.bresenhamLineExists(sourceX, sourceY, pointX, pointY, sourceZ)
          || Utils.bresenhamLineExists(pointX, pointY, sourceX, sourceY, sourceZ)) {
  ...
  public void addEffect(EffectManagerItem emi, boolean applyEffectStuff, boolean verbose) {
    // Le metemos el efecto si no lo tiene ya (o es immune), si ya lo tiene reseteamos el contador
    // de lasts
    ...
    // Si llega aquÝ es que no es immune al efecto por via efectos
    if (iIndexSameEffect == -1) {
      ...
      getLivingEntityData()
          .getEffects()
          .get(iIndexSameEffect)
          .setLasts(Utils.launchDice(emi.getLasts()));
    }
    ...
    if (iIndexSameEffect == -1
        && emi.isMessageWhenGain()) { // Efecto nuevo que muestra mensajes de gain
      if (verbose && emi.getName() != null && emi.getName().length() > 0) {
        ...
        if (iLivingType == TYPE_HERO) {
          MessagesPanel.addMessage(
              ...
              getLivingEntityData().getName() + Messages.getString("Hero.9") + emi.getName() + "]",
              ColorGL.WHITE,
              getCoordinates(),
  ...
  public void delete(boolean dead) {
    ...
    if (dead) {
      if (lemi.getType() == TYPE_HERO) {
        // Caso hÚroes, si se han pirado del mapa no hay que sacar drop
        Hero hero = (Hero) this;
    ...
    if (dead) {
      ...
      if (alDrops != null && alDrops.size() > 0) {
        ...
        for (int i = 0; i < alDrops.size(); i++) {
          if (Utils.getRandomBetween(1, 100) <= Utils.launchDice(alDrops.get(i).getPCT())) {
            ...
            String sItemID = null;
            if (alDrops.get(i).getItem() != null
                && !alDrops.get(i).getItem().equalsIgnoreCase(DropData.ITEM_RANDOM)) {
              // Item fijo
              sItemID = alDrops.get(i).getItem();
            } else {
              // Item by level (random)
              ItemManagerItem imi = ItemManager.getRandomItemByLevel(
                  alDrops.get(i).getLevelMin(),
                  alDrops.get(i).getLevelMax());
              if (imi != null) {
              ...
              if (p3dDrop != null) {
                ...
                if (item instanceof MilitaryItem) {
                  ...
                  if (Utils.getRandomBetween(1, 100) <= 5) { // 5% de tener prefijo
                    ((MilitaryItem) item)
                        .setPrefix(PrefixSuffixManager.getRandomPrefix().getRandom());
                  }
                  // Sufijo
                  if (Utils.getRandomBetween(1, 100) <= 5) { // 5% de tener sufijo
                    ((MilitaryItem) item)
                        .setSuffix(PrefixSuffixManager.getRandomSuffix().getRandom());
                  }
                }
        ...
        if (enemy.getCarryingData() != null) {
          ...
          if (enemy.getCarryingData().getCarrying() != null) {
            ...
            if (p3dDrop != null) {
              ...
              // Si no puede soltarlo nos lo trincamos, s¾lo hay que mirar containers para que se
              // limpien bien las listas
              if (imi.isContainer()) {
                Game.getWorld().deleteContainer(itemCarrying.getID());
              }
            }
      ...
      if (lemi.getType() == TYPE_CITIZEN) {
        Game.updateTutorialFlow(
            TutorialTrigger.TYPE_INT_POPULATION,
            (World.getNumCitizens() + World.getNumSoldiers()),
            null);
      }
    } else {
      // No lo matamos, eso es que lo ha pillado alg·n aldeano, le ponemos la direction pertinente
      // para que quede bien
      // TambiÚn podrÝa ser un hero que se pira (o un enemy robbery, en los 2 casos borramos
      // containers si llevaban alguno)
    ...
    if (lemi.getType() == TYPE_CITIZEN) {
      ...
      if (iIndex != -1) {
        // Lo sacamos de la lista de hÚroes
      ...
      if (bHeroLeaved) {
        // Lo metemos tambiÚn en la lista de hÚroes que se han pirado (para que quizß vuelvan en el
        // futuro)
        Game.getWorld().getOldHeroes().add(this);
      } else {
        // HÚroe muerto, si tiene nombre especial (caso Sips, punchwood, ...) lo metemos en la lista
        // de muertos para que no aparezca de nuevo
      ...
      for (int h = 0; h < alHeros.size(); h++) {
        // Si estß en la lista de amigos de otros, lo borramos
        ...
        if (heroAux != null) {
          while (heroAux.getHeroData().getFriendships().remove(Integer.valueOf(getID()))) {
            // Vamos borrando (aunque no deberÝa haber mßs de 1)
          }
        }
      }
    }
    ...
    if (getMaxAge() > 0 && getAge() >= getMaxAge()) {
      ...
      if (lemi.getMaxAgeLiving() != null) {
        ...
        World.addNewLiving(
            lemi.getIniHeader(),
            lemi.getType(),
            World.getCell(getCoordinates()).isDiscovered(),
            getX(),
            getY(),
            getZ(),
            true);
      ...
      if (iTmp >= 0) {
        for (int i = (x - radio); i <= (x + radio); i++) {
          if (i >= 0 && i < World.MAP_WIDTH) {
            ...
            if (cell.getAstarZoneID() == iASZID
                && cell.containsSpecificLiving(iEntityToCheck) != null) {
              // if (Utils.bresenhamLineExists (x, y, i, iTmp, z, iEntitySource) ||
              // Utils.bresenhamLineExists (i, iTmp, x, y, z, iEntitySource)) {
              if (Utils.bresenhamLineExists(x, y, i, iTmp, z)
                  || Utils.bresenhamLineExists(i, iTmp, x, y, z)) {
                return true;
              }
            }
          }
      ...
      if (iTmp >= 0) {
        for (int i = (y - radio); i <= (y + radio); i++) {
          if (i >= 0 && i < World.MAP_HEIGHT) {
            ...
            if (cell.getAstarZoneID() == iASZID
                && cell.containsSpecificLiving(iEntityToCheck) != null) {
              // if (Utils.bresenhamLineExists (x, y, iTmp, i, z, iEntitySource) ||
              // Utils.bresenhamLineExists (iTmp, i, x, y, z, iEntitySource)) {
              if (Utils.bresenhamLineExists(x, y, iTmp, i, z)
                  || Utils.bresenhamLineExists(iTmp, i, x, y, z)) {
                return true;
              }
            }
          }
  ...
  public boolean equip(MilitaryItem militaryItem) {
    if (getEquippedData()
        .isWearing(ItemManager.getItem(militaryItem.getIniHeader()).getLocation())) {
      return false;
    }
  ...
  /**
   * Mueve a la living SI o SI (si puede, claro) Se usa cuando estß encima de un muro (ocurre cuando se acaba de
   * construir), encima de fluidos o en encima de un hole
   *
   * @return true si ha podido, false si no puede (en ese caso deberÝa palmar o algo)
   */
  public boolean forceMove() {
    ...
    // Hay que mirar que no haya un hated ahÝ
        ...
        if (i != 0 || j != 0) {
          if (Utils.isInsideMap(currentX + i, currentY + j, currentZ)) {
            if (isCellAllowed(currentX + i, currentY + j, currentZ)) {
              ...
              // Por cada living en el destino miramos si la entity los odia
              ArrayList<LivingEntity> alLivings = World.getCell(currentX + i, currentY + j, currentZ)
                  .getLivings();
              ...
              if (alLivings != null) {
                ...
                for (int h = 0; h < alLivings.size(); h++) {
                  le = alLivings.get(h);
                  if (hateData.isHate(le)) {
                    continue nextcell; // Hay hate, la celda no estß disponible para el move
                  }
                }
              }
    ...
    // Si la casilla de arriba estß minada, miramos las casillas diagonal/parriba
          ...
          if (i != 0 || j != 0) {
            if (Utils.isInsideMap(currentX + i, currentY + j, currentZ - 1)) {
              if (isCellAllowed(currentX + i, currentY + j, currentZ - 1)) {
                ...
                // Por cada living en el destino miramos si la entity los odia
                ArrayList<LivingEntity> alLivings = World
                    .getCell(currentX + i, currentY + j, currentZ - 1).getLivings();
                ...
                alPoints.add(
                    Point3DShort.getPoolInstance(currentX + i, currentY + j, currentZ - 1));
              }
            }
          }
        }
    ...
    // Si llega aquÝ es que no hay ni un punto bueno
    // En caso de hole (digged) caerß abajo, en otro caso palmarß
  ...
  public ArrayList<LivingEntity> getAllLivingsInRadius(int iRadius, boolean hated) {
    ...
    for (int radio = 1; radio <= iRadius; radio++) {
      ...
      if (iTmp >= 0) {
        for (short i = (short) (x - radio); i <= (x + radio); i++) {
          if (i >= 0 && i < World.MAP_WIDTH) {
            // if (Utils.bresenhamLineExists (x, y, i, iTmp, z, iLivingType) &&
            // Utils.bresenhamLineExists (i, iTmp, x, y, z, iLivingType)) {
            if (Utils.bresenhamLineExists(x, y, i, iTmp, z)
                && Utils.bresenhamLineExists(i, iTmp, x, y, z)) {
              cell = World.getCell(i, iTmp, z);
      ...
      if (iTmp >= 0) {
        for (short i = (short) (y - radio); i <= (y + radio); i++) {
          if (i >= 0 && i < World.MAP_HEIGHT) {
            // if (Utils.bresenhamLineExists (x, y, iTmp, i, z, iLivingType) &&
            // Utils.bresenhamLineExists (iTmp, i, x, y, z, iLivingType)) {
            if (Utils.bresenhamLineExists(x, y, iTmp, i, z)
                && Utils.bresenhamLineExists(iTmp, i, x, y, z)) {
              cell = World.getCell(iTmp, i, z);
              if (cell.isDiscovered() && cell.getLivings() != null) {
   ...
   * @return
   */
  public MilitaryItem hasBetterItemInLOSAtCell(
      int x,
      int y,
      int z,
      int iCurrentASZID,
      boolean citizenAround,
      LivingEntityManagerItem lemi) {
    ...
    if (cell.getAstarZoneID() == iCurrentASZID) {
      ...
      if (item != null
          && item instanceof MilitaryItem
          && MilitaryItem.isBetterItem(this, (MilitaryItem) item)) {
        ...
        if (bTagsOK) {
          ...
          if (bItemOK) {
            if (Utils.bresenhamLineExists(x, y, getX(), getY(), z) /* , TYPE_HERO) */
                || Utils.bresenhamLineExists(getX(), getY(), x, y, z)) { // , TYPE_HERO)) {
  ...
  /**
   * Este mÚtodo es para notificar a una living entity que ha sido golpeado, se le pasa el atacante
   *
   * @param le
   *            El atacante
   * @param bHitted
   *            Indica si le ha pegado o s¾lo lo ha intentado
   */
  ...
  public void init(int x, int y, int z, boolean bDiscovered) {
    ...
    // Caso aldeanos o heros, se mete en la lista pertinente y se suma 1 al num cits/soldiers (si es
    // el caso)
    LivingEntityManagerItem lemi = LivingEntityManager.getItem(getIniHeader());
    if (lemi.getType() == TYPE_CITIZEN) {
  ...
  public boolean nextTurn() {
    ...
    for (int i = (getLivingEntityData().getEffects().size() - 1); i >= 0; i--) {
      ...
      if (effectData.getDOT() != 0) {
        getLivingEntityData()
            .setHealthPoints(getLivingEntityData().getHealthPoints() - effectData.getDOT());
      ...
      if (effectData.getHappy() != 0) {
        if (lemi.getType() == TYPE_CITIZEN) {
          ...
          if (!cit.getSoldierData().isSoldier()) {
            cit.getCitizenData()
                .setHappiness(cit.getCitizenData().getHappiness() + effectData.getHappy());
          }
      ...
      if (effectData.isFlee() && !isWaitingForPath()) {
        if (lemi.getType() == TYPE_CITIZEN) {
          ...
          if (cit.getCitizenData().getZoneID() != -1) {
            ...
            if (zone != null) {
              Point3DShort p3ds = Zone
                  .getFreeCellAtRandom(zone, World.getCell(getCoordinates()).getAstarZoneID());
              if (p3ds != null) {
                  ...
                  if (Game.getWorld().getZones() != null) {
                    for (int z = 0; z < Game.getWorld().getZones().size(); z++) {
                      ...
                      if (zone.getIniHeader().equals(sZone)) {
                        Point3DShort p3ds = Zone.getFreeCellAtRandom(
                            zone,
        ...
        if (effectData.getCastCooldown() >= effectData.getCastCooldownMAX()) {
          ...
          if (effectData.getCastTrigger() == SkillManagerItem.USE_UNKNOWN) {
            ...
            if (effectData.getCastTrigger() == SkillManagerItem.USE_ALWAYS) {
              ...
              if (getLivingEntityData().getHealthPoints() < getLivingEntityData()
      ...
      if (effectData.getLasts() <= 0) {
        ...
        if (emi.getLasts() != null) { // Tiene lasts, pues puede acabar
          ...
          // No tiene lasts, reseteo para que no se pase de rango (aunque podrÝan pasar a±os reales
          // para que esto pase, creo)
    ...
    if (getLivingEntityData().getHealthPoints() <= 0) {
      if (lemi.getType() == TYPE_CITIZEN) {
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_COMBAT,
            ((Citizen) this).getCitizenData().getFullName() + Messages.getString("Citizen.3"),
            ColorGL.RED,
            getCoordinates()); // $NON-NLS-1$
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_ANNOUNCEMENT,
            ((Citizen) this).getCitizenData().getFullName() + Messages.getString("Citizen.3"),
            ColorGL.RED,
        ...
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_HEROES,
            ((Hero) this).getCitizenData().getFullName() + Messages.getString("Citizen.3"),
            ColorGL.RED,
            getCoordinates()); // $NON-NLS-1$
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_ANNOUNCEMENT,
        ...
        if (getFocusData() != null && getFocusData().getEntityType() == TYPE_HERO) {
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_HEROES,
              getDeadMessage(),
              new ColorGL(Color.GREEN.darker()),
              getCoordinates());
        } else {
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_COMBAT,
    ...
    // Food needed (nunca valdrß 0, si vale 0 es que el bicho no necesita comer)
    if (getFoodNeededTurns() != 0) {
      setFoodNeededTurns(getFoodNeededTurns() - 1);
      if (getFoodNeededTurns() == 0) {
        // Ha LLEGADO a 0, creamos la tarea de comer, y lo ponemos a -1 (ya que nunca vale 0, s¾lo
        // cuando el living no come)
        ...
        // Estß en negativo, miramos si ya toca palmar
        if (Math.abs(getFoodNeededTurns()) >= lemi.getFoodNeededDieTurns()) {
          // BAM, muere de hambre
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_ANNOUNCEMENT,
              lemi.getName() + Messages.getString("Citizen.8"),
              new ColorGL(Color.ORANGE),
              getCoordinates()); // $NON-NLS-1$
          return true;
        }
    ...
    if (!isCellAllowed(x, y, z)) {
      boolean palma = true;
      // En caso de muro (o puerta bloqueada) o agua o hole no palma, son asÝ de guais
      Cell cell = World.getCell(x, y, z);
      ...
      if (cell.isDigged()) { // Cuidado con el ¾rden, la comprobaci¾n de digged debe ir aquÝ ya que
        // sabemos que no hay item en la celda
        palma = false;
        bWallFluidsHole = true;
      }
      ...
      if (palma) {
        if (lemi.getType() == TYPE_CITIZEN || lemi.getType() == TYPE_HERO) {
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_ANNOUNCEMENT,
              getCellDeadMessage(x, y, z, lemi.getType()),
              ColorGL.RED,
              getCoordinates());
        }
        return true;
    ...
    if (lemi.getHPRegeneration() > 0) {
      ...
      if (getLivingEntityData().getHealthPoints() < getLivingEntityData().getHealthPointsMAXCurrent()) {
        getLivingEntityData()
            .setHealthPoints(getLivingEntityData().getHealthPoints() + lemi.getHPRegeneration());
        if (getLivingEntityData().getHealthPoints() > getLivingEntityData().getHealthPointsMAXCurrent()) {
          getLivingEntityData().setHealthPoints(getLivingEntityData().getHealthPointsMAXCurrent());
        }
      }
    ...
    if (lemi.getType() == TYPE_CITIZEN) {
      ...
      if (!cit.getSoldierData().isSoldier()) {
        if (Task.isWorkingTask(cit.getCurrentTask())) {
          ...
          if (cd.getHappinessIdleCounter() <= 0) {
            if (Game.getWorld().getTurn() % (World.TIME_MODIFIER_HOUR / 2) == 0) {
              // S¾lo restamos happiness si realmente estß idle, no cuando come o duerme
              if (cit.getCurrentTask() == null
                  || cit.getCurrentTask().getTask() == Task.TASK_NO_TASK) {
                cd.setHappiness(cd.getHappiness() - 1);
              }
            }
          } else {
      ...
      // Eat sleep + turnos mÝnimos de stay (HEROS)
      ((Hero) this).updateCounters();
    }

    // Si estß en un muro o fluidos lo movemos si o si
    if (bWallFluidsHole) {
      if (!forceMove()) {
        if (lemi.getType() == TYPE_CITIZEN || lemi.getType() == TYPE_HERO) {
          MessagesPanel.addMessage(
    ...
    if (lemi.getType() == TYPE_CITIZEN || lemi.getType() == TYPE_HERO) {
      ...
      if (getCheckLOSCounter() == 0) {
        if (getCoordinates().z > 0) { // Las celdas de arriba estßn todas discovered
    ...
    if (getFocusData().getEntityID() == -1 && fleeing == 0) {
      ...
      if (getCheckLOSCounter() == 0) {
        if (lemi.getType() == TYPE_HERO
            && (getLivingEntityData().getHealthPointsMAXCurrent() / 3) > getLivingEntityData()
                .getHealthPoints()) {
          ...
          fd = hasEnemyInLOS(
              getIniHeader(),
              getCoordinates(),
              getLivingEntityData().getLOSCurrent(),
              bAttackAllies);

          // En el caso de heroes, miramos si tiene una skill lista para ser lanzada con
          // use=ENEMIES_IN_LOS
        ...
        if (fd != null && fd.getEntityID() != -1) {
          ...
          if (lemi.getType() == TYPE_HERO
              && getCheckLOSCounter() == 0) { // Cuidado con esto, se resetea a cero arriba
            Hero hero = (Hero) this;
            if (hero.canChangeTask(true, true)
                && hero.getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_EQUIPING) {
    ...
    if (getPath().isEmpty()) {
      ...
      if (getFocusData().getEntityID() != -1) {
        ...
        if (lemi.getFollowEntity() != null && lemi.getFollowEntity().length > 0) {
          getLivingEntityData()
              .setFollowTurnsCounter(getLivingEntityData().getFollowTurnsCounter() - 1);
          if (getLivingEntityData().getFollowTurnsCounter() <= 0) {
            ...
            // Buscamos una living de estas (normalmente s¾lo habrß 1 en el mundo, caso Sips) y le
            // hacemos un setdestination
            int[] alLivings = new int[1];
            // Obtenemos una living al azar
            alLivings[0] = lemi.getFollowEntity()[Utils
                .getRandomBetween(0, (lemi.getFollowEntity().length - 1))];
            int iTrys = 4;
            ...
            if (iTrys > 0) {
              // Para hacer que vaya a items a random, pillarÚ unas coordenadas a random y
              // buscaremos la living mßs cercana
              Point3DShort p3dRandom = Point3DShort.getPoolInstance(
                  Utils.getRandomBetween(0, World.MAP_WIDTH - 1),
                  Utils.getRandomBetween(0, World.MAP_HEIGHT - 1),
                  Utils.getRandomBetween(0, World.MAP_DEPTH - 1));
              LivingEntity le = searchLivingForcedASZID(
                  p3dRandom,
                  alLivings,
                  true,
                  null,
      ...
      if (point == null || (point.x == x && point.y == y && point.z == z)) {
        ...
        if (leHate != null) {
          ...
          if (leHate.getLivingEntityData().getHealthPoints() <= 0) {
            ...
            resetPathOffset();
            if (getAttackAnimationCounter() > 0
                && getAttackAnimationCounter() < ATTACK_ANIMATION_MAX_COUNTER) {
              setAttackAnimationCounter(0);
            }
          }
        } else {
          // No hay hater, intentamos movernos
          // Miramos que no haya un wallConector (puerta), en ese caso le metemos hostias hasta que
          // pete (s¾lo si no estß abierta y es enemy)
          // En caso de friendly no puede pasar si estß cerrada o locked (excepto caravanas)
          boolean bWallConector = false;
          if (lemi.getType() != TYPE_CITIZEN
              && lemi.getType() != TYPE_HERO
              && lemi.getType() != TYPE_ALLY
              && lemi.getCaravan() == null) {
            ...
            if (item != null && item.isOperative()) {
              ...
              if (lemi.getType() == TYPE_FRIENDLY) {
                ...
                if (imi != null
                    && imi.isDoor()
                    && !item.isDoorStatus(Item.FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_OPENED)) {
                  bWallConector = true;
          ...
          if (!bWallConector) {
            ...
            if (lemi.getType() != TYPE_CITIZEN
                || (((Citizen) this).getCurrentTask() == null
                    || ((Citizen) this).getCurrentTask().getTask() != Task.TASK_HEAL)) {
              // No hay enemigo, miramos si tenemos enemigo en una casilla al lado (esto evita lo de
              // citizen y enemigo corriendo arriba y abajo sin tocarse)
              FocusData fd = null;
              if (lemi.getType() == TYPE_HERO
                  && (getLivingEntityData().getHealthPointsMAXCurrent() / 3) > getLivingEntityData()
                      .getHealthPoints()) {
              ...
              // Citizen heleandose (curßndose, si)
              bCamina = true;
            }
            ...
            if (bCamina) {
              if (canWalkNext(this, lemi.getType())) {
                point = getPath()
                    .remove(0); // En principio no harÝa falta igualar otra vez el point a esto
                if (isCellAllowed(point.x, point.y, point.z)) {
                  ...
                  if (bPuede) {
                    ...
                    // Se acaba de mover, si es un citizen con tarea de drop y puede soltar el item,
                    // lo soltamos aquÝ mismo
                    if (lemi.getType() == TYPE_CITIZEN) {
                      Citizen cit = (Citizen) this;
                      if (cit.getCurrentTask() != null
                          && cit.getCurrentTask().getTask() == Task.TASK_DROP) {
                        if (cit.getCurrentTask().getParameter() == null
                            || !cit.getCurrentTask().getParameter().equals("P")) {
                          Cell cell = World.getCell(point);
        ...
        for (int i = 0; i < getLivingEntityData().getEffects().size(); i++) {
          ...
          if (emi != null) {
            getLivingEntityData()
                .getEffects()
                .get(i)
                .setCastTargets(new HateData(emi.getCastTargets()));
          }
        }
      }
  ...
  public void removeEffect(String id, int iLivingType, boolean verbose) {
    ...
    for (int i = (getLivingEntityData().getEffects().size() - 1); i >= 0; i--) {
      ...
      if (effectData.getEffectID().equals(id)) {
        ...
        if (verbose && emi.isMessageWhenVanish()) {
          if (iLivingType == TYPE_HERO) {
            MessagesPanel.addMessage(
                MessagesPanel.TYPE_HEROES,
                "["
                    + EffectManager.getItem(effectData.getEffectID()).getName()
                    + Messages.getString("LivingEntity.8")
                    + getLivingEntityData().getName(),
                ColorGL.ORANGE,
                getCoordinates(),
                getID()); // $NON-NLS-1$ //$NON-NLS-2$
        ...
        // Si era un effect de atacar aliados, borramos el target para que no siga atacßndolos
        if (effectData.isAttackAllies()) {
          getFocusData().setEntityID(-1);
          getFocusData().setEntityType(TYPE_UNKNOWN);
          setFighting(false);
  ...
  public void setCoordinates(short iX, short iY, short iZ) {
    int iIndex = -1;
    if (getX() != -1) { // Con esto miro que no sea una living nueva (que no tendrÝa celda source)
   ...
   * @param pointDestination
   *            Punto
   * @param bRemoveTaskIfFails
   *            Indica si hay que sacar al aldeano de la tarea si falla
   */
  public void setDestination(
      Point3DShort pointDestination,
      boolean bRemoveTaskIfFails,
      int iNumCellsToRemove) {
    setDestination(
        pointDestination.x,
        pointDestination.y,
        pointDestination.z,
  ...
  public void setDestination(
    ...
    // Si origen y destino tienen distinto A* zone ID, es que no existe camino posible, lo sacamos
    // de la tarea (si tiene)
    ...
    // Buscamos camino
    // Bresenham para empezar (linea recta) (si estß en el mismo nivel)
    // if (getZ () == z) {
    // setPath (Utils.bresenhamLine (getX (), getY (), x, y, z, livingType));
    // }
    // Camino vacÝo, eso es que en linea recta topa con algo (o estß en distintos niveles), usamos
    // A* para buscar la ruta
    // if (getPath ().isEmpty ()) {
    ...
    AStarQueueItem item = new AStarQueueItem(
        getID(),
        iLivingType,
        Point3DShort.getPoolInstance(getX(), getY(), getZ()),
        Point3DShort.getPoolInstance(x, y, z),
        bSiege);
  ...
  /**
   * @param positionOffsetConstants
   *            the positionOffsetConstants to set
   */
  ...
  /**
   * Desequipa un objeto, modifica los livingEntityData. Se le pasa la zona (cabeza, cuerpo, ....) que quiere
   * desequipar
   *
   * @param location
   *            Zona a desequipar
  ...
  public void updateFacingDirection(int x1, int y1, int z1, int x2, int y2, int z2) {
    ...
    if (x1 == x2) {
      ...
      if (y1 == y2) {
        ...
        // Si llega aquÝ es que es una diagonal si o si
        if (x1 < x2) {
          if (y1 < y2) {
  ...
  protected void doFocus(LivingEntityManagerItem lemi) {
    // Si el "enemigo" (<hate>) a·n existe vamos hacia Úl
    LivingEntity leHate = World.getLivingEntityByID(getFocusData().getEntityID());

    if (leHate != null) {
      // Lo tenemos
      // Ataque melee o ranged?
      boolean bRanged = getEquippedData().getWeapon() != null
          && ItemManager.getItem(getEquippedData().getWeapon().getIniHeader()).isRanged();
      if (bRanged) {
        if (Math.abs(leHate.getCoordinates().x - getX()) <= getLivingEntityData().getLOSCurrent()
            && Math.abs(leHate.getCoordinates().y - getY()) <= getLivingEntityData().getLOSCurrent()) {
          // Estß a rango, disparamos
          doRangeAttack(this, leHate, true);
          setPath(null); // Importante
        } else {
          // No estß a rango, nos olvidamos, pero nos acercamos si podemos
          if (World.getCell(leHate.getCoordinates()).getAstarZoneID() == World.getCell(getCoordinates())
              .getAstarZoneID()) {
            // Misma zona
      ...
      if (hasGrouping()) {
        for (int x = getX() - (getLivingEntityData().getLOSCurrent() * 2); x <= (getX()
            + (getLivingEntityData().getLOSCurrent() * 2)); x++) {
          for (int y = getY() - (getLivingEntityData().getLOSCurrent() * 2); y <= (getY()
              + (getLivingEntityData().getLOSCurrent() * 2)); y++) {
            if (x >= 0 && x < World.MAP_WIDTH && y >= 0 && y < World.MAP_HEIGHT) {
              ...
              if (le != null
                  && le.getFocusData().getEntityID() == -1
                  && le.getIniHeader().equals(getIniHeader())) {
   ...
   *
   * @param iPCT
   *            Porcentaje de moverse (suele ser menor que el parßmetro, ya que intentarß ir a un sitio y si no es
   *            allowed no se moverß)
  ...
  protected void moveAtRandom(int iPCT, int iType, int counter) {
    ...
    if (Utils.getRandomBetween(1, 100) <= iPCT) {
      ...
      if (Utils.getRandomBetween(1, 10) <= 3) {
        ...
        if (currentX >= 0
            && currentY >= 0
            && currentX < World.MAP_WIDTH
            && currentY < World.MAP_HEIGHT) {
          Cell cell = World.getCell(currentX, currentY, currentZ);
          if (isCellAllowed(cell)) {
            if (counter > 0
                && cell.hasBuilding()
                && !Building.getBuilding(cell.getBuildingCoordinates()).isOperative()
                || cell.containsSpecificLiving(iType) != null) {
            } else {
      ...
      if (!bMoved) {
        ...
        while (iTrys > 0) {
          ...
          switch (iRandom) {
            case 1 :
              // if (nodo.z > 0 && nodo.z < World.MAP_NUM_LEVELS_OUTSIDE && World.getCell (nodo.x,
              // nodo.y, nodo.z - 1).isMined ()) {
              if (currentZ > 0 && World.getCell(currentX, currentY, currentZ - 1).isMined()) {
                iAuxX = currentX - 1;
                iAuxY = currentY - 1;
                iAuxZ = currentZ - 1;
            ...
            case 18 :
              if (currentZ < (World.MAP_DEPTH - 1)
                  && currentX > 0
                  && currentY > 0
                  && World.getCell(currentX - 1, currentY - 1, currentZ).isMined()) {
                iAuxX = currentX - 1;
                iAuxY = currentY - 1;
                iAuxZ = currentZ + 1;
              }
            ...
            case 19 :
              if (currentZ < (World.MAP_DEPTH - 1)
                  && currentY > 0
                  && World.getCell(currentX, currentY - 1, currentZ).isMined()) {
                iAuxX = currentX;
                iAuxY = currentY - 1;
                iAuxZ = currentZ + 1;
              }
            ...
            case 21 :
              if (currentZ < (World.MAP_DEPTH - 1)
                  && currentX > 0
                  && World.getCell(currentX - 1, currentY, currentZ).isMined()) {
                iAuxX = currentX - 1;
                iAuxY = currentY;
                iAuxZ = currentZ + 1;
              }
          ...
          if (isCellAllowed(iAuxX, iAuxY, iAuxZ)) {
            ...
            if (counter > 0
              ...
              if (iDirection >= 0 && iDirection < 8 && Utils.getRandomBetween(1, 2) == 1) {
                ...
                if (iAuxX >= 0
                    && iAuxX < World.MAP_WIDTH
                    && iAuxY >= 0
                    && iAuxY < World.MAP_HEIGHT) {
                  if (isCellAllowed(iAuxX, iAuxY, iAuxZ)) {
                    Point3DShort p3d2 = Point3DShort.getPoolInstance(iAuxX, iAuxY, iAuxZ);
                    getPath().add(p3d2);

                    // Un 10% mßs de moverse otra casilla
                    if (Utils.getRandomBetween(1, 10) == 1) {
                      ...
                      if (iAuxX >= 0
                          ...
                          && iAuxY < World.MAP_HEIGHT) {
                        if (isCellAllowed(iAuxX, iAuxY, iAuxZ)) {
                          Point3DShort p3d3 = Point3DShort
                              .getPoolInstance(iAuxX, iAuxY, iAuxZ);
                          getPath().add(p3d3);
                        }
                      }
                    }
  ...
  /** Setea la constante, tambiÚn cambia el facing direction */
  protected void updatePathConstantOffsets() {
    ...
    if (getPath().size() == 0) {
      ...
      Point2D.Float pTMP2 = getPositionOffsetConstants();
      pTMP2.x = (((p3dDestino.x - getX()) + (p3dDestino.y - getY())) * (Tile.TERRAIN_ICON_WIDTH / 2))
          / totalFrames;
      pTMP2.y = (((p3dDestino.y - getY()) - (p3dDestino.x - getX())) * (Tile.TERRAIN_ICON_HEIGHT / 2))
          / totalFrames;

      // Si estß cambiando de nivel hay que sumar, restar a la Y
  ...
  /**
   * Analiza la cadena de offset (que es "x, y") y devuelve el valor numÚrico de una de las 2
   *
   * @param sValue
   *            Cadena de offset
   * @param firstValue
   *            Indica si hay que devolver el primer valor de la cadena o el segundo
   * @return
   */
  private int getOffsetValue(String sValue, boolean firstValue) {
    ...
    if (firstValue) {
      ...
      try {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("LivingEntity.0") + sValue + "][" + e.toString() + "]",
            getClass().toString()); // $NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
        return 0;
      }
    } else {
      // La Y
      try {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("LivingEntity.3") + sValue + "][" + e.toString() + "]",
            getClass().toString()); // $NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
        return 0;
      }
    }
