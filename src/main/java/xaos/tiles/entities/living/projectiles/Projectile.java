...
public class Projectile extends Tile implements Externalizable {
  ...
  public boolean nextTurn() {
    ...
    LivingEntity.doHit(
        ...
        getVictim(),
        false,
        ItemManager.getItem(attackerWeapon),
        damage,
        maxDistance,
        false);
    return true;
    ...
    if (point.x != -1) { // No se si es realmente necesario
      ...
      if (imi != null && imi.getLightRadius() > 0) {
        Cell.generateLightsItemRemovedCellMined(
            ...
            point.y,
            point.z,
            imi.getLightRadius() + 1); // +1 para que borre lo de al lado (donde estaba antes)
      }
    }
  }
  ...
  public void setPath(ArrayList<Point3DShort> path) {
    ...
    if (path != null && path.size() > 1) {
      ...
      // Miramos qué parte es más corta (servirá para girarlo bien en modo isométrico) (difícil de
      // entender)
