...
public class HeroManager {
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "heroes.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
  ...
  private static void loadXML(String sXMLName) {
    try {
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (node.getNodeName().equals("comePrerequisites")) { // $NON-NLS-1$
            ...
            throw new Exception(
                Messages.getString("HeroManager.3")
                    + node.getNodeName()
                    + "]"); //$NON-NLS-1$ //$NON-NLS-2$
          }
          ...
          for (int j = 0; j < nodesPrerequisites.getLength(); j++) {
            ...
            if (nodePrerequisite.getNodeType() == Node.ELEMENT_NODE) {
              ...
              for (int v = 0; v < nodePrerequisiteChilds.getLength(); v++) {
                ...
                if (nodePrerequisiteChild.getNodeType() == Node.ELEMENT_NODE) {
                  ...
                  if (iType == 1 || iType == 2) {
                    ...
                    // Lo ańadimos a la lista
                    alPrerequisites.add(hp);
                  } else if (iType == 3) {
                    // Behaviours
                    if (sChildID.equalsIgnoreCase("IDLEPCT")) { // $NON-NLS-1$
                      behaviour.setIdlePCT(
                          UtilsXML.getChildValue(nodePrerequisiteChilds, sChildID));
                    } else if (sChildID.equalsIgnoreCase("EXPLOREPCT")) { // $NON-NLS-1$
                      behaviour.setExplorePCT(
                          UtilsXML.getChildValue(nodePrerequisiteChilds, sChildID));
                    } else {
                      throw new Exception(
                          Messages.getString("HeroManager.1")
                              + sChildID
                              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
                    }
                  } else if (iType == 4) {
              ...
              // Ańadimos la lista a la hash que toque
              if (iType == 1) {
                ...
                if (behaviour.checkPCTs()) {
                  ...
                  throw new Exception(
                      Messages.getString("HeroManager.4") + sID + "]"); // $NON-NLS-1$ //$NON-NLS-2$
                }
              } else if (iType == 4) {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("HeroManager.0") + sXMLName + "] [" + e.toString() + "]",
          "HeroManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
}
