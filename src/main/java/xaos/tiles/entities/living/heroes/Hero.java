...
public class Hero extends LivingEntity implements Externalizable {
          ...
          if (lemi.getType() == TYPE_HERO) {
            ...
            if (TownsProperties.DEBUG_MODE) {
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Task ID: " + hero.getHeroData().getHeroTask().getTaskID(),
                      null,
                      null,
                      null,
                      null,
                      p3d.toPoint3D())); // $NON-NLS-1$
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      "Exploring counter: " + hero.getHeroData().getExploringCounter(),
                      null,
                      null,
                      null,
                      null,
            ...
            sm.addItem(
                new SmartMenu(
                    SmartMenu.TYPE_TEXT, hero.getLivingEntityData().toString(), null, null, null));
  ...
  public boolean canChangeTask(boolean checkEat, boolean checkSleep) {
    ...
    return iTaskID != HeroTask.TASK_LEAVING
        && iTaskID != HeroTask.TASK_RAGE
        && iTaskID != HeroTask.TASK_DESTROY_BLOCKING;
  }

  public void checkAndUseSkills(int useType) {
    if (getHeroData().getHeroTask() != null
        && getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_EATING
        && getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_SLEEPING
        && getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_LEAVING) {
      SkillData sd;
    ...
    if (getHeroData().updateBehaviour()) {
      ...
      getHeroData()
          .setCurrentBehaviourID(
              HeroBehaviour.next(
                  getHeroData().getCurrentBehaviourID(),
                  getHeroData().getHeroBehaviour()));
      // Turnos
      getHeroData()
          .setTurnsToNextBehaviour(
              HeroBehaviour.getTurns(
                  getHeroData().getCurrentBehaviourID(),
                  getHeroData().getHeroBehaviour()));
  ...
  public void hitted(LivingEntity le, boolean bHitted, int iDamage) {
    // super.hitted (le, bHitted); // El comentario es para que no responda a las hostias, ya se
    // hace abajo si hace falta

    if (bHitted) {
      // Si tiene los puntos de vida bajos se va a la habitación personal (o taberna si no tiene) a
      // descansar
      ...
      if ((getLivingEntityData().getHealthPointsMAXCurrent() / 3) > getLivingEntityData().getHealthPoints()) {
        ...
        if (!bADescansar && !bYaEstabaDescansando) {
          ...
          if (cell.getZoneID() > 0) {
            ...
            if (zone != null
                && ZoneManager.getItem(zone.getIniHeader()).getType() == ZoneManagerItem.TYPE_DINING) {
      ...
      if (bADescansar) {
        // El hero se va a descansar, le quitamos el focus y le pondremos el exploring counter alto
        // para que no se vaya de marcha hasta al cabo de un rato
        getFocusData().setEntityID(-1);
        getFocusData().setEntityType(TYPE_UNKNOWN);
        setFighting(false);
        int turnosDescansando = getLivingEntityData().getHealthPointsMAXCurrent()
            - (getLivingEntityData().getHealthPointsMAXCurrent() / 3);
      ...
      if (bYaEstabaDescansando) { // Esto es para que responda a ataques si ya estaba en la zona de
        // descanso
    ...
    // Si está al borde de la muerte (10% HP) miramemos también si puede usar skills de tipo
    // NEAR_DEATH
    if ((getLivingEntityData().getHealthPointsMAXCurrent() / 3) >= getLivingEntityData().getHealthPoints()) {
  ...
  public void startLeaving() {
    ...
    if (getHeroData().getStartingPoint() != null
        && World.getCell(getHeroData().getStartingPoint()).getAstarZoneID() == iASZID) {
      ...
      if (p3dLeave != null) {
        ...
        MessagesPanel.addMessage(
            ...
            getCitizenData().getFullName() + Messages.getString("Hero.2"),
            ColorGL.RED,
            getCoordinates(),
            getID()); // $NON-NLS-1$
      }
  ...
  /**
   * Resta los contadores de stay, comida y sueńo, suma el contador de puntos de vida
   *
   * @return
   */
  public void updateCounters() {
    ...
    if (getCitizenData().getHungry() < 0) {
      ...
      if (getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_EATING
          && getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_SLEEPING
          && getHeroData().getHeroTask().getTaskID() != HeroTask.TASK_LEAVING) {
        // Buscamos comida
        Point3DShort p3d = Item.searchFood(
            getCoordinates(),
            getID()); // Héroes no le quitan la comida a los aldeanos, pero sí al revés
        if (p3d == null) {
          // No hay comida
          getCitizenData().setHungryEating(getCitizenData().getHungryEating() - 1);

          if (getCitizenData().getHungryEating() < -(2 * World.TIME_MODIFIER_HOUR)) { // 2 horas sin comida en
            // el mundo, se pira
            if (canChangeTask(false, false)) { // Falses pq comida y sleep ya están mirados arriba
              getHeroData().getHeroTask().setTaskID(HeroTask.TASK_LEAVING);
              MessagesPanel.addMessage(
                  MessagesPanel.TYPE_HEROES,
                  getCitizenData().getFullName() + Messages.getString("Hero.1"),
                  ColorGL.ORANGE,
                  getCoordinates(),
                  getID()); // $NON-NLS-1$
            }
  ...
  private void doEating() {
    if (getCarrying() == null) {
      ...
      if (imiCarrying.canBeEaten()) {
        ...
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_HEROES,
            getCitizenData().getFullName() + Messages.getString("Hero.3") + imiCarrying.getName(),
            ColorGL.ORANGE,
            getCoordinates(),
            getID()); // $NON-NLS-1$
      }
  ...
  private void doExploring() {
    if (getHeroData().getExploringCounter() == 0) {
      ...
      Point3DShort p3dExploringHP = Game.getWorld()
          .getRandomExploringHotpoint(World.getCell(getCoordinates()).getAstarZoneID());
  ...
  /**
   * Focus a citizen in the zone (to attack him). If no citizen he enters in "DESTROY_BLOCKING_ITEMS" mode
   */
  private void doRage() {
  ...
  private void moveFriendsTo(Point3DShort p3ds) {
    ...
    if (getHeroData().getFriendships().size() > 0) {
      ...
      for (int h = 0; h < World.getHeroIDs().size(); h++) {
        ...
        if (hero != null
            && hero.getID() != getID()
            && hero.getHeroData().getHeroTask().getTaskID() == HeroTask.TASK_EXPLORING) {
          ...
          if (hero.getFocusData() == null || hero.getFocusData().getEntityID() == -1) {
            ...
            if ((hero.getLivingEntityData().getHealthPointsMAXCurrent() / 3) <= hero.getLivingEntityData()
                .getHealthPoints()) {
  ...
  private void useSkill(SkillManagerItem smi) {
    ...
    if (alSkillEffects != null && alSkillEffects.size() > 0) {
      ...
      for (int i = 0; i < alSkillEffects.size(); i++) {
        ...
        if (sei.getTarget() != SkillEffectItem.TARGET_INT_NONE) {
          // Tiene target, hay que ańadir el effect al target
          ...
          if (alTargets != null) {
            if (!bVerbosed) {
              MessagesPanel.addMessage(
                  MessagesPanel.TYPE_HEROES,
                  getCitizenData().getFullName()
                      + Messages.getString("Hero.6")
                      + smi.getName()
                      + "]",
                  ColorGL.WHITE,
                  getCoordinates(),
    ...
    if (smi.isTaunt()) {
      ...
      if (alTargets.size() > 0) {
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_HEROES,
            getCitizenData().getFullName() + Messages.getString("Hero.6") + smi.getName() + "]",
            ColorGL.WHITE,
