...
public class Enemy extends LivingEntity implements Externalizable {
    ...
    if (alLivings != null) {
      for (int liv = 0; liv < alLivings.size(); liv++) {
        ...
        if (lemi.getType() == TYPE_ENEMY) {
          ...
          sm.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_TEXT, le.getLivingEntityData().toString(), null, null, null));
  ...
  private void doSiegeRobbery(LivingEntityManagerItem lemi) {
    if (getCarryingData() != null
        && (getCarryingData().getCarrying() != null
            || getCarryingData().getCarryingLiving() != null)) {
    ...
    if (lemi.getStealLivings() != null) {
      ...
      if (cell.getLivings() != null) {
        for (int i = 0; i < cell.getLivings().size(); i++) {
          if (UtilsIniHeaders.contains(
              lemi.getStealLivings(),
              cell.getLivings().get(i).getNumericIniHeader())) {
    ...
    if (lemi.getSteal() != null) {
      ...
      if (itemCell != null) {
        ...
        if (imi.isContainer()
            || UtilsIniHeaders.contains(lemi.getSteal(), imi.getNumericalIniHeader())) {
          // BINGO! Lo pillamos
          if (getCarryingData() == null) {
            setCarryingData(new CarryingData());
          }
    ...
    if (Game.getWorld().getContainers().size() > 0) {
      ...
      for (int i = 0; i < alContainers.size(); i++) {
        ...
        if (container != null) {
          ...
          if (itemContainer != null
              && World.getCell(itemContainer.getCoordinates()).getAstarZoneID() == iASZI) {
      ...
      if (alContainersInArea.size() > 0) {
        ...
        getSiegeData()
            .setCount(
                World.TIME_MODIFIER_HOUR * LivingEntityManager.getItem(getIniHeader()).getLevel());
        if (getSiegeData().getCount() <= 0) {
  ...
  private void doSiegeStandard() {
    if ((World.getNumCitizens() + World.getNumSoldiers()) > 0) {
      // En caso de siege y con aldeanos en el mundo le metemos un focus a alguien en el ASZI (el
      // citizen más cercano)
      int iASZI = World.getCell(getCoordinates()).getAstarZoneID();
      Citizen citizen;
      ...
      if (alCitsInArea.size() > 0) {
        getFocusData()
            .setEntityID(alCitsInArea.get(Utils.getRandomBetween(0, (alCitsInArea.size() - 1))));
