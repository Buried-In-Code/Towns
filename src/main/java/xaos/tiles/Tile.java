...
public class Tile implements Externalizable {
  ...
  public static final int TERRAIN_ICON_WIDTH = 64;

  public static final int TERRAIN_ICON_HEIGHT = 32;
  ...
  public void changeGraphic(String sNewIniHeader) {
    ...
    setAnimationTiles(
        Towns.getPropertiesInt(
            PropertyFile.PROPERTY_FILE_GRAPHICS,
            "[" + sNewIniHeader + "]ANIMATION_TILES")); // $NON-NLS-1$ //$NON-NLS-2$
    setAnimationFrameDelay(
        Towns.getPropertiesInt(
            PropertyFile.PROPERTY_FILE_GRAPHICS,
            "[" + sNewIniHeader + "]ANIMATION_FRAME_DELAY",
            Game.FPS_INGAME)); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public void refreshTransients() {
    ...
    setAnimationTiles(
        Towns.getPropertiesInt(
            PropertyFile.PROPERTY_FILE_GRAPHICS,
            "[" + getIniHeader() + "]ANIMATION_TILES")); // $NON-NLS-1$ //$NON-NLS-2$
    setAnimationFrameDelay(
        Towns.getPropertiesInt(
            PropertyFile.PROPERTY_FILE_GRAPHICS,
            "[" + getIniHeader() + "]ANIMATION_FRAME_DELAY",
            Game.FPS_INGAME)); // $NON-NLS-1$ //$NON-NLS-2$
    setCurrentAnimationTile(0);
    setCurrentFrameDelay(Utils.getRandomBetween(0, getAnimationFrameDelay() - 1));
  }
  ...
  public void setTextureID(String sIniHeader, String sIniHeader2) {
    TextureData texture = UtilsGL.loadTexture(
        Towns.getPropertiesString(
            PropertyFile.PROPERTY_FILE_GRAPHICS,
            "[" + sIniHeader + "]TEXTURE_FILE"),
        GL11.GL_REPLACE);
    if (texture == null) {
      texture = UtilsGL.loadTexture(
          Towns.getPropertiesString(
              PropertyFile.PROPERTY_FILE_GRAPHICS,
              "[" + sIniHeader2 + "]TEXTURE_FILE"),
          GL11.GL_REPLACE); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      if (texture == null) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("Tile.6")
                + sIniHeader
                + Messages.getString("Tile.7")
                + sIniHeader2
                + "]",
  ...
  public void setTileSetCoordinates(String iniHeader, String iniHeader2) {
    this.tileX = (byte) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_X",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    this.tileY = (byte) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_Y",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    this.tileXRot = (byte) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_X_ROT",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    this.tileYRot = (byte) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_Y_ROT",
        -1); // $NON-NLS-1$ //$NON-NLS-2$

    if (this.tileX == -1 || this.tileY == -1) {
      this.tileX = (byte) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_X"); // $NON-NLS-1$ //$NON-NLS-2$
      this.tileY = (byte) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_Y"); // $NON-NLS-1$ //$NON-NLS-2$
      this.tileXRot = (byte) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_X_ROT"); // $NON-NLS-1$ //$NON-NLS-2$
      this.tileYRot = (byte) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_Y_ROT"); // $NON-NLS-1$ //$NON-NLS-2$
    }
  }
  ...
  public boolean setTileSetCoordinatesReturn(String iniHeader, String iniHeader2) {
    ...
    if (this.tileX == -1 || this.tileY == -1) {
      ...
      this.tileXRot = (byte) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_X_ROT",
          -1); // $NON-NLS-1$ //$NON-NLS-2$
      this.tileYRot = (byte) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_Y_ROT",
          -1); // $NON-NLS-1$ //$NON-NLS-2$
      if (this.tileX == -1 || this.tileY == -1) {
        return false;
      }
  ...
  public void setTileSetTexCoordinates() {
    ...
    this.tileTexX1 = this.tileTexX0
        + (TEXTURE_ICON_WIDTH * ((float) this.tileWidth / (float) TERRAIN_ICON_WIDTH));
    this.tileTexY1 = this.tileTexY0
        + (TEXTURE_ICON_HEIGHT * ((float) this.tileHeight / (float) TERRAIN_ICON_HEIGHT));
    if (this.tileXRot != -1 && this.tileYRot != -1) {
      ...
      this.tileTexX1Rot = this.tileTexX0Rot
          + (TEXTURE_ICON_WIDTH * ((float) this.tileWidth / (float) TERRAIN_ICON_WIDTH));
      this.tileTexY1Rot = this.tileTexY0Rot
          + (TEXTURE_ICON_HEIGHT * ((float) this.tileHeight / (float) TERRAIN_ICON_HEIGHT));
    } else {
  ...
  public void setTileSize(String iniHeader, String iniHeader2) {
    // Width
    this.tileWidth = (short) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_WIDTH",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    if (this.tileWidth == -1) {
      this.tileWidth = (short) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_WIDTH"); // $NON-NLS-1$ //$NON-NLS-2$
      if (this.tileWidth == 0) {
        this.tileWidth = TERRAIN_ICON_WIDTH;
      }
    }
    this.tileWidthOffset = (short) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_WIDTH_OFFSET",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    if (this.tileWidthOffset == -1) {
      this.tileWidthOffset = (short) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_WIDTH_OFFSET"); // $NON-NLS-1$ //$NON-NLS-2$
    }

    // Height
    this.tileHeight = (short) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_HEIGHT",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    if (this.tileHeight == -1) {
      this.tileHeight = (short) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_HEIGHT"); // $NON-NLS-1$ //$NON-NLS-2$
      if (this.tileHeight == 0) {
        this.tileHeight = TERRAIN_ICON_HEIGHT;
      }
    }
    this.tileHeightOffset = (short) Towns.getPropertiesInt(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + iniHeader + "]TILE_HEIGHT_OFFSET",
        -1); // $NON-NLS-1$ //$NON-NLS-2$
    if (this.tileHeightOffset == -1) {
      this.tileHeightOffset = (short) Towns.getPropertiesInt(
          PropertyFile.PROPERTY_FILE_GRAPHICS,
          "[" + iniHeader2 + "]TILE_HEIGHT_OFFSET"); // $NON-NLS-1$ //$NON-NLS-2$
    }
  }
