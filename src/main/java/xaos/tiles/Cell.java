...
public class Cell implements Externalizable {
  ...
  // Flags
  public static final int FLAG_ORDERS = 1; // 00000001 (Indica si la celda tiene órdenes)
  public static final int FLAG_DISCOVERED = 2; // 00000010 (Indica si la celda está descubierta)
  public static final int FLAG_CAVE = 4; // 00000100 (Indica si la celda es de tipo caverna (se usa para descubrir la
  // caverna entera
  // cuando descubres una celda tipo cave))
  public static final int FLAG_PATROL_POINT = 8; // 0000001000 (Indica si algún soldado tiene ese punto de patrulla)
  public static final int FLAG_HERO_EXPLORING_POINT = 16; // 0000010000 (Indica si el punto es uno de los puntos de
  // exploring de los heroes)
  public static final int FLAG_SHOULD_PAINT_UNDER = 32; // 0000100000 (Indica si hay que pintar casillas de niveles
  // inferiores)
  public static final int FLAG_DIGGED = 64; // 0001000000 (Indica si la casilla de abajo está minada)
  public static final int FLAG_SHADOW = 128; // 0010000000 (Indica si la casilla está en la sombra)
  public static final int FLAG_BLOCKY = 256; // 0100000000 (Indica si la casilla está ocupada por un bloque que tapa
  // las cosas de
  // debajo)
  public static final int FLAG_MINED = 512; // 10 00000000 (Indica si la casilla está minada)
  public static final int FLAG_LIGHT_RFULL = 1024; // 100 00000000 (Indica si la casilla tiene luz roja full cerca)
  public static final int FLAG_LIGHT_GFULL = 2048; // 1000 00000000 (Indica si la casilla tiene luz verde full cerca)
  public static final int FLAG_LIGHT_BFULL = 4096; // 10000 00000000 (Indica si la casilla tiene luz azul full cerca)
  public static final int FLAG_LIGHT_RHALF = 8192; // 100000 00000000 (Indica si la casilla tiene luz roja floja
  // cerca)
  public static final int FLAG_LIGHT_GHALF = 16384; // 1000000 00000000 (Indica si la casilla tiene luz verde floja
  // cerca)
  public static final int FLAG_LIGHT_BHALF = 32768; // 10000000 00000000 (Indica si la casilla tiene luz azul floja
  // cerca)
  public static final int FLAG_BURY = 65536; // 1 00000000 00000000 (Indica si la casilla tiene luz azul floja cerca)
  public static final int FLAG_FLUID_CHECK_LIST = 131072; // 10 00000000 00000000 (Indica si la casilla está en la
  // lista de fluidos a chequear
  public static final int FLAG_OPEN = 262144; // 10 00000000 00000000 (Indica si la casilla está en la lista de
  // fluidos a chequear
  public static final int FLAG_BLINK = 524288; // 100 00000000 00000000 (Indica si la casilla está en la lista de
  // fluidos a chequear
  ...
  public static boolean checkPaintUnder(Cell[][][] cells, int x, int y, int z) {
    ...
    // if (cellTmp.isDiscovered () && !cellTmp.isBlocky () && cellTmp.isMined () &&
    // cellTmp.getTerrain ().getFluidCount () <= 4) {
    if (cellTmp.isDiscovered()
        && !cellTmp.isBlocky()
        && cellTmp.getTerrain().getFluidCount() <= 4) {
      return true;
    }
  ...
  public static void createTerrain(Point3DShort p3d, String sTerrainID) {
    ...
    if (tmi != null) {
      ...
      // cell.getTerrain ().setTerrainTileID(TerrainManagerItem.TERRAIN_AIR_ID *
      // TerrainManager.SLOPES_INIHEADER.length);
      ...
      for (short i = -1; i <= 1; i++) {
        for (short j = -1; j <= 1; j++) {
          for (short k = 0; k <= 1; k++) {
            ...
            // Game.getWorld().addFluidCellToProcess(x + i, y + j, z + k,
            // false);
          }
        }
      }
      ...
      for (int y = 0; y < World.MAP_HEIGHT; y++) {
        for (int z = 0; z < (World.MAP_DEPTH - 1); z++) {
          ...
          if (cells[x][y][z].isMined()) {
            ...
            cells[x][y][z].setBlocky(
                TerrainManager.getItemByID(cells[x][y][z].getTerrain().getTerrainID()).isBlocky());
          }
        }
      }
    }
  ...
  public static void generateLights(Cell[][][] cells, int x, int y, int z, boolean bProjectiles) {
    ...
    if (bProjectiles && Projectile.getLocations() != null) {
      if (Projectile.getLocations()[x][y][z] > 0) {
        ...
        for (int c = 0; c < Game.getWorld().getProjectiles().size(); c++) {
          ...
          if (projectile.getCoordinates().x == x
              && projectile.getCoordinates().y == y
              && projectile.getCoordinates().z == z) {
    ...
    if (imi != null) {
      if (imi.getLightRadius() != 0) {
        ...
        // Todas las casillas de las lineas (y las que están por arriba y abajo, teniendo en cuenta
        // el radio) tendrán luz, hasta que lleguen a un non-mined o wall
        int iX, iY;
        // Norte del cuadrado
        iY = y - iRadius;
        if (iY < 0) {
  ...
  public static void generateLightsItemRemovedCellMined(int x, int y, int z, int iRadius) {
    ...
    // Ahora, teniendo en cuenta el radio máximo de todos los objetos que producen luz, miramos las
    // celdas a iluminar
  ...
  public static int getDepth(int x, int y, int z) {
    ...
    // return ((getCoordinates ().y - MainPanel.depthYMin) * MainPanel.maxTilesWidthHeight +
    // (MainPanel.maxTilesWidthHeight - 1 - (getCoordinates ().x - MainPanel.depthXMin))) +
    // (World.MAP_DEPTH - getCoordinates ().z);
  }

  /**
   * Indica si la celda es allowed para dig. Sin fluidos, sin diggar, descubierta, sin stockpile, sin zona, minada,
   * sin edificio
   *
   * @param cell
  ...
  public static boolean isDiggable(Cell cell) {
    if (cell.getCoordinates().z < (World.MAP_DEPTH - 2)
        && !cell.isDigged()
        && cell.isDiscovered()
        && !cell.hasStockPile()
        && !cell.hasZone()) {
   ...
   *            Z
   * @param bFull
   *            Indica si se está haciendo un merge de todo, en ese caso el getNeighbour no hace falta que mire todos
   *            los vecinos
   */
  public static void mergeZoneID(int x, int y, int z, boolean bFull) {
    ...
    if (neighborIDsIndex == 0) {
      ...
      // IDs distintos, acabamos de abrir el punto que une varias zonas, actualizamos todas las
      // celdas que tengan estos IDs
  ...
  private static void checkConnectorsRoads(int x, int y, int z, Item item, ItemManagerItem imi) {
    ...
    if (sbConnectors.toString().length() > 1) {
      ...
      item.setTileSetCoordinates(
          item.getIniHeader() + sbConnectors.toString(),
          item.getIniHeader());
  ...
  private static void checkConnectorsWalls(int x, int y, int z, Item item, ItemManagerItem imi) {
    ...
    if (y > 0) {
      ...
      if (!cellTemp.isMined()) {
        ...
        if (entityTemp != null && entityTemp.isOperative()) {
          imiTemp = ItemManager.getItem(entityTemp.getIniHeader());
          // popoif (entityTemp.isOperative () && ((imi.isWallConnector () && imiTemp.isWall ()) ||
          // (imiTemp.isWallConnector () && imi.isWall ()) || entityTemp.getIniHeader ().equals
          // (entity.getIniHeader ()))) {
    ...
    if (sbConnectors.toString().length() > 1) {
      // Buscamos el connector que toca
      if (bInteriors) {
        // Si tiene interiors y no existen miraremos el _NSEW (sin minúsculas), si tampoco existe
        // miraremos el iniHeader normal
        boolean bOK = item.setTileSetCoordinatesReturn(
            item.getIniHeader() + sbConnectors.toString(),
            item.getIniHeader() + "_NSEW"); // $NON-NLS-1$
        if (!bOK) {
  ...
  public void setDiscovered(boolean discovered) {
    // Si no estaba descubierta de antes, actualizamos el contador de livings y le decimos al
    // minimapa que se actualice
    ...
    if (bFromNonDiscoveredToDiscovered) {
      ...
      if (alLivings != null
          && alLivings.size() > 0) { // Hay livings, los sacamos de undiscovered y los ponemos en discovereds
        LivingEntity le;
                ...
                if (iIniHeader != null) {
                  ...
                  if (sIniHeader != null) {
                    ...
                    if (imi != null && isMined()) {
                      ...
                      if (!imi.isBuryDestroyItem()) {
                        ...
                        if (alBuryItem != null && alBuryItem.size() > 0) {
                          ...
                          for (int p = 0; p < alBuryItem.size(); p++) {
                            if (Utils.getRandomBetween(1, 100) <= imi.getBuryItemPCT()
                                .get(p).intValue()) {
                              // Bingo
                        ...
                        if (imiItem != null
                          ...
                          if (newItem != null) {
                            ...
                            if (alTexts != null && alTexts.size() > 0) {
                              World.getItemsText()
                                  .put(Integer.valueOf(newItem.getID()), alTexts);
                            }
                          }
                        }
                      }
                      ...
                      // buryLivings
                      if (!bWall
                          && imi.getBuryLivings() != null
                          && imi.getBuryLivings().size() > 0) {
                        String sLiving;
                        for (int i = 0; i < imi.getBuryLivings().size(); i++) {
                          ...
                          if (Utils.getRandomBetween(1, 100) <= iPCT) {
                            ...
                            LivingEntityManagerItem lemi = LivingEntityManager
                                .getItem(sLiving);
                            if (lemi != null
                                && LivingEntity.isCellAllowed(getCoordinates())) {
                              World.addNewLiving(
                                  sLiving,
                                  lemi.getType(),
                                  true,
      ...
      if (getAstarZoneID() != -1) {
        ...
        if (getCoordinates().z < (World.MAP_DEPTH - 1)) {
          World.getCell(getCoordinates().x, getCoordinates().y, getCoordinates().z + 1)
  ...
  public void setEntity(Entity entity) {
    ...
    if (entity == null || (this.entity != null)) {
      // Se va a borrar la celda (o se va a meter algo encima de otro algo), restamos el townvalue
      // de lo que hubiera
        ...
        if (imi.isContainer()) {
          ...
          if (container != null) {
            for (int i = 0; i < container.getItemsInside().size(); i++) {
              World.setTownValue(
                  World.getTownValue() - PricesManager.getPrice(container.getItemsInside().get(i)));
            }
          }
        }
    ...
    // Miramos si el item puede ir aquí, en caso contrario lo meteremos en la lista de objetos para
    // haulear
    if (entity != null && entity instanceof Item) {
      if (stockpile != null) {
        ...
        // No hay pila, lo metemos en la lista de haul, el taskmanager ya mirará si es un item haul
        // o no
        Game.getWorld().addItemToBeHauled((Item) entity);
      }
    }
    ...
    if (entity != null && entity instanceof Item) {
      ...
      Game.updateTutorialFlow(
          TutorialTrigger.TYPE_INT_PLACE,
          imi.getNumericalIniHeader(),
          getCoordinates().toPoint3D());
    }
    ...
    if (iLightRadiusItemRemoved >= 0) {
      ...
      if (imi != null && imi.getLightRadius() > 0) {
        ...
        if (Utils.isCellBlockingLightItem(imi, newCellItem)) {
          generateLightsItemRemovedCellMined(
              coord.x,
              coord.y,
              coord.z,
