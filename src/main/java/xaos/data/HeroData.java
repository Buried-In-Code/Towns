...
public class HeroData implements Externalizable {
  ...
  private static final int MIN_TURNS_TO_STAY = World.TIME_MODIFIER_DAY;
  public static final int TURNS_BETWEEN_EXPLORE = World.TIME_MODIFIER_HOUR;

  // Idle
  public static final int IDLE_NONE = 0;
  public static final int IDLE_STEAL = 1;
  public static final int IDLE_SOCIAL = 2;
  ...
  public HeroData(LivingEntityManagerItem lemi) {
    ...
    setTurnsToNextBehaviour(
        0); // This way the first hero turn will change the behavior to the first one
  ...
  /**
   * Mira si tiene que subir de nivel (o niveles), en ese caso lo hace y devuelve true. También ańade las skills que
   * se obtienen en ese nivel
   *
   * @return
   */
  public boolean checkAndUpdateLevelUp(Hero hero, boolean verboseSkills) {
    if (getLevel() < 50 && getXp() >= (300 * getLevel())) {
      while (getLevel() < 50 && getXp() >= (300 * getLevel())) {
        ...
        HeroSkills hSkills = HeroManager
            .getSkills(LivingEntityManager.getItem(hero.getIniHeader()).getHeroSkills());
        if (hSkills != null) {
              ...
              if (verboseSkills) {
                MessagesPanel.addMessage(
                    ...
                    hero.getCitizenData().getFullName()
                        + Messages.getString("HeroData.0")
                        + SkillManager.getItem(newSkills.get(i)).getName()
                        + "]",
                    ColorGL.GREEN,
                    hero.getCoordinates(),
                    hero.getID()); // $NON-NLS-1$ //$NON-NLS-2$
              }
            }
          }
        }
  ...
  /**
   * Resta 1 a los turnos de behaviour, si llegan a 0 devuelve true para poder cambiar la tarea
   *
   * @return
   */
