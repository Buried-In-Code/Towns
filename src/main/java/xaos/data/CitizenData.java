...
public class CitizenData implements Externalizable {
  ...
  // Turnos (se usa para la "animación" (cada X pasos se resetea a 0) (también al pillar un objeto,
  // para que no todos los aldeanos tengan la misma animación)
  ...
  private int hungryEating; // Cuando tiene hambre se pone a 0 y luego a cada turno irá sumando 1 hasta
  // llegar a un tope, en ese momento ya habrá comido
  ...
  private int sleepSleeping; // Cuando tiene sueńo se pone a 0 y luego a cada turno irá sumando 1 hasta
  // llegar a un tope (6h), en ese momento ya habrá dormido

  // Heal
  private int healHealing; // Cuando va a curarse se pone a 0, si está en un hospital se irá sumando 1 hasta
  // llegar a un tope (1h), en ese momento se curará 1PV
  ...
  public CitizenData(LivingEntityManagerItem lemi) {
        ...
        if (Utils.getRandomBetween(1, 100) == 1) {
          ...
          sFullName.append(
              Names.getName(
                  "nick",
                  Game.getWorld().getCampaignID(),
                  Game.getWorld().getMissionID())); // $NON-NLS-1$
          sFullName.append("' "); // $NON-NLS-1$
        }

        sFullName.append(
            Names.getName(
                lemi.getSurnamePoolTag(),
                Game.getWorld().getCampaignID(),
                Game.getWorld().getMissionID()));
      }
    ...
    setSleep(
        Utils.getRandomBetween(
            getMaxSleep() / 2,
            getMaxSleep() * 2)); // Al crearse randomizo un poco para que no vayan a dormir a la vez
  ...
  /**
   * @param blinkAnimationTurns
   *            the blinkAnimationTurns to set
   */
