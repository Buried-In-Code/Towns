...
public class CaravanData implements Externalizable {
  ...
  public boolean confirmTrade() {
    ...
    for (int i = 0; i < getMenuTownToSell().getItems().size(); i++) {
      ...
      for (int q = 0; q < smItem.getDirectCoordinates().y; q++) {
        ...
        task.setPointIni(
            new Point3D(
                getLivingId(),
                smItem.getDirectCoordinates().z,
                -1)); // Caravan ID y itemID (caso militares)
    ...
    if (getMenuCaravanToBuy().getItems().size() == 0
        && getMenuTownToSell().getItems().size() == 0) {
  ...
  public boolean itemCarried(Item item) {
    ...
    for (int i = 0; i < getMenuTownToSell().getItems().size(); i++) {
      ...
      if (bMilitary) {
        ...
        if (smItem.getDirectCoordinates().x == iPrice
    ...
    if (iIndexItem != -1) {
      ...
      if (p3dInfo.y <= 0) {
        ...
        getMenuTownToSell()
            .getItems()
            .get(iIndexItem)
            .setParameter2("x" + p3dInfo.y); // $NON-NLS-1$
      }
  ...
  public void setStatus(int status) {
    ...
    if (status == STATUS_IN_PLACE) {
      ...
      if (le != null) {
        MessagesPanel.addMessage(
            MessagesPanel.TYPE_ANNOUNCEMENT,
            Messages.getString("CaravanData.1"),
            ColorGL.ORANGE,
            le.getCoordinates(),
            le.getID()); // $NON-NLS-1$
      }
  ...
  /**
   * Chequea si la caravana está lista para trade Cambia el flag "ready" de la misma (tanto true como false)
   *
    ...
    switch (getStatus()) {
            ...
            if (le.getPath().size() == 0) {
              ...
              if (iZoneID != 0) {
                ...
                if (zone != null
                    && cmi != null
                    && cmi.getZone() != null
                    && zone.getIniHeader().equals(cmi.getZone())) {
                ...
                if (cmi != null) {
                  ...
                  if (alDestinations == null || alDestinations.size() == 0) {
                    ...
                    while (alDestinations.size() > 0) {
                      p3dDestination = alDestinations.remove(
                          Utils.getRandomBetween(0, alDestinations.size() - 1));
      ...
      case STATUS_LEAVING :
        ...
        if (le != null) {
          ...
          if ((le.getFocusData() != null || le.getFocusData().getEntityID() == -1)
              && le.getPath().size() == 0) {
            // Lista para irse
            if (World.getCell(le.getCoordinates()).getAstarZoneID() == World.getCell(getStartingPoint())
                .getAstarZoneID()) {
              // Via libre
              le.setDestination(getStartingPoint());
            } else {
              // Buscamos una salida
              Point3DShort p3dLeave = World
                  .getRandomBorderPoint(World.getCell(le.getCoordinates()).getAstarZoneID());
              if (p3dLeave != null) {
      ...
      if (iIndexItem != -1) {
        ...
        for (int i = 0; i < alItems.size(); i++) {
          ...
          if (itemAux != null && itemAux instanceof MilitaryItem) {
            ...
            // NO militar
            if (((cidi.getPrice() * getPricePCT()) / 100) == smItem.getDirectCoordinates().x
                && cidi.getItem().getIniHeader().equals(smItem.getParameter())) {
              // Encontrado!
              iIndexRealItem = i;
              break;
            }
        ...
        if (iIndexRealItem != -1) {
          ...
          // Item no encontrado ż?ż?ż? rarísimo, lo eliminamos de la lista de to-buy
          getMenuCaravanToBuy().getItems().remove(iIndexItem);
        }
      }
    }
  ...
  private Point3DShort getDropPoint(Point3DShort p3dIniPoint, ItemManagerItem imi) {
    ...
    if (!bCellFound) {
      ...
      radiusloop : for (int radius = 1; radius <= 8; radius++) {
        // Arriba y abajo
        for (int i = -radius; i <= radius; i++) {
          p3dTemp = Point3DShort.getPoolInstance(
              p3dIniPoint.x + i,
              p3dIniPoint.y - radius,
              p3dIniPoint.z);
          if (p3dTemp.x >= 0
              && p3dTemp.x < World.MAP_WIDTH
              && p3dTemp.y >= 0
              && p3dTemp.y < World.MAP_HEIGHT) {
