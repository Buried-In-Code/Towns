...
public class Type implements Externalizable {
  ...
  /**
   * Devuelve el tipo principal de un nombre de tipo ej: food.meat devolvería food
   *
   * @param sTypeName
  ...
  private static void loadTypeNamesAndIcons() {
    ...
    // Cargar de fichero
    loadXML(
        Towns.getPropertiesString("DATA_FOLDER") + "types.xml", // $NON-NLS-2$
        true); // $NON-NLS-1$
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "types.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
  ...
  private static void loadXML(String sXMLPath, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los nombres a la hash
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bLoadingMain && typeNames.containsKey(sTypeID)) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("Type.0") + sTypeID + "]",
          ...
          if (node.getAttributes() != null
              && node.getAttributes().getNamedItem("icon") != null
              && node.getAttributes().getNamedItem("icon").getNodeValue() != null) {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Type.3") + e.toString() + "]",
          "Type"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
