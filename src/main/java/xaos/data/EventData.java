...
public class EventData implements Externalizable {
          ...
          if (i != 0 || j != 0 || n != 0) {
            if (Utils.isInsideMap(x + i, y + j, n + z)) {
              ...
              if (cell.getTerrain().hasFluids()) {
                ...
                if (iSpecialTerrain == MapGeneratorItem.SPECIAL_INT_WATER
                    || iSpecialTerrain == MapGeneratorItem.SPECIAL_INT_WATER_1
                    || iSpecialTerrain == MapGeneratorItem.SPECIAL_INT_WATER_INF) {
                  // Agua
                  if (cell.getTerrain().getFluidType() == Terrain.FLUIDS_WATER) {
                    return true;
                  }
                } else if (iSpecialTerrain == MapGeneratorItem.SPECIAL_INT_LAVA
                    || iSpecialTerrain == MapGeneratorItem.SPECIAL_INT_LAVA_1
                    || iSpecialTerrain == MapGeneratorItem.SPECIAL_INT_LAVA_INF) {
                  // Lava
  ...
  public void checkAllItemsDeletePCTs(EventManagerItem emi) {
    if (emi.getItems() != null
        && emi.getItemsDeletePCT() != null
        && emi.getItems().size() > 0
        && emi.getItems().size() == emi.getItemsDeletePCT().size()) {
      ...
      for (int i = 0; i < emi.getItems().size(); i++) {
        ...
        if (Item.getNumItemsTotal(sIniHeader, World.MAP_DEPTH - 1) > 0) {
          ...
          if (imi != null) {
            ...
            ArrayList<Integer> alItems = Item.getMapItemsLocked()
                .get(UtilsIniHeaders.getIntIniHeader(sIniHeader));
            if (alItems != null) {
              Item item;
   ...
   * @param emi
   * @param bRestore
   *            Restoring items to the original state
   */
  ...
  public void checkAllItemsSpawnLiving(EventManagerItem emi) {
    if (emi.getItems() != null
        && emi.getItemsSpawnLiving() != null
        && emi.getItemsSpawnLivingSize() != null
        && emi.getItems().size() > 0
        && emi.getItems().size() == emi.getItemsSpawnLiving().size()
        && emi.getItemsSpawnLiving().size() == emi.getItemsSpawnLivingSize().size()) {
      // Obtenemos todos los items
                  ...
                  if (item != null) {
                    ...
                    for (int n = 0; n < iSize; n++) {
                      World.addNewLiving(
                          ...
                          lemi.getType(),
                          true,
                          item.getCoordinates().x,
                          item.getCoordinates().y,
                          item.getCoordinates().z,
                          true);
                    }
                  }
                }
   ...
   * @param emi
   * @param effectsList
   *            afterEat, afterSleep, effects, ...
   * @param singleLiving
   *            Si se pasa una living sólo intenta aplicarlo a ella, no a todos
   */
  public void launchEffects(
      EventManagerItem emi,
      ArrayList<String> effectsList,
      LivingEntity singleLiving) {
    if (emi.getTargets() != null && emi.getTargets().size() > 0) {
      ...
      if (effectsList != null && effectsList.size() > 0) {
        ...
        // Por si acaso han cambiado el xml, comprobamos tamańos y esas cosas
        if (alTargets != null && alPCTs != null && alTargets.size() == alPCTs.size()) {
          ...
          if (singleLiving != null
              && emi.isTargetsOpenCell()
              && !World.getCell(singleLiving.getCoordinates()).isOpen()) {
            return;
          }
          ...
          if (emi.getTargetsRandomCell() == null || emi.getTargetsRandomCell().length() == 0) {
            ...
            foreffects : for (int ef = 0; ef < effectsList.size(); ef++) {
                ...
                for (int i = 0; i < alTargets.size(); i++) {
                  ...
                  if (singleLiving != null) {
                    LivingEntityManagerItem lemi = LivingEntityManager
                        .getItem(singleLiving.getIniHeader());
                    if (lemi != null) {
                      if (Utils.getRandomBetween(1, 100) <= iPCT
                          && emi.getTargetsHateData().isHate(singleLiving)) {
                  ...
                  if (iTargetType == LivingEntity.TYPE_HERO) {
                    ...
                    for (int h = 0; h < World.getHeroIDs().size(); h++) {
                      ...
                      if (le != null && Utils.getRandomBetween(1, 100) <= iPCT) {
                        // Miramos el openCell
                        if (emi.isTargetsOpenCell()
                            && !World.getCell(le.getCoordinates()).isOpen()) {
                          continue;
                        }
                  ...
                  } else if (iTargetType == LivingEntity.TYPE_ALLY
                      || iTargetType == LivingEntity.TYPE_FRIENDLY
                      || iTargetType == LivingEntity.TYPE_ENEMY) {
            ...
            while (iNumCells > 0) {
              ...
              if (cell.isDiscovered()
                  && cell.getLivings() != null
                  && cell.getLivings().size() > 0) {
                  ...
                  if (lemi != null) {
                    if (emi.getTargetsHateData().isHate(le)) {
                      ...
                      for (int t = 0; t < alTargets.size(); t++) {
                        if (iType == LivingEntity.TYPE_HERO
                            && alTargets.get(t).equals(LivingEntityManagerItem.TYPE_HERO)) {
                          iTargetIndex = t;
                          break;
                        } else if (iType == LivingEntity.TYPE_CITIZEN
                            && alTargets.get(t)
                                .equals(LivingEntityManagerItem.TYPE_CITIZEN)) {
                          iTargetIndex = t;
                          break;
                        } else if (iType == LivingEntity.TYPE_ALLY
                            && alTargets.get(t).equals(LivingEntityManagerItem.TYPE_ALLY)) {
                          iTargetIndex = t;
                          break;
                        } else if (iType == LivingEntity.TYPE_FRIENDLY
                            && alTargets.get(t)
                                .equals(LivingEntityManagerItem.TYPE_FRIENDLY)) {
                          iTargetIndex = t;
                          break;
                        } else if (iType == LivingEntity.TYPE_ENEMY
                            && alTargets.get(t)
                                .equals(LivingEntityManagerItem.TYPE_ENEMY)) {
                          iTargetIndex = t;
                          break;
                        } else {
                          // Direct living
                          LivingEntityManagerItem lemiTarget = LivingEntityManager
                              .getItem(alTargets.get(t));
                      ...
                      if (iTargetIndex != -1) {
                        ...
                        if (Utils.getRandomBetween(1, 100) <= Utils
                            .launchDice(alPCTs.get(iTargetIndex))) {
                          // BAM, efectos
                          EffectManagerItem efmi;
      ...
      if ((z >= cd.iHeightMin || cd.iHeightMin == -1)
        ...
        for (int x = 0; x < World.MAP_WIDTH; x++) {
          for (int y = 0; y < World.MAP_HEIGHT; y++) {
            if (cd.source == null
                || (World.getCell(x, y, z).getTerrain().getTerrainID() == iSourceTerrainID)) {
              if (iSourceTerrainID != TerrainManagerItem.TERRAIN_AIR_ID) {
                ...
                if (Utils.getRandomBetween(1, 100) <= cd.pct) {
                  ...
                  if (iSpecialType == MapGeneratorItem.SPECIAL_INT_NONE) {
                    if (cd.terrain == null
                        || checkNeighbors(x, y, z, iTerrainTerrainID, cd.radius)) {
                      abMap[x][y][z] = true;
                    }
                  } else {
          ...
          if (abMap[x][y]) {
            ...
            for (int z = 0; z <= World.MAP_NUM_LEVELS_OUTSIDE; z++) {
              ...
              if (cell.getTerrain().getTerrainID() == TerrainManagerItem.TERRAIN_AIR_ID) {
                ...
                // Tenemos un no-air
                if (bHayAIRs) {
                  // Tiene espacio por encima, así que simplemente cambiamos el terrain por el de
                  // abajo
