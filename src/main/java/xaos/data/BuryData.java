...
public class BuryData implements Externalizable {
  ...
  private int height; // Altura (SÓLO SE ACTUALIZA CUANDO SE GENERA EL BURY EN UN NUEVO MAPA, NO VIENE DE
  // SERIE, YA QUE DEPENDE DE LO QUE HAYA EN EL MAPA)
  private HashMap<Point3DShort, Integer> hash; // Hash por cada punto del bury con su numeric iniHeader
  private HashMap<Point3DShort, ArrayList<String>> hashTexts; // Hash por cada punto del bury con los textos de item
        ...
        for (int z = 0; z < World.MAP_DEPTH; z++) {
          ...
          if (item != null) {
            ...
            if (imi.isBury()) {
              ...
              // Si es un wall, vaciamos lo de debajo hasta encontrar otro wall o una celda
              // non-mined
              if (imi.isWall()) {
