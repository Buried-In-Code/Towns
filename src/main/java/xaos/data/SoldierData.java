...
public class SoldierData implements Externalizable {
  ...
  /**
   * Mira si tiene que subir de nivel (o niveles), en ese caso lo hace y devuelve true.
   *
   * @return
   */
  ...
  public void setGroup(int group, int livingID) {
    if (getState() == STATE_NOT_A_SOLDIER) {
      ...
      if (bDifferentGroups) {
        ...
        // Lo ańadimos a la lista que toque
        ...
        // Si pasa de aldeano a soldado podría ser que sea el mismo grupo (-1), así que miramos eso
        // y lo ańadimos a la lista de soldados sin grupo
        if (!Game.getWorld()
            .getSoldierGroups()
            .getSoldiersWithoutGroup()
            .contains(Integer.valueOf(livingID))) {
          Game.getWorld().getSoldierGroups().addSoldierToGroup(livingID, this.group);
        }
      }
    }
  ...
  /**
   * Cambia el estado del soldier/citizen, y lo mueve de las listas si hace falta
   *
   * @param state
   * @param livingID
   */
