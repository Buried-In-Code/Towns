...
public class LivingEntityData implements Externalizable {
   ...
   * @param mi
   * @param level
   *            Se le pasa 0 o el nivel del héroe
   */
  public void setModifiers(LivingEntity le) {
    ...
    for (int i = 0; i < effects.size(); i++) {
      ...
      if (effect.getHealthPointsPCT() != 100) {
        setHealthPointsMAXCurrent(
            (getHealthPointsMAXCurrent() * effect.getHealthPointsPCT()) / 100);
      }
  ...
  public String toString() {
    return Messages.getString("LivingEntityData.10")
        + getHealthPoints()
        + "/"
        + getHealthPointsMAXCurrent()
        + Messages.getString("LivingEntityData.13")
        + getAttackCurrent()
        + Messages.getString("LivingEntityData.14")
        + getDefenseCurrent()
        + Messages.getString("LivingEntityData.15")
        + getDamageCurrent(); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
  }
  ...
  /**
   * Setea los modificadores según un objeto dado, mete efectos si hace falta pero no les activa los modificadores
   *
   * @param mi
   *            Objeto
   */
