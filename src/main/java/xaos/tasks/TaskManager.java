...
public final class TaskManager implements Externalizable {
  ...
  // pero
  // no hay que eliminarla (performance)
  ...
  /**
   * Ańade una custom action, controla que no esté metida, para no repetirla
   *
   * @param action
  ...
  /**
   * Ejecuta todas las tareas necesarias del manager (sincronizar, tareas directas, eliminar tareas vencidas, asignar
   * nuevas tareas...
   */
  public void executeAll(boolean bOnlyDirect) {
    ...
    // Miramos si tenemos tareas automáticas del panel de producción, sólo lo hacemos cada
    // FPS_INGAME turnos
    ...
    // Tiene que hacerse lo primero de todo, ya que si no podrían pasar tareas sin hotpoint (tareas
    // directas) al método de asignar tareas normales
    synchronized (taskItemsTemp) {
    ...
    synchronized (alProductionToRemove) {
      synchronized (customActionsTemp) {
        ...
        // Primero metemos 4 de las wait (esto es para performance, si hay 2000 acciones no las
        // chequeará todas cada vez)
        for (int i = 0; i < 4; i++) {
  ...
  /**
   * Marca un hotpoint como acabado. Mira si hay más, en caso contrario marca la tarea como acabada. Los aldeanos la
   * llaman cuando acaban su tarea.
   *
   * @param task
   *            Tarea
   * @param hotPointIndex
  ...
  /**
   * Obtiene el ID más alto de una tarea y pone ese ID+1 como el actual ID Se usa al cargar una partida, para no tener
   * taskID's repetidos
   */
  ...
  private void addToNonAvailableHash(
      Action action,
  ...
  /**
   * Asigna tareas de create (construcción de items), se le pasa la hash de aldeanos ociosos
   *
   * @param hmCitizensSinTarea
   */
  private int assignCreateTasks(
      HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea,
      int iLibres) {
    ArrayList<Integer> citizens = World.getCitizenIDs();
    Citizen citizen;

    // Recorremos todos los edificios a ver si hay alguno con tarea de construcción y ningún aldeano
    // haciéndola
    Building building;
    ...
    breakBuildings : for (int i = 0; i < World.getBuildings().size(); i++) {
      ...
      if (building.isOperative() && building.hasItemsInQueue()) {
        ...
        // Edificio con tareas, primero miramos si hay aldeanos en la zona y después miramos si no
        // se está haciendo por otro aldeano
   ...
   * @param hmCitizensSinTarea
   */
  private int assignCustomActions(
      HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea,
      int iLibres,
      int priorityIndex) {
    ArrayList<Citizen> alCits;
    ...
    synchronized (customActions) {
      while (iActionsProcesadas < MAX_CUSTOM_ACTIONS_PER_TURN_AND_PRIORITY_LEVEL
          ...
          for (int i = 0; i < action.getQueue().size(); i++) {
            ...
            if (queueItem.getType() == QueueItem.TYPE_CREATE_ITEM) {
              ...
              if (imi == null) {
                Log.log(
                    Log.LEVEL_ERROR,
                    Messages.getString("TaskManager.10") + queueItem.getValue() + "]",
              ...
              if (alItems == null || alItems.isEmpty()) {
                Log.log(
                    Log.LEVEL_ERROR,
                    Messages.getString("TaskManager.11") + " [" + queueItem.getValue() + "]",
        ...
        // Me aseguro que el primer "pick/pickLiving" y TODOS los "move" estén disponibles para el
        // aldeano (de momento sólo pillo valores, más adelante lo miro)
        ...
        for (int i = 0; i < action.getQueue().size(); i++) {
          ...
          } else if (iFirstPickItemOrLiving == 0
              && queueItem.getType() == QueueItem.TYPE_PICK_FRIENDLY) {
            sPickItemLiving = queueItem.getValue();
        ...
        // Cola chequeada, miramos que los items tengan el mismo A*zoneID y que existan (y
        // operativas las de move)
        ...
        for (int i = 0; i < citizens.size(); i++) {
          ...
          if (!alASZIDAMirar.contains(Integer.valueOf(iCitASZID))) {
            ...
            alASZIDAMirarCoordinates.add(
                World.getLivingEntityByID(citizens.get(i)).getCoordinates());
          }
        }

        // Si tiene forcepick y ningún aldeano tiene el mismo ASZID, pos p'acasa
        if (p3dForcedPick != null
            && !alASZIDAMirar.contains(World.getCell(p3dForcedPick).getAstarZoneID())) {
        ...
        if (alMoveITem.size() > 0) {
          // Miraremos según los aldeanos (NO miramos los libres, que quizá no hay ninguno, miramos
          // TODOS)
          ...
          while (iMovesEncontrados < alMoveITem.size() && alASZIDAMirarCoordinates.size() > 0) {
            ...
            for (int i = 0; i < alMoveITem.size(); i++) {
              ...
              // Item.SEARCH_TRUE, Item.SEARCH_TRUE, null, true); // Buscamos ordenadamente (el
              // último true)
              Point3DShort puntoMoveItem = Item.searchItem(
                  false,
                  p3d,
                  UtilsIniHeaders.getIntsArray(itemToSearch),
                  false,
                  Item.SEARCH_DOESNTMATTER,
                  Item.SEARCH_TRUE,
                  null,
                  ...
                  false,
                  Game.getWorld()
                      .getRestrictHaulEquippingLevel()); // Buscamos ordenadamente, evitamos
              // containers
            ...
            if (!action.isSilent()) {
              ...
              if (moveItems.size() > 0) {
                ...
                MessagesPanel.addMessage(
                    ...
                    ami.getName()
                        + Messages.getString("TaskManager.5")
                        + sName
                        + Messages.getString("TaskManager.2"),
                    ColorGL.RED); // $NON-NLS-1$ //$NON-NLS-2$
              }
            }
            removeAction(iIndex, false, null);
            if (action.getDestinationPoint() != null) {
        ...
        // Si llega aquí es que los moves están OK, miramos que el primero de la lista no esté en
        // uso, en ese caso saltamos la tarea por el momento
        if (alMoveITem.size() > 0) {
          ...
          while (itAldeanos.hasNext()) {
            ...
            if (alCits != null && alCits.size() > 0) {
              ...
              if (Item.searchItem(
                  true,
                  alCits.get(0).getCoordinates(),
                  UtilsIniHeaders.getIntsArray(itemsToSearch),
                  false,
                  Item.SEARCH_DOESNTMATTER,
                  Item.SEARCH_TRUE,
                  alItemsInUse,
        ...
        if (sPickItemLiving != null) {
          ...
          if (alMoveITem.size() > 0) {
            ...
            if (iFirstPickItemOrLiving == 1) {
              ...
              puntoPickItemLiving = Item.searchItem(
                  false,
                  p3dMove,
                  aiPicks,
                  false,
              ...
              LivingEntity le = LivingEntity.searchLiving(
                  p3dMove,
            ...
            if (p3dMoveTerrain != null) {
              ...
              if (cellTerrain.isMined()) {
                ...
                if (p3dMoveTerrain.z > 0) {
                  cellTerrain = World
                      .getCell(p3dMoveTerrain.x, p3dMoveTerrain.y, p3dMoveTerrain.z - 1);
                  alCits = hmCitizensSinTarea.get(cellTerrain.getAstarZoneID());
                  // } else {
                  // No se puede acceder
                }
        ...
        if (p3dMoveTerrain != null) {
          ...
          alCits = hmCitizensSinTarea.get(
              Integer.valueOf(World.getCell(p3dDestination).getAstarZoneID()));
          ...
          // Ni move ni pick..... cualquier aldeano nos vale (que esté en el mismo A*ZID del
          // destination !)
          if (action.getDestinationPoint() != null
              && World.getCell(action.getDestinationPoint()).getAstarZoneID() != -1) {
            alCits = hmCitizensSinTarea.get(
                Integer.valueOf(World.getCell(action.getDestinationPoint()).getAstarZoneID()));
            if (alCits != null && alCits.size() > 0) {
              p3dDestination = alCits.get(0).getCoordinates();
            }

            if (p3dDestination == null) {
              // No hay aldeanos en la zona, miramos aldeanos en otras zonas adyacentes para ver si
              // pueden construir desde otra celda
              Point3DShort p3dNeighbour;
              int iASZIDNeighbour;
              bucleNeighbours : for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                  for (int z = -1; z <= 1; z++) {
                    p3dNeighbour = Point3DShort.getPoolInstance(
                        action.getDestinationPoint().x + x,
                        action.getDestinationPoint().y + y,
                        action.getDestinationPoint().z + z);
        ...
        if (alCits != null && alCits.size() > 0) {
          ...
          if (iIndexCit != -1) {
            ...
            hmCitizensSinTarea.put(
                Integer.valueOf(World.getCell(citizen.getCoordinates()).getAstarZoneID()),
                alCits);
            ...
            // Tarea metida, la metemos en las non-available para que espere al menos 1 turno (por
            // los <locks>)
   ...
   * @param hmCitizensSinTarea
   */
  private void assignHaulTasks(
      HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea,
      int iLibres,
      int iHaulers) {
    // Las tareas de haul (put_in_container) usan algo de CPU, tampoco mucho (casi nada de hecho),
    // pero hay que recorrerse todos los items del mundo (árboles inclusive)
    ...
    if (stockpiles.size() > 0 || containers.size() > 0) {
      ...
      // Vamos a recorrernos los items del mundo y ir viendo si está en una stockpile válida, en
      // caso negativo asignaremos a un aldeano
      ...
      break1 : for (int i = haulIndex; i < MAX_INDEX; i++) {
        ...
        // boolean bItemCanBeStockpiled = !cell.hasStockPile () || (!Stockpile.getStockpile
        // (item.getCoordinates ()).itemAllowed (item.getIniHeader ()));
        ...
        if (alCits != null && alCits.size() > 0) {
          ...
          fd = LivingEntity.hasEnemyInLOS(
              alCits.get(0).getIniHeader(),
              item.getCoordinates(),
              6,
              false);
          if (fd != null && fd.getEntityID() != -1) { // TODO: 6 de LOS a pińon
            continue;
          }
          ...
          if (!bStockEncontrada && World.getCell(item.getCoordinates()).hasStockPile()) {
            ...
            // En todo el nivel no hay ni un hueco, haremos el truqui de buscar en un radio de 8x8
                              ...
                              if (cell.isEmpty()
                                ...
                                if (!cell.hasStockPile()
                                  ...
                                  if (iIndexCit != -1) {
                                    ...
                                    iLibres--;
                                    hmCitizensSinTarea
                                        .put(new Integer(iMatZoneID), alCits);
                                    if (iLibres <= 0) {
                                      // No más aldeanos libres
                                      break break1; // Fin
                                    }
   ...
   * @param citizens
   * @param iTaskType
   *            indica el tipo de tarea a procesar
   * @return
   */
  private void assignNormalTasks(
      HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea,
    ...
    fortasks : for (int i = 0; i < taskItems.size(); i++) {
      ...
      if (item.getTask().getTask() == Task.TASK_WEAR
          || item.getTask().getTask() == Task.TASK_WEAR_OFF
      ...
      // Miramos si es tarea de construcción, en ese caso no asignamos aldeano si no hay items (o
      // livings) en el mundo
      ...
      if (item.getTask().getTask() == Task.TASK_BUILD) {
        ...
        if (alPrerequisites == null || alPrerequisites.isEmpty()) {
          ...
          for (int it = 0; it < alPrerequisites.get(0).length; it++) {
            iNumItems += Item.getNumItems(
                alPrerequisites.get(0)[it],
                false,
                Game.getWorld().getRestrictHaulEquippingLevel());
            if (iNumItems != 0) {
              break;
          ...
          if (iNumItems <= 0) {
            ...
            // Si hay materiales miramos que nos vayan bien como prerequisito y que estén en el
            // mismo A*ZoneID que el edificio
            boolean bMatsEnZona = false;
            for (int it = 0; it < alPrerequisites.size(); it++) {
              bMatsEnZona = (Item.searchItem(
                  true,
                  building.getCoordinates(),
                  alPrerequisites.get(it),
                  false,
                  Item.SEARCH_FALSE,
                  Item.SEARCH_DOESNTMATTER,
                  null,
        ...
        if (bItemPrerequisitesDone) {
          ...
          if (alPrerequisites != null && alPrerequisites.size() > 0) {
            // Hay prerequisitos living, antes de asignar la tarea miramos que exista alguno de esos
            // livings en la zona del edificio
            boolean bLivingEnZona = false;
            for (int li = 0; li < alPrerequisites.size(); li++) {
              bLivingEnZona = (LivingEntity.searchLiving(
                  building.getCoordinates(),
                  alPrerequisites.get(li),
                  false,
                  null) != null);
              if (bLivingEnZona) {
                break;
        ...
        if (iItemID != -1) {
          ...
          if (p3dMilitar == null) {
            // No está actualmente en el mundo, quizá en el carrying de algún aldeano, de momento
            // pasamos
            continue;
          } else {
            // El item existe, podemos asignar tarea a algún aldeano (si está en la misma zona que
            // la caravana)
        ...
        if (leToFeed == null) {
          // Si la living la tiene pillada alguien de momento no hacemos nada, en otro caso (ha
          // muerto) eliminamos la tarea
          Citizen cit;
        ...
        if (lemi.getFoodNeeded() == null) {
          // Curiosamente no tiene feed, quizá se ha modificado el .xml y el animal es antiguo,
          // pacasa
          item.getTask().setFinished(true);
          continue;
        }
      ...
      while (iCitizens < iMaxCitizens && !bTaskEnded) {
        ...
        iIndex = 0; // Marcará el índice de aldeano que está más cerca y que esté en la misma zona que
        // alguno de sus places
        int iHPIndex = 0;
        HotPoint hotPoint;
        ...
        while (!bHotPointEncontrado && !bTaskEnded) {
          ...
          // Miramos que NO esté terminado y que ningún aldeano ya lo esté haciendo y que las places
          // del hotpoint sean accesible por aldeanos
          boolean bHotpointLibre = !hotPoint.isFinished();

          if (bHotpointLibre) {
            Game.iError = 66731;
            // Miramos las places del hotpoint a ver si hay alguna accesible y con aldeanos que
            // puedan llegar
          ...
          if (bHotpointLibre) { // Hotpoint sin terminar y sin nadie haciéndolo, le metemos el
            // aldeano INACTIVO más cercano con A*ZoneID igual a alguna de sus
            // places
            Game.iError = 6674;
            ...
            // Para evitar stucks raros primero miramos esta casuistica (haul con hotpoint distinto
            // de pointini)
            ...
            for (int c = 0; c < hotPoint.getPlaces().size(); c++) {
              ...
              ArrayList<Citizen> alCits = hmCitizensSinTarea.get(
                  new Integer(
                      World.getCells()[p3dPlaces.x][p3dPlaces.y][p3dPlaces.z]
                          .getAstarZoneID()));
            ...
            if (alCitizensSinTarea.size() == 0) {
              ...
              if (iIndex != -1) {
                ...
                Integer iZoneID = new Integer(
                    World.getCells()[citizen.getX()][citizen.getY()][citizen.getZ()]
                        .getAstarZoneID());
   ...
   * @return
   */
  private int assignRemoveFromContainerTasks(
      HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea,
      int iLibres) {
    ArrayList<Container> containers = Game.getWorld().getContainers();
    Container container;
  ...
  private void assignTasks() {
    ...
    // assignFightTasks ();
    // Obtenemos aldeanos inactivos (y que no estén cargando nada) y los metemos en una hash, la key
    // será el número de zona A*
    ...
    for (int i = 0; i < citizens.size(); i++) {
      citizen = (Citizen) World.getLivingEntityByID(citizens.get(i));
      if (!citizen.isWaitingForPath()
          && citizen.isIdle()
          && citizen.getCarrying() == null
          && citizen.getCarryingLiving() == null
          && citizen.getCitizenData().getSleep() > 0) {
        // Tenemos aldeano sin tarea, miramos la zona A* y lo metemos en la hash
        Integer iZID = new Integer(
            World.getCells()[citizen.getX()][citizen.getY()][citizen.getZ()].getAstarZoneID());
    ...
    // Primero borramos tareas que puedan ser null ż?ż?ż?, esto evita un pete
    ...
    Game.iError = 670;
    // Nos hemos "pulido" todas las tareas, miramos si quedan aldeanos libres para las tareas
    // mine/dig, move_caravan, build_building, feed_animals, custom y de haul
    iLibres = getNumCitizens(hmCitizensSinTarea);
    if (iLibres == 0) {
      return;
    }
    ...
    int iBuildingsPriority = ActionPriorityManager.getPriorityValueByPriorityID(
        ActionPriorityManager.PRIORITY_BUILD_BUILDINGS);
    int iFeedAnimalsPriority = ActionPriorityManager.getPriorityValueByPriorityID(
  ...
  /**
   * Asigna tareas de equiparse/desequiparse, se le pasa la hash de aldeanos ociosos
   *
   * @param hmCitizensSinTarea
   */
    ...
    for (int i = 0; i < taskItems.size(); i++) {
      ...
      if (task.getTask() == Task.TASK_WEAR) {
        ...
        if (task.getParameter() == null || task.getPointIni() == null) {
          // Raro, pero a la peńa le peta en algún sitio (quizá el aldeano está muerto cuando clican
          // en el menú de equipar o algo)
          task.setFinished(true);
          continue;
        }
          ...
          if (World.getCell(citizen.getCoordinates()).getAstarZoneID() == World.getCell(p3dItem)
            ...
            if (citizen.isIdle()) {
              ...
              if (alCits != null) {
                ...
                hmCitizensSinTarea.put(
                    World.getCell(citizen.getCoordinates()).getAstarZoneID(),
                    alCits);
              }
            } else {
              Game.iError = 920;
              // No ocioso, si no está comiendo ni durmiendo ni equipándose lo sacamos de su tarea y
              // le metemos la nueva
              if (citizen.getCurrentTask() == null
                  || (citizen.getCurrentTask().getTask() != Task.TASK_EAT
            ...
            MessagesPanel.addMessage(
                MessagesPanel.TYPE_ANNOUNCEMENT,
                citizen.getCitizenData().getFullName() + Messages.getString("TaskManager.0"),
                ColorGL.ORANGE); // $NON-NLS-1$
          }
        }
        ...
        if (citizen != null) {
          if (citizen.isIdle()) {
            ...
            // No ocioso, si no está comiendo ni bebiendo ni desequipándose lo sacamos de su tarea y
            // le metemos la nueva
            if (citizen.getCurrentTask() == null
                || (citizen.getCurrentTask().getTask() != Task.TASK_EAT
                    && citizen.getCurrentTask().getTask() != Task.TASK_SLEEP
        ...
        if (citizen != null) {
          ...
          if (citizen.isIdle()) {
            ...
            // No ocioso, si no está comiendo ni durmiendo ni curándose ni equipándose lo sacamos de
            // su tarea y le metemos la nueva
            if (citizen.getCurrentTask() == null
                || (citizen.getCurrentTask().getTask() != Task.TASK_EAT
                    && citizen.getCurrentTask().getTask() != Task.TASK_SLEEP
                    && citizen.getCurrentTask().getTask() != Task.TASK_WEAR
  ...
  /**
   * Comprueba cuantas acciones de tipo queue se podrían meter en base a las que hay en el APS
   *
   * @param ami
   * @param iNumOnAPS
   * @return
            ...
            if (qi.getType() == QueueItem.TYPE_MOVE) {
              ...
              if (alList != null && alList.size() > 0) {
                ...
                for (int t = 0; t < alList.size(); t++) {
                  iTmp += Item.getNumItems(
                      UtilsIniHeaders.getIntIniHeader(alList.get(t)),
                      true,
                      Game.getWorld().getRestrictHaulEquippingLevel());
                }
                if (iTmp > 0) {
  ...
  private void executeDirectTasks() {
    ...
    for (int i = (taskItems.size() - 1); i >= 0; i--) {
      ...
      if (item.getTask().getTask() == Task.TASK_DESTROY_BUILDING) {
          ...
          for (int j = 0; j < taskItems.size(); j++) {
            ...
            if (itemAux.getTask().getTask() == Task.TASK_BUILD) {
              if (itemAux
                  ...
                  .getHotPoint(0)
                  .getHotPoint()
                  .equals(item.getTask().getPointIni())) {
        ...
        // int id = Terrain.getTileHeightID (item.getTask ().getPointIni ().x, item.getTask
        // ().getPointIni ().y, item.getTask ().getPointIni ().z);
        ...
        Game.getWorld()
            .addFluidCellToProcess(
                item.getTask().getPointIni().x,
                item.getTask().getPointIni().y,
                item.getTask().getPointIni().z,
                false);
        ...
        if (imi != null) {
          ...
          if (sBuildingIniHeader != null) {
            // Buscamos edificio donde se pueda construir, si tiene la cola llena seguimos a ver si
            // encontramos uno sin cola
            Building building;
            ...
            if (iBuildingIndex == -1) {
              ...
              if (bmi == null) {
                Log.log(
                    Log.LEVEL_ERROR,
                    Messages.getString("TaskManager.1") + sBuildingIniHeader + "]",
                    getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$
              } else {
                MessagesPanel.addMessage(
                    MessagesPanel.TYPE_ANNOUNCEMENT,
                    Messages.getString("TaskManager.3")
                        + imi.getName()
                        + Messages.getString("TaskManager.4")
                        + bmi.getName()
                        + Messages.getString("TaskManager.2"),
            ...
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("TaskManager.6")
                    + sItemIniHeader
                    + Messages.getString("TaskManager.7"),
                getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$
          }
        } else {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("TaskManager.8") + sItemIniHeader + "]",
              getClass().toString()); // $NON-NLS-1$ //$NON-NLS-2$
        }
        ...
        Building building = Building.getBuilding(item.getTask().getPointIni());
        if (building != null) {
          // Ańadimos un elemento a la cola
          BuildingManagerItem bmi = BuildingManager.getItem(building.getIniHeader());
        ...
        if (building != null) {
          for (int b = iItemIndex; b >= 0; b--) {
            if (building
                ...
                .get(b)
                .getIniHeader()
                .equals(item.getTask().getParameter())) {
        ...
        if (itemToLock != null) {
          itemToLock.setWallConnectorStatus(
              Item.FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED,
              true);
        }
        item.getTask().setFinished(true);
        ...
        if (itemToUnlock != null) {
          itemToUnlock.setWallConnectorStatus(
              Item.FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_OPENED,
              true);
        }
        item.getTask().setFinished(true);
        ...
        if (itemToUnlock != null) {
          itemToUnlock.setWallConnectorStatus(
              Item.FLAG_WALL_CONNECTOR_STATUS_UNLOCKED_AND_CLOSED,
              true);
        }
        item.getTask().setFinished(true);
        ...
        // Container container = Game.getWorld ().getContainer (Integer.parseInt (item.getTask
        // ().getParameter ()));
        // if (container != null) {
        ...
        if (citizen != null
          ...
          if (item.getTask().getTask() == Task.TASK_CONVERT_TO_SOLDIER) {
            ...
            if (iOldGroup != -1) {
              ...
              Game.getWorld()
                  .getCitizenGroups()
                  .getCitizensWithoutGroup()
                  .remove(new Integer(citID));
            }
            ...
            // Lo ańadimos al job group
            Game.getWorld()
                .getCitizenGroups()
                .getCitizensWithoutGroup()
                .add(new Integer(citizen.getID()));
            // Borramos sus jobs
        ...
        if (citizen != null) {
          ...
          if (citizen.getSoldierData().getState() != iState
              || (iState == SoldierData.STATE_IN_A_GROUP
                  && citizen.getSoldierData().getGroup() != item.getTask().getPointIni().x)) {
            citizen
                .getSoldierData()
                .setState(iState, item.getTask().getPointIni().x, citizen.getID());
            ...
            if (iState == SoldierData.STATE_GUARD) {
              Game.updateTutorialFlow(
                  ...
                  TutorialTrigger.ICON_INT_LIVINGS_GUARD,
                  null);
            } else if (iState == SoldierData.STATE_PATROL) {
              Game.updateTutorialFlow(
                  TutorialTrigger.TYPE_INT_ICONHIT,
                  TutorialTrigger.ICON_INT_LIVINGS_PATROL,
                  null);
            } else if (iState == SoldierData.STATE_BOSS_AROUND) {
              Game.updateTutorialFlow(
                  TutorialTrigger.TYPE_INT_ICONHIT,
                  TutorialTrigger.ICON_INT_LIVINGS_BOSS,
                  null);
            }
          }
        }
        ...
        // AŃADIR PUNTO DE PATROL A UN SOLDADO PATROL
        int citID = Integer.parseInt(item.getTask().getParameter());
        Citizen citizen = (Citizen) World.getLivingEntityByID(citID);
        if (citizen != null) {
        ...
        // AŃADIR PUNTO DE PATROL A UN GRUPO PATROL
            ...
            if (citizenNuevo.getCitizenData().hasZone()) {
              ...
              if (zoneNuevo != null && zoneNuevo instanceof ZonePersonal) {
                ...
                if (citizenCurrent != null) {
                  ...
                  // La zona destino no tenía dueńo, así que borramos el dueńo de la zona origen
        ...
        if (groupID >= 0 && groupID < SoldierGroups.MAX_GROUPS) {
          ...
          if (zone != null && zone instanceof ZoneBarracks) {
            ...
            if (zoneBarracks.getGroupID() >= 0
                && zoneBarracks.getGroupID() < SoldierGroups.MAX_GROUPS) {
  ...
  /**
   * Retorna el índice, dentro de un array, del aldeano más cercano al punto pasado
   *
   * @param p3d
   *            Punto
   * @param alCits
   *            Lista de Citiens
   * @return el índice, dentro del array, del aldeano más cercano al punto pasado, -1 en caso de no encontrarse
   */
   ...
   *            Lista de Citiens
   * @param sPriorityID
   *            ID de la prioridad, si se le pasa buscará aldeanos que no tengan esa priority (job) denegada
   * @return el índice, dentro del array, del aldeano más cercano al punto pasado, -1 en caso de no encontrarse
   */
  ...
  /**
   * Indica cuantas actions "iguales" hay en curso No se miran duplicados reales, sólo que exista una action con mismo
   * ID
   *
   * @param newAction
  ...
  private boolean isDuplicateAction(Action newAction, Action action) {
    ...
    if (newAction.getDestinationPoint() != null
        && action.getDestinationPoint() != null
  ...
  private boolean isOnNonAvailableHash(
      Action action,
      HashMap<String, ArrayList<Integer>> hmNonAvailableActions) {
    Point3DShort p3dsDestination = action.getDestinationPoint();
    if (p3dsDestination != null) {
  ...
  /**
   * Comprueba las places de las órdenes de mine/dig Se llama cuando se acaba de digar/minar en un sitio, por si ha
   * cambiado algo También cancelaremos tareas de dig mine si ya están hechas
   */
  private void recheckMinePlaces() {
    ...
    for (int i = 0; i < taskItems.size(); i++) {
      ...
      if (!item.getTask().isFinished()
          && (iTaskType == Task.TASK_MINE || iTaskType == Task.TASK_MINE_LADDER)) {
          ...
          for (int h = 0; h < alHotpoints.size(); h++) {
            ...
            if (!alHotpoints.get(h).isFinished()) {
              alHotpoints
                  .get(h)
                  .setPlaces(
                      Task.getAccesingPoints(hPoint3D.x, hPoint3D.y, hPoint3D.z, iTaskType),
                      true);
            }
          }
        }
  ...
  private void removeFinishedTasks() {
    ...
    for (int i = (taskItems.size() - 1); i >= 0; i--) {
      ...
      if (item.getTask().isFinished()) {
        ...
        // Si era tarea de haul reseteamos el contador de turnos, para que vuelva a chequear el haul
        // sin esperar MAX_HAUL_TURNS turnos
