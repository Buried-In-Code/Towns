...
public class SkillManager {
  ...
  private static ArrayList<SkillEffectItem> loadEffectData(NodeList nodeList) throws Exception {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE
          && node.getNodeName().equals("effect")) { // $NON-NLS-1$
        ...
        skillEffectItem.setTarget(
            UtilsXML.getChildValue(node.getChildNodes(), "target")); // $NON-NLS-1$
        alEffects.add(skillEffectItem);

        // Miramos que exista el efecto
        if (EffectManager.getItem(skillEffectItem.getId()) == null) {
          throw new Exception(
              Messages.getString("SkillManager.2")
                  + skillEffectItem.getId()
                  + "]"); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
    }
  ...
  private static void loadItems() {
    ...
    // Cargar de fichero
    loadXMLSkills(
        Towns.getPropertiesString("DATA_FOLDER") + "skills.xml", // $NON-NLS-2$
        true); // $NON-NLS-1$
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "skills.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
  ...
  private static void loadXMLSkills(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            item.setCoolDown(
                UtilsXML.getChildValue(node.getChildNodes(), "coolDown")); // $NON-NLS-1$
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("SkillManager.0") + e.toString() + "]",
          "SkillManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
}
