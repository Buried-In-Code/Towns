...
public final class Game {
  ...
  public static final int SAVEGAME_V9_V10a = 1;
  public static final int SAVEGAME_V10b = 2;
  public static final int SAVEGAME_V11 = 3;
  public static final int SAVEGAME_V12 = 4;
  public static final int SAVEGAME_V13 = 5;
  public static final int SAVEGAME_V14 = 6;
  public static final int SAVEGAME_V14d = 7;
  public static final int SAVEGAME_V14e = 8;
  public static final int SAVEGAME_VERSION = SAVEGAME_V14e;
  public static int SAVEGAME_LOADING_VERSION = 0;
  public static boolean SAVE_MISSION = true;

  public static final String SAVE_FOLDER1 = "save"; // $NON-NLS-1$
  public static final String MODS_FOLDER1 = "mods"; // $NON-NLS-1$
  public static final String BURY_FOLDER1 = "bury"; // $NON-NLS-1$
  public static final String SCREENSHOTS_FOLDER1 = "screenshots"; // $NON-NLS-1$
  ...
  public static final int SIEGE_DIFFICULTY_OFF = 0;
  public static final int SIEGE_DIFFICULTY_EASY = 1;
  public static final int SIEGE_DIFFICULTY_NORMAL = 2;
  public static final int SIEGE_DIFFICULTY_HARD = 3;
  public static final int SIEGE_DIFFICULTY_HARDER = 4;
  public static final int SIEGE_DIFFICULTY_INSANE = 5;

  public static final int STATE_NO_STATE = 0;
  public static final int STATE_CREATING_TASK = 1;
  public static final int STATE_SHOWING_CONTEXT_MENU = 2;
  public static final int STATE_SHOWING_INFO_PANEL = 3;
  ...
  public Game() {
    ...
    panelUI = new UIPanel();
    if (getWorld() == null) {
      panelCommand = new CommandPanel(
          UtilsGL.getWidth() - World.MAP_WIDTH,
          World.MAP_HEIGHT,
          World.MAP_WIDTH,
          UtilsGL.getHeight() - World.MAP_HEIGHT,
          null,
          null);
    } else {
      panelCommand = new CommandPanel(
    ...
    // Esto aquí, después de cargar las texturas, que si no empieza el contador del SMP logo a bajar
    // antes de tiempo
  ...
  public static void continueGame(String savegameName, String sCampaignMissionAndForcedZipPath) {
    ...
    if (!bNewGame) {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Game.1") + " [" + savegameName + ".zip]",
          "Game"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exitToMainMenu();
      getPanelMainMenu()
          .setErrorToShow(
              Messages.getString("Game.1")
                  + " ["
                  + savegameName
                  + ".zip]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      getPanelMainMenu().createMenu();
      return;
    }
  }
  ...
  public static void showPauseStatus() {
    if (isPaused()) {
      MessagesPanel.addMessage(
          ...
          Messages.getString("Game.3"),
          ColorGL.YELLOW); // $NON-NLS-1$
    } else {
      MessagesPanel.addMessage(
          MessagesPanel.TYPE_SYSTEM,
          Messages.getString("Game.4"),
          ColorGL.YELLOW); // $NON-NLS-1$
    }
  }
   ...
   *
   * @param campaignID
   *            ID de la campaña, null en caso de partida normal
   * @param missionID
   *            ID de la misión, null en caso de partida normal
   */
    ...
    if (campaignID == null || missionID == null) {
      fMapa = new File(
          Game.getUserFolder()
              ...
              + Game.SAVE_FOLDER1
              + Game.getFileSeparator()
              + "freemode.zip"); //$NON-NLS-1$ //$NON-NLS-2$
    } else {
      fMapa = new File(
          Game.getUserFolder()
              + Game.getFileSeparator()
  ...
  public static void updateTutorialFlow(
      int iTutorialType,
      int iParam,
      Point3D p3d,
      String sParam,
    ...
    if (missionData != null
        && missionData.getTutorialFlowIndex() < missionData.getTutorialFlows().size()
          ...
          if (iTutorialType == trigger.getType()) {
            ...
            switch (iTutorialType) {
              ...
              case TutorialTrigger.TYPE_INT_WASD :
                ...
                if (trigger.getParam2() >= (TutorialTrigger.WASD_UP
                    + TutorialTrigger.WASD_DOWN
                    + TutorialTrigger.WASD_LEFT
                    + TutorialTrigger.WASD_RIGHT)) {
                  bTriggerEnd = true;
                }
              ...
              case TutorialTrigger.TYPE_INT_ICONHIT :
                if (iParam == TutorialTrigger.ICON_INT_FLAT) {
                  ...
                  bTriggerEnd = (trigger.getParam1() != null
                      && trigger
                          .getParam1()
                          .equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_BODY));
                  ...
                  bTriggerEnd = (trigger.getParam1() != null
                      && trigger
                          .getParam1()
                          .equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_JOBS));
                  ...
                  bTriggerEnd = (trigger.getParam1() != null
                      && trigger
                          .getParam1()
                          .equalsIgnoreCase(TutorialTrigger.ICON_LIVINGS_BOSS));
              ...
              case TutorialTrigger.TYPE_INT_PLACE :
                if (p3d != null && trigger.getParamXYZ() != null) {
                  if (UtilsIniHeaders.getIntIniHeader(trigger.getParam1()) == iParam) {
                    if (p3d.x == trigger.getParamXYZ().x
                        && p3d.y == trigger.getParamXYZ().y
            ...
            // Here I'll check for NONE triggers and delete the PLACE ones if the items are already
            // set
            if (iTutorialType == TutorialTrigger.TYPE_INT_NONE
              ...
              if (trigger.getParamXYZ() != null) {
                ...
                if (item != null
                    && item.getNumericIniHeader() == UtilsIniHeaders
  ...
  private static void setupGame(
      ...
      if (le != null) {
        ...
        if (lemi != null && lemi.getType() == LivingEntity.TYPE_CITIZEN) {
          ...
          if (cit.getCitizenData() != null && cit.getCitizenData().getGroupID() == -1) {
            // Ciudadano sin grupo (pq no tiene grupo, pq es un save de la v10 o pq es una nueva
            // partida)
    ...
    // Esto es un parche para solventar un bug. Antes no se borraban los ciudadanos de los job
    // grupos al morir, y podría estar corrupto
      ...
      if (sCampaignID != null && sMissionID != null) {
        ...
        if (missionData == null) {
          if (bNewGame) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("Game.14") + " [" + sCampaignID + "][" + sMissionID + "]",
                "Game"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
            Game.exit();
            // } else {
          ...
          // } else {
          // MessagesPanel.addMessage (MessagesPanel.TYPE_SYSTEM, Messages.getString ("Game.5"),
          // ColorGL.GREEN); //$NON-NLS-1$
        }
  ...
  private static void takeScreenshot() {
    ...
    try {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Game.17") + " [" + file.getName() + "]",
          "Game"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    }
  }
  ...
  public void run() {
    ...
    // int checkTrigger = (4 * FPS_INGAME); // Para que lo mire nada más empezar, en el primer
    // ciclo
    // boolean bVictory = false;

    // int iMissionCompletedWidth = (UtilFont.getWidth (Messages.getString ("Game.2"))) / 2;
    // //$NON-NLS-1$
    while (!Display.isCloseRequested()) {
      ...
      if (!getPanelMainMenu().isActive()) {
        world.nextTurn();
        /*
         * // Comprobamos la victoria cada 4xFPS_INGAME turnos (para ganar un poquillo de performance) if
         * (bVictory == false && world != null && getWorld ().getCampaignID () != null) { checkVictory++; if
         * (checkVictory >= (4 * FPS_INGAME)) { checkVictory = 0; MissionData md = CampaignManager.getMission
         * (getWorld ().getCampaignID (), getWorld ().getMissionID ()); if (md != null &&
         * ObjectiveData.checkCompleted (md.getObjectives ())) { // Victoria !! bVictory = true; } } } } else {
         * bVictory = false;
         */

        /*
         * // Tutorial trigger? checkTrigger++; if (checkTrigger >= (4 * FPS_INGAME)) { checkTrigger = 0; if
         * (Game.getCurrentMissionData () != null && Game.getCurrentMissionData ().getTutorialFlowIndex () <
         * Game.getCurrentMissionData ().getTutorialFlows ().size ()) { if (Game.getCurrentMissionData
         * ().getTutorialFlows ().get (Game.getCurrentMissionData ().getTutorialFlowIndex ()).getTriggers () !=
         * null && Game.getCurrentMissionData ().getTutorialFlows ().get (Game.getCurrentMissionData
         * ().getTutorialFlowIndex ()).getTriggers ().size () > 0) { // There are triggers to check if
         * (TutorialTrigger.checkCompleted (Game.getCurrentMissionData ().getTutorialFlows ().get
         * (Game.getCurrentMissionData ().getTutorialFlowIndex ()).getTriggers ().get (0))) { // Trigger
         * completed! Game.getCurrentMissionData ().getTutorialFlows ().get (Game.getCurrentMissionData
         * ().getTutorialFlowIndex ()).getTriggers ().remove (0); // If no more triggers we update the index if
         * (Game.getCurrentMissionData ().getTutorialFlows ().get (Game.getCurrentMissionData
         * ().getTutorialFlowIndex ()).getTriggers ().size () == 0) { Game.getCurrentMissionData
         * ().setTutorialFlowIndex (Game.getCurrentMissionData ().getTutorialFlowIndex () + 1); }
         *
         * // We trigger a new image on the images panel ImagesPanel.triggerNewFlow (); } } else { // No more
         * triggers, we update the index Game.getCurrentMissionData ().setTutorialFlowIndex
         * (Game.getCurrentMissionData ().getTutorialFlowIndex () + 1); //ImagesPanel.notifyFlowChanged (); } }
         * }
         */
      }
      ...
      // }
      // fps++;
      // Pongo este método pq dice la doc de LWJGL que actualiza los timers y tiene que llamarse lo
      // más posible
      Sys.getTimerResolution();

      // Render
      // if (getPanelMainMenu ().isActive () || (world != null && World.getCitizenIDs ().size () ==
      // 0)) {
      render();

      // Misión completada?
      /*
       * if (bVictory) { GL11.glBindTexture (GL11.GL_TEXTURE_2D, Game.TEXTURE_FONT_ID); GL11.glTexEnvf
       * (GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE, GL11.GL_MODULATE); UtilsGL.glBegin (GL11.GL_QUADS);
       * UtilsGL.drawStringWithBorder (Messages.getString ("Game.2"), (MainPanel.renderWidth / 2) -
       * iMissionCompletedWidth, MainPanel.renderHeight / 2 - UtilFont.MAX_HEIGHT / 2, ColorGL.RED,
       * ColorGL.WHITE); //$NON-NLS-1$ UtilsGL.glEnd ();
       *
       * if (getCurrentState () == STATE_SHOWING_CONTEXT_MENU && getCurrentContextMenu () != null) { //
       * ContextMenu getCurrentContextMenu ().render (); } }
       */
        ...
        if (UIPanel.typingPanel != null) {
          ...
          if (iKEY == Keyboard.KEY_ESCAPE && getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
            ...
            if (iFN == UtilsKeyboard.FN_SHOW_MISSION) {
              ...
              // if (getCurrentMissionData () != null && getCurrentMissionData ().getText() != null
              // && getCurrentMissionData ().getObjectives() != null && getCurrentMissionData
              // ().getObjectives().size() > 0) {
              // showInfoPanel(panel);
  ...
  private void checkMouseEvents() {
    ...
    while (Mouse.next()) {
      ...
      if (Mouse.getEventButtonState()) {
        ...
        // if (mouseX >= getCurrentInfoPanel().getX() && mouseX < (getCurrentInfoPanel().getX() +
        // getCurrentInfoPanel().getWidth()) && mouseY >= getCurrentInfoPanel().getY() && mouseY <
        // (getCurrentInfoPanel().getY() + getCurrentInfoPanel().getHeight())) {
        // getCurrentInfoPanel().mousePressed(mouseX - getCurrentInfoPanel().getX(), mouseY -
        // getCurrentInfoPanel().getY());
        // }
        // continue;
        // }

        if (mouseButton == 0) {
          // Botón izquierdo pulsado, miramos a qué panel pertenece y llamamos a su función
          // mousePressed (x, y) (con x e y relativas al panel)
          ...
          if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
            ...
            if (mouseX >= getCurrentContextMenu().getX()
              ...
              getCurrentContextMenu()
                  .mousePressed(
                      mouseX - getCurrentContextMenu().getX(),
                      mouseY - getCurrentContextMenu().getY());
            } else {
              // Cierra el menú
              deleteCurrentContextMenu();
          ...
          if (UIPanel.typingPanel != null) {
            ...
            if (mouseX >= 0
              ...
              // } else if (mouseX >= getPanelMessages ().renderX && mouseX < (getPanelMessages
              // ().renderX + getPanelMessages ().renderWidth) && mouseY >= getPanelMessages
              // ().renderY && mouseY < (getPanelMessages ().renderY + getPanelMessages
              // ().renderHeight)) {
              // Messages panel
      ...
      if (!getPanelMainMenu().isActive()) {
        ...
        if (mouseX < BORDE) {
          if (Game.isMouseScrollEarsON()
              || !UIPanel.isMouseCloseToOpenCloseProductionIcon(mouseX, mouseY)) {
            if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
              if (!(mouseX >= getCurrentContextMenu().getX()
                  && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth())
                  && mouseY >= getCurrentContextMenu().getY()
          ...
          if (Game.isMouseScrollEarsON()
              || !UIPanel.isMouseCloseToOpenCloseMenuIcon(mouseX, mouseY)) {
            if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
              if (!(mouseX >= getCurrentContextMenu().getX()
                  && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth())
                  && mouseY >= getCurrentContextMenu().getY()
        ...
        if (mouseY < BORDE) {
          ...
          if (Game.isMouseScrollEarsON()
              || !UIPanel.isMouseCloseToOpenCloseBottomIcon(mouseX, mouseY)) {
            if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
              if (!(mouseX >= getCurrentContextMenu().getX()
                  && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth())
                  && mouseY >= getCurrentContextMenu().getY()
    ...
    if (Display.wasResized() || Display.isFullscreen() != displayFullscreen) {
      ...
      if (resetDisplayMode) {
        try {
          ...
          Log.log(
              Log.LEVEL_ERROR,
              "Problem encountered when setting the display mode: " + ex,
              "Game");
        }
      }
      ...
      Game.getPanelCommand()
          .resize(
              iWidth - World.MAP_WIDTH,
              World.MAP_HEIGHT,
              World.MAP_WIDTH,
