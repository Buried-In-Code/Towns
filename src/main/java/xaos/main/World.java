...
public final class World implements Externalizable {
  ...
  public static final int FLUIDS_MOVED_PER_INVOCATION = 64; // Max fluidos a procesar por turno (tambiÚn se usa para
  // la evaporaci¾n)
  public static final int FLUIDS_NOT_MOVED_PER_INVOCATION = 64;
  public static final int FLUIDS_MAX_EVAPORATION = 48;
  ...
  // ID Mßximo (current) de los entities (livingentities, items, edificios, special)
  ...
  // tiempo
  private static transient int maxDemoDays;
  ...
  private transient int indexViewCitizen; // Indica el n·mero de aldeano en la lista global de aldeanos que se estß

  // "viendo"
  private transient int indexViewSoldier; // Indica el n·mero de soldado en la lista global de aldeanos que se estß

  // "viendo"
  private transient int indexViewHero; // Indica el n·mero de hÚroe en la lista global de hÚroes que se estß "viendo"
  ...
  public static LivingEntity addNewLiving(
      String sIniHeader,
      int type,
      boolean bDiscovered,
      int x,
  ...
  public static LivingEntity addNewLiving(
    ...
    if (bInit) {
      ...
      if (lemi != null && lemi.getCaravan() != null) {
        Game.getWorld()
            .setCurrentCaravanData(
                CaravanManager.getItem(lemi.getCaravan()).getInstance(le.getID(), x, y, z));
      }
    }
  ...
  public static boolean addSiegeEnemies(
    ...
    if (bSiegeType == SiegeData.SIEGE_ROBBERY) {
      ...
      if (aiSteal != null && aiSteal.length > 0) {
        for (int i = 0; i < aiSteal.length; i++) {
          ...
          if (Item.getNumItemsTotal(
              UtilsIniHeaders.getStringIniHeader(aiSteal[i]),
              World.MAP_DEPTH - 1) > 0) {
    ...
    for (int i = 0; i < numEnemies; i++) {
      LivingEntity le = World.addNewLiving(
          lemi.getIniHeader(),
          LivingEntity.TYPE_ENEMY,
          true,
          p3dSpawnPoint.x,
          p3dSpawnPoint.y,
          p3dSpawnPoint.z,
  ...
  public static boolean addSiegeEnemies(
    ...
    LivingEntityManagerItem lemi = LivingEntityManager.getItemByLevel(
        level,
        LivingEntity.TYPE_ENEMY,
   ...
   * @param cell
   * @param bFluids
   *            Indica si hay que borrar por cul`pa de fluidos
   * @param bRaisedLowered
   *            Indica si hay que borrar debido a un raise/lower del terreno
   */
  ...
  /**
   * Devuelve un punto a random, que estarß en los bordes y en el nivel 0, basßndose en el ASZI. Se usa en la llegada
   * de inmigrantes/heroes.
   *
   * @param iASZID
   * @return
   */
  public static Point3DShort getRandomBorderPoint(int iASZI) {
    ...
    // Si asÝ no encontramos nos recorreremos todo el mapa
    ...
    // Si llega aquÝ es que los randoms no han funcionado, nos recorremos todo el mundo
    // Norte/sur
   ...
   *
   * @param iASZI
   * @param iMinLevelToCheck
   *            Nivel mÝnimo donde puede spawnear
   * @param bUseBorderPointIfFails
   *            Si no encuentra punto buscarß un punto normal en los bordes del mapa
   * @return
   */
  public static Point3DShort getRandomUndergroundPoint(
      int iASZI,
      int iMinLevelToCheck,
      int maxLevelDiscovered,
    ...
    while (iIndexLevel > iMinLevelToCheck) {
      // Miramos 200 randoms como mßximo (MAP_WIDTH)
    ...
    // Si llega aquÝ es que no hemos encontrado una casilla a random, recorremos el mundo a manija
    ...
    // Si llega aquÝ es que nada de nada
  ...
  /**
   * Mueve el fluido de una celda origen (se asume que hay fluido ahÝ) a una destino, y hace las cosas necesarias
   * (borrar items de celdas, minimapa, ...)
   */
  public static boolean moveSingleFluid(
      Cell cellSource,
      Cell cellDestination,
      boolean bPreloading,
      Point3DShort p3dSource,
      Point3DShort p3dDestination,
      ArrayList<Point3DShort> alNewPoints) {
    ...
    if (!bMovedFromFluidToFluid) {
      ...
      // Celda destino ya tenÝa fluidos
      ...
      if (cellSource.getTerrain().getFluidType() != cellDestination.getTerrain().getFluidType()) {
        ...
        cellDestination
            .getTerrain()
            .setFluidCount(cellDestination.getTerrain().getFluidCount() - 1);
  ...
  public void addContainer(int iItemID) {
    // Miramos que no exista ya (por si lo estßn moviendo de sitio)
    Container container;
    for (int i = 0; i < getContainers().size(); i++) {
      container = getContainers().get(i);
      if (container.getItemID() == iItemID) {
        // Ya existe, quizß lo ha movido de sitio, por lo que setearemos las coordinates de los
        // items de dentro
  ...
  // // Si al cabo de X intentos no se consigue uno que no estÚ en la lista, se recorre a manija la
  // lista, uno a uno
  //
  // String sGodID;
  // GodManagerItem gmi;
  ...
  // }
  //
  // // Si llega aquÝ ya lo tenemos
  // gods.add (gmi.getGodDataInstance ());
  // return;
  // }
  // }
  //
  // // Si llega aquÝ nos recorreremos la lista de gods a manija
  ...
  public boolean addEvent(EventManagerItem emi) {
    ...
    // Si llega aquÝ es que no existe en la lista actualmente
    ...
    // Si llega aquÝ es que los prerequisitos estßn ok, creamos una instancia y la metemos
    ...
    if (bNeighbours) {
      ...
      if (z > 0) {
        if (Utils.isInsideMap(x, y, z - 1)) {
          if (!cells[x][y][z - 1].isFluidCheckList()
              && cells[x][y][z - 1].getTerrain().hasFluids()) {
  ...
  public void checkCaravansCome() {
    ...
    // Obtenemos una lista por si hay mßs de una
    Zone zone;
    ...
    // Tenemos los 2 puntos, metemos la living y que vaya para allß
    LivingEntity leCaravan = World.addNewLiving(
        lemi.getIniHeader(),
        lemi.getType(),
        true,
        p3dCome.x,
    ...
    String sCaravanMessage = Messages.getString("World.19")
        + " ("
        + leCaravan.getLivingEntityData().getName()
  ...
  /** Mira si llegan hÚroes. En ese caso los mete */
  public void checkHeroesCome() {
    if (Utils.getRandomBetween(1, 12) != 1) {
      return;
    }

    // Antes de nada miramos si tenemos el mßximo de hÚroes, nos basamos en el n·mero de aldeanos
    ...
    if (bOldHeroToCome) {
      // Miramos si puede llegar un hÚroe de los que ya se han ido
    ...
    if (lemi != null) {
      // Antes de mirar los prerequisitos miramos que no sea un hÚroe ·nico (con nombre fijo), en
      // ese caso miramos que no lo tengamos ya en el mundo o en la lista de hÚroes que se han ido
      if (lemi.getName() != null
          && (lemi.getNamePoolTag() == null || lemi.getNamePoolTag().length() == 0)) {
        // HÚroe con nombre ·nico, miramos que no tengamos uno de estos ya en el mundo (o en los old
        // heros)
        Hero hero;
        ...
        for (int i = 0; i < World.getHeroIDs().size(); i++) {
          ...
          if (hero != null) {
            ...
            if (lemiWorld != null
                && lemiWorld.getName() != null
                && lemiWorld.getName().equalsIgnoreCase(lemi.getName())) {
              // Mismo hÚroe, pos ala, pacasa, no viene
              return;
            }
          }
        }

        // Si llega aquÝ es que el nuevo hÚroe no estß en el mundo, miramos si estß en los old heros
        if (!bOldHeroToCome) {
        ...
        if (prerequisite.getId() == HeroPrerequisite.ID_MIN_CITIZENS) {
          ...
          for (int z = 0; z < getZones().size(); z++) {
            if (ZoneManager.getItem(getZones().get(z).getIniHeader())
              ...
              if (zoneHero.getPoints().size() > 0) {
                ...
                // Buscamos un punto que tenga ASZID vßlido
          ...
          for (int z = 0; z < getZones().size(); z++) {
            if (ZoneManager.getItem(getZones().get(z).getIniHeader())
              ...
              ZoneHeroRoom zoneHero = (ZoneHeroRoom) getZones().get(z);
              if (zoneHero.getPoints().size() > 0) {
                // Comprobamos la altura mÝnima
                if (zoneHero.getPoints().size() > 0
                    && zoneHero.getPoints().get(0).z > (MAP_NUM_LEVELS_OUTSIDE
      ...
      if (bPrerequisitesOK) {
        // Todo OK, hÚroe tiene que aparecer
        ...
        if (p3dDestinationPoint == null) {
          // No hay punto de room, metemos al hÚroe en el mismo ASZID que el primer aldeano
          if (getCitizenIDs().size() > 0) {
        ...
        if (p3dDestinationPoint != null) {
          // Tenemos punto de destino, metemos al hÚroe y lo movemos a ese punto
          int iASZID = getCell(p3dDestinationPoint).getAstarZoneID();
          Point3DShort p3dArrival = getRandomBorderPoint(iASZID);
          if (p3dArrival != null) {
            Hero hero = (Hero) addNewLiving(
                lemi.getIniHeader(),
                LivingEntity.TYPE_HERO,
                true,
                p3dArrival.x,
                p3dArrival.y,
                p3dArrival.z,
            ...
            if (hero != null) {
              ...
              MessagesPanel.addMessage(
                  ...
                  hero.getCitizenData().getFullName() + Messages.getString("World.15"),
                  ColorGL.GREEN,
                  p3dArrival,
              ...
              MessagesPanel.addMessage(
                  MessagesPanel.TYPE_HEROES,
                  Messages.getString("World.16"),
                  ColorGL.RED); // $NON-NLS-1$
            }
          } else {
            MessagesPanel.addMessage(
                MessagesPanel.TYPE_HEROES,
   ...
   *
   * @param emi
   *            Evento, si se pasa null funciona como siempre, en caso contrario se usan los datos del emi (y la siege
   *            aparece si o si, si cabe, claro)
   */
  public void checkSiege(EventManagerItem emi) {
    ...
    if (iSiegeRandom == 1 || iSiegeRandom == 2 || (emi != null && emi.isSiegeUnderground())) {
      ...
      // Pueden aparecer underground (20% y algunos niveles mßs descubiertos
  ...
  /**
   * Devuelve un hotpoint de exploraci¾n para un hÚroe
   *
   * @param iASZID
   * @return
   */
  public Point3DShort getRandomExploringHotpoint(int iASZID) {
    ...
    // Si una de esas casillas coincide con el ASZID pasado y undiscovered vecinas se devuelve, en
    // otro caso se devuelve null
  ...
  // }
  //
  // // Si hay mßs like que dislike el status sube, en caso contrario baja
  ...
  // Messages.getString("World.23") + " " + emi.getName (), ColorGL.ORANGE); //$NON-NLS-1$
  // //$NON-NLS-2$ //$NON-NLS-3$
  // }
  // }
  // }
  ...
  public void keyPressed(int tecla, int fn) {
    // if (tecla == Keyboard.KEY_W || tecla == Keyboard.KEY_S || tecla == Keyboard.KEY_A || tecla ==
    // Keyboard.KEY_D || tecla == Keyboard.KEY_UP || tecla == Keyboard.KEY_DOWN || tecla ==
    // Keyboard.KEY_LEFT || tecla == Keyboard.KEY_RIGHT) {
    if (fn == UtilsKeyboard.FN_UP
        || fn == UtilsKeyboard.FN_DOWN
        || fn == UtilsKeyboard.FN_LEFT
    ...
    } else if (fn == UtilsKeyboard.FN_NEXT_HERO) { // Siguiente hÚroe
      CommandPanel.executeCommand(CommandPanel.COMMAND_NEXT_HERO, null, null, null, null, 0);
    } else if (fn == UtilsKeyboard.FN_PREVIOUS_HERO) { // Anterior hÚroe
  ...
  public void nextTurn() {
    // Cursores (si no estß sacando el panel de typing)
    ...
    // }
    // Demo version, codificado de forma rara para evitar que se toque con un editor hexadecimal o
    // asÝ
      ...
      if (iSave > 0) {
        ...
        if ((currentAutosaveDays % iSave) == 0) {
          ...
          UtilsGL.drawStringWithBorder(
              sText,
              MainPanel.renderWidth / 2 - UtilFont.getWidth(sText) / 2,
              MainPanel.renderHeight / 2 - UtilFont.MAX_HEIGHT / 2,
              ColorGL.YELLOW,
              ColorGL.BLACK);
          UtilsGL.glEnd();
          Display.update();
    ...
    for (int i = (aItems.length - 1); i >= 0; i--) {
      ...
      if (oLiving != null) {
        if (oLiving.nextTurn()) {
          ...
          // Tutorial flow
          Game.updateTutorialFlow(
              TutorialTrigger.TYPE_INT_KILL,
              oLiving.getNumericIniHeader(),
              null);
        }
      }
    }
    ...
    while (iIndex >= 0) {
      if (iIndex < events.size()
          && events
              .get(iIndex)
              .nextTurn()) { // Cuidado, que al hacer deletes pueden pasar cosas raras (no deberÝa)
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    if (Game.SAVEGAME_LOADING_VERSION >= Game.SAVEGAME_V12) {
      ...
      // Comentado pq el n·mero de levels a·n no estß seteado, despuÚs de cargar el mundo los seteo
  ...
  public void refreshTransients() {
    ...
    for (short z = 0; z < MAP_DEPTH; z++) { // Importante poner la Z aquÝ para que el iMaxDiscovered no se lie
  ...
  /**
   * @param readyForNextTurnFrameCounter
   *            the readyForNextTurnFrameCounter to set
   */
  ...
  private Point3DShort checkFluidMovement(Point3DShort p3d) {
    ...
    // Si llega aquÝ es que el agua no se va para abajo ni arriba
  ...
  private short checkFluidMovementCells(
      Cell cellSource,
      int x,
      int y,
      int z,
      boolean bElevatorOnSource) {
    if (!Utils.isInsideMap(x, y, z)) {
      return 0;
    }
    ...
    if (!cellDestination.isMined()
        || cellDestination.getTerrain().getFluidCount() == Terrain.FLUIDS_COUNT_INFINITE
    ...
    if (item != null) {
      ...
      if (imi != null) {
        if (imi.isBlockFluids()) {
          ...
          if (imi.isDoor()) {
            // Es puerta, miramos si estß cerrada o bloqueada
    ...
    if (iSourceCount > 1 && iSourceCount > (iDestinationCount + 1)) {
      ...
      // Si llega aquÝ es que no se puede mover (por lo menos a una casilla con <2 de fuerza)
      // Vamos a mirar si se puede mover a una casilla con <1 fuerza
      // S¾lo podrß si tiene una casilla vecina con >1 fuerza
  ...
  private void checkHeroesFriendships() {
    // Para cada hÚroe miramos si obtien alg·n nuevo amigo
      ...
      for (int h = (i + 1); h < iNumHeroes; h++) {
        ...
        if (hero.getHeroData().getMinTurnsToStay() <= 0) {
          ...
          if (!hero.getHeroData().getFriendships().contains(Integer.valueOf(iPossibleFriendID))) {
            // Si no tiene amigos o si no tiene al posible amigo, comprararemos moral y quizß se
            // harßn amigos
            possibleHeroFriend = (Hero) World.getLivingEntityByID(iPossibleFriendID);

            if (possibleHeroFriend.getHeroData().getMinTurnsToStay() <= 0) {
              int iMoraleDifference = Math.abs(
                  hero.getLivingEntityData().getMoral()
              ...
              if (iMoraleDifference <= 5) {
                ...
                // Y viceversa
                if (!possibleHeroFriend
                    .getHeroData()
                    .getFriendships()
                    .contains(Integer.valueOf(iID))) { // ╔sto deberÝa cumplirse siempre
  ...
  /** Mira si los hÚroes tienen que pirarse. En ese caso les setea el flag de leave */
  private void checkHeroesLeave() {
    // Para cada hÚroe miramos si se tiene que pirar o no
    int iID;
    ...
    for (int i = 0; i < World.getHeroIDs().size(); i++) {
      ...
      if (hero.getHeroData().getHeroTask().getTaskID() == HeroTask.TASK_LEAVING) {
        // Si ya se estß pirando no hacemos nada
        continue;
      } else if (hero.getHeroData().getMinTurnsToStay() > 0) {
        // Turnos mÝnimos a pasar en la ciudad antes de pirarse
        continue;
      }
          ...
          if (prerequisite.getId() == HeroPrerequisite.ID_MIN_CITIZENS) {
            ...
            if (hero.getCitizenData().getZoneID() == -1) {
              ...
              if (zone == null) {
                ...
                if (zone.getPoints().size() > 0
                    && zone.getPoints().get(0).z > (MAP_NUM_LEVELS_OUTSIDE
                        - prerequisite.getValueInt())) {
                  bPrerequisitesOK = false;
                  break;
                }
    ...
    if (alLeavingHeroes != null) {
      ...
      for (int i = 0; i < alLeavingHeroes.size(); i++) {
        ...
        hero = (Hero) World.getLivingEntityByID(iID);
        if (hero != null) {
          // Un hÚroe que se pira
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_HEROES,
              hero.getCitizenData().getFullName() + Messages.getString("World.17"),
              ColorGL.ORANGE,
              hero.getCoordinates(),
              hero.getID()); // $NON-NLS-1$
  ...
  private void checkImmigrants() {
    ...
    if (getCitizenIDs().size() > 0) {
      ...
      if (iHappiness > happinessMin) {
        ...
        // Miramos si hay Zones personales libres, en otro caso no vendrßn
        int iASZID;
        ...
        for (int i = 0; i < getZones().size(); i++) {
          if (ZoneManager.getItem(getZones().get(i).getIniHeader()).getType() == ZoneManagerItem.TYPE_PERSONAL
            ...
            // Buscamos un punto ASZID vßlido en la zona
            for (int p = 0; p < zonePersonal.getPoints().size(); p++) {
              ...
              if (iASZID != -1) {
                ...
                alZonesPoint.add(
                    Point3DShort.getPoolInstance(getZones().get(i).getPoints().get(p)));
                iQtty--;
                break;
              }
            }
        ...
        if (alASZID.size() > 0) {
          ...
          for (int i = 0; i < alASZID.size(); i++) {
            // Buscamos el sitio donde saldrßn, nos basamos en el A*ZI de las Zones libres
            if (p3dArrival == null) {
              ...
              if (getCell(p3dArrival).getAstarZoneID() != alASZID.get(i).intValue()) {
                ...
                p3dArrival = getRandomBorderPoint(alASZID.get(i));
              } // Else, mismo zone ID, asÝ que mantenemos el punto de arrival
            }

            if (p3dArrival != null) {
              Citizen citizen = (Citizen) addNewLiving(
                  null,
          ...
          if (iImmigrantsOK > 0) {
            MessagesPanel.addMessage(
                MessagesPanel.TYPE_ANNOUNCEMENT,
                Messages.getString("World.1") + " (" + iImmigrantsOK + ")",
                ColorGL.GREEN); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            ...
            Game.updateTutorialFlow(
                ...
                (getNumCitizens() + getNumSoldiers()),
                null);
          }
          if (iImmigrantsNotOK > 0) {
            MessagesPanel.addMessage(
                MessagesPanel.TYPE_ANNOUNCEMENT,
                Messages.getString("World.2") + " (" + iImmigrantsNotOK + ")",
                ColorGL.RED); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          }
        }
      }
    }
  ...
  /**
   * Comprueba si un evento estß en la lista de eventos
   *
   * @param sEvent
   * @return
   */
  ...
  /**
   * Evapora fluidos con fuerza 1 y sin fluidos con mßs fuerza a los lados Evapora el 12.5% primero de fluids
   */
    ...
    if (fluidEvaporation.size() > 0) {
      ...
      while (iNumFluidsToEvaporate > 0 && fluidEvaporation.size() > 0) {
        ...
        if (cell.getTerrain().getFluidCount() == 1) {
          // Miramos que no haya fluidos con mßs fuerza a los lados
          boolean bMasFuerza = false;
          forvecinas : for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
              if (x != 0 || y != 0) {
                if (Utils.isInsideMap(
                    cell.getCoordinates().x + x,
                    cell.getCoordinates().y + y,
                    cell.getCoordinates().z)) {
                  int iStr = World.getCell(
                      cell.getCoordinates().x + x,
                      cell.getCoordinates().y + y,
                      cell.getCoordinates().z)
                      .getTerrain()
  ...
  private void generateCitizens() {
    ...
    // Buscamos un punto de llegada vßlido (10.000 intentos)
    // int outsideIndex = 1;
    int trys = 0;
    Point3DShort p3d = null;
    // boolean bFinished = false;
    // while (!bFinished) {
    // p3d = new Point3D (Utils.getRandomBetween (0, World.MAP_WIDTH - 0), Utils.getRandomBetween
    // (0, World.MAP_HEIGHT - 1), World.MAP_NUM_LEVELS_OUTSIDE - outsideIndex);
    if (MapGenerator.STARTING_X != -1 && MapGenerator.STARTING_Y != -1) {
      p3d = Point3DShort.getPoolInstance(
          MapGenerator.STARTING_X,
          MapGenerator.STARTING_Y,
          MapGenerator.STARTING_LEVEL);
    } else {
      p3d = Point3DShort.getPoolInstance(
          (short) Utils.getRandomBetween(0, World.MAP_WIDTH - 0),
          (short) Utils.getRandomBetween(0, World.MAP_HEIGHT - 1),
          MapGenerator.STARTING_LEVEL);
    }
    trys = 10000;
  ...
  private void generateDungeons(DungeonData dungeonData) {
    ...
    for (int i = 0; i < alMonsters.size(); i++) {
      ...
      // Metemos a random el monster indicado tantas veces como estÚ indicado
      if (monsterData.getId().equalsIgnoreCase(MonsterData.ID_RANDOM)) {
        // Enemigo a random, obtenemos la lista por rango de nivel
        alMonsterList = LivingEntityManager.getItemByLevelList(
            monsterData.getLevelMin(),
            monsterData.getLevelMax(),
            LivingEntity.TYPE_ENEMY,
            false);
      }
      ...
      for (int n = 0; n < iNumber; n++) {
        ...
        if (lemi == null) {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("World.5") + monsterData.getId() + "]",
  ...
  private boolean isFreeRoomItemsOK(
      ArrayList<HeroPrerequisite> alPrerequisites,
      ZoneHeroRoom zoneHeroRoom) {
    ...
    // Hay items que comprobar, allß vamos
    ...
    whileitems : while (tokenizer.hasMoreTokens()) {
      ...
      if (imi != null) {
        // Item vßlido, a ver si hay alguno en la zona
        ...
        // Si llega aquÝ es que el item no existe
        return false;
      } else {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("World.24") + " [" + sItem + "]",
  ...
  /** Recorre todos los aldeanos y les modifica la happiness seg·n lo que estÚn haciendo / viendo */
  private void modifyHappiness() {
    Citizen citizen;
    // Cada hora modificamos la happiness seg·n lo que estÚn haciendo/viendo en ese momento
    for (int i = 0; i < getCitizenIDs().size(); i++) {
  ...
  private void modifyHappiness(Citizen citizen) {
    ...
    // POPO citizen.getCitizenData ().setHappiness (citizen.getCitizenData ().getHappiness () +
    // Task.getHappiness (citizen.getCurrentTask ()));
    ...
    if (citizen.getCitizenData().getHappinessWorkCounter() != 0
      ...
      // Llenamos la lista con los valores de happiness que encontramos en LOS, quitando los que
      // tienen happiness = 0
          ...
          if (Utils.isInsideMap(x, y, citizen.getZ())) {
            ...
            if (cell.hasEntity()) {
              ...
              if (imi != null && imi.getHappiness() != 0) {
                // Evitamos la infravisi¾n (teniendo en cuenta que si que hay camino hasta la misma
                // casilla donde estß)
                // if ((x == citizen.getX () && y == citizen.getY ()) || Utils.bresenhamLineExists
                // (citizen.getX (), citizen.getY (), x, y, citizen.getZ (),
                // LivingEntity.TYPE_CITIZEN) || Utils.bresenhamLineExists (x, y, citizen.getX (),
                // citizen.getY (), citizen.getZ (), LivingEntity.TYPE_CITIZEN)) {
                if ((x == citizen.getX() && y == citizen.getY())
                    || Utils.bresenhamLineExists(
                        citizen.getX(),
                        citizen.getY(),
                        x,
                        y,
        ...
        citizen
            ...
            .setHappiness(
                citizen.getCitizenData().getHappiness()
                    + alItemsHappy
                        .get(Utils.getRandomBetween(0, alItemsHappy.size() - 1))
                        .intValue());
      }
    }
  }
  ...
  private void moveFluids(boolean bPreloading) {
    ...
    // Seteamos los zone IDs en caso de que ya no se muevan fluidos y el contador de fluidos movidos
    // sea mayor que 0
    // TambiÚn si el contador de fluidos llega a 128 (por ejemplo)
    if (!bPreloading) {
      if (fluidsMoved) {
        fluidMovedCounter++;
      }
    ...
    while (iCounterMoved < FLUIDS_MOVED_PER_INVOCATION
        && iCounterNotMoved < FLUIDS_NOT_MOVED_PER_INVOCATION
        && fluidCellsToProcess.size() > 0) {
      p3dSource = fluidCellsToProcess.remove(
          Utils.getRandomBetween(
              0,
              (fluidCellsToProcess.size() - 1) / 4)); // Random del primer 25% de tiles

      // Miramos si estß dentro del mapa
  ...
  private boolean validStartingPoint(Point3DShort p3d) {
    // Miraremos que tenca una cuadrÝcula de 17x17 sin agua
    final int NUM_CELLS = 8;
    if (p3d.x < NUM_CELLS
        || p3d.y < NUM_CELLS
        || p3d.x > MAP_WIDTH - 1 - NUM_CELLS
    ...
    // TambiÚn miramos si la celda es ok para ellos
