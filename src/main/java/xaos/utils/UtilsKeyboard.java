...
public final class UtilsKeyboard {
  ...
  private static final String FNSTRINGS[] = {
      "FN_UP", //$NON-NLS-1$
      "FN_DOWN", //$NON-NLS-1$
      "FN_LEFT", //$NON-NLS-1$
      "FN_RIGHT", //$NON-NLS-1$
  ...
  private static final String FNHUMANSTRINGS[] = {
   ...
   *
   * @param iIndex
   *            Index that recently changed the key (0/1)
   * @param iFN
   *            Function that recently changed the key
   * @param iKey
   *            Key to check
   */
  ...
  public static String getTooltip(int iFN) {
    ...
    return " ("
        + Messages.getString("UtilsKeyboard.15")
        + sAux
   ...
   *
   * @return true si el usuario está pulsando alguna tecla de la función pasadas
   */
