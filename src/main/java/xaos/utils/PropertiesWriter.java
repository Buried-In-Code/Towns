...
public class PropertiesWriter {
  ...
  public <T> void setProperty(Property<T> property, T value) {
    if (property.getPropertyFile() != file) {
      throw new IllegalArgumentException(
          "property " + property.getKey() + " is not part of " + file);
    }
  ...
  public void store(File file) throws IOException {
    ...
    try {
      pw = new PrintWriter(
          new OutputStreamWriter(new FileOutputStream(file), Charset.forName("ISO8859-1")));
      int counter = 0;
      for (Entry<EntryKey, String> entry : map.entrySet()) {
        ...
        if (entryKey instanceof SectionEntryKey) {
          ...
          pw.append(propertiesEntryKey.getKey())
              .append(" = ")
