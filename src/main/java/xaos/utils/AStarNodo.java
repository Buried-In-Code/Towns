...
public final class AStarNodo extends Point3DShort implements Comparable<AStarNodo> {
  ...
  public static AStarNodo getPoolInstance(
      int x,
      int y,
      int z,
      AStarNodo padre,
  ...
  public void generateAll(int x, int y, int z, AStarNodo padre, int xDest, int yDest, int zDest) {
    ...
    if (z >= (World.MAP_NUM_LEVELS_OUTSIDE - 1)) {
      if (zDest >= (World.MAP_NUM_LEVELS_OUTSIDE - 1)) {
        ...
        h += ((32)
            * Math.abs(
                zDest - z)); // Método Manhattan, funciona mejor incluso permitiendo movimiento
        // diagonal que la "Distancia Chebyshev" (20% mejor)
      } else {
        // El origen es underground pero el destino no
        h += ((32) * Math.abs(z - (World.MAP_NUM_LEVELS_OUTSIDE - 1))
            + 2
                * (World.MAP_NUM_LEVELS_OUTSIDE
                    - 1
                    - zDest)); // Método Manhattan, funciona mejor incluso permitiendo
      ...
      if (zDest >= (World.MAP_NUM_LEVELS_OUTSIDE - 1)) {
        ...
        h += ((32) * Math.abs(zDest - (World.MAP_NUM_LEVELS_OUTSIDE - 1))
            + 2
                * (World.MAP_NUM_LEVELS_OUTSIDE
                    - 1
                    - z)); // Método Manhattan, funciona mejor incluso permitiendo movimiento
        // diagonal que la "Distancia Chebyshev" (20% mejor)
      } else {
        ...
        h += (2
            * Math.abs(
                zDest - z)); // Método Manhattan, funciona mejor incluso permitiendo movimiento
        // diagonal que la "Distancia Chebyshev" (20% mejor)
        // h += (8 * Math.abs (zDest - z)); // Método Manhattan, funciona mejor incluso
        // permitiendo movimiento diagonal que la "Distancia Chebyshev" (20% mejor)
        // h += (4 * Math.abs (zDest - z)); // Método Manhattan, funciona mejor incluso
        // permitiendo movimiento diagonal que la "Distancia Chebyshev" (20% mejor)
      }
    }
