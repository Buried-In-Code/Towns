...
public final class AStarQueueItem {
  ...
  public AStarQueueItem(
      int livingEntityID,
      int livingEntityType,
      Point3DShort startPoint,
      Point3DShort endPoint,
      boolean bSiege) {
  ...
  /**
   * Busca bresenham lines en el camino y reemplaza (y elminina) los puntos que toque con la misma Esto evita
   * diagonales raras al andar El método tiene que ser rápido para no perder performance
   *
   * @param alPath
   */
  ...
  private static int smoothPath(
      ArrayList<Point3DShort> alPath,
      int startIndex,
      int endIndex,
      short z) { // , int livingType) {
    ...
    while ((iIndexStart + 3) < returnNewIndex) {
      ...
      if (alBresenham != null
          && alBresenham.size() > 2
          && countWalk(alPath, iIndexStart, alBresenham.size()) >= countWalk(
              alBresenham,
              0,
              alBresenham.size())) {
        // if (alBresenham != null && alBresenham.size () > 2) {
        // Linea recta, sustituimos
        // Miramos el tamańo de lo que tenemos en el path
        int subPathSize = (iIndexEnd - iIndexStart);
        if (alBresenham.size() <= (subPathSize + 1)) {
          // Linea del mismo tamańo o más corta, primero sustituimos el path

          for (int j = 0; j < alBresenham.size(); j++) {
            alPath
                .get(iIndexStart + j)
                .setPoint(alBresenham.get(j).x, alBresenham.get(j).y, alBresenham.get(j).z);
          }
          ...
          iIndexStart += alBresenham.size();

          // Reseteamos el end index
          iIndexEnd = returnNewIndex;
        } else {
          // Lo encontrado es más largo.... ż?ż? raro
          iIndexEnd -= stepSize;
  ...
  /**
   * Continua (o empieza) la búsqueda A* hasta un máximo de "iMaxIterations" iteraciones.
   *
   * @param iMaxIterations
   * @return el número de iteraciones efectuadas
   */
  public int search(int iMaxIterations) {
    // Miramos si las 2 celdas aún están en la misma zona (por si acaso alguien ha digado o hecho
    ...
    if (isNewItem()) {
      setNewItem(false);
      // Miramos las casillas cercanas al punto final y origen, empezaremos la búsqueda por el que
      // tenga más
      ...
      if (getClosedList() == null
          || getClosedList()[0][0].length != (Game.getWorld().getNumFloorsDiscovered() + 1)) {
        setClosedList(
            new boolean[World.MAP_WIDTH][World.MAP_HEIGHT][Game.getWorld().getNumFloorsDiscovered() + 1]);
        openListIndexes = new short[World.MAP_WIDTH][World.MAP_HEIGHT][Game.getWorld().getNumFloorsDiscovered()
            + 1];
      ...
      // Ańadimos el primer nodo en la lista abierta
      nodo = AStarNodo.getPoolInstance(
          startPoint.x,
          startPoint.y,
          startPoint.z,
          null,
    ...
    while (iIterations < iMaxIterations && !openList.isEmpty()) {
      ...
      if (nodo.x == endPoint.x
          && nodo.y == endPoint.y
          && nodo.z == endPoint.z) { // Hemos encontrado el destino
        bEncontrado = true;
        break;
      } else {
        ...
        // Aunque eso se soluciona con la optimización de caminos que hacemos una vez encontrado el
        // mismo
        // El problema es que a veces no encuentra camino debido a que llega a un final de caminillo
        // donde no hay diagonales posibles
  ...
  /**
   * @param livingEntityType
   *            the livingEntityType to set
   */
  ...
  /**
   * @param numCellsToRemove
   *            the numCellsToRemove to set
   */
  ...
  private boolean checkNewNode(
      AStarNodo currentNode,
      AStarBinaryHeap alListaAbierta,
      int x,
      int y,
      int z) {
    // Miro la closedList[0][0].length, no fuera que el levels discovered creciera mientras busca un
    // camino y decidiera buscar por ahí
  ...
  private boolean checkNewNodeDown(
      AStarNodo currentNode,
      AStarBinaryHeap alListaAbierta,
      int x,
      int y,
  ...
  private boolean isAllowed(int x, int y, int z) {
    if (x < 0
        ...
        || y >= World.MAP_HEIGHT
        || z < 0
        || z > Game.getWorld().getNumFloorsDiscovered()) {
      return false;
    }
    // return World.getCell (currentNode.x, currentNode.y, currentNode.z).getAstarZoneID () ==
