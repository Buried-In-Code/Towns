...
public final class Utils {
  ...
  public static ArrayList<Point3DShort> bresenhamLine(
      Point3DShort p0,
      Point3DShort p1,
      short zLevel) { // , int livingType) {
   ...
   * @param zLevel
   *            Z
   * @return an array of Point's representing a line between 2 points, null if found a not allowed point
   */
  public static ArrayList<Point3DShort> bresenhamLine(
      int x0,
      int y0,
  ...
  public static boolean bresenhamLineExists(
      int x0,
      int y0,
      int x1,
      int y1,
  ...
  /**
   * Pone luz a las celdas de la linea (y las de arriba y abajo teniendo en cuenta el radio) hasta que encuentra un
   * no-mined o un wall
   *
   * @param x0
   *            Ini X
   ...
   *            Z
   * @param iRadious
   *            Radio para poner luz a las casillas de arriba/abajo
   */
  public static void bresenhamLineLight(
      int x0,
      int y0,
      int x1,
      int y1,
  ...
  public static File createUserFolder(String sUserFolder) {
    ...
    if (!fUserFolder.exists() || !fUserFolder.isDirectory()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.10") + fUserFolder.getAbsolutePath() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      return null;
    }

    File fFolderTowns = new File(
        fUserFolder.getAbsolutePath()
            + System.getProperty("file.separator")
            + ".towns"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    if (!fFolderTowns.exists()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.20") + " [" + fFolderTowns.getAbsolutePath() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      return null;
    }

    File fFolderSave = new File(
        fFolderTowns.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.SAVE_FOLDER1); // $NON-NLS-1$
    ...
    if (!fFolderSave.exists()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.25") + " [" + fFolderSave.getAbsolutePath() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      return null;
    }

    File fFolderMods = new File(
        fFolderTowns.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.MODS_FOLDER1
    ...
    if (!fFolderMods.exists()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.29") + " [" + fFolderMods.getAbsolutePath() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      return null;
    }

    File fFolderBury = new File(
        fFolderTowns.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.BURY_FOLDER1
            + System.getProperty("file.separator")); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    if (!fFolderBury.exists()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.29") + " [" + fFolderBury.getAbsolutePath() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      return null;
    }

    File fFolderScreenshots = new File(
        fFolderTowns.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.SCREENSHOTS_FOLDER1
            + System.getProperty("file.separator")); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    if (!fFolderScreenshots.exists()) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.18") + " [" + fFolderScreenshots.getAbsolutePath() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      return null;
    }
  ...
  public static void deleteSavegame(String sName) {
    String sSaveFolder = Game.getUserFolder()
  ...
  /**
   * Devuelve un array de strings a partir de un string con elementos separados por comas ","
   *
   * @param sChain
   *            Cadena
   * @return un array de strings a partir de un string con elementos separados por comas ",". Null en caso de
   *         error/cadena vacía
   */
  ...
  /**
   * Devuelve un array de Integers a partir de un string con elementos separados por comas ","
   *
   * @param sChain
   *            Cadena
   * @return un array de Integers a partir de un string con elementos separados por comas ",". Null en caso de
   *         error/cadena vacía
   */
    ...
    if (sChain != null && sChain.trim().length() > 0) {
      ...
      while (tokenizer.hasMoreElements()) {
        ...
        try {
          ...
          throw new Exception(
              Messages.getString("Utils.22")
                  + tokenizer.nextToken()
                  + Messages.getString("Utils.24")); // $NON-NLS-1$ //$NON-NLS-2$
        }
      }
   ...
   *            B-control point
   * @param dIteration
   *            Iteration from 0.0 (first point) to 1.0 (last point)
   * @return
   */
  public static Point getBezierPoint(
      Point pointSource,
      Point pointDest,
      Point pointControlA,
      Point pointControlB,
   ...
   *
   * @param sColor
   *            Color string (ex: Green or GREEN)
   * @return a java.awt.Color from a String. Color.BLACK if parameter is null or empty
   */
    ...
    if (sColor.equals("GREEN")) { // $NON-NLS-1$
      ...
      try {
        ...
        if (coloresOK) {
          ...
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("Utils.6") + sColor + "]",
              "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          return new ColorGL(null);
        }
      } catch (Exception e) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("Utils.6") + sColor + "] [" + e.toString() + "]",
            "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        return new ColorGL(null);
      }
    }
      ...
      if (iIndexD != -1) {
        try {
          ...
          if (iIndexPlus == -1) {
            ...
            if (iIndexPlus == -1) {
              return new Point(
                  iNumber, iNumber * Integer.parseInt(sStr.substring(iIndexD + 1).trim()));
            } else {
              ...
              return new Point(
                  ...
                  (iNumber
                      * Integer.parseInt(
                          sStr.substring(iIndexD + 1, (iIndexPlus + 1 - iIndexD)).trim()))
                      - iMinus);
            }
          } else {
        ...
        try {
          ...
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("Utils.3") + sStr + "] [" + e.toString() + "]",
              "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          return new Point(0, 0);
        }
      }
  ...
  public static int getDistance(int xs, int ys, int zs, int xd, int yd, int zd) {
    ...
    // h += ((World.MAP_WIDTH / 4) * Math.abs (zd - zs)); // Método Manhattan, funciona mejor
    // incluso permitiendo movimiento diagonal que la "Distancia Chebyshev" (20% mejor)
    // } else {
    // // El origen es underground pero el destino no
    // h += ((World.MAP_WIDTH / 4) * Math.abs (zs - (World.MAP_NUM_LEVELS_OUTSIDE - 1)) + 2 *
    // (World.MAP_NUM_LEVELS_OUTSIDE - 1 - zd)); // Método Manhattan, funciona mejor incluso
    // permitiendo movimiento diagonal que la "Distancia Chebyshev" (20% mejor)
    // }
    // } else {
    // // El origen es outside
    // if (zd >= (World.MAP_NUM_LEVELS_OUTSIDE - 1)) {
    // // El destino es underground (y origen outside)
    // h += ((World.MAP_WIDTH / 4) * Math.abs (zd - (World.MAP_NUM_LEVELS_OUTSIDE - 1)) + 2 *
    // (World.MAP_NUM_LEVELS_OUTSIDE - 1 - zs)); // Método Manhattan, funciona mejor incluso
    // permitiendo movimiento diagonal que la "Distancia Chebyshev" (20% mejor)
    // } else {
    // // El destino es outside, como el origen
    // h += (2 * Math.abs (zd - zs)); // Método Manhattan, funciona mejor incluso permitiendo
    // movimiento diagonal que la "Distancia Chebyshev" (20% mejor)
    // }
    // }
    //
    // return h;
  ...
  // * STRING UTILS *
  // ***************
  /** Retorna una cadena haciendo cambios dinámicos (sustituyendo ciertas cadenas por otras) */
  public static String getDynamicString(String sString) {
    ...
    if (iIndex != -1) {
      // Música ON?
      String sMusicON = Game.isMusicON()
          ? Messages.getString("Utils.4")
          : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
      sAux = sString.substring(0, iIndex)
          + sMusicON
          + sString.substring(iIndex + "__MUSIC__".length()); // $NON-NLS-1$
      return getDynamicString(sAux);
    } else {
      iIndex = sString.indexOf("__FX__"); // $NON-NLS-1$
      if (iIndex != -1) {
        // FX ON?
        String sFXON = Game.isFXON()
            ? Messages.getString("Utils.4")
            : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
        sAux = sString.substring(0, iIndex)
            + sFXON
            + sString.substring(iIndex + "__FX__".length()); // $NON-NLS-1$
        return getDynamicString(sAux);
      } else {
        iIndex = sString.indexOf("__MOUSE_SCROLL__"); // $NON-NLS-1$
        if (iIndex != -1) {
          // Mouse scroll ON?
          String sMouseScrollON = Game.isMouseScrollON()
              ? Messages.getString("Utils.4")
              : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
          sAux = sString.substring(0, iIndex)
              + sMouseScrollON
              + sString.substring(iIndex + "__MOUSE_SCROLL__".length()); // $NON-NLS-1$
          return getDynamicString(sAux);
        } else {
          iIndex = sString.indexOf("__MOUSE_SCROLL_EARS__"); // $NON-NLS-1$
          if (iIndex != -1) {
            // Mouse scroll EARS ON?
            String sMouseScrollEarsON = Game.isMouseScrollEarsON()
                ? Messages.getString("Utils.4")
                : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
            sAux = sString.substring(0, iIndex)
                + sMouseScrollEarsON
                + sString.substring(iIndex + "__MOUSE_SCROLL_EARS__".length()); // $NON-NLS-1$
            return getDynamicString(sAux);
          } else {
            iIndex = sString.indexOf("__MOUSE_2D_CUBES__"); // $NON-NLS-1$
            if (iIndex != -1) {
              String sMouse2DCubes = Game.isMouse2DCubesON()
                  ? Messages.getString("Utils.4")
                  : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
              sAux = sString.substring(0, iIndex)
                  + sMouse2DCubes
                  + sString.substring(iIndex + "__MOUSE_2D_CUBES__".length()); // $NON-NLS-1$
              return getDynamicString(sAux);
            } else {
              iIndex = sString.indexOf("__DISABLE_ITEMS__"); // $NON-NLS-1$
              if (iIndex != -1) {
                // Disabled items ON?
                String sDisabledItemsON = Game.isDisabledItemsON()
                    ? Messages.getString("Utils.4")
                    : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
                sAux = sString.substring(0, iIndex)
                    + sDisabledItemsON
                    + sString.substring(iIndex + "__DISABLE_ITEMS__".length()); // $NON-NLS-1$
                return getDynamicString(sAux);
              } else {
                iIndex = sString.indexOf("__DISABLE_GODS__"); // $NON-NLS-1$
                if (iIndex != -1) {
                  // Disabled gods ON?
                  String sDisabledGodsON = Game.isDisabledGodsON()
                      ? Messages.getString("Utils.4")
                      : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
                  sAux = sString.substring(0, iIndex)
                      + sDisabledGodsON
                      + sString.substring(iIndex + "__DISABLE_GODS__".length()); // $NON-NLS-1$
                  return getDynamicString(sAux);
                } else {
                  iIndex = sString.indexOf("__PAUSE_START__"); // $NON-NLS-1$
                  if (iIndex != -1) {
                    // Pause at start ON?
                    String sPauseStartON = Game.isPauseStartON()
                        ? Messages.getString("Utils.4")
                        : Messages.getString("Utils.5"); // $NON-NLS-1$ //$NON-NLS-2$
                    sAux = sString.substring(0, iIndex)
                        + sPauseStartON
                        + sString.substring(iIndex + "__PAUSE_START__".length()); // $NON-NLS-1$
                    return getDynamicString(sAux);
                  } else {
                    ...
                    if (iIndex != -1) {
                      ...
                      sAux = sString.substring(0, iIndex)
                          + sAutosave
                          + sString.substring(iIndex + "__SAVE_DAYS__".length()); // $NON-NLS-1$
                      return getDynamicString(sAux);
                    } else {
                      ...
                      if (iIndex != -1) {
                        ...
                        sAux = sString.substring(0, iIndex)
                            + sSiegeDifficulty
                            + sString.substring(iIndex + "__SIEGES__".length()); // $NON-NLS-1$
                        return getDynamicString(sAux);
                      } else {
                        iIndex = sString.indexOf("__SIEGE_PAUSE__"); // $NON-NLS-1$
                        if (iIndex != -1) {
                          String sSiegePauseON = Game.isSiegePause()
                              ? Messages.getString("Utils.4")
                          ...
                          sAux = sString.substring(0, iIndex)
                              + sSiegePauseON
                              + sString.substring(
                                  iIndex + "__SIEGE_PAUSE__".length()); // $NON-NLS-1$
                          return getDynamicString(sAux);
                        } else {
                          iIndex = sString.indexOf("__CARAVAN_PAUSE__"); // $NON-NLS-1$
                          if (iIndex != -1) {
                            String sCaravanPauseON = Game.isCaravanPause()
                                ? Messages.getString("Utils.4")
                                : Messages.getString("Utils.5"); // $NON-NLS-1$
                            // //$NON-NLS-2$
                            sAux = sString.substring(0, iIndex)
                                + sCaravanPauseON
                                + sString.substring(
                                    iIndex + "__CARAVAN_PAUSE__".length()); // $NON-NLS-1$
                            return getDynamicString(sAux);
                          } else {
                            ...
                            if (iIndex != -1) {
                              ...
                              sAux = sString.substring(0, iIndex)
                                  + sVolMusic
                                  + sString.substring(
                                      iIndex + "__VOLMUSIC__".length()); // $NON-NLS-1$
                              return getDynamicString(sAux);
                            } else {
                              ...
                              if (iIndex != -1) {
                                ...
                                sAux = sString.substring(0, iIndex)
                                    + sVolFX
                                    + sString.substring(
                                        iIndex + "__VOLFX__".length()); // $NON-NLS-1$
                                return getDynamicString(sAux);
                              } else {
                                ...
                                if (iIndex != -1) {
                                  ...
                                  if (iIndexEndMod != -1) {
                                    // Tenemos mod y /mod
                                    String sModName = sSubStr.substring(
                                        "__MOD__".length(),
                                        iIndexEndMod); // $NON-NLS-1$
                                    String sModON = Game.isModLoaded(sModName)
                                        ? Messages.getString("Utils.4")
                                        : Messages.getString(
                                            "Utils.5"); //$NON-NLS-1$ //$NON-NLS-2$
                                    sAux = sString.substring(0, iIndex)
                                        + sModON
                                        + sSubStr.substring(
                                            iIndexEndMod
                                                + "__/MOD__".length()); // $NON-NLS-1$
                                  } else {
                                    // Raro, raro, borramos el __MOD__ para que siga
                                    // mirando dynamic
                                    // strings
                                    sAux = sString.substring(0, iIndex)
                                        + sString.substring(
                                            iIndex + "__MOD__".length()); // $NON-NLS-1$
                                  }
                                  return getDynamicString(sAux);
                                } else {
                                  iIndex = sString.indexOf("__BURY__"); // $NON-NLS-1$
                                  if (iIndex != -1) {
                                    String sBury = Game.isAllowBury()
                                        ? Messages.getString("Utils.4")
                                        : Messages.getString(
                                            "Utils.5"); //$NON-NLS-1$ //$NON-NLS-2$
                                    sAux = sString.substring(0, iIndex)
                                        + sBury
                                        + sString.substring(
                                            iIndex + "__BURY__".length()); // $NON-NLS-1$
                                    return getDynamicString(sAux);
                                  } else {
                                    iIndex = sString.indexOf("__CPUPF__"); // $NON-NLS-1$
                                    if (iIndex != -1) {
                                      String sLevel = Integer.toString(
                                          Game.getPathfindingCPULevel());
                                      sAux = sString.substring(0, iIndex)
                                          + sLevel
                                          + sString.substring(
                                              iIndex + "__CPUPF__"
  ...
  /**
   * Devuelve una lista de Strings con los languages encontrados (es_ES, en_US, ....)
   *
  ...
  public static ArrayList<LanguageData> getLanguages() {
    ...
    if (fLanguagesFolder.exists()) {
      ...
      for (int i = 0; i < asFiles.length; i++) {
        if (asFiles[i] != null
            && asFiles[i].startsWith("messages")
            && asFiles[i].endsWith(".properties")) { // $NON-NLS-1$ //$NON-NLS-2$
          ...
          try {
            prop.load(
                new FileInputStream(
                    fLanguagesFolder.getAbsolutePath() + File.separator + asFiles[i]));
    ...
    for (int m = 0; m < alModsLoaded.size(); m++) {
      ...
      String sModLanguagesPath = fUserFolder.getAbsolutePath()
          + System.getProperty("file.separator")
          + Game.MODS_FOLDER1
          + System.getProperty("file.separator")
          + sModName
          ...
          + "/languages/"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
  ...
  public static ArrayList<File> getModsFolders() {
    String sModsFolder = Game.getUserFolder()
        + Game.getFileSeparator()
        + Game.MODS_FOLDER1
        + Game.getFileSeparator();
  ...
  /**
   * Retorna la ruta entera a un fichero, teniendo en cuenta si está en una misión o no. Tambien mira los mods
   * cargados. Si es misión, mirará primero la carpeta general y después dentro de las carpetas de campańa
   *
   * @param sOriginalFile
   * @param sCampaignID
   * @param sMissionID
   * @return la ruta entera a un fichero, teniendo en cuenta si está en una misión o no
   */
  public static ArrayList<String> getPathToFile(
    ...
    if (sMissionID == null || sMissionID.trim().isEmpty()) {
      ...
      // Después la carpeta de campańa
      sPath = Towns.getPropertiesString("CAMPAIGNS_FOLDER")
          + sCampaignID
          + File.separator
          + sOriginalFile; // $NON-NLS-1$
      f = new File(sPath);
      if (f.exists()) {
        alReturn.add(sPath);
      }

      // Ahora la de campańa + misión
      sPath = Towns.getPropertiesString("CAMPAIGNS_FOLDER")
          + sCampaignID
          + File.separator
          + sMissionID
   ...
   * @param iMax
   *            Max
   * @return a random number between the 2 params. If iMin > iMax then return iMin
   */
  ...
  public static BuryData getRandomBuryData(String sServerName) {
    ...
    String sBuryFolder = Game.getUserFolder()
        + Game.getFileSeparator()
        + Game.BURY_FOLDER1
        + Game.getFileSeparator();
    ...
    try {
      ...
      if (fBuryZip != null && fBuryZip.exists()) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("Utils.17"), // $NON-NLS-1$
            "Utils"); // $NON-NLS-1$
      }
    } catch (Exception e) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("BuryData.1") + " [" + e.toString() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    }
  ...
  /**
   * loads a resource bundle that is located in the "./data" directory rather than from the classpath
   *
   * @param name
   * @return
   */
  ...
  public static ArrayList<File> getSaveFiles() {
    ...
    for (int i = 0; i < aFiles.length; i++) {
      if (aFiles[i] != null
          && aFiles[i].getName() != null
          && aFiles[i].isFile()
          && aFiles[i].getName().endsWith(".zip")) { // $NON-NLS-1$
        // Bingo
        alFiles.add(aFiles[i]);
      }
    }
  ...
  public static boolean isInsideMap(Point3DShort p3d) {
    return p3d.x >= 0
        && p3d.x < World.MAP_WIDTH
        && p3d.y >= 0
        && p3d.y < World.MAP_HEIGHT
        && p3d.z >= 0
        && p3d.z < World.MAP_DEPTH;
  }
  ...
  /**
   * Launch N dices taking a String as an input ex: 4d8+34 (this wil launch 4 times a dice of 8 sides, then addes 34
   * to the result
   *
   * <p>
   * Input string can be passed with commas ',' Ex: 2d8,3d6+1,1d3 (lauch 2 dices of 8 sides, then 3 dices of 6 sides,
   * then adds 1, then launch 1 dice of 3 sides
   *
   * @param str
   * @return
   */
        ...
        try {
          ...
          if (iIndexPlus == -1) {
            ...
            if (iIndexPlus == -1) {
              ...
              return launchDice(
                  iNumber,
                  Integer.parseInt(sStr.substring(iIndexD + 1, (iIndexPlus + 1 - iIndexD)).trim()),
                  -Integer.parseInt(sStr.substring(iIndexPlus + 1)));
            }
          } else {
            return launchDice(
                iNumber,
          ...
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("Utils.0") + sStr + "] [" + e.toString() + "]",
              "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          return 0;
        }
      } else {
   ...
   *
   * @param iNumber
   *            Number of times the dice was launched
   * @param iSides
   ...
   *            Number of sides of the dice
   * @param add
   *            Number to add (or substract if negative) to the end result
   * @return
   */
  ...
  public static void load(String sFolder, String sZipFilename) throws Exception {
    try {
      ...
      ZipInputStream zis = new ZipInputStream(
          new BufferedInputStream(new FileInputStream(new File(sFolder + sZipFilename))));
      ZipEntry zipEntry = zis.getNextEntry();
      InputStream is = zipFile.getInputStream(zipEntry);
            ...
            if (alLivings != null) {
              for (int i = 0; i < alLivings.size(); i++) {
                if (LivingEntityManager.getItem(alLivings.get(i).getIniHeader()) == null) {
                  throw new Exception(
                      Messages.getString("LivingEntity.10")
                          + " ["
                          + alLivings.get(i).getIniHeader()
                          + "]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                }
              }
            }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.23") + e.toString() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      if (e.getMessage() != null) {
        throw new Exception(
            Messages.getString("Utils.23") + e.getMessage() + "]"); // $NON-NLS-1$ //$NON-NLS-2$
      } else {
        throw new Exception(
            Messages.getString("Utils.23") + e.toString() + "]"); // $NON-NLS-1$ //$NON-NLS-2$
      }
    }
  }
  ...
  public static void saveOptions() {
    ...
    try {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("Utils.13") + e.toString() + "]",
          "Utils"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
  }
   ...
   * @param imiSource
   * @param iDirection
   *            0 = misma casilla, 1-NW 2-N 3-NE 4-W 5-E 6-SW 7-S 8-SE
   */
  private static void checkSetLights(
      boolean bNonMinedOrWall,
      Cell cell,
      ItemManagerItem imiSource,
      int iDirection) {
    if (!bNonMinedOrWall) {
   ...
   */
  private static boolean checkSetLightsUp(
      boolean bNonMinedOrWall,
      Cell cell,
      ItemManagerItem imiSource,
      int iDirection) {
  ...
  /**
   * Retorna la ruta entera a un fichero de mods, teniendo en cuenta si está en una misión o no Si es misión, mirará
   * primero la carpeta general y después dentro de las carpetas de campańa
   *
   * @param sOriginalFile
   * @param sCampaignID
   * @param sMissionID
   * @return la ruta entera a un fichero, teniendo en cuenta si está en una misión o no
   */
  private static void getPathToFileMods(
    ...
    for (int i = 0; i < alMods.size(); i++) {
      ...
      if (sMissionID == null || sMissionID.trim().isEmpty()) {
        // Sin misión, lo pillamos de la carpeta data
        String sModActionsPath = fUserFolder.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.MODS_FOLDER1
            + System.getProperty("file.separator")
            + sModName
            + System.getProperty("file.separator")
            + Towns.getPropertiesString("DATA_FOLDER")
            + sOriginalFile; // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        ...
        String sPath = fUserFolder.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.MODS_FOLDER1
            + System.getProperty("file.separator")
            + sModName
        ...
        // Ahora miramos la carpeta de la campańa
        sPath = fUserFolder.getAbsolutePath()
            + System.getProperty("file.separator")
            + Game.MODS_FOLDER1
            + System.getProperty("file.separator")
            + sModName
            + System.getProperty("file.separator")
            + Towns.getPropertiesString("CAMPAIGNS_FOLDER")
            + sCampaignID
            + File.separator
            + sOriginalFile; // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        if (new File(sPath).exists()) {
