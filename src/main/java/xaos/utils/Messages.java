    ...
    if (sModName == null) {
      ...
      if (!fUserFolder.exists() || !fUserFolder.isDirectory()) {
        ...
        if (sLanguage.equals("en") && sCountry.equals("US")) {
          sModLanguagePath = fUserFolder.getAbsolutePath()
              ...
              + "/languages/messages.properties"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          // //$NON-NLS-4$ //$NON-NLS-5$
        } else {
          sModLanguagePath = fUserFolder.getAbsolutePath()
              + System.getProperty("file.separator")
              ...
              + "_"
              + sCountry
              + ".properties"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
