...
public final class UtilsGL {
  ...
  // }
  // UtilsGL.drawTextureZ (xOffset, y + cd.yoffset, xOffset + cd.width, y + cd.yoffset + cd.height,
  // cd.xTex, cd.yTex, cd.xTex + cd.widthTex, cd.yTex + cd.heightTex, z);
  // xOffset += cd.xadvance;
  // }
  // }
  ...
  public static void drawString(String sMessage, int x, int y, ColorGL color) {
    ...
    for (int j = 0; j < aiMessage.length; j++) {
      ...
      UtilsGL.drawTexture(
          ...
          y + cd.yoffset,
          xOffset + cd.width,
          y + cd.yoffset + cd.height,
          cd.xTex,
          cd.yTex,
          cd.xTex + cd.widthTex,
          cd.yTex + cd.heightTex);
      xOffset += cd.xadvance;
    }
    GL11.glColor3f(1, 1, 1);
  }
  ...
  public static void drawStringWithBorderZ(
      String sMessage,
      int x,
      int y,
      ColorGL color,
  ...
  // public static void drawTextureZ (int x0, int y0, int x1, int y1, int x2, int y2, int x3, int
  // y3, float texX0, float texY0, float texX1, float texY1, int z) {
  // ATI_drawed = true;
  // GL11.glTexCoord2f (texX0, texY0);
  ...
  // public static void drawTextureZ (int x0, int y0, int x1, int y1, float texX0, float texY0,
  // float texX1, float texY1, ColorGL color, int z) {
  // GL11.glColor3f (color.r, color.g, color.b);
  ...
  //
  // public static void drawTextureZ (int x0, int y0, int x1, int y1, float texX0, float texY0,
  // float texX1, float texY1, ColorGL color, float transparency, int z) {
  // GL11.glColor4f (color.r, color.g, color.b, transparency);
  // drawTextureZ (x0, y0, x1, y1, texX0, texY0, texX1, texY1, z);
  // }
  public static void drawTextureZ(
      int x0,
      int y0,
      int x1,
      int y1,
  ...
  public static boolean[][] generateAlpha(Tile tile) {
    String sFileName = Towns.getPropertiesString(
        PropertyFile.PROPERTY_FILE_GRAPHICS,
        "[" + tile.getIniHeader() + "]TEXTURE_FILE"); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public static boolean[][] generateAlpha(boolean[][] alphaArray, int width, int height) {
    // Si llega aquÝ es que es un tama±o personalizado
  ...
  // BufferedImage imageCursor = ImageIO.read(new
  // File(Towns.getPropertiesString("GRAPHICS_FOLDER") +
  // Towns.getPropertiesString(Towns.PropertyFile.PROPERTY_FILE_GRAPHICS, "CURSOR_FILE")));
  // //$NON-NLS-1$ //$NON-NLS-2$
  //
  ...
  //
  // // Giramos los pixels pq sale al revÚs
  // int iPixel1, iPixel2;
  // for (int x = 0; x < 16; x++) {
  // for (int y = 0; y < 16; y++) {
  // iPixel1 = (y * 16 * 4) + (x * 4);
  ...
  //
  // /*
  // * Note that the IntBuffer contains packed pixel data, so one int contains all four pixel
  // components.
  // */
  // IntBuffer intbuf = ByteBuffer.allocateDirect(16 * 16 * 4 /* sizeof(int)
  ...
  public static void initGLModes() {
    ...
    GL11.glOrtho(
        ...
        UtilsGL.getHeight(),
        0,
        -((World.MAP_WIDTH * World.MAP_HEIGHT * 1) + 68),
        1);
  ...
  public static ImageData loadImage(String imageFile, boolean reload) {
    ...
    try {
      // Primero miramos que no estÚn en alguna carpeta de mod
      ...
      if (fUserFolder.exists() && fUserFolder.isDirectory()) {
        ...
        if (alMods != null && alMods.size() > 0) {
          for (int i = 0; i < alMods.size(); i++) {
            String sModTexture = fUserFolder.getAbsolutePath()
                + System.getProperty("file.separator")
                + Game.MODS_FOLDER1
                + System.getProperty("file.separator")
                + alMods.get(i)
                + System.getProperty("file.separator")
                + Towns.getPropertiesString("GRAPHICS_FOLDER")
                + System.getProperty("file.separator")
                + imageFile;
      ...
      Log.log(
          Log.LEVEL_DEBUG,
          "Fast decoding of image [" + imageFile + "] failed: " + e.toString(),
          "UtilsGL");
      // return null;
    }
    ...
    if (imageData == null) {
      try {
        imageData = loadImageDataImageIO(
            Towns.getPropertiesString("GRAPHICS_FOLDER")
                + System.getProperty("file.separator")
                + imageFile,
            imageFile);
        ...
        Log.log(
            Log.LEVEL_ERROR,
            "ImageIO decoding of image [" + imageFile + "] failed: " + e.toString(),
            "UtilsGL");
      }
    }
  ...
  /**
   * Loads the given texture and returns it. If the texture has been loaded before then it is returned from cache or
   * if reloaded is true then it is reloaded from disk.
   *
   * @param imageFile
   * @param textureMode
   * @param reload
   *            should the texture be reloaded when it was cached before?
   * @return
   */
  ...
  // * @return texture ID
  // */
  // public static int loadTexture(float[] textureFloats, int ID, float textureMode, int width, int
  // height, int format) {
  ...
  //
  // GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, format, width, height, 0, format, GL11.GL_FLOAT,
  // buffer);
  //
  // return ID;
  // }
   ...
   * @return texture ID
   */
  public static int reloadTexture(
      float[] textureFloats,
      int ID,
      float textureMode,
      int width,
  ...
  private static ImageData loadImageDataImageIO(String imageFile, String imageName)
      throws IOException {
  ...
  private static void loadTexture(ImageData image, int id, int textureMode) {
    ...
    GL11.glTexImage2D(
        ...
        image.getFormat(),
        GL11.GL_UNSIGNED_BYTE,
        image.getImagePixels());
  }

  private static void setNativeCursor() throws Exception {
    final File cursorFile = new File(
        Towns.getPropertiesString("GRAPHICS_FOLDER")
            + Towns.getPropertiesString(PropertyFile.PROPERTY_FILE_GRAPHICS, "CURSOR_FILE"));
    BufferedImage imageCursor = ImageIO.read(cursorFile);

    int[] buffer = new int[imageCursor.getWidth()];
    IntBuffer intBuffer = ByteBuffer.allocateDirect(imageCursor.getWidth() * imageCursor.getHeight() * 4)
    ...
    try {
      Mouse.setNativeCursor(
          new Cursor(imageCursor.getWidth(), imageCursor.getHeight(), 0, 15, 1, intBuffer, null));
    } catch (LWJGLException e) {
      Log.log(Log.LEVEL_ERROR, "No native cursor support.", UtilsGL.class.getCanonicalName());
    }
  }
