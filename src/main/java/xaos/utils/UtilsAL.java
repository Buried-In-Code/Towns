...
public final class UtilsAL {
  ...
  public static final String SOURCE_MUSIC_MAINMENU = "musicMM"; // $NON-NLS-1$
  public static final String SOURCE_MUSIC_INGAME = "musicIG"; // $NON-NLS-1$

  public static final String SOURCE_FX_CLICK = "fxclick"; // $NON-NLS-1$

  // public final static String SOURCE_FX_CHOP = 6;
  public static final String SOURCE_FX_MINE = "fxmine"; // $NON-NLS-1$
  // public final static String SOURCE_FX_DIG = 8;
  public static final String SOURCE_FX_EAT = "fxeat"; // $NON-NLS-1$

  public static final String SOURCE_FX_DEAD = "fxdead"; // $NON-NLS-1$

  public static final String SOURCE_FX_BUILDING = "fxbuilding"; // $NON-NLS-1$
    ...
    if (Game.isMusicON() || Game.isFXON()) {
      if (!isOpenALON()) {
        if (!loadALData()) {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("UtilsAL.8"),
              "UtilsAL"); //$NON-NLS-1$ //$NON-NLS-2$
          Game.exit();
        }
        setOpenALON(true);
      ...
      if (fUserFolder.exists() && fUserFolder.isDirectory()) {
        ...
        if (alMods != null && alMods.size() > 0) {
          for (int i = 0; i < alMods.size(); i++) {
            String sModAudioIniPath = fUserFolder.getAbsolutePath()
   ...
   * @param sKey
   * @param sFile
   *            Fichero, si se le pasa null (o vacío) no pasa nada, mete null en la hash
   * @return
   */
  private static boolean loadAudio(String sKey, String sFile) {
    ...
    if (!fAudio.exists()) {
      ...
      if (fUserFolder.exists() && fUserFolder.isDirectory()) {
        ...
        if (alMods != null && alMods.size() > 0) {
          for (int i = 0; i < alMods.size(); i++) {
            String sModAudioPath = fUserFolder.getAbsolutePath()
                + System.getProperty("file.separator")
                + Game.MODS_FOLDER1
                + System.getProperty("file.separator")
                + alMods.get(i)
                + System.getProperty("file.separator")
                + Towns.getPropertiesString("AUDIO_FOLDER")
                + sFile;
    ...
    if (audio == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("UtilsAL.5") + sFilePath + "]",
          "UtilsAL"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      return false;
    }
