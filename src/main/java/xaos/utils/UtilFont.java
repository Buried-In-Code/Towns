...
public final class UtilFont {
  ...
  // public static short MAX_HEIGHT_NUMBERS = 8; // Altura, se calcula con la info de la fuente, 16
  // es lo mínimo
  ...
  /**
   * Returns the number of chars where the string has to be splitted It takes account words
   *
   * @param sText
   *            Text
   * @param iWidth
  ...
  public static void parseFont() {
    try {
      ...
      String sFontFilePath = Towns.getPropertiesString(
        ...
        if (fUserFolder.exists() && fUserFolder.isDirectory()) {
          ...
          if (alMods != null && alMods.size() > 0) {
            for (int i = 0; i < alMods.size(); i++) {
              String sModFontFilePath = fUserFolder.getAbsolutePath()
                  ...
                  + Towns.getPropertiesString(
                      PropertyFile.PROPERTY_FILE_GRAPHICS,
                      "FONT_FILE"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      ...
      // Hemos acabado, ahora recorro todos para restar pixels al yoffset (para tener lo mínimo
      // yoffset=0)
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("UtilFont.0") + " " + e.toString(),
          "UtilFont"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
  }
  ...
  private static CharDef parseChar(String line) {
    ...
    // if (((def.id >= '0' && def.id <= '9') || (def.id == '/')) && def.height >
    // MAX_HEIGHT_NUMBERS) {
