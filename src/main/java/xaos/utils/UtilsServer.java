...
public class UtilsServer {
  ...
  public static String getBuriedTown(String serverURL, String buryFolder) {
    ...
    try {
      ...
      if (sXML == null || sXML.trim().isEmpty()) {
        Log.log(
            Log.LEVEL_DEBUG,
            Messages.getString("UtilsServer.0"),
            "UtilsServer"); //$NON-NLS-1$ //$NON-NLS-2$
        return null;
      }
      ...
      if (doc == null
        ...
        Log.log(
            Log.LEVEL_DEBUG,
            Messages.getString("UtilsServer.2"),
            "UtilsServer"); //$NON-NLS-1$ //$NON-NLS-2$
        return null;
      }
              ...
              for (int j = 0; j < nodeListBurieds.getLength(); j++) {
                ...
                if (nodeBurieds.getNodeType() == Node.ELEMENT_NODE) {
                  ...
                  if (sAux.equalsIgnoreCase("BURIEDFILE")) { // $NON-NLS-1$
                    ...
                    String sName = UtilsXML.getChildValue(
                        nodeBurieds.getChildNodes(),
                        "fileName"); // $NON-NLS-1$
                    String sID = UtilsXML.getChildValue(
                        nodeBurieds.getChildNodes(),
                        "fileID"); // $NON-NLS-1$
                    if (sName != null
                        && sID != null
      ...
      if (sDownloadURL != null && alNames.size() > 0) {
        ...
        if (iIndexID == -1) {
          Log.log(
              Log.LEVEL_DEBUG,
              Messages.getString("UtilsServer.13"),
              "UtilsServer"); //$NON-NLS-1$ //$NON-NLS-2$
        } else {
          while (iIndexID != -1) {
            sDownloadURL = sDownloadURL.substring(0, iIndexID)
                + alIDs.get(iIndexRandom)
                + sDownloadURL.substring(iIndexID + "__ID__".length()); // $NON-NLS-1$
            iIndexID = sDownloadURL.indexOf("__ID__"); // $NON-NLS-1$
          }

          if (downloadBuryFile(
              sDownloadURL,
              buryFolder + File.separator + alNames.get(iIndexRandom))) {
            return alNames.get(iIndexRandom);
          } else {
            Log.log(
                Log.LEVEL_DEBUG,
                Messages.getString("UtilsServer.17"),
                "UtilsServer"); //$NON-NLS-1$ //$NON-NLS-2$
          }
        }
      } else {
        if (sDownloadURL == null) {
          Log.log(
              Log.LEVEL_DEBUG,
              Messages.getString("UtilsServer.19"),
              "UtilsServer"); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
    } catch (Exception e) {
      ...
      Log.log(
          Log.LEVEL_DEBUG,
          Messages.getString("UtilsServer.21") + " [" + e.toString() + "]",
          "UtilsServer"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    }
  ...
  public static String getServerName(String serverURL) {
    ...
    return Messages.getString("UtilsServer.1")
        + " ["
        + serverURL
