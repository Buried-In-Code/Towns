...
public class ImagesPanel {
  ...
  public static void loadImages(MissionData missionData) {
    ...
    if (missionData != null) {
      for (int i = 0; i < missionData.getTutorialFlows().size(); i++) {
        if (missionData.getTutorialFlows().get(i).getImage() != null
            && !hashImageTextureData.containsKey(
                missionData.getTutorialFlows().get(i).getImage())) {
          TextureData td = UtilsGL.loadTexture(
              missionData.getTutorialFlows().get(i).getImage(),
              GL11.GL_MODULATE);
          if (td != null) {
            td.clearPixels();
  ...
  public static boolean nextMission() {
    ...
    if (Game.getCurrentMissionData() != null) {
      if (getCurrentFlowIndex() >= 0
          && getCurrentFlowIndex() <= Game.getCurrentMissionData().getTutorialFlows().size()) {
        String sNextMission = Game.getCurrentMissionData()
            .getTutorialFlows()
            .get(getCurrentFlowIndex())
            .getNextMission();
    ...
    if (sCampaign != null && sMission != null) {
      CommandPanel.executeCommand(
          CommandPanel.COMMAND_EXIT_TO_MAIN_MENU_NOSAVE,
          null,
          null,
          null,
          null,
  ...
  public static void render(int mousePanel) {
    ...
    if (tutorialFlow != null) {
      ...
      if (tutorialFlow.getTitle() != null) {
        UtilsGL.drawStringWithBorder(
            tutorialFlow.getTitle(),
            panelPoint.x + (WIDTH / 2) - UtilFont.getWidth(tutorialFlow.getTitle()) / 2,
            iY,
            ColorGL.WHITE,
            ColorGL.BLACK);
      }
      ...
      if (tutorialFlow.getTexts().size() > 0) {
        ...
        if (alTexts.size() > 0) {
          ...
          for (int i = 0; i < alTexts.size(); i++) {
            ...
            // UtilsGL.drawStringWithBorder (alTexts.get (i), TEXT_X, iY, ColorGL.WHITE,
            // ColorGL.BLACK);
      ...
      if (tutorialFlow.getImage() != null) {
        iCurrentTexture = UtilsGL.setTexture(
            iCurrentTexture,
            hashImageTextureData.get(tutorialFlow.getImage()).getTextureID());
        // UtilsGL.drawTexture (panelPoint.x, panelPoint.y, panelPoint.x + WIDTH, panelPoint.y +
        // HEIGHT, 0, 0, 1, 1);
        // UtilsGL.drawTexture (panelPoint.x + WIDTH / 2 - 256, panelPoint.y + HEIGHT / 2 - 256,
        // panelPoint.x + WIDTH / 2 + 256, panelPoint.y + HEIGHT / 2 + 256, 0, 0, 1, 1);
        UtilsGL.drawTexture(
            panelPoint.x - MINI_IMAGE_WIDTH / 2,
            panelPoint.y - 64,
            panelPoint.x + MINI_IMAGE_WIDTH / 2,
            panelPoint.y + (MINI_IMAGE_HEIGHT - 64),
            0,
            0,
            1,
            1);
      ...
      if (currentTile != null) {
        ...
        UtilsGL.drawTexture(
            panelPoint.x - currentTile.getTileWidth() / 2,
            panelPoint.y - 64,
            panelPoint.x + currentTile.getTileWidth() / 2,
            panelPoint.y + (currentTile.getTileHeight() - 64),
            currentTile.getTileSetTexX0(),
            currentTile.getTileSetTexY0(),
            currentTile.getTileSetTexX1(),
            currentTile.getTileSetTexY1());
      }
      ...
      if (tutorialFlow.getNextMission() != null) {
        ...
        UIPanel.drawTile(
            tileNextMission,
            nextMissionPoint,
            mousePanel == UIPanel.MOUSE_IMAGES_PANEL_NEXT_MISSION);
      }
    }
    ...
    if (getCurrentFlowIndex() <= 0) {
      ...
      UIPanel.drawTile(
          tilePrevious,
          previousImagePoint,
          mousePanel == UIPanel.MOUSE_IMAGES_PANEL_PREVIOUS);
    }
  ...
  public static void resize(int renderWidth, int renderHeight) {
    ...
    panelPoint.setLocation(
        UIPanel.PIXELS_TO_BORDER
            + iIconSize
            + MINI_IMAGE_WIDTH / 2
            + UIPanel.getImagesPanelOffset(),
        renderHeight / 2 - (UtilsGL.getHeight() - 256 + 2 * tilePanel[1].getTileHeight()) / 2);

    // Close button
    closeButtonPoint.setLocation(
        panelPoint.x + WIDTH - UIPanel.tileButtonClose.getTileWidth(),
        panelPoint.y);

    // Previous button
    previousImagePoint.setLocation(
        panelPoint.x + WIDTH / 2 - 2 * tilePrevious.getTileWidth(),
        panelPoint.y + HEIGHT - tilePrevious.getTileHeight() - tilePrevious.getTileHeight() / 4);

    // Next button
    nextImagePoint.setLocation(
        panelPoint.x + WIDTH / 2 + tileNext.getTileWidth(),
        panelPoint.y + HEIGHT - tileNext.getTileHeight() - tileNext.getTileHeight() / 4);

    // Next mission button
    nextMissionPoint.setLocation(
        panelPoint.x + WIDTH - 2 * tileNextMission.getTileWidth(),
        panelPoint.y
            + HEIGHT
            - tileNextMission.getTileHeight()
            - tileNextMission.getTileHeight() / 4);
  ...
  public static void setCurrentFlowIndex(int currentFlowIndex) {
    ...
    if (Game.getCurrentMissionData() != null
        && currentFlowIndex >= 0
        && currentFlowIndex < Game.getCurrentMissionData().getTutorialFlows().size()) {
  ...
  public static void setVisible(boolean visible) {
    ...
    if (Game.getCurrentMissionData() == null) { // || Game.getCurrentMissionData ().getTutorialFlowIndex () >=
      // Game.getCurrentMissionData ().getTutorialFlows ().size ()) {
