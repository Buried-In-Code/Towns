...
public final class MainPanel {
  ...
  /**
   * Devuelve la Z Mßxima donde se deberß dibujar el 3D mouse
   *
   * @param x
   * @param y
   * @param z
  ...
  public static void render() {
    ...
    int iBaseXGeneral = (-xView) * (Tile.TERRAIN_ICON_WIDTH / 2)
        + ((-yView) * Tile.TERRAIN_ICON_WIDTH / 2)
        + xCentro;
    int iBaseYGeneral = (-yView) * (Tile.TERRAIN_ICON_HEIGHT / 2)
        - ((-xView) * Tile.TERRAIN_ICON_HEIGHT / 2)
        + yCentro;
    ...
    if (bMouseInMainArea) {
      int iTextureID = renderAllTerrains(
          zView,
          cellXMin,
          cellXMax,
          cellYMin,
      ...
      renderTask(
          zView,
          iBaseXGeneral,
          iBaseYGeneral + Tile.TERRAIN_ICON_HEIGHT,
          pointTileMouse,
          iTextureID,
          cellXMin,
          cellYMin);
  ...
  public static int renderAllEntities(
      int zView,
      int cellXMin,
      int cellXMax,
      int cellYMin,
      ...
      for (int j = cellYMin; j <= cellYMax; j++) {
        ...
        if (cell != null
          ...
          currentTextureID = renderEntities(
              ...
              cell,
              iXGeneral,
              iYGeneral + Tile.TERRAIN_ICON_HEIGHT,
              currentTextureID,
              pointTileMouse,
              fColorShadowLight,
              zLevelOffset);
        ...
        // iBaseXGeneral, iBaseYGeneral + (Tile.TERRAIN_ICON_WIDTH / 2), null, (zLevelOffset + 1),
        // currentTextureID);
        // }
        // }
        if (cell == null
            || cell.isShouldPaintUnder()
            || (flatMouseON
                && zLevelOffset == 0
                && pointTileMouse != null
                && isMouseNearCell(
                    cell,
                    pointTileMouse))) { // El zLevelOffset es para que se vea bien cuando acercas al
          // rat¾n a un item con _block
          ...
          if (zView < (World.MAP_DEPTH - 1)) {
            currentTextureID = renderAllEntities(
                ...
                j - 1,
                iBaseXGeneral,
                iBaseYGeneral + (Tile.TERRAIN_ICON_WIDTH / 2),
                null,
                (zLevelOffset + 1),
                currentTextureID);
          }
          ...
          if (gridON && zLevelOffset == 0 && cell.isDiscovered() && cell.isMined()) {
            if (!cell.isDigged()) {
              ...
              UtilsGL.drawTextureZ(
                  ...
                  GRID_TILE.getTileSetTexX0(),
                  GRID_TILE.getTileSetTexY0(),
                  GRID_TILE.getTileSetTexX1(),
                  GRID_TILE.getTileSetTexY1(),
                  Cell.getDepth(i, j, zView));
            } else {
              ...
              if (cell.getAstarZoneID() != -1) {
                ...
                UtilsGL.drawTextureZ(
                    ...
                    GRID_NOT_ALLOWED_TILE.getTileSetTexY1(),
                    Cell.getDepth(i, j, zView));
              }
            }
          }
  ...
  public static int renderAllTerrains(
      int zView,
      int cellXMin,
      int cellXMax,
      int cellYMin,
          ...
          if (cell.hasZone()) {
            ...
            if (zone != null) {
              ...
              if (zView < (World.MAP_DEPTH - 1)) {
                ...
                currentTextureID = setColorShadowLightCellTerrain(
                    cellDown,
                    tile,
                    zLevelOffset + 1,
                    currentTextureID,
              ...
              UtilsGL.drawTextureZ(
                  ...
                  iYGeneral + Tile.TERRAIN_ICON_HEIGHT,
                  iXGeneral + tile.getTileWidth(),
                  iYGeneral + tile.getTileHeight() + Tile.TERRAIN_ICON_HEIGHT,
                  tile.getTileSetTexX0(),
                  tile.getTileSetTexY0(),
                  tile.getTileSetTexX1(),
                  tile.getTileSetTexY1(),
  ...
  public static void renderTask(
    ...
    if (pointTileMouse != null) {
      if (Game.getCurrentState() == Game.STATE_CREATING_TASK
        ...
        int z3D;
        if (tDMouseON
            && Game.getCurrentState() == Game.STATE_CREATING_TASK
            && Game.getCurrentTask().getState() == Task.STATE_CREATING_ENDZONE) {
        ...
        // Miramos donde estß el rat¾n para mostrar info por pantalla y el cuadradito blanco
        ...
        if (Game.getCurrentTask().getState() == Task.STATE_CREATING_ENDZONE) {
          if (Game.getCurrentTask().getTask() == Task.TASK_STOCKPILE) {
            ...
            zmi = ZoneManager.getItem(
                expandZone.getIniHeader()); // Usamos el expandzone solo por "performance"
          ...
          if (Game.getCurrentTask().getTask() == Task.TASK_BUILD
            ...
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("MainPanel.3") + Game.getCurrentTask().getTask() + "]",
                "MainPanel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          }
        }
        ...
        if (bBuilding) {
          ...
          if (queuing) {
            // En caso de colas el item estß en el parßmetro 2
            imi = ItemManager.getItem(Game.getCurrentTask().getParameter2());
          } else {
            imi = ItemManager.getItem(Game.getCurrentTask().getParameter());
          }
          if (imi == null) {
            // No ha encontrado el item??? Quizß es un createItemByType y el valor es un type, asÝ
            // que pillamos el primer item con ese type
            if (queuing) {
              // En caso de colas el item estß en el parßmetro 2
            ...
            if (mouseOK) {
              ...
              if (bBuilding) {
                ...
                if (!Item.isCellAvailableForItem(imi, i, j, z3D, true, true)) {
                  ...
                  // El item puede ir ahÝ, pero miraremos que no estÚ construyendo en el aire y no
                  // haya camino posible
              ...
              if (bDraw) {
                ...
                iXGeneral = iBaseXGeneral
                    + (i + j) * (Tile.TERRAIN_ICON_WIDTH / 2)
                    + terrainSpecialTile.getTileWidthOffset();
                iYGeneral = iBaseYGeneral
                    - (i - j) * (Tile.TERRAIN_ICON_HEIGHT / 2)
                    - terrainSpecialTile.getTileHeight()
                    - terrainSpecialTile.getTileHeightOffset()
                ...
                UtilsGL.drawTextureZ(
                    ...
                    terrainSpecialTile.getTileSetTexX0(),
                    terrainSpecialTile.getTileSetTexY0(),
                    terrainSpecialTile.getTileSetTexX1(),
                    terrainSpecialTile.getTileSetTexY1(),
   ...
   * @param iTexture
   *            <p>
   *            private static void unsetBrightNess (int iTexture) { //UtilsGL.glEnd (); // GL11.glBindTexture
   *            (GL11.GL_TEXTURE_2D, iTexture); // GL11.glTexEnvf (GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE,
   *            GL11.GL_MODULATE); // GL11.glTexEnvf (GL11.GL_TEXTURE_ENV, GL13.GL_COMBINE_RGB, GL11.GL_MODULATE);
   *            //UtilsGL.glBegin (GL11.GL_QUADS); }
   */
  public static void resize(int renderWidth, int renderHeight) {
    ...
    // Obtenemos el n·mero de tiles mßximo que caben por pantalla
  ...
  public static int setColorShadowLightCell(
      Cell cell,
      Tile tile,
      int zLevelOffset,
    ...
    if (cell.isLight() && Game.OPENGL_13_AVAILABLE) {
      ...
      return setColorShadowLightCell(
          cell,
          getColorShadowLightCell(cell, zLevelOffset),
          tile,
          bTransparency,
          iCurrentTextureID,
          zLevelOffset);
  ...
  public static int setColorShadowLightCell(
    ...
    if (cell.isLight()) {
      ...
      if (bTransparency) {
        GL11.glColor4f(
            fShadowLightColor + ged.getRed(),
            fShadowLightColor + ged.getGreen(),
  ...
  public static int setColorShadowLightCellNoLight(
      Cell cell,
      Tile tile,
      int zLevelOffset,
      int iCurrentTextureID,
      boolean bTransparency) {
    return setColorShadowLightCellNoLight(
        cell,
        getColorShadowLightCell(cell, zLevelOffset),
        tile,
        bTransparency,
  ...
  public static int setColorShadowLightCellTerrain(
      Cell cell,
      Tile tile,
      int zLevelOffset,
      int iCurrentTextureID,
      boolean bTransparency) {
    if (cell.isLight() && Game.OPENGL_13_AVAILABLE) {
      return setColorShadowLightCellTerrain(
          cell,
          0,
          tile,
          bTransparency,
  ...
  public static int setColorShadowLightCellTerrain(
    ...
    if (cell.isLight()) {
      ...
      if (Game.OPENGL_13_AVAILABLE && zLevelOffset == 0) {
        ...
        if (zLevelOffset <= 1
            && !cell.isShadow(true)
            && !Game.getWorld().getGlobalEvents().isHalfShadows()) {
  ...
  public static void toggle3DMouse() {
    ...
    Game.updateTutorialFlow(
        ...
        TutorialTrigger.ICON_INT_3DMOUSE,
        null);

    // if (tDMouseON) {
    // // Acaba de activarlo
    // if (Game.getCurrentState () == Game.STATE_CREATING_TASK && Game.getCurrentTask ().getState ()
    // == Task.STATE_CREATING_ENDZONE) {
    // Point3D p3dIni = Game.getCurrentTask ().getPointIni ();
    // // Game.getCurrentTask ().setPointIni (new Point3D (p3dIni.x, p3dIni.y, getMaxZ3DMouse
    // (p3dIni.x, p3dIni.y, p3dIni.z)));
  ...
  public static void toggleFlatMouse() {
    ...
    Game.updateTutorialFlow(
        TutorialTrigger.TYPE_INT_ICONHIT,
        TutorialTrigger.ICON_INT_FLATCURSOR,
        null);
  }
  ...
  /**
   * Retorna la celda en la que el mouse estß apuntando. Se le pasa la view
   *
   * @param x
   * @param y
   * @param xView
  ...
  private static Point3D getTileMouse(int x, int y, int xView, int yView, int zView) {
    ...
    if (tDMouseON) {
      ...
      if (pointTileMouse.x < 0
          || pointTileMouse.x >= World.MAP_WIDTH
          || pointTileMouse.y < 0
  ...
  private static int renderCarrying(
      Cell cell,
      Citizen cit,
      int iXGeneral,
      int iYGeneral,
      int currentTextureID,
      float fColorShadowLight,
      boolean bLight,
      int iDepth,
      int zLevelOffset) {
    int iFacingDirection = cit.getFacingDirection();

    // Miramos si estß cargando algo para dibujarlo
    if (cit.getCarrying() != null) {
      int iXSpecific = iXGeneral + cit.getOffset_carry_x() + (int) cit.getPositionOffset().x;
      int iYSpecific = iYGeneral
          - (cit.getCarrying().getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
          + cit.getOffset_carry_y()
      ...
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecific,
            iXSpecific + cit.getCarrying().getTileWidth(),
            iYSpecific + cit.getCarrying().getTileHeight(),
            cit.getCarrying().getTileSetTexX1(),
            cit.getCarrying().getTileSetTexY0(),
            cit.getCarrying().getTileSetTexX0(),
            cit.getCarrying().getTileSetTexY1(),
            iDepth);
      } else {
        UtilsGL.drawTextureZ(
            iXSpecific,
      ...
      cit.getCarrying()
          .updateAnimation(cit.getCarrying().isFacingEast() || cit.getCarrying().isFacingNorth());
    }

    if (cit.getCarryingLiving() != null) {
      int iXSpecific = iXGeneral + cit.getOffset_carry_x() + (int) cit.getPositionOffset().x;
      int iYSpecific = iYGeneral
          - (cit.getCarryingLiving().getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
          + cit.getOffset_carry_y()
          + (int) cit.getPositionOffset().y;

      currentTextureID = setColorShadowLightCell(
          cell,
          cit.getCarryingLiving(),
          zLevelOffset,
          currentTextureID,
          false);
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecific + cit.getCarryingLiving().getTileHeight(),
            iXSpecific + cit.getCarryingLiving().getTileWidth(),
            iYSpecific,
            cit.getCarryingLiving().getTileSetTexX1(),
            cit.getCarryingLiving().getTileSetTexY0(),
            cit.getCarryingLiving().getTileSetTexX0(),
            cit.getCarryingLiving().getTileSetTexY1(),
            iDepth);
      } else {
        UtilsGL.drawTextureZ(
            iXSpecific,
    ...
    if (equippedData.isWearing(MilitaryItem.LOCATION_WEAPON)) {
      ...
      int iYSpecific = iYGeneral
          - (cit.getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
          + (int) cit.getPositionOffset().y;
      ...
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        // Flip
        UtilsGL.drawTextureZ(
            iXSpecific + cit.getOffset_weapon_x(),
            iYSpecific + cit.getOffset_weapon_y(),
            iXSpecific + cit.getOffset_weapon_x() + mi.getTileWidth(),
            iYSpecific + cit.getOffset_weapon_y() + mi.getTileHeight(),
            mi.getTileSetTexX1(),
            directionOffset,
            mi.getTileSetTexX0(),
  ...
  private static int renderCarrying(
    ...
    if (hero.getCarrying() != null) {
      ...
      int iYSpecific = iYGeneral
          - (hero.getCarrying().getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
          + hero.getOffset_carry_y()
      ...
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecific,
            iXSpecific + hero.getCarrying().getTileWidth(),
            iYSpecific + hero.getCarrying().getTileHeight(),
            hero.getCarrying().getTileSetTexX1(),
            hero.getCarrying().getTileSetTexY0(),
            hero.getCarrying().getTileSetTexX0(),
            hero.getCarrying().getTileSetTexY1(),
            iDepth);
      } else {
        UtilsGL.drawTextureZ(
            iXSpecific,
      ...
      hero.getCarrying()
          .updateAnimation(hero.getCarrying().isFacingEast() || hero.getCarrying().isFacingNorth());
    }

    if (hero.getCarryingLiving() != null) {
      int iXSpecific = iXGeneral + hero.getOffset_carry_x() + (int) hero.getPositionOffset().x;
      int iYSpecific = iYGeneral
          - (hero.getCarryingLiving().getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
          + hero.getOffset_carry_y()
          + (int) hero.getPositionOffset().y;

      currentTextureID = setColorShadowLightCell(
          cell,
          hero.getCarryingLiving(),
          zLevelOffset,
          currentTextureID,
          false);
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecific + hero.getCarryingLiving().getTileHeight(),
            iXSpecific + hero.getCarryingLiving().getTileWidth(),
            iYSpecific,
            hero.getCarryingLiving().getTileSetTexX1(),
            hero.getCarryingLiving().getTileSetTexY0(),
            hero.getCarryingLiving().getTileSetTexX0(),
            hero.getCarryingLiving().getTileSetTexY1(),
            iDepth);
      } else {
        UtilsGL.drawTextureZ(
            iXSpecific,
    ...
    if (equippedData.isWearing(MilitaryItem.LOCATION_WEAPON)) {
      ...
      int iYSpecific = iYGeneral
          - (hero.getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
          + (int) hero.getPositionOffset().y;
      MilitaryItem mi = equippedData.getWeapon();
      float height = mi.getBaseTileSetTexY1() - mi.getBaseTileSetTexY0();
      float directionOffset = mi.getFacingDirectionYOffset(iFacingDirection);
      ...
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        // Flip
        UtilsGL.drawTextureZ(
            iXSpecific + hero.getOffset_weapon_x(),
            iYSpecific + hero.getOffset_weapon_y(),
            iXSpecific + hero.getOffset_weapon_x() + mi.getTileWidth(),
            iYSpecific + hero.getOffset_weapon_y() + mi.getTileHeight(),
            mi.getTileSetTexX1(),
            directionOffset,
            mi.getTileSetTexX0(),
            directionOffset + height,
  ...
  private static int renderCitizen(
      Cell cell,
      Citizen cit,
      int iXGeneral,
      int iYGeneral,
      int currentTextureID,
      boolean bGraphicChanged,
      float fColorShadowLight,
      boolean bLight,
      int iDepth,
      int zLevelOffset) {
    ...
    if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
      ...
      UtilsGL.drawTextureZ(
          iXSpecific,
          iYSpecific,
          iXSpecific + cit.getTileWidth(),
          iYSpecific + cit.getTileHeight(),
          cit.getTileSetTexX1(),
          cit.getTileSetTexY0(),
          cit.getTileSetTexX0(),
          cit.getTileSetTexY1(),
    ...
    if (!bGraphicChanged) {
      ...
      if (equippedData.isWearing(MilitaryItem.LOCATION_BODY)) {
        ...
        if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
          ...
          // Flip
          UtilsGL.drawTextureZ(
              iXSpecific + cit.getOffset_body_x(),
              iYSpecific + cit.getOffset_body_y(),
              iXSpecific + cit.getOffset_body_x() + mi.getTileWidth(),
              iYSpecific + cit.getOffset_body_y() + mi.getTileHeight(),
              mi.getTileSetTexX1(),
              directionOffset,
              mi.getTileSetTexX0(),
              directionOffset + height,
      ...
      if (equippedData.isWearing(MilitaryItem.LOCATION_HEAD)) {
        ...
        if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
          ...
          // Flip
          UtilsGL.drawTextureZ(
              iXSpecific + cit.getOffset_head_x(),
              iYSpecific + cit.getOffset_head_y(),
              iXSpecific + cit.getOffset_head_x() + mi.getTileWidth(),
              iYSpecific + cit.getOffset_head_y() + mi.getTileHeight(),
              mi.getTileSetTexX1(),
              directionOffset,
              mi.getTileSetTexX0(),
              directionOffset + height,
      ...
      if (equippedData.isWearing(MilitaryItem.LOCATION_FEET)) {
        ...
        if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
          ...
          // Flip
          UtilsGL.drawTextureZ(
              iXSpecific + cit.getOffset_feet_x(),
              iYSpecific + cit.getOffset_feet_y(),
              iXSpecific + cit.getOffset_feet_x() + mi.getTileWidth(),
              iYSpecific + cit.getOffset_feet_y() + mi.getTileHeight(),
              mi.getTileSetTexX1(),
              directionOffset,
              mi.getTileSetTexX0(),
              directionOffset + height,
      ...
      if (equippedData.isWearing(MilitaryItem.LOCATION_LEGS)) {
        ...
        if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
          ...
          // Flip
          UtilsGL.drawTextureZ(
              iXSpecific + cit.getOffset_legs_x(),
              iYSpecific + cit.getOffset_legs_y(),
              iXSpecific + cit.getOffset_legs_x() + mi.getTileWidth(),
              iYSpecific + cit.getOffset_legs_y() + mi.getTileHeight(),
              mi.getTileSetTexX1(),
              directionOffset,
              mi.getTileSetTexX0(),
              directionOffset + height,
  ...
  private static int renderEntities(
      int i,
      int j,
      int zView,
      Cell cell,
    ...
    if (cell.isDiscovered()) {
      if (cell.isMined()) {
          ...
          for (int c = 0; c < Game.getWorld().getProjectiles().size(); c++) {
            ...
            if (projectile.getCoordinates().x == (i)
              ...
              UtilsGL.drawTextureZ(
                  iXGeneral,
                  iYSpecific,
                  iXGeneral + projectile.getTileWidth(),
                  iYSpecific + projectile.getTileHeight(),
                  texX0,
                  texY0,
                  texX1,
                  texY1,
        ...
        if (cell.hasBuilding()) {
          ...
          if (building != null) {
            ...
            if ((building.getCoordinates().x) == cell.getCoordinates().x
              ...
              iYSpecific = iYGeneral
                  - building.getTileHeight()
                  + building.getTileHeightOffset()
                  + ((bmi.getHeight() - 1) * Tile.TERRAIN_ICON_HEIGHT / 2);
              // currentTextureID = UtilsGL.setTexture (building, currentTextureID);
              boolean bTransparency = (flatMouseON
                  && pointTileMouse != null
              ...
              UtilsGL.drawTextureZ(
                  iXSpecific,
                  iYSpecific,
                  iXSpecific + building.getTileWidth(),
              ...
              // Edificio NO construido (NO operativo) o item no construido (no operativo) (cruz
              // roja)
              if (!building.isOperative()) {
                ...
                UtilsGL.drawTextureZ(
                    ...
                    World.getTileRedCross().getTileSetTexY1(),
                    iDepth);
              }
            }
          }
        ...
        if (cell.getTerrain().hasFluids()) {
          ...
          //
          // UtilsGL.drawStringZ (Integer.toString (cell.getTerrain ().getFluidCount ()), iXGeneral
          // + Tile.TERRAIN_ICON_WIDTH / 2 - UtilFont.MAX_WIDTH / 2, iYSpecific +
          // UtilFont.MAX_HEIGHT / 2, iDepth + 1);
          // }
        }

        // Livings
        currentTextureID = renderLivings(
            cell,
            iXGeneral,
            iYGeneral,
            currentTextureID,
  ...
  private static int renderHero(
    ...
    if (!bGraphicChanged) {
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_NORTH
        ...
        currentTextureID = renderCarrying(
            cell,
            hero,
            iXGeneral,
            iYGeneral,
    ...
    if (LivingEntityManager.getItem(hero.getIniHeader()).isFacingDirections()) {
      if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecific,
            iXSpecific + hero.getTileWidth(),
            iYSpecific + hero.getTileHeight(),
    ...
    if (hero.getCitizenData().getBlinkAnimationTurns() > (CitizenData.MAX_BLINK_ANIMATION_TURNS / 2)) {
      ...
      if (tileTask != null) {
        ...
        iYSpecific = iYGeneral
            - (tileTask.getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
            + tileTask.getTileHeightOffset()
            + (int) hero.getPositionOffset().y;

        // En el caso de comer, miramos si estß pasando hambre porque no hay comida
        if (!hero.isSleeping() && hero.getCitizenData().getHungryEating() < 0) {
          // Dibujamos la cruz roja
          Tile tileRedCross = World.getTileRedCross();
          currentTextureID = setColorShadowLightCellNoLight(
              cell,
              tileRedCross,
              zLevelOffset,
              currentTextureID,
          ...
          UtilsGL.drawTextureZ(
              iXSpecific,
              iYSpecific,
              iXSpecific + tileRedCross.getTileWidth(),
              iYSpecific + tileRedCross.getTileHeight(),
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecific,
            iXSpecific + tileTask.getTileWidth(),
            iYSpecific + tileTask.getTileHeight(),
  ...
  private static int renderItems(
      Cell cell,
      Item item,
      int iXGeneral,
      int iYGeneral,
    ...
    if (item != null) {
      ...
      if (item.isDoorStatus(Item.FLAG_WALL_CONNECTOR_STATUS_LOCKED_AND_CLOSED)) {
        ...
        UtilsGL.drawTextureZ(
            iXSpecific,
            iYSpecificLocked,
            iXSpecific + lockedConnectorTile.getTileWidth(),
            iYSpecificLocked + lockedConnectorTile.getTileHeight(),
  ...
  private static int renderLivings(
      Cell cell,
      int iXGeneral,
      int iYGeneral,
      int currentTextureID,
      ...
      boolean bLight,
      int zLevelOffset) {
    // Miramos si aquÝ hay friendlies
    ...
    for (int liv = 0; liv < alLivings.size(); liv++) {
      ...
      Item item = (le.getPath().size() == 0)
          ? cell.getItem()
          : World.getCell(le.getPath().get(0)).getItem();
      if (item != null) {
      ...
      if (iNumEffects > 0) {
        ...
        if (iIndexSkill < iNumEffects && (iCurrentCounter % 16 < 8)) {
          ...
          if (tile != null) {
            ...
            iYSpecific = iYGeneral
                - (tile.getTileHeight() + tile.getTileHeight())
                + tile.getTileHeightOffset()
      ...
      if (lemi.getType() == LivingEntity.TYPE_CITIZEN) {
        ...
        if (lemi.isFacingDirections()) {
          ...
          if (iFacingDirection == LivingEntity.FACING_DIRECTION_NORTH
            ...
            currentTextureID = renderCitizen(
                cell,
                cit,
                iXGeneral,
                iYGeneral,
        ...
        if (cit.getShowExclamationTurns() > 0) {
          ...
          iYSpecific = iYGeneral
              - (tileExclamation.getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
              + tileExclamation.getTileHeightOffset()
              + (int) cit.getPositionOffset().y;
          ...
          UtilsGL.drawTextureZ(
              iXSpecific,
              iYSpecific,
              iXSpecific + tileExclamation.getTileWidth(),
              iYSpecific + tileExclamation.getTileHeight(),
        ...
        iYSpecific = iYGeneral
            - (le.getTileHeight() - Tile.TERRAIN_ICON_HEIGHT)
            + (int) le.getPositionOffset().y;
        ...
        if (lemi.isFacingDirections()) {
            ...
            if (enemy.getCarryingData() != null) {
              if (enemy.getCarryingData().getCarrying() != null) {
                ...
                UtilsGL.drawTextureZ(
                    iXCarrying,
                    iYCarrying,
                    iXCarrying + itemCarrying.getTileWidth(),
                    iYCarrying + itemCarrying.getTileHeight(),
              ...
              if (enemy.getCarryingData().getCarryingLiving() != null) {
                ...
                UtilsGL.drawTextureZ(
                    iXCarrying,
                    iYCarrying + le.getTileHeight(),
                    iXCarrying + leCarrying.getTileWidth(),
                    iYCarrying,
          ...
          if (iFacingDirection == LivingEntity.FACING_DIRECTION_EAST
            ...
            UtilsGL.drawTextureZ(
                iXSpecific,
                iYSpecific,
                iXSpecific + le.getTileWidth(),
                iYSpecific + le.getTileHeight(),
                le.getTileSetTexX1(),
                le.getTileSetTexY0(),
                le.getTileSetTexX0(),
                le.getTileSetTexY1(),
          ...
          if (enemy.getCarryingData() != null) {
            if (enemy.getCarryingData().getCarrying() != null) {
              ...
              UtilsGL.drawTextureZ(
                  iXSpecific,
                  iYSpecific,
                  iXSpecific + itemCarrying.getTileWidth(),
                  iYSpecific + itemCarrying.getTileHeight(),
                  itemCarrying.getTileSetTexX0(),
                  itemCarrying.getTileSetTexY0(),
                  itemCarrying.getTileSetTexX1(),
                  itemCarrying.getTileSetTexY1(),
      ...
      if (le.getAttackAnimationCounter() > 0) {
        ...
        UtilsGL.drawTextureZ(
            iXGeneral,
            iYGeneral,
            iXGeneral + lockedConnectorTile.getTileWidth(),
            iYGeneral + lockedConnectorTile.getTileHeight(),
            lockedConnectorTile.getTileSetTexX0(),
            lockedConnectorTile.getTileSetTexY0(),
            lockedConnectorTile.getTileSetTexX1(),
            lockedConnectorTile.getTileSetTexY1(),
  ...
  private static int renderMouse(
      int iBaseXGeneral,
      int iBaseYGeneral,
      int zView,
      Point3D pointTileMouse,
    ...
    if (pointTileMouse != null && UIPanel.typingPanel == null) {
      if (!(Game.getCurrentState() == Game.STATE_CREATING_TASK
        ...
        if (tDMouseON) {
          ...
          UtilsGL.drawStringZ(
              sLevel,
              iXGeneral + tile.getTileWidth() / 2 - sLevelW,
              iYGeneral + tile.getTileHeight() / 2 + UtilFont.MAX_HEIGHT / 2 - 2,
              ColorGL.WHITE,
              Cell.getDepth(pointTileMouse.x, pointTileMouse.y, pointTileMouse.z));
        }
      }
   ...
   *
   * @param x
   *            X de la celda en pantalla
   * @param y
   *            Y de la celda en pantalla
   * @param pointTileMouse
   *            Celda a mostrar info
   */
  private static void renderTooltip(int x, int y, Point3D pointTileMouse) {
    ...
    if (cell.isDiscovered()) {
      if (cell.hasZone()) {
        ...
        if (stockpile != null
            && stockpile.getType() != null
            && stockpile.getType().getName() != null) {
        ...
        if (cell.getTerrain().hasFluids()) {
          ...
          if (cell.isMined() && pointTileMouse.z < (World.MAP_DEPTH - 1)) {
            sMessage = TerrainManager.getItemByID(
                World.getCell(pointTileMouse.x, pointTileMouse.y, pointTileMouse.z + 1)
                    .getTerrain()
                    .getTerrainID())
                .getName();
          } else {
        ...
        for (int i = 0; i < alLivings.size(); i++) {
          ...
          if (lemi.getType() == LivingEntity.TYPE_CITIZEN) {
            ...
            if (cit.getSoldierData().isSoldier()
              ...
              alMessages.add(
                  cit.getCitizenData().getFullName()
                      + " ("
                      + cit.getLivingEntityData().getHealthStatus()
                      + ") ("
                      + Game.getWorld()
                          .getSoldierGroups()
                          .getGroup(cit.getSoldierData().getGroup())
                          .getName()
                      + ")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
              alColor.add(ColorGL.YELLOW);
            } else {
              // Grupo
              if (cit.getCitizenData().getGroupID() != -1
                ...
                alMessages.add(
                    cit.getCitizenData().getFullName()
                        ...
                        + Game.getWorld()
                            .getCitizenGroups()
                            .getGroup(cit.getCitizenData().getGroupID())
                            .getName()
                        + ")"); //$NON-NLS-1$ //$NON-NLS-2$);
              } else {
                alMessages.add(
            ...
            if (cit.getSoldierData().isSoldier()) {
              alMessages.add(
                  Messages.getString("Hero.4")
                      + cit.getSoldierData().getLevel()
                      + " ("
                      + cit.getSoldierData().getXp()
                      + Messages.getString("Hero.5")
                      + cit.getSoldierData().getXpPCT()
            ...
            if (TownsProperties.DEBUG_MODE) {
              alMessages.add(
                  "Hap/HWork/HIdle: "
                      + cit.getCitizenData().getHappiness()
                      + "/"
            ...
            alMessages.add(
                hero.getCitizenData().getFullName()
                    + " ("
                    + le.getLivingEntityData().getHealthStatus()
                    + ")"); //$NON-NLS-1$ //$NON-NLS-2$);
            alColor.add(ColorGL.YELLOW);
            alMessages.add(
                Messages.getString("Hero.4")
            ...
            if (lemi.getCaravan() != null) {
              ...
              alMessages.add(
                  le.getLivingEntityData().getName()
                      + " ("
                      + le.getLivingEntityData().getHealthStatus()
                      + ")"); //$NON-NLS-1$ //$NON-NLS-2$
            }
          ...
          if (le.getLivingEntityData().getEffects().size() > 0) {
            ...
            alMessages.add(
                Messages.getString("LivingEntity.7") + sBuffer.toString()); // $NON-NLS-1$
    ...
    // alMessages.add ("x " + cell.getCoordinates ().x + " y " + cell.getCoordinates ().y + " z " +
    // cell.getCoordinates ().z + " XMin " + depthXMin + " YMin " + depthYMin + " Depth " +
    // cell.getDepth ());
    // alColor.add (ColorGL.ORANGE);
  ...
  /**
   * Devuelve un contextmen·segun la casilla en la que estÓ
   *
   * @param x
   * @param y
   * @return
  ...
  public ContextMenu getContextMenu(int x, int y) {
    ...
    if (sm.getItems().size() > 0) {
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              Messages.getString("MainPanel.57"),
              null,
              CommandPanel.COMMAND_CLOSE_CONTEXT,
              null)); // $NON-NLS-1$
  ...
  public void mousePressed(int x, int y, int mouseButton) {
    ...
    if (Game.getCurrentState() == Game.STATE_CREATING_TASK) {
      // if (tDMouseON) {
      // //Game.getCurrentTask ().setPoint (new Point3D (p3d.x, p3d.y, getMaxZ3DMouse (p3d.x, p3d.y,
      // p3d.z)));
