...
public final class MessagesPanel {
  ...
  public MessagesPanel(int renderWidth, int renderHeight) {
    final TextureData fontTexture = UtilsGL.loadTexture(
        Towns.getPropertiesString(PropertyFile.PROPERTY_FILE_GRAPHICS, "TILESET_FONT_FILE"),
        GL11.GL_MODULATE);
    if (fontTexture == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MessagesPanel.2"),
  ...
  /**
   * Ańade un mensaje al final de la cola de mensajes, lo parte si no cabe
   *
   * @param iMessageType
   * @param sMessage
   *            Mensaje
  ...
  /**
   * Ańade un mensaje al final de la cola de mensajes Lo parte para meter en la cola a renderizar
   *
   * @param iMessageType
   * @param sMessage
   *            Mensaje
   * @param color
   *            ColorGL del fondo del mensaje
   */
    ...
    for (int i = iIndex; i < iIndex + maxRenderLines; i++) {
      if (i >= 0 && messagesDataRender.length > i && messagesDataRender[i] != null) {
        ...
        if (messagesDataRender[i].getView() != null || messagesDataRender[i].getEntityID() != -1) {
          if (mouseX >= xM
              && mouseX < (xM + messagesDataRender[i].getWidth())
              && mouseY >= yM
              && mouseY < (yM + UtilFont.MAX_HEIGHT)) {
            // POM POM
            ...
            } else if (messagesDataRender[i].getView() != null) { // Si llega aquí debería ser != null
              // siempre
              Game.getWorld().setView(messagesDataRender[i].getView());
              return true;
            }
        ...
        if (messagesDataRender[i].getView() != null || messagesDataRender[i].getEntityID() != -1) {
          if (mouseX >= xM
            ...
            UtilsGL.drawStringWithBorder(
                messagesDataRender[i].getMessage(),
                xM,
                yM,
                messagesDataRender[i].getColor(),
                ColorGL.WHITE);
          } else {
            UtilsGL.drawString(
                messagesDataRender[i].getMessage(),
  ...
  public static void resize(int width, int height) {
    ...
    // mpd = messagesData[i];
    // if (mpd != null) {
    // addMessageRender (m / 2, mpd.getMessage (), mpd.getColor (), mpd.getView (),
    // mpd.getEntityID());
    // }
    // }
    // }
    ...
    maxRenderLines = (UIPanel.MESSAGES_PANEL_SUBPANEL_HEIGHT - 2 * UIPanel.tileMessagesPanel[1].getTileHeight())
        / (UtilFont.MAX_HEIGHT);
    if (maxRenderLines < 1) {
      maxRenderLines = 1;
    }
  ...
  /**
   * Ańade el mensaje a la cola de mensajes a renderizar
   *
   * @param sMessage
   * @param color
   * @param view
   */
  private static void addMessageRender(
      int iMessageType,
      String sMessage,
      ColorGL color,
      Point3DShort view,
    ...
    int iMaxChars = UtilFont.getMaxCharsByWidth(
        sMessage,
        UIPanel.MESSAGES_PANEL_SUBPANEL_WIDTH
            - 2 * UIPanel.tileMessagesPanel[3].getTileWidth());
    if (sMessage.length() > iMaxChars) {
      // No cabe, lo partimos
      messagesDataRender[messagesDataRender.length - 1] = new MessagesPanelData(
          sMessage.substring(0, iMaxChars).trim(), color, view, iEntityID);
