...
public class MatsPanelData {
  ...
  public static void loadGroups() {
    ...
    loadXMLGroups(
        Towns.getPropertiesString("DATA_FOLDER") + "matspanel.xml",
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "matspanel.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
  ...
  private static void loadXMLGroups(String sXMLPath, boolean bLoadingMain) {
    ...
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo los grupos
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (sAux == null || sAux.trim().isEmpty()) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("MatsPanelData.5"),
                "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$
            Game.exit();
          }
          ...
          if (bModChangingValues) {
            ...
            if (sAux == null || sAux.trim().isEmpty()) {
              Log.log(
                  Log.LEVEL_ERROR,
                  Messages.getString("MatsPanelData.0"),
                  "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$
              Game.exit();
            }
            nameGroups.add(sAux);
          ...
          if (bModChangingValues) {
            ...
            if (sAux == null || sAux.trim().isEmpty()) {
              Log.log(
                  Log.LEVEL_ERROR,
                  Messages.getString("MatsPanelData.2"),
                  "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$
              Game.exit();
            }
            iconGroups.add(sAux);
          ...
          if (alTypes == null || alTypes.isEmpty()) {
            if (!bModChangingValues) {
              Log.log(
                  Log.LEVEL_ERROR,
                  Messages.getString("MatsPanelData.4"),
                  "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$
              Game.exit();
            }
          }
          ...
          if (alTypes != null && alTypes.size() > 0) {
            ...
            if (alItems.isEmpty()) {
              Log.log(
                  Log.LEVEL_ERROR,
                  Messages.getString("MatsPanelData.6"),
                  "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$
              Game.exit();
            }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MatsPanelData.8") + e.toString() + "]",
          "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      Game.exit();
    }
    ...
    if (numGroups == 0) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MatsPanelData.11"),
          "MatsPanelData"); //$NON-NLS-1$ //$NON-NLS-2$
      Game.exit();
    }
  }
