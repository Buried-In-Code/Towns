...
public class TradePanel {
  ...
  public TradePanel(CaravanData caravanData, Point coordinates, int width, int height) {
    ...
    UIPanel.resizeIcons(
        getMenuCaravan(),
        UIPanel.TRADE_PANEL_BUTTON_WIDTH,
        UIPanel.TRADE_PANEL_BUTTON_HEIGHT);
    UIPanel.resizeIcons(
        getMenuTown(),
        UIPanel.TRADE_PANEL_BUTTON_WIDTH,
        UIPanel.TRADE_PANEL_BUTTON_HEIGHT);
  ...
  public void recheckPriceToBuySell(CaravanData caravanData) {
    ...
    int iTotal = 0;
    if (caravanData != null
        && caravanData.getMenuCaravanToBuy() != null
            & caravanData.getMenuCaravanToBuy().getItems().size() > 0) {
      int iPrice = 0;
    ...
    // Sell
    if (caravanData != null
        && caravanData.getMenuTownToSell() != null
        && caravanData.getMenuTownToSell().getItems().size() > 0) {
      int iPrice = 0;
  ...
  public void resize(CaravanData caravanData) {
    ...
    // Icons
    setCaravanCoinsIconPoint(
        new Point(
            COLUMN1X
                + tileTradeButton.getTileWidth() / 2
                - tileTradeCaravanCoins.getTileWidth() / 2,
            coordinates.y + 12));
    setTownCoinsIconPoint(
        new Point(
            COLUMN4X + tileTradeButton.getTileWidth() / 2 - tileTradeTownCoins.getTileWidth() / 2,
            coordinates.y + 12));
    setBuyIconPoint(
        new Point(
            COLUMN2X
                + tileTradeButton.getTileWidth() / 2
                - tileTradeBuy.getTileWidth() / 2
                + (tileTradeButton.getTileWidth() * 2),
            coordinates.y + 48));
    setSellIconPoint(
        new Point(
            COLUMN3X + tileTradeButton.getTileWidth() / 2 - tileTradeSell.getTileWidth() / 2,
            coordinates.y + 48));
    setCostPoint(
        new Point(
            coordinates.x + UIPanel.TRADE_PANEL_WIDTH / 2 - tileTradeCost.getTileWidth() / 2,
            coordinates.y + 12));

    int iYMin = getCaravanCoinsIconPoint().y
        + tileTradeCaravanCoins.getTileHeight()
        + UtilFont.MAX_HEIGHT * 2;
    int iYMax = coordinates.y
        + UIPanel.TRADE_PANEL_HEIGHT
        - 48
        - UIPanel.tileScrollDown.getTileHeight()
        - 16
        - UtilFont.MAX_HEIGHT * 2;
    ...
    setScrollUpCaravanPoint(
        new Point(
            COLUMN1X + tileTradeButton.getTileWidth() / 2 - UIPanel.tileScrollUp.getTileWidth() / 2,
            iYMin));
    iYMin += UIPanel.tileScrollUp.getTileHeight()
        + 16; // Tamańo del primer scroll + espacio en blanco (16)
    ...
    setScrollDownCaravanPoint(
        new Point(
            COLUMN1X
                + tileTradeButton.getTileWidth() / 2
                - UIPanel.tileScrollDown.getTileWidth() / 2,
            iYMin));

    // Caravan buttons to-buy
    iYMin = getCaravanCoinsIconPoint().y
        + tileTradeCaravanCoins.getTileHeight()
        + UtilFont.MAX_HEIGHT * 2;
    // Index, miramos si el actual es válido con el nuevo size
    if (getIndexButtonsToBuyCaravan() + MAX_VERTICAL_BUTTONS > caravanData.getMenuCaravanToBuy().getItems()
        .size()) {
      setIndexButtonsToBuyCaravan(0);
    }

    setScrollUpCaravanToBuyPoint(
        new Point(
            COLUMN2X
                + tileTradeButton.getTileWidth() / 2
                - UIPanel.tileScrollUp.getTileWidth() / 2
                + (tileTradeButton.getTileWidth() * 2),
            iYMin));
    iYMin += UIPanel.tileScrollUp.getTileHeight()
        + 16; // Tamańo del primer scroll + espacio en blanco (16)
    ...
    setScrollDownCaravanToBuyPoint(
        new Point(
            COLUMN2X
                + tileTradeButton.getTileWidth() / 2
                - UIPanel.tileScrollDown.getTileWidth() / 2
    ...
    setConfirmPoint(
        new Point(
            coordinates.x + UIPanel.TRADE_PANEL_WIDTH / 2 - tileTradeConfirm.getTileWidth() / 2,
            coordinates.y + UIPanel.TRADE_PANEL_HEIGHT - tileTradeConfirm.getTileHeight() - 8));
    ...
    iYMin = getCaravanCoinsIconPoint().y
        + tileTradeCaravanCoins.getTileHeight()
        + UtilFont.MAX_HEIGHT * 2;
    setScrollUpTownPoint(
        new Point(
            COLUMN4X + tileTradeButton.getTileWidth() / 2 - UIPanel.tileScrollUp.getTileWidth() / 2,
            iYMin));
    iYMin += UIPanel.tileScrollUp.getTileHeight()
        + 16; // Tamańo del primer scroll + espacio en blanco (16)
    iMaxButtons = MAX_VERTICAL_BUTTONS;
    ...
    setScrollUpTownToSellPoint(
        new Point(
            COLUMN3X + tileTradeButton.getTileWidth() / 2 - UIPanel.tileScrollUp.getTileWidth() / 2,
            iYMin));
    iYMin += UIPanel.tileScrollUp.getTileHeight()
        + 16; // Tamańo del primer scroll + espacio en blanco (16)
  ...
  public void scrollDownToBuyCaravan() {
    CaravanData caravanData = Game.getWorld().getCurrentCaravanData();
    if (caravanData != null
        && getIndexButtonsToBuyCaravan() + MAX_VERTICAL_BUTTONS < caravanData.getMenuCaravanToBuy().getItems()
            .size()) {
  ...
  public void scrollDownToSellTown() {
    CaravanData caravanData = Game.getWorld().getCurrentCaravanData();
    if (caravanData != null
        && getIndexButtonsToSellTown() + MAX_VERTICAL_BUTTONS < caravanData.getMenuTownToSell().getItems()
            .size()) {
  ...
  public void selectItemToBuy(int iIndex) {
    ...
    if (p3dFlags.y > 0) {
      boolean bControl = p3dFlags.y > 99
          && (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL)
              || Keyboard.isKeyDown(Keyboard.KEY_RCONTROL));
      boolean bShift = p3dFlags.y > 9
      ...
      // Ańadimos 1 item al menú de items a comprar
      ...
      for (int i = 0; i < caravanData.getMenuCaravanToBuy().getItems().size(); i++) {
        ...
        if (menuAux.getDirectCoordinates().z != -1) {
          ...
          // Item genérico
          if (menuAux.getParameter().equals(menuItem.getParameter())
              && menuAux.getDirectCoordinates().x == menuItem.getDirectCoordinates().x) {
            iFoundIndex = i;
            break;
          }
        }
      ...
      if (iFoundIndex != -1) {
        ...
        SmartMenu sm = new SmartMenu(
            ...
            null,
            null,
            menuItem.getParameter(),
            (bControl ? "x100" : (bShift ? "x10" : "x1")),
            new Point3D(
                p3dFlags.x,
                (bControl ? 100 : (bShift ? 10 : 1)),
                p3dFlags.z)); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    if (p3dFlags.y > 0) {
      ...
      for (int i = 0; i < getMenuCaravan().getItems().size(); i++) {
        ...
        if (menuAux.getDirectCoordinates().z != -1) {
          ...
          // Item genérico
          if (menuAux.getParameter().equals(menuBuy.getParameter())
              && menuAux.getDirectCoordinates().x == menuBuy.getDirectCoordinates().x) {
            iFoundIndex = i;
            break;
          }
        }
  ...
  public void selectItemToNonSell(int iIndex) {
    ...
    if (p3dFlags.y > 0) {
      ...
      // Ańadimos 1 item al menú de items a vender
      // Primero miramos si existe
      int iFoundIndex = -1;
      SmartMenu menuAux;
      ...
      if (bMilitary) {
        for (int i = 0; i < getMenuTown().getItems().size(); i++) {
          ...
          if (menuAux.getParameter().equals(menuSell.getParameter())
              && menuAux.getDirectCoordinates().z == menuSell.getDirectCoordinates().z) {
            iFoundIndex = i;
            break;
          }
        }
        ...
        for (int i = 0; i < getMenuTown().getItems().size(); i++) {
          menuAux = getMenuTown().getItems().get(i);
          if (menuAux.getParameter().equals(menuSell.getParameter())
              && menuAux.getDirectCoordinates().x == menuSell.getDirectCoordinates().x) {
            iFoundIndex = i;
            break;
          }
        }
  ...
  public void selectItemToSell(int iIndex) {
    ...
    if (p3dFlags.y > 0) {
      ...
      // Ańadimos 1 (o 10) item al menú de items a vender
      // Primero miramos si existe
      int iFoundIndex = -1;
      SmartMenu menuAux;
  ...
  public void setCost(CaravanData caravanData, int cost) {
    ...
    int iCostoins = cost;
    if (caravanData.getMenuCaravanToBuy().getItems().size() > 0
        || caravanData.getMenuTownToSell().getItems().size() > 0) {
  ...
  /**
   * Mira toda la lista de to-sell para ver si hay items en el menu de town por cada uno
   *
   * @param caravanData
   */
    ...
    for (int i = caravanData.getMenuTownToSell().getItems().size() - 1; i >= 0; i--) {
      ...
      for (int j = 0; j < getMenuTown().getItems().size(); j++) {
        ...
        if (smTown.getDirectCoordinates().z != -1) {
          ...
          // Item genérico
          if (smTown.getDirectCoordinates().x == smToSell.getDirectCoordinates().x
              && smTown.getParameter().equals(smToSell.getParameter())) {
            bExists = true;
            break;
          }
        }
    ...
    for (int i = 0; i < caravanData.getAlItems().size(); i++) {
      ...
      SmartMenu sm = new SmartMenu(
          ...
          (item instanceof MilitaryItem)
              ? ((MilitaryItem) item).getExtendedTilename()
              : imi.getName(),
          null,
          null,
          sIniHeader,
          "x" + (caravanData.getAlItems().get(i).getQuantity() - iNumItemsToBuy),
          new Point3D(
              (caravanData.getAlItems().get(i).getPrice() * caravanData.getPricePCT()) / 100,
              caravanData.getAlItems().get(i).getQuantity() - iNumItemsToBuy,
              (item instanceof MilitaryItem) ? item.getID() : -1)); // $NON-NLS-1$
  ...
  private void createTownMenu(
      CaravanData caravanData,
      CaravanManagerItem cmi,
      Point3DShort p3dCaravanCoordinates) {
          ...
          if (World.getCell(oItem.getCoordinates()).getAstarZoneID() == iASZID) {
            ...
            if (imi.getType() != null && alBuys.contains(imi.getType())) {
              if (oItem instanceof MilitaryItem) {
                ...
                // Si el número de items en el menú de town da negativo, restamos items del
                // menoToSell
                ...
                SmartMenu sm = new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    ((MilitaryItem) oItem).getExtendedTilename(),
                    null,
                    null,
                    imi.getIniHeader(),
                    "x" + iNumItemsTownMenu,
                    new Point3D(
                        PricesManager.getPrice(oItem), iNumItemsTownMenu, oItem.getID()));
          ...
          if (imi.getType() != null && alBuys.contains(imi.getType())) {
            if (oItem instanceof MilitaryItem) {
              ...
              SmartMenu sm = new SmartMenu(
                  ...
                  imi.getIniHeader(),
                  "x" + iNumItemsTownMenu,
                  new Point3D(PricesManager.getPrice(oItem), iNumItemsTownMenu, oItem.getID()));
              sm.setIcon(imi.getIniHeader());
              sm.setIconType(SmartMenu.ICON_TYPE_ITEM);
              getMenuTown().addItem(sm);
            } else {
      ...
      for (int m = 0; m < alIniHeaders.size(); m++) {
        ...
        if (alQtty.get(m) > 0) {
          SmartMenu sm = new SmartMenu(
              ...
              imi.getIniHeader(),
              "x" + iNumItemsTownMenu,
              new Point3D(imi.getValue(), iNumItemsTownMenu, -1));
          sm.setIcon(imi.getIniHeader());
          sm.setIconType(SmartMenu.ICON_TYPE_ITEM);
          getMenuTown().addItem(sm);
        }
    ...
    // Repasamos toda la lista de to-sell para ver si todos los items que hay ahí existen en la
    // lista de town
  ...
  private int getNumItemsToBuy(CaravanData caravanData, Item item) {
    ...
    for (int i = 0; i < caravanData.getMenuCaravanToBuy().getItems().size(); i++) {
      ...
      if (bMilitary) {
        ...
        if (smItem.getDirectCoordinates().x == ((PricesManager.getPrice(item) * caravanData.getPricePCT())
            / 100)
            && smItem.getParameter().equals(item.getIniHeader())) {
          return smItem.getDirectCoordinates().y;
        }
      }
    }
  ...
  private int getNumItemsToSell(CaravanData caravanData, Item item) {
    ...
    for (int i = 0; i < caravanData.getMenuTownToSell().getItems().size(); i++) {
      ...
      if (bMilitary) {
        ...
        if (smItem.getDirectCoordinates().x == PricesManager.getPrice(item)
            && smItem.getParameter().equals(item.getIniHeader())) {
          return smItem.getDirectCoordinates().y;
        }
      }
  ...
  private int getNumItemsToSell(CaravanData caravanData, String sIniHeader) {
    ...
    for (int i = 0; i < caravanData.getMenuTownToSell().getItems().size(); i++) {
      smItem = caravanData.getMenuTownToSell().getItems().get(i);
      if (smItem.getDirectCoordinates().x == PricesManager.getPrice(sIniHeader)
          && smItem.getParameter().equals(sIniHeader)) {
        return smItem.getDirectCoordinates().y;
      }
    }
  ...
  /**
   * Cambia el valor de un item en la lista de to-sell. Si el valor llega a 0 (o menos ż?) el item se borra y se
   * devuelve true
   *
   * @param caravanData
   * @param sIniHeader
  ...
  private boolean setItemsToSellQtty(CaravanData caravanData, Item oITem, int iQtty) {
    ...
    for (int i = 0; i < caravanData.getMenuTownToSell().getItems().size(); i++) {
      ...
      if (p3dInfo.x == PricesManager.getPrice(oITem)
          && smItem.getParameter().equals(oITem.getIniHeader())) {
        if (iQtty == 0) {
          caravanData.getMenuTownToSell().getItems().remove(i);
          return true;
        } else {
  ...
  private boolean setItemsToSellQtty(CaravanData caravanData, String sIniHeader, int iQtty) {
    ...
    for (int i = 0; i < caravanData.getMenuTownToSell().getItems().size(); i++) {
      smItem = caravanData.getMenuTownToSell().getItems().get(i);
      p3dInfo = smItem.getDirectCoordinates();
      if (p3dInfo.x == PricesManager.getPrice(sIniHeader)
          && smItem.getParameter().equals(sIniHeader)) {
        if (iQtty == 0) {
          caravanData.getMenuTownToSell().getItems().remove(i);
          return true;
