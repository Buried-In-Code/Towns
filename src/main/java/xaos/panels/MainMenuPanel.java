...
public final class MainMenuPanel implements Runnable {
  ...
  public MainMenuPanel(int renderX, int renderY, int renderWidth, int renderHeight) {
    ...
    TextureData textureSMPLogo = UtilsGL.loadTexture(
        Towns.getPropertiesString(PropertyFile.PROPERTY_FILE_GRAPHICS, "SMP_LOGO_FILE"),
        GL11.GL_MODULATE); // $NON-NLS-1$
    if (textureSMPLogo == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MainMenuPanel.21"),
    ...
    TextureData textureLoading = UtilsGL.loadTexture(
        Towns.getPropertiesString(PropertyFile.PROPERTY_FILE_GRAPHICS, "LOADING_FILE"),
        GL11.GL_REPLACE); // $NON-NLS-1$
    if (textureLoading == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MainMenuPanel.24"),
          getClass().getName()); // $NON-NLS-1$
      Game.exit();
    }
    ...
    textureLoading = UtilsGL.loadTexture(
        Towns.getPropertiesString(PropertyFile.PROPERTY_FILE_GRAPHICS, "TOWNS_LOGO_FILE"),
        GL11.GL_REPLACE); // $NON-NLS-1$ //$NON-NLS-2$
    if (textureLoading == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MainMenuPanel.25"),
          getClass().getName()); // $NON-NLS-1$
      Game.exit();
    }
  ...
  public void createMenu() {
    ...
    if (getErrorToShow() != null) {
      mainMenu.addItem(
          new SmartMenu(
              ...
              getErrorToShow(),
              null,
              CommandPanel.COMMAND_MM_DELETE_ERROR,
              null,
              null,
              null,
              Color.RED));
    ...
    if (alCampaigns != null && alCampaigns.size() > 0) {
      // Tutorial
      SmartMenu menuTutorial = new SmartMenu(
          SmartMenu.TYPE_MENU,
          Messages.getString("MainMenuPanel.82"),
          mainMenu,
          null,
          null,
          null,
      ...
      // New game
      SmartMenu menuCampaign = new SmartMenu(
          SmartMenu.TYPE_MENU,
          Messages.getString("MainMenuPanel.53"),
          mainMenu,
          null,
          null,
          null,
      ...
      for (int i = 0; i < alCampaigns.size(); i++) {
        ...
        // alCampaigns.get(i).getName(), menuCampaign, null, null, null, null, textColor);
        ...
        for (int j = 0; j < alCampaigns.get(i).getMissions().size(); j++) {
          ...
          // Ańadimos la opción de bajar burieds
          if (Game.isAllowBury() && alCampaigns.get(i).getMissions().get(j).isAllowBury()) {
            SmartMenu loadBurieds = new SmartMenu(
                SmartMenu.TYPE_MENU, missionName, campaign, null, null, null, null, textColor);
            loadBurieds.setTrasparency(true);
            loadBurieds.setBorderColor(borderColor);

            loadBurieds.addItem(
                new SmartMenu(SmartMenu.TYPE_TEXT, missionName, null, null, null, null));
            loadBurieds.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null, null));

            // No bury
            menuAux = new SmartMenu(
                SmartMenu.TYPE_ITEM,
            ...
            if (Game.getServerNames().size() > 0) {
              for (int s = 0; s < Game.getServerNames().size(); s++) {
                menuAux = new SmartMenu(
                    SmartMenu.TYPE_ITEM,
                    Messages.getString("MainMenuPanel.68")
                        + " ["
                        + Game.getServerNames().get(s)
                        + "]",
                    null,
                    CommandPanel.COMMAND_MM_NEWGAME_SET_SAVE_NAME,
                    sCampaignID,
                    sMissionID,
                    new Point3D(s, s, s),
                    textColor); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                menuAux.setBorderColor(borderColor);
                loadBurieds.addItem(menuAux);
                loadBurieds.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
              }
        ...
        // menuTutorial.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null,
        // null, null));
        // } else {
        // menuCampaign.addItem(campaign);
        // menuCampaign.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null,
        // null, null));
        // }
      }
      ...
      // Messages.getString("MainMenuPanel.7"), null, CommandPanel.COMMAND_BACK, null, null, null,
      // textColor); //$NON-NLS-1$
      // menuAux.setBorderColor(borderColor);
    ...
    if (alSavegames != null && alSavegames.size() > 0) {
      SmartMenu menuLoad = new SmartMenu(
          SmartMenu.TYPE_MENU,
          Messages.getString("MainMenuPanel.38"),
          mainMenu,
          null,
          null,
          null,
      ...
      for (int i = 0; i < alSavegames.size(); i++) {
        ...
        if (cal.get(Calendar.DAY_OF_MONTH) == calToday.get(Calendar.DAY_OF_MONTH)
          ...
          sDate = " ("
              ...
              + "/"
              + cal.get(Calendar.YEAR); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          // //$NON-NLS-5$
        }
        sDate += " "
        ...
        menuLoad.addItem(
            new SmartMenu(
                ...
                Messages.getString("MainMenuPanel.40")
                    + Utils.removeExtension(fAux.getName())
                    + sDate,
                null,
        ...
        SmartMenu menuDelete = new SmartMenu(
            SmartMenu.TYPE_MENU,
    ...
    // Mods
    SmartMenu smMods = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.58"),
        mainMenu,
        null,
        null,
        null,
    ...
    smMods.addItem(
        new SmartMenu(
            SmartMenu.TYPE_ITEM,
            Messages.getString("MainMenuPanel.70") + " [" + sModsFolder + "]",
            null,
    ...
    // Servers
    SmartMenu smServers = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.71"),
        mainMenu,
        null,
        null,
        null,
    ...
    if (alServerNames.size() > 0) {
      ...
      for (int i = 0; i < alServerNames.size(); i++) {
        ...
        smServers.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("MainMenuPanel.73") + sServer,
                null,
                CommandPanel.COMMAND_SERVER_REMOVE,
                Game.getServers().get(i))); // $NON-NLS-1$
      ...
      smServers.addItem(
          new SmartMenu(
              SmartMenu.TYPE_TEXT,
              Messages.getString("MainMenuPanel.74"),
              null,
              null,
              null,
              null)); // $NON-NLS-1$
    ...
    smServers.addItem(
        new SmartMenu(
            SmartMenu.TYPE_ITEM,
            Messages.getString("MainMenuPanel.75"),
            null,
            CommandPanel.COMMAND_SERVER_ADD,
            null)); // $NON-NLS-1$
    smServers.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
    ...
    SmartMenu menuOptions = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.4"),
        mainMenu,
        null,
        null,
        null,
    ...
    SmartMenu menuOptionsGraphics = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.16"),
        menuOptions,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.17"),
        null,
    ...
    // Options - Audio
    SmartMenu menuOptionsAudio = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.13"),
        menuOptions,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.5"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.56"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.6"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.57"),
        null,
    ...
    // Options - Game
    SmartMenu menuOptionsGame = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.19"),
        menuOptions,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.20"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.33"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.72"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.27"),
        null,
    ...
    if (TownsProperties.GODS_ACTIVATED) {
      menuAux = new SmartMenu(
          SmartMenu.TYPE_ITEM,
          Messages.getString("MainMenuPanel.77"),
          null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.28"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.31"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.32"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.36"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.66"),
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.62"),
        null,
    ...
    // Options - Controls
    SmartMenu menuOptionsControls = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.41"),
        menuOptions,
        null,
        null,
        null,
    ...
    menuOptionsControls.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.43"),
            null,
            null,
            null,
            null,
            null,
            creditsColor)); // $NON-NLS-1$
    ...
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_LEVEL_UP, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_LEVEL_DOWN, textColor, borderColor));
    menuOptionsControls.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null, null));
    menuOptionsControls.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.44"),
            null,
            null,
            null,
            null,
            ...
            creditsColor)); // $NON-NLS-1$
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SHOW_MISSION, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SHOW_STOCK, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SHOW_PRIORITIES, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SHOW_TRADE, textColor, borderColor));
    menuOptionsControls.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null, null));
    menuOptionsControls.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.45"),
            null,
            null,
            null,
            null,
            ...
            creditsColor)); // $NON-NLS-1$
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_NEXT_CITIZEN, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_PREVIOUS_CITIZEN, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_NEXT_SOLDIER, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_PREVIOUS_SOLDIER, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_NEXT_HERO, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_PREVIOUS_HERO, textColor, borderColor));
    menuOptionsControls.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null, null));
    menuOptionsControls.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.46"),
            null,
            null,
            null,
            null,
            ...
            creditsColor)); // $NON-NLS-1$
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_FULLSCREEN, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_FLAT_MOUSE, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_3D_MOUSE, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_GRID, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_MINIBLOCKS, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_HIDE_UI, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_TOGGLE_ITEM_BUILD_FACE, textColor, borderColor));
    menuOptionsControls.addItem(createKeyboardMenu(UtilsKeyboard.FN_PAUSE, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SPEED_UP, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SPEED_DOWN, textColor, borderColor));
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_SCREENSHOT, textColor, borderColor));
    menuOptionsControls.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null, null));
    menuOptionsControls.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.61"),
            null,
            null,
            null,
            null,
    ...
    menuOptionsControls.addItem(
        createKeyboardMenu(UtilsKeyboard.FN_BOT_10, textColor, borderColor));
    ...
    // Options - Performance
    SmartMenu menuOptionsPerformance = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.8"),
        menuOptions,
        null,
        null,
        null,
    ...
    menuOptionsPerformance.addItem(
        new SmartMenu(
            ...
            null)); // $NON-NLS-1$
    menuOptionsPerformance.addItem(
        new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null, null));

    menuOptionsPerformance.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.59"),
            null,
            null,
            null,
            null,
            null,
            Color.LIGHT_GRAY)); // $NON-NLS-1$
    menuOptionsPerformance.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.64"),
            null,
            null,
            null,
            null,
    ...
    menuOptionsPerformance.addItem(
        new SmartMenu(
            SmartMenu.TYPE_TEXT,
            Messages.getString("MainMenuPanel.65"),
            null,
            null,
            null,
            null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.63"),
        null,
    ...
    if (alLanguages != null && alLanguages.size() > 1) {
      menuOptionsLanguage = new SmartMenu(
          SmartMenu.TYPE_MENU,
          Messages.getString("MainMenuPanel.14"),
          menuOptions,
          null,
          null,
          null,
    ...
    menuOptions.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
    menuOptions.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
    // menuAux = new SmartMenu (SmartMenu.TYPE_ITEM, Messages.getString("MainMenuPanel.26"), null,
    // CommandPanel.COMMAND_SAVE_OPTIONS, null, null, null, textColor); //$NON-NLS-1$
    // menuAux.setBorderColor (borderColor);
    ...
    // Credits
    SmartMenu smCredits = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("MainMenuPanel.34"),
        mainMenu,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_TEXT,
        Messages.getString("MainMenuPanel.35"),
        null,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_TEXT,
        Messages.getString("MainMenuPanel.37"),
        null,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_TEXT,
        Messages.getString("MainMenuPanel.39"),
        null,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_TEXT,
        Messages.getString("MainMenuPanel.78"),
        null,
        null,
        null,
        null,
    ...
    menuAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("MainMenuPanel.3"),
        null,
        CommandPanel.COMMAND_EXIT_GAME,
        null,
        null,
        null,
        textColor); // $NON-NLS-1$
    ...
    if (isSettingSavegameName()) {
      ...
      if (TypingPanel.keyPressed(iKey)) {
        ...
        if (TypingPanel.getNewText() != null && TypingPanel.getNewText().length() > 0) {
          ...
          if (settingHotkey == 1) {
            UtilsKeyboard.redefineKey(
                0,
                TypingPanel.TYPING_PARAMETER,
                Integer.parseInt(TypingPanel.getNewText()));
            settingHotkey = 2;
            TypingPanel.setTitle(
                Messages.getString("MainMenuPanel.52")
                    ...
                    + ((UtilsKeyboard.getKey(TypingPanel.TYPING_PARAMETER, 0) == Keyboard.KEY_NONE)
                        ? ""
                        : Messages.getString("MainMenuPanel.50")
                            + Keyboard.getKeyName(
  ...
  public void loadMenuTexture(boolean bUnload) {
    final TextureData textureMainMenu = UtilsGL.loadTexture(
        Towns.getPropertiesString(PropertyFile.PROPERTY_FILE_GRAPHICS, "MAINMENU_BG_FILE"),
        GL11.GL_REPLACE,
        true); // $NON-NLS-1$
    if (textureMainMenu == null) {
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("MainMenuPanel.0"),
          getClass().getName()); // $NON-NLS-1$
      Game.exit();
    }
        ...
        if (isSettingSavegameName()) {
          ...
          if (iMousePanel == UIPanel.MOUSE_TYPING_PANEL_CLOSE) {
            ...
            if (TypingPanel.getNewText() != null && TypingPanel.getNewText().length() > 0) {
              ...
              if (!Utils.existsSavegame(
                  TypingPanel.getNewText())) { // Sólo si no existe en disco previamente
                startGame(TypingPanel.getNewText());
              }
            }
          }
  ...
  public void render() {
    ...
    UtilsGL.drawTexture(
        ...
        centerY - 10 - imageTownsLogoHeight,
        centerX + imageTownsLogoWidth / 2,
        centerY - 10,
        0,
        0,
    ...
    UtilsGL.drawStringWithBorder(
        sVersion,
        renderX + renderWidth - UtilFont.getWidth(sVersion) - 10,
        renderY + renderHeight - UtilFont.MAX_HEIGHT - 10,
        COLORGL_WHITE,
        COLORGL_BLACK);
    ...
    if (TownsProperties.DEMO_VERSION) {
      UtilsGL.drawStringWithBorder(
          Messages.getString("MainMenuPanel.23"),
          11,
          renderY + renderHeight - UtilFont.MAX_HEIGHT - 10,
          COLORGL_WHITE,
          COLORGL_BLACK); // $NON-NLS-1$
    }
    UtilsGL.glEnd();
  ...
  public void setSettingHotkey(boolean settingHotkey, int iFN) {
    if (settingHotkey) {
      ...
      new TypingPanel(
          renderWidth,
          renderHeight,
          Messages.getString("MainMenuPanel.48")
              + UtilsKeyboard.getFNHumanString(iFN)
              + ((UtilsKeyboard.getKey(iFN, 0) == Keyboard.KEY_NONE)
                  ? ""
                  : Messages.getString("MainMenuPanel.50")
                      + Keyboard.getKeyName(UtilsKeyboard.getKey(iFN, 0))
                      + ")"),
          new String(),
          TypingPanel.TYPE_REDEFINE_KEYS,
          iFN); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    } else {
  ...
  public void setSettingNewServer(boolean bSettingNewServer) {
    ...
    if (bSettingNewServer) {
      new TypingPanel(
          renderWidth,
          renderHeight,
          Messages.getString("MainMenuPanel.76"),
          new String(),
          TypingPanel.TYPE_ADD_SERVER,
          0); // $NON-NLS-1$
    }
  }

  public void setSettingSavegameName(
      boolean settingSavegameName,
      String sCampaign,
      String sMission) {
    ...
    if (settingSavegameName) {
      new TypingPanel(
          renderWidth,
          renderHeight,
          Messages.getString("MainMenuPanel.47"),
          new String(),
          TypingPanel.TYPE_SAVEGAME_NAME,
          0); // $NON-NLS-1$
    }
  }

  /**
   * Cambia los textos de todos los menus COMMAND_CHANGE_HOTKEY Se llama después deredifinir alguna tecla.
   */
  private void checkChangeHotkeyMenusText(SmartMenu sm) {
    ...
    } else if (sm.getType() == SmartMenu.TYPE_ITEM
        && sm.getCommand() != null
  ...
  private void startGame(String savegameName) {
    ...
    CommandPanel.executeCommand(
        CommandPanel.COMMAND_MM_NEWGAME,
        getSaveGameCampaignID(),
