...
public class SmartMenu implements Externalizable {
  ...
  public static final int ICON_TYPE_UI = 0;
  public static final int ICON_TYPE_ITEM = 1;

  public static final Color COLOR_SUBMENU = Color.ORANGE.brighter();
  public static final ColorGL COLORGL_SUBMENU = new ColorGL(COLOR_SUBMENU);
  ...
  public static void readXMLMenu(
    ...
    try {
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("SmartMenu.1")
              + sFilename
              + Messages.getString("SmartMenu.2")
              + e.toString()
              + "]",
          "SmartMenu"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  ...
  private static void readXMLItem(
      Document doc,
      NodeList list,
      SmartMenu smartMenu,
      boolean bLoadingMain) {
    Node node;
    for (int i = 0; i < list.getLength(); i++) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (node.getNodeName().equalsIgnoreCase("ITEM")) { // $NON-NLS-1$
          ...
          if (map.getNamedItem("delete") != null
              && map.getNamedItem("delete").getNodeValue() != null
              && map.getNamedItem("delete")
                  .getNodeValue()
                  .equalsIgnoreCase(
          ...
          if (code == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("SmartMenu.7"),
                "SmartMenu"); //$NON-NLS-1$ //$NON-NLS-2$
            Game.exit();
          } else if (code.getNodeValue() != null
              && code.getNodeValue().equalsIgnoreCase("BLANKLINE")) { // $NON-NLS-1$
            // Linea en blanco
            ...
            if (sName == null || sName.isEmpty()) {
              ...
              // CREATEANDPLACEROW o BUILD para obtener la cadena de la definición del item/edificio
              if (parameter != null
                ...
                if (sName == null || sName.trim().isEmpty()) {
                  Log.log(
                      Log.LEVEL_ERROR,
                      Messages.getString("SmartMenu.0") + parameter.getNodeValue() + "]",
                      Messages.getString("SmartMenu.5")); // $NON-NLS-1$ //$NON-NLS-2$
                  // //$NON-NLS-3$
                  Game.exit();
                }
              } else {
                Log.log(
                    Log.LEVEL_ERROR,
                    Messages.getString("SmartMenu.4"),
                    "SmartMenu"); //$NON-NLS-1$ //$NON-NLS-2$
                Game.exit();
              }
            }
            ...
            if (parameter != null) {
              ...
              item = new SmartMenu(
                  SmartMenu.TYPE_ITEM, sName, null, code.getNodeValue(), null); // $NON-NLS-1$
            }
            ...
            if (map.getNamedItem("icon") != null) { // $NON-NLS-1$
              ...
              if (code != null
                ...
                // Miramos si es un código de crear objeto, en ese caso el icono se pilla según el
                // mismo
            ...
            // Si es un back lo ańadimos tal cual, en otro caso miramos que no haya un back, para
            // ańadirlo justo antes
            if (item.getCommand() != null
              ...
              if (smartMenu.getItems().size() > 0) {
                ...
                if (smLast.getCommand() != null
                  ...
                  // Hay un back, ańadimos el item justo antes
          ...
          if (map.getNamedItem("delete") != null
            ...
            continue;
          } else if (map.getNamedItem("deleteContent") != null
              && map.getNamedItem("deleteContent").getNodeValue() != null
              && map.getNamedItem("deleteContent")
                  .getNodeValue()
                  .equalsIgnoreCase(
                      "TRUE")) { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            if (bModChangingValues) {
          ...
          if (sName == null || sName.isEmpty()) {
            if (map.getNamedItem("name") != null
                && map.getNamedItem("name").getNodeValue() != null
                && map.getNamedItem("name").getNodeValue().length() > 0) { // $NON-NLS-1$ //$NON-NLS-2$
              // //$NON-NLS-3$
              sName = map.getNamedItem("name").getNodeValue(); // $NON-NLS-1$
            }
          }
            ...
            if (alQueue.get(i).getType() == QueueItem.TYPE_MOVE
              ...
              if (imi != null) {
                ...
                if (imi.isBlockFluids()) {
                  alMessages.add(
                      "("
                          + Messages.getString("SmartMenu.8")
            ...
            for (int i = 0; i < alPrerequisites.size(); i++) {
              ...
              for (int ite = 0; ite < aItems.length; ite++) {
                if (ite == 0) {
                  ...
                  sName += Messages.getString("SmartMenu.3")
                      + ItemManager.getItem(UtilsIniHeaders.getStringIniHeader(aItems[ite]))
                          .getName(); // $NON-NLS-1$
                }
              }
              if (sName != null) {
              ...
              for (int liv = 0; liv < aLivings.length; liv++) {
                if (liv == 0) {
                  sName = LivingEntityManager
                      .getItem(UtilsIniHeaders.getStringIniHeader(aLivings[liv]))
                      .getName();
                } else {
                  sName += Messages.getString("SmartMenu.3")
                      + LivingEntityManager.getItem(
                          UtilsIniHeaders.getStringIniHeader(aLivings[liv]))
                          .getName(); // $NON-NLS-1$
                }
              }
              if (sName != null) {
  ...
  /**
   * Comprueba si se ha clicado en un submenu o en un item En el primer caso devuelve dicho submenú En el segundo caso
   * ejecuta la acción correspondiente y se devuelve él mismo
   *
   * @param x
   *            X Relativa al menú
   * @param y
