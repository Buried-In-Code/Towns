      ...
      if (sCommand.equals(COMMAND_BACK)) {
        ...
        if (tile != null) {
          ...
          if (ami != null && ami.getGeneratedItem() != null) {
            Game.getCurrentTask()
                .setTile(new Tile(ami.getGeneratedItem()), SmartMenu.ICON_TYPE_ITEM);
          }
        }
        ...
        if (stockpile != null) {
          ...
          for (int h = 0; h < stockpile.getPoints().size(); h++) {
            Game.getWorld()
                .addItemToBeHauled(World.getCell(stockpile.getPoints().get(h)).getItem());
          }
        }
        ...
        if (pileSource != null) {
          ...
          for (int i = 0; i < alStockpiles.size(); i++) {
            ...
            if (pileDest.getID() != pileSource.getID()) {
              if (pileDest.getType().getID().equals(typeSource.getID())
                  && !pileDest.isLockedToCopy()) {
                // Bingo!
                // Let's copy the type
                ...
                for (int j = (typeSource.getElements().size() - 1); j >= 0; j--) {
                  if (!typeDestination.contains(typeSource.getElements().get(j))) {
                    typeDestination.addElement(
                        typeSource.getElements().get(j),
            ...
            if (bSomethingRemoved) {
              for (int h = 0; h < pileDest.getPoints().size(); h++) {
                Game.getWorld()
                    .addItemToBeHauled(World.getCell(pileDest.getPoints().get(h)).getItem());
              }
            }
          }
        }
        ...
        // Como esto es en diferido, quizá el usuario hace botón derecho, deja pasar el tiempo y
        // luego clica
        ...
        if (TownsProperties.DEBUG_MODE) {
          ...
          if (view.z < (Game.getWorld().getNumFloorsDiscovered() - 1)
        ...
        try {
          ...
          Log.log(
              ...
              "CommandPanel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_SYSTEM,
              Messages.getString("CommandPanel.38") + ex.toString() + "]",
        ...
        if (UIPanel.typingPanel == null) {
          UIPanel.typingPanel = new TypingPanel(
              UIPanel.renderWidth,
              UIPanel.renderHeight,
              Messages.getString("CommandPanel.14"),
              "",
              TypingPanel.TYPE_ADD_TEXT_TO_ITEM,
              Integer.valueOf(sParameter)); // $NON-NLS-1$ //$NON-NLS-2$
        }
        ...
        if (item != null
            && item.isLocked()
            && item.getCoordinates() != null
            && item.getCoordinates().x != -1) {
        ...
        try {
          ...
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_SYSTEM,
              Messages.getString("CommandPanel.9")); // $NON-NLS-1$
        } catch (Exception ex) {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("CommandPanel.8") + " [" + ex.toString() + "]",
              "CommandPanel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          MessagesPanel.addMessage(
              MessagesPanel.TYPE_SYSTEM,
              Messages.getString("CommandPanel.8") + " [" + ex.toString() + "]",
        ...
        if (TownsProperties.DEBUG_MODE) {
          smExit.addItem(
              new SmartMenu(
                  SmartMenu.TYPE_ITEM,
                  "Admin save, no mission",
                  null,
                  COMMAND_SAVE_NO_MISSIONDATA,
                  null)); // $NON-NLS-1$
        ...
        smExit.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("CommandPanel.4"),
                null,
                COMMAND_SAVE,
                null)); // $NON-NLS-1$
        smExit.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
        smExit.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("CommandPanel.0"),
                null,
                COMMAND_EXIT_TO_MAIN_MENU_SAVE,
                null)); // $NON-NLS-1$
        smExit.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
        SmartMenu smSure = new SmartMenu(
            SmartMenu.TYPE_MENU,
            Messages.getString("CommandPanel.1"),
            smExit,
            null,
            null); // $NON-NLS-1$
        smSure.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("CommandPanel.3"),
                null,
                COMMAND_EXIT_TO_MAIN_MENU_NOSAVE,
                null)); // $NON-NLS-1$
        smSure.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
        smExit.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("CommandPanel.7"),
                null,
                COMMAND_BURY,
                null)); // $NON-NLS-1$
        smExit.addItem(new SmartMenu(SmartMenu.TYPE_TEXT, null, null, null, null));
        smSure.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("CommandPanel.5"),
                null,
                COMMAND_BACK,
                null)); // $NON-NLS-1$
        ...
        smExit.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("CommandPanel.2"),
                null,
                COMMAND_CLOSE_CONTEXT,
                null)); // $NON-NLS-1$
        ...
        executeCommand(
            COMMAND_MM_NEWGAME_SET_SAVE_NAME,
            sParameter,
            sParameter2,
            p3dDirect,
            tile,
        ...
        // If the campaign/mission folder contains a "save.zip", then we will just load that one and
        // set the missionData on campaigns.xml
        ArrayList<String> alPaths = Utils.getPathToFile("save.zip", sParameter, sParameter2); // $NON-NLS-1$
        if (alPaths.size() > 0) {
          executeCommand(
              COMMAND_MM_CONTINUEGAME,
              "save.zip",
              sParameter + "," + sParameter2 + "," + alPaths.get(0),
              null,
              null,
        ...
        if (TownsProperties.TEST_COMMANDS) {
          if (sCommand.equals(COMMAND_TEST)) {
            ...
            // Game.getWorld ().getGods ().get (i).setStatus (Game.getWorld ().getGods ().get
            // (i).getStatus () - 5);
            // break;
            // }
            // }
            ...
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("CommandPanel.6") + sCommand + "] [" + sParameter + "]",
                "CommandPannel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          }
        } else {
          Log.log(
              Log.LEVEL_ERROR,
