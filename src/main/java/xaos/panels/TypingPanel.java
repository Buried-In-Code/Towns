...
public class TypingPanel {
  ...
  public TypingPanel(
      int renderWidth, int renderHeight, String title, String oldText, int type, int parameter) {
    setTitle(title);
    setOldText(oldText);
  ...
  public static void createPanel(int renderWidth, int renderHeight) {
    ...
    // Width / height
    WIDTH = UtilFont.MAX_WIDTH * MAX_CHARS
        + (UtilFont.MAX_WIDTH * MAX_CHARS) / 8
        + 2 * tilePanel[3].getTileWidth();
    ...
    closeButtonPoint.setLocation(
        ...
        panelPoint.y);
    titlePoint.setLocation(
        panelPoint.x + WIDTH / 2 - UtilFont.getWidth(getTitle()) / 2,
        panelPoint.y + tilePanel[1].getTileHeight());
    oldTextPoint.setLocation(
        panelPoint.x + WIDTH / 2 - UtilFont.getWidth(getOldText()) / 2,
        titlePoint.y + 2 * UtilFont.MAX_HEIGHT);

    panelNewTextPoint.setLocation(
        panelPoint.x + WIDTH / 2 - WIDTH_SUBPANEL / 2,
        oldTextPoint.y + 2 * UtilFont.MAX_HEIGHT);
    updateNewTextPosition();

    confirmPoint.setLocation(
        panelPoint.x + WIDTH / 2 - tileConfirm.getTileWidth() / 2,
        panelNewTextPoint.y + HEIGHT_SUBPANEL + UtilFont.MAX_HEIGHT);
  }
  ...
  /**
   * Ańade un caracter al texto y retorna true en caso de que ya haya terminado.
   *
   * @param key
   * @return
   */
  ...
  public static void render(int mousePanel) {
    ...
    UtilsGL.drawStringWithBorder(
        "_",
        newTextPoint.x + UtilFont.getWidth(getNewText()),
        newTextPoint.y,
        ColorGL.WHITE,
  ...
  /**
   * Determina donde está el mouse y llama al render (mousePanel) Se usa, de momento, desde el main menu, para teclear
   * el nombre de la partida grabada
   */
  ...
  public static int whereIsMouse(int mouseX, int mouseY) {
    if (UIPanel.isMouseOnAnIcon(
        mouseX,
        mouseY,
        closeButtonPoint,
        UIPanel.tileButtonClose,
        UIPanel.tileButtonCloseAlpha)) {
      return UIPanel.MOUSE_TYPING_PANEL_CLOSE;
    } else if (UIPanel.isMouseOnAnIcon(
        mouseX,
        mouseY,
        confirmPoint,
        tileConfirm,
  ...
  private static void updateNewTextPosition() {
    newTextPoint.setLocation(
        panelNewTextPoint.x + WIDTH_SUBPANEL / 2 - UtilFont.getWidth(getNewText()) / 2,
        panelNewTextPoint.y + HEIGHT_SUBPANEL / 2 - UtilFont.MAX_HEIGHT / 2);
  }

  private static void updateSubTitlePosition() {
    if (getSubTitle() != null) {
      subTitlePoint.setLocation(
          panelPoint.x + WIDTH / 2 - UtilFont.getWidth(getSubTitle()) / 2,
          titlePoint.y + 2 * UtilFont.MAX_HEIGHT);
    }
  }
}
