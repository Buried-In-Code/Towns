...
/** Clase de tipo "managerItem", no es la que se ańade a la lista de gods */
public class GodManagerItem {
  ...
  public void setItemsDislikePCT(ArrayList<String> itemsDislikePCT) throws Exception {
    if (itemsDislikePCT == null) {
      ...
      // Si no hay items o el tamańo no es el mismo, BAM, error
      ...
      for (int i = 0; i < itemsDislikePCT.size(); i++) {
        try {
          ...
          throw new Exception(
              Messages.getString("GodManagerItem.6")
                  + " ["
                  + itemsDislikePCT.get(i)
    ...
    if (itemsLikePCT == null) {
      ...
      for (int i = 0; i < itemsLikePCT.size(); i++) {
        try {
          ...
          throw new Exception(
              Messages.getString("GodManagerItem.3")
                  + " ["
                  + itemsLikePCT.get(i)
                  + "]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
