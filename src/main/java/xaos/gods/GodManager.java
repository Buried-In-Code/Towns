...
public class GodManager {
  ...
  public static void loadItems() {
    ...
    // Cargar de fichero
    loadXMLEvents(
        Towns.getPropertiesString("DATA_FOLDER") + "gods.xml", // $NON-NLS-2$
        true); // $NON-NLS-1$
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "gods.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
      ...
      if (gmi.getItemsLike() != null && gmi.getItemsDislike().size() > 0) {
        for (int i = 0; i < gmi.getItemsLike().size(); i++) {
          if (ItemManager.getItem(gmi.getItemsLike().get(i)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("GodManager.2") + " [" + gmi.getItemsLike().get(i) + "]",
                "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      }
      ...
      if (gmi.getItemsDislike() != null && gmi.getItemsDislike().size() > 0) {
        for (int i = 0; i < gmi.getItemsDislike().size(); i++) {
          if (ItemManager.getItem(gmi.getItemsDislike().get(i)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("GodManager.6") + " [" + gmi.getItemsDislike().get(i) + "]",
                "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (gmi.getEventsWhenHappy() != null && gmi.getEventsWhenHappy().size() > 0) {
        for (int i = 0; i < gmi.getEventsWhenHappy().size(); i++) {
          if (EventManager.getItem(gmi.getEventsWhenHappy().get(i)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("GodManager.3") + " [" + gmi.getEventsWhenHappy().get(i) + "]",
                "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (gmi.getEventsWhenReallyHappy() != null && gmi.getEventsWhenReallyHappy().size() > 0) {
        for (int i = 0; i < gmi.getEventsWhenReallyHappy().size(); i++) {
          if (EventManager.getItem(gmi.getEventsWhenReallyHappy().get(i)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("GodManager.8")
                    + " ["
                    + gmi.getEventsWhenReallyHappy().get(i)
                    + "]",
                "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
      ...
      if (gmi.getEventsWhenAngry() != null && gmi.getEventsWhenAngry().size() > 0) {
        for (int i = 0; i < gmi.getEventsWhenAngry().size(); i++) {
          if (EventManager.getItem(gmi.getEventsWhenAngry().get(i)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("GodManager.12") + " [" + gmi.getEventsWhenAngry().get(i) + "]",
                "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (gmi.getEventsWhenReallyAngry() != null && gmi.getEventsWhenReallyAngry().size() > 0) {
        for (int i = 0; i < gmi.getEventsWhenReallyAngry().size(); i++) {
          if (EventManager.getItem(gmi.getEventsWhenReallyAngry().get(i)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("GodManager.16")
                    + " ["
                    + gmi.getEventsWhenReallyAngry().get(i)
                    + "]",
                "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
  ...
  private static void loadXMLEvents(String sXMLName, boolean bLoadingMain) {
    ...
    try {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            sAux = UtilsXML.getChildValue(
                node.getChildNodes(),
                "eventsWhenReallyHappy"); // $NON-NLS-1$
            if (sAux != null) {
            ...
            sAux = UtilsXML.getChildValue(
                node.getChildNodes(),
                "eventsWhenReallyAngry"); // $NON-NLS-1$
            if (sAux != null) {
              gmi.setEventsWhenReallyAngry(Utils.getArray(sAux));
            }
          } else {
            gmi.setNamePool(
                UtilsXML.getChildValue(node.getChildNodes(), "namePool")); // $NON-NLS-1$
            gmi.setSurName(UtilsXML.getChildValue(node.getChildNodes(), "surName")); // $NON-NLS-1$
            gmi.setItemsLike(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "itemsLike"))); // $NON-NLS-1$
            gmi.setItemsLikePCT(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "itemsLikePCT"))); // $NON-NLS-1$
            gmi.setItemsDislike(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "itemsDislike"))); // $NON-NLS-1$
            gmi.setItemsDislikePCT(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "itemsDislikePCT"))); // $NON-NLS-1$
            gmi.setEventsWhenHappy(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "eventsWhenHappy"))); // $NON-NLS-1$
            gmi.setEventsWhenReallyHappy(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "eventsWhenReallyHappy"))); // $NON-NLS-1$
            gmi.setEventsWhenAngry(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "eventsWhenAngry"))); // $NON-NLS-1$
            gmi.setEventsWhenReallyAngry(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "eventsWhenReallyAngry"))); // $NON-NLS-1$
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("GodManager.1") + " [" + e.getMessage() + "]",
          "GodManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
