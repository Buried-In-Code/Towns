...
public class EventManager {
  ...
  public static void loadItems() {
    ...
    // Cargar de fichero
    loadXMLEvents(
        Towns.getPropertiesString("DATA_FOLDER") + "events.xml", // $NON-NLS-2$
        true); // $NON-NLS-1$
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "events.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
    ...
    // Miramos que los items, itemSpawnLivings, siegeLivings, afterEvents, prerequisites,
    // eventsInmune, effects, effectsAfterEat y effectsAfterSleep existan
    Iterator<EventManagerItem> itEffects = eventsList.values().iterator();
    EventManagerItem emi;
    while (itEffects.hasNext()) {
      emi = itEffects.next();
      ...
      if (emi.getItems() != null && emi.getItems().size() > 0) {
        for (int e = 0; e < emi.getItems().size(); e++) {
          if (ItemManager.getItem(emi.getItems().get(e)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.0") + " [" + emi.getItems().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      }
      ...
      if (emi.getItemsSpawnLiving() != null && emi.getItemsSpawnLiving().size() > 0) {
        for (int e = 0; e < emi.getItemsSpawnLiving().size(); e++) {
          if (LivingEntityManager.getItem(emi.getItemsSpawnLiving().get(e)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.2")
                    + " ["
                    + emi.getItemsSpawnLiving().get(e)
                    + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
      ...
      if (emi.getSiegeLivings() != null && emi.getSiegeLivings().size() > 0) {
        for (int e = 0; e < emi.getSiegeLivings().size(); e++) {
          if (LivingEntityManager.getItem(emi.getSiegeLivings().get(e)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.3") + " [" + emi.getSiegeLivings().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (emi.getAfterEvents() != null && emi.getAfterEvents().size() > 0) {
        for (int e = 0; e < emi.getAfterEvents().size(); e++) {
          if (!eventsList.containsKey(emi.getAfterEvents().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.4") + " [" + emi.getAfterEvents().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (emi.getPrerequisites() != null && emi.getPrerequisites().size() > 0) {
        for (int e = 0; e < emi.getPrerequisites().size(); e++) {
          if (!eventsList.containsKey(emi.getPrerequisites().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.8") + " [" + emi.getPrerequisites().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (emi.getEventsImmune() != null && emi.getEventsImmune().size() > 0) {
        for (int e = 0; e < emi.getEventsImmune().size(); e++) {
          if (!eventsList.containsKey(emi.getEventsImmune().get(e))) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.12") + " [" + emi.getEventsImmune().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (emi.getEffects() != null && emi.getEffects().size() > 0) {
        for (int e = 0; e < emi.getEffects().size(); e++) {
          if (EffectManager.getItem(emi.getEffects().get(e)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.16") + " [" + emi.getEffects().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (emi.getEffectsAfterEat() != null && emi.getEffectsAfterEat().size() > 0) {
        for (int e = 0; e < emi.getEffectsAfterEat().size(); e++) {
          if (EffectManager.getItem(emi.getEffectsAfterEat().get(e)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.1") + " [" + emi.getEffectsAfterEat().get(e) + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
        }
      ...
      if (emi.getEffectsAfterSleep() != null && emi.getEffectsAfterSleep().size() > 0) {
        for (int e = 0; e < emi.getEffectsAfterSleep().size(); e++) {
          if (EffectManager.getItem(emi.getEffectsAfterSleep().get(e)) == null) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("EventManager.24")
                    + " ["
                    + emi.getEffectsAfterSleep().get(e)
                    + "]",
                "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
          }
  ...
  private static void loadXMLEvents(String sXMLName, boolean bLoadingMain) {
    ...
    try {
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            emi.setMinPopulation(
                UtilsXML.getChildValue(node.getChildNodes(), "minPopulation")); // $NON-NLS-1$
            emi.setSpawnAtRandom(
                UtilsXML.getChildValue(node.getChildNodes(), "spawnAtRandom")); // $NON-NLS-1$
            emi.setEventCooldown(
                UtilsXML.getChildValue(node.getChildNodes(), "eventCooldown")); // $NON-NLS-1$
            emi.setAfterEvents(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "afterEvents"))); // $NON-NLS-1$
            emi.setShadows(UtilsXML.getChildValue(node.getChildNodes(), "shadows")); // $NON-NLS-1$
            emi.setHalfShadows(
                UtilsXML.getChildValue(node.getChildNodes(), "halfShadows")); // $NON-NLS-1$
            ...
            emi.setPrerequisites(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "prerequisites"))); // $NON-NLS-1$
            emi.setEventsImmune(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "eventsImmune"))); // $NON-NLS-1$
            emi.setWaitPCT(UtilsXML.getChildValue(node.getChildNodes(), "waitPCT")); // $NON-NLS-1$
            emi.setWalkSpeedPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "walkSpeedPCT")); // $NON-NLS-1$
            emi.setHappinessPCT(
                UtilsXML.getChildValue(node.getChildNodes(), "happinessPCT")); // $NON-NLS-1$
            emi.setSiege(UtilsXML.getChildValue(node.getChildNodes(), "siege")); // $NON-NLS-1$
            emi.setSiegeUnderground(
                UtilsXML.getChildValue(node.getChildNodes(), "siegeUnderground")); // $NON-NLS-1$
            emi.setSiegeLivings(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "siegeLivings"))); // $NON-NLS-1$
            emi.setSiegeSize(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "siegeSize"))); // $NON-NLS-1$
            emi.setTargets(UtilsXML.getChildValue(node.getChildNodes(), "targets")); // $NON-NLS-1$
            emi.setTargetsPCT(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "targetsPCT"))); // $NON-NLS-1$
            emi.setTargetsRandomCell(
                UtilsXML.getChildValue(node.getChildNodes(), "targetsRandomCell")); // $NON-NLS-1$
            emi.setTargetsOpenCell(
                UtilsXML.getChildValue(node.getChildNodes(), "targetsOpenCell")); // $NON-NLS-1$
            emi.setEffects(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "effects"))); // $NON-NLS-1$
            emi.setEffectsAfterEat(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "effectsAfterEat"))); // $NON-NLS-1$
            emi.setEffectsAfterSleep(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "effectsAfterSleep"))); // $NON-NLS-1$
            emi.setItems(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "items"))); // $NON-NLS-1$
            emi.setItemsMaxAgePCT(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "itemsMaxAgePCT"))); // $NON-NLS-1$
            emi.setItemsDeletePCT(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "itemsDeletePCT"))); // $NON-NLS-1$
            emi.setItemsSpawnLiving(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "itemsSpawnLiving"))); // $NON-NLS-1$
            emi.setItemsSpawnLivingSize(
                Utils.getArray(
                    UtilsXML.getChildValue(
                        node.getChildNodes(),
                        "itemsSpawnLivingSize"))); // $NON-NLS-1$
            emi.setFxBeforeCooldown(
                UtilsXML.getChildValue(node.getChildNodes(), "fxBeforeCooldown")); // $NON-NLS-1$
            emi.setFxAfterCooldown(
                UtilsXML.getChildValue(node.getChildNodes(), "fxAfterCooldown")); // $NON-NLS-1$
            emi.setFxRunning(
                UtilsXML.getChildValue(node.getChildNodes(), "fxRunning")); // $NON-NLS-1$
            emi.setFxRunningTurns(
                UtilsXML.getChildValue(node.getChildNodes(), "fxRunningTurns")); // $NON-NLS-1$
            emi.setFxFinish(
                UtilsXML.getChildValue(node.getChildNodes(), "fxFinish")); // $NON-NLS-1$
            emi.setInjectActions(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "injectActions"))); // $NON-NLS-1$
            emi.setUseFile(UtilsXML.getChildValue(node.getChildNodes(), "useFile")); // $NON-NLS-1$
            emi.setUseFileIDs(
                Utils.getArray(
                    UtilsXML.getChildValue(node.getChildNodes(), "useFileIDs"))); // $NON-NLS-1$
            emi.loadUseFile(sXMLName);
          }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("EventManager.32") + " [" + e.getMessage() + "]",
          "EventManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
