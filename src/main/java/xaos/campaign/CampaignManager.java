...
public class CampaignManager {
  ...
  // if (node.getNodeType () == Node.ELEMENT_NODE && node.getNodeName ().equals ("objective")) {
  // //$NON-NLS-1$
  // ObjectiveData objectiveData = new ObjectiveData ();
  // objectiveData.setType (UtilsXML.getChildValue (node.getChildNodes (), "type"));
  // //$NON-NLS-1$
  // objectiveData.setParam1 (UtilsXML.getChildValue (node.getChildNodes (), "param1"));
  // //$NON-NLS-1$
  // objectiveData.setParam2 (UtilsXML.getChildValue (node.getChildNodes (), "param2"));
  // //$NON-NLS-1$
  ...
  private static void loadCampaignXML(String sXMLPath, boolean bLoadingMain) {
    try {
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (iIndex != -1 && !bLoadingMain) {
            ...
            campaignData.setName(
                UtilsXML.getChildValue(node.getChildNodes(), "name")); // $NON-NLS-1$
            campaignData.setTutorial(
                UtilsXML.getChildValue(node.getChildNodes(), "tutorial")); // $NON-NLS-1$
          }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("CampaignManager.0") + " [" + e.toString() + "]",
          "CampaignManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
  ...
  private static void loadCampaigns() {
    ...
    // Cargar de fichero
    loadCampaignXML(
        Towns.getPropertiesString("DATA_FOLDER") + "campaigns.xml",
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "campaigns.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        // //$NON-NLS-5$
  ...
  private static ArrayList<MissionData> loadMissions(
      NodeList nodeList,
      CampaignData campaignData,
      boolean bLoadingMain) throws Exception {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      node = nodeList.item(i);
      if (node.getNodeType() == Node.ELEMENT_NODE
          && node.getNodeName().equals("mission")) { // $NON-NLS-1$
        sID = UtilsXML.getChildValue(node.getChildNodes(), "id"); // $NON-NLS-1$
        ...
        if (iIndex != -1 && !bLoadingMain) {
          ...
          missionData.setAllowBury(
              UtilsXML.getChildValue(node.getChildNodes(), "allowBury")); // $NON-NLS-1$
  ...
  private static ArrayList<TutorialFlow> loadTutorialFlows(NodeList nodeList) throws Exception {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      node = nodeList.item(i);
      if (node.getNodeType() == Node.ELEMENT_NODE
          && node.getNodeName().equals("tutorialFlow")) { // $NON-NLS-1$
        ...
        if (nodeListFlows != null && nodeListFlows.getLength() > 0) {
          for (int j = 0; j < nodeListFlows.getLength(); j++) {
            ...
            if (nodeFlow.getNodeType() == Node.ELEMENT_NODE
                && nodeFlow.getNodeName().equals("flow")) { // $NON-NLS-1$
              // Tenemos un flow, lo metemos en la lista
              TutorialFlow flow = new TutorialFlow();
              flow.setTitle(
                  UtilsXML.getChildValue(nodeFlow.getChildNodes(), "title")); // $NON-NLS-1$
              flow.setTexts(
                  UtilsXML.getChildValues(nodeFlow.getChildNodes(), "text")); // $NON-NLS-1$
              flow.setImage(
                  UtilsXML.getChildValue(nodeFlow.getChildNodes(), "image")); // $NON-NLS-1$
              flow.setTile(UtilsXML.getChildValue(nodeFlow.getChildNodes(), "tile")); // $NON-NLS-1$
              flow.setStartEvents(
                  Utils.getArray(
                      UtilsXML.getChildValue(
                          nodeFlow.getChildNodes(),
                          "startEvents"))); // $NON-NLS-1$
              flow.setNextMission(
                  UtilsXML.getChildValue(nodeFlow.getChildNodes(), "nextMission")); // $NON-NLS-1$
              flow.setBlinkItems(
                  UtilsXML.getChildValue(nodeFlow.getChildNodes(), "blinkItems")); // $NON-NLS-1$
              flow.setBlinkPiles(
                  Utils.getArray(
                      UtilsXML.getChildValue(
                          nodeFlow.getChildNodes(),
                          "blinkPiles"))); // $NON-NLS-1$
              flow.setBlinkBottom(
                  Utils.getArray(
                      UtilsXML.getChildValue(
                          nodeFlow.getChildNodes(),
                          "blinkBottom"))); // $NON-NLS-1$
              flow.setBlinkRight(
                  Utils.getArray(
                      UtilsXML.getChildValue(
                          nodeFlow.getChildNodes(),
                          "blinkRight"))); // $NON-NLS-1$
              loadBlinkProduction(flow, nodeFlow.getChildNodes());
              flow.setBlinkMinis(
                  Utils.getArray(
                      UtilsXML.getChildValue(
                          nodeFlow.getChildNodes(),
                          "blinkMinis"))); // $NON-NLS-1$

              // Cargamos los triggers
              flow.setOrderedTriggers(
                  UtilsXML.getChildValue(
                      nodeFlow.getChildNodes(),
                      "orderedTriggers")); // $NON-NLS-1$
        ...
        // System.out.println (trigger.getType () + "-> " + trigger.getParam1 () + ", " +
        // trigger.getParam2 ());
        // }
        // }
        // }
  ...
  private static ArrayList<TutorialTrigger> loadTutorialTriggers(NodeList nodeList)
      throws Exception {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      node = nodeList.item(i);
      if (node.getNodeType() == Node.ELEMENT_NODE
          && node.getNodeName().equals("triggers")) { // $NON-NLS-1$
        // Lista de triggers
        ...
        if (nodeListTriggers != null && nodeListTriggers.getLength() > 0) {
          for (int j = 0; j < nodeListTriggers.getLength(); j++) {
            ...
            if (nodeTrigger.getNodeType() == Node.ELEMENT_NODE
                && nodeTrigger.getNodeName().equals("trigger")) { // $NON-NLS-1$
              // Tenemos un trigger, lo cargamos
              TutorialTrigger trigger = new TutorialTrigger();
              trigger.setType(
                  UtilsXML.getChildValue(nodeTrigger.getChildNodes(), "type")); // $NON-NLS-1$
              trigger.setParam1(
                  UtilsXML.getChildValue(nodeTrigger.getChildNodes(), "param1")); // $NON-NLS-1$
              trigger.setParam2(
                  UtilsXML.getChildValue(nodeTrigger.getChildNodes(), "param2")); // $NON-NLS-1$
              trigger.setParamXYZ(
                  UtilsXML.getChildValue(nodeTrigger.getChildNodes(), "paramXYZ")); // $NON-NLS-1$
