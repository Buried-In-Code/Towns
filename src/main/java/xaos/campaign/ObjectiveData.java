...
public class ObjectiveData {
  ...
  public static boolean checkCompleted(ObjectiveData data) {
    ...
    if (data.getType().equals(TYPE_COLLECT)) {
      ...
      // Crear zonas
      return Game.getWorld().getZones() != null
          && Game.getWorld().getZones().size() >= data.getParam2();
      ...
      // Crear pilas
      return Game.getWorld().getStockpiles() != null
          && Game.getWorld().getStockpiles().size() >= data.getParam2();
  ...
  public void setType(String type) throws Exception {
    if (type == null
        || (!type.equalsIgnoreCase(TYPE_COLLECT)
            && !type.equalsIgnoreCase(TYPE_BUILD)
            && !type.equalsIgnoreCase(TYPE_ZONE)
            && !type.equalsIgnoreCase(TYPE_KILL)
            && !type.equalsIgnoreCase(TYPE_PILE) /* && !type.equalsIgnoreCase (TYPE_ROOF) */
            && !type.equalsIgnoreCase(TYPE_SOLDIER))) {
      throw new Exception(
          Messages.getString("ObjectiveData.2")
              + " ["
              + type
  ...
  public String toString() {
    if (getType().equalsIgnoreCase(TYPE_COLLECT)) {
      ...
      if (number >= getParam2()) {
        return Messages.getString("ObjectiveData.0")
            + getParam2()
            + " "
            + imi.getName()
            + Messages.getString("ObjectiveData.7"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
        return Messages.getString("ObjectiveData.0")
            + getParam2()
            + " "
      ...
      if (number >= getParam2()) {
        return Messages.getString("ObjectiveData.3")
            + getParam2()
            + " "
            + bmi.getName()
            + Messages.getString("ObjectiveData.7"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      if (number >= getParam2()) {
        return Messages.getString("ObjectiveData.1")
            + getParam2()
            + Messages.getString("ObjectiveData.4")
            + Messages.getString("ObjectiveData.7"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
        return Messages.getString("ObjectiveData.1")
            + getParam2()
      ...
      if (killed >= getParam2()) {
        return Messages.getString("ObjectiveData.6")
            + getParam2()
            + " "
            + sName
            + Messages.getString("ObjectiveData.7"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      if (number >= getParam2()) {
        return Messages.getString("ObjectiveData.1")
            + getParam2()
            + Messages.getString("ObjectiveData.8")
            + Messages.getString("ObjectiveData.7"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
        return Messages.getString("ObjectiveData.1")
            + getParam2()
      ...
      // if (number >= getParam2 ()) {
      // return Messages.getString("ObjectiveData.1") + getParam2 () +
      // Messages.getString("ObjectiveData.9") + Messages.getString("ObjectiveData.7");
      // //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      // } else {
      // return Messages.getString("ObjectiveData.1") + getParam2 () +
      // Messages.getString("ObjectiveData.9") + " (" + number + ")"; //$NON-NLS-1$ //$NON-NLS-2$
      // //$NON-NLS-3$ //$NON-NLS-4$
      // }
      ...
      if (World.getNumSoldiers() >= getParam2()) {
        return Messages.getString("ObjectiveData.1")
            + getParam2()
            + Messages.getString("ObjectiveData.10")
            + Messages.getString("ObjectiveData.7"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
        return Messages.getString("ObjectiveData.1")
            + getParam2()
