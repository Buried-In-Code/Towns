...
public class DungeonManager {
    ...
    try {
      ...
      for (int p = 0; p < alPathToFiles.size(); p++) {
        ...
        if (doc != null) {
          ...
          // Lo recorremos entero y vamos ańadiendo los datos al array
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
              ...
              if (iIndexExists != -1) {
                if (i == 0) {
                  Log.log(
                      Log.LEVEL_DEBUG,
                      Messages.getString("Generator.1") + " [" + sID + "]",
                      "DungeonManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                }
              ...
              if (sNodename != null && sNodename.equalsIgnoreCase("DELETE")) { // $NON-NLS-1$
                ...
                dungeonData.setLevel(
                    UtilsXML.getChildValue(node.getChildNodes(), "level")); // $NON-NLS-1$
                // Le sumamos los niveles outside al level
                dungeonData.setLevel(
                    (short) (dungeonData.getLevel() + World.MAP_NUM_LEVELS_OUTSIDE - 1));
                dungeonData.setType(
                    UtilsXML.getChildValue(node.getChildNodes(), "type")); // $NON-NLS-1$

                // Monsters
                dungeonData.setMonsters(loadMonsters(node.getChildNodes()));

                // Lo ańadimos al array
                if (iIndexExists != -1) {
          ...
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("DungeonManager.0"),
              "DungeonManager"); //$NON-NLS-1$ //$NON-NLS-2$
          Game.exit();
        }
      }
    } catch (Exception e) {
  ...
  private static ArrayList<MonsterData> loadMonsters(NodeList nodeList) throws Exception {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      node = nodeList.item(i);
      if (node.getNodeType() == Node.ELEMENT_NODE
          && node.getNodeName().equals("monster")) { // $NON-NLS-1$
        MonsterData monsterData = new MonsterData();

        monsterData.setId(UtilsXML.getChildValue(node.getChildNodes(), "id")); // $NON-NLS-1$
        monsterData.setNumber(
            UtilsXML.getChildValue(node.getChildNodes(), "number")); // $NON-NLS-1$
        monsterData.setLevelMin(
            UtilsXML.getChildValue(node.getChildNodes(), "levelMin")); // $NON-NLS-1$
        monsterData.setLevelMax(
