...
public class DungeonGenerator {
  ...
  public static void generateDungeons(String sCampaignID, String sMissionID) {
    ...
    for (int i = 0; i < alDungeons.size(); i++) {
      ...
      if (dungeonData.getType().equalsIgnoreCase(DungeonData.TYPE_CAVE)) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("DungeonGenerator.1") + dungeonData.getType() + "]",
            "DungeonGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      }
    }
  }
   ...
   * @param iLevel
   */
  private static void generateDungeonRooms(
      Cell[][][] cells,
      boolean[][] cellsCopia,
      int roomX,
      int roomY,
      ...
      if (orden[o] == 1) {
        ...
        if (bCabe) {
          ...
          if (!placeRoom(
              cells,
              cellsCopia,
              xNorth - newRoomWidth / 2,
              roomY - xNorthLength - newRoomHeigth,
              newRoomWidth,
              newRoomHeigth,
              iLevel)) {
            for (int i = 0; i < xNorthLength; i++) {
        ...
        if (bCabe) {
          ...
          if (!placeRoom(
              cells,
              cellsCopia,
              xSouth - newRoomWidth / 2,
              roomY + roomHeight + xSouthLength,
              newRoomWidth,
              newRoomHeigth,
              iLevel)) {
            for (int i = 0; i < xSouthLength; i++) {
        ...
        if (bCabe) {
          ...
          if (!placeRoom(
              cells,
              cellsCopia,
              roomX + roomWidth + yEastLength,
              yEast - newRoomHeigth / 2,
              newRoomWidth,
              newRoomHeigth,
              iLevel)) {
            for (int i = 0; i < yEastLength; i++) {
        ...
        if (bCabe) {
          ...
          if (!placeRoom(
              cells,
              cellsCopia,
              roomX - yWestLength - newRoomWidth,
              yWest - newRoomHeigth / 2,
              newRoomWidth,
              newRoomHeigth,
              iLevel)) {
            for (int i = 0; i < yWestLength; i++) {
        ...
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("DungeonGenerator.0") + orden[o] + "]",
            "DungeonGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      }
    }
  }
  ...
  private static void generateDungeonRooms(Cell[][][] cells, int iLevel) {
    ...
    for (;;) {
      ...
      liquidos : for (int x = xRoom; x < (xRoom + ROOM_WIDTH); x++) {
  ...
  /**
   * Devuelve el número de vecinos discovereds Se usa en la generación de dungeons
   *
   * @param cellsCopia
   *            Array de booleans indicando los discovered
   * @param x
   * @param y
   * @return
   */
  ...
  /**
   * Intenta meter una room en el sitio pasado, si lo consigue llama al generateDungeonRooms(....) para que meta
   * pasillos, más rooms, ....
   *
   * @param cells
   * @param cellsCopia
   ...
   * @param iLevel
   */
  private static boolean placeRoom(
      Cell[][][] cells,
      boolean[][] cellsCopia,
      int roomX,
      int roomY,
    ...
    if (roomX < 0
        || roomY < 0
        || (roomX + roomWidth - 1) >= World.MAP_WIDTH
        || (roomY + roomHeight - 1) >= World.MAP_HEIGHT) {
      return false;
    }

    // Habitación dentro del mapa, comprobamos que ninguna casilla ya está ocupada o tenga fluidos
    // alrededor
    boolean bOK = true;
    room1 : for (int x = roomX; x < (roomX + roomWidth); x++) {
