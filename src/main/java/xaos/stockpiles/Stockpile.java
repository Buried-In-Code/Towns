...
public class Stockpile implements Externalizable {
  ...
  public static SmartMenu createPilePanelMenu(int iStockpileID) {
    ...
    SmartMenu smReturn = new SmartMenu(
        SmartMenu.TYPE_MENU,
        Messages.getString("Stockpile.6"),
        null,
        null,
        sPileID); // $NON-NLS-1$
    ...
    SmartMenu smAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("Stockpile.4"),
        null,
        CommandPanel.COMMAND_STOCKPILE_ENABLE_ALL,
        Integer.toString(iStockpileID),
        null,
        null,
    ...
    smAux = new SmartMenu(
        SmartMenu.TYPE_ITEM,
        Messages.getString("Stockpile.5"),
        null,
      ...
      if (iIndex != -1) {
        ...
        if (iIndexSubtype != -1) {
          ...
          alElementsWithSubtype
              .get(iIndexSubtype)
              .addElement(
                  type.getElements().get(i),
                  stockpileType.contains(type.getElements().get(i)));
        } else {
      ...
      for (int i = 0; i < stockpiles.size(); i++) {
        ...
        if (stockpile.getID() == iID) {
          ...
          for (int t = 0; t < type.getElements().size(); t++) {
            imi = ItemManager.getItem(type.getElements().get(t));
            if (imi.getType() != null && imi.getType().equals(sSubType)) {
              // Item a habilitar si no lo estß ya
              if (!stockpile.getType().contains(imi.getIniHeader())) {
                stockpile
                    .getType()
                    .addElement(type.getElements().get(t), type.getElementNames().get(t));
              }
            }
          }
          break;
      ...
      if (stockpile != null) {
        ...
        sm.addItem(
            new SmartMenu(
                ...
                Integer.toString(stockpile.getID()),
                null,
                null,
                Color.GREEN)); // $NON-NLS-1$
        ...
        sm.addItem(
            new SmartMenu(
                SmartMenu.TYPE_ITEM,
                Messages.getString("Stockpile.3"),
                null,
  ...
  /**
   * Devuelve el objeto Stockpile de una coordenada, lo saca de la lista de stockpiles Devuelve nulo si no la
   * encuentra
   *
   * @param p3d
   * @return
  ...
  public static boolean isCellAvailableForStockpile(int x, int y, int z) {
    ...
    boolean checkBasics = cell.isDiscovered()
        && cell.isMined()
        && !cell.hasZone()
  ...
  public static void updateIndexID() {
    /** Obtiene el ID mßs alto de Stockpile */
    ArrayList<Stockpile> stockpiles = Game.getWorld().getStockpiles();
    Stockpile pile;
    int iMaxID = -1;
    for (int i = 0; i < stockpiles.size(); i++) {
  ...
  /**
   * Indica si el item pasado (iniheader) estß permitido por la stockpiles
   *
   * @param sItemIniHeader
   * @return true si el item pasado (iniheader) estß permitido por la stockpiles
   */
