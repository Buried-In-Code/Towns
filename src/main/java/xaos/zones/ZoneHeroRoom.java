...
public class ZoneHeroRoom extends Zone implements Externalizable {
  ...
  /**
   * Asigna una zona personal libre a un héroe. Si el héroe YA tiene ID de zona NO hace nada y devolvemos false.
   *
   * @param hero
  ...
  /**
   * Asigna una zona personal CONCRETA y libre a un héroe Si el héroe YA tiene ID de zona NO hace nada y devolvemos
   * false.
   *
   * @param hero
   * @return true si ha podido
    ...
    for (int j = 0; j < Game.getWorld().getZones().size(); j++) {
      ...
      if (zmi.getType() == ZoneManagerItem.TYPE_HERO_ROOM) {
        if ((iZoneID == -1 || zone.getID() == iZoneID)
            && ((ZoneHeroRoom) zone).getOwnerID() == -1) {
          // La tenemos
  ...
  /**
   * Libera una personal hero zone, se usa si el héroe muere o se pira, por ejemplo.
   *
   * @param hero
   */
  public static void unAssignZone(Hero hero) {
    ...
    if (iID != 0) {
      ...
      for (int i = 0; i < Game.getWorld().getZones().size(); i++) {
        ...
        if (zone.getID() == iID
            && ZoneManager.getItem(zone.getIniHeader()).getType() == ZoneManagerItem.TYPE_HERO_ROOM) {
