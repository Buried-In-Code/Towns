...
public class ZonePersonal extends Zone implements Externalizable {
  ...
  /**
   * Asigna una zona personal libre a un aldeano Si el aldeano YA tiene ID de zona NO hace nada y devolvemos false.
   *
   * @param citizen
  ...
  /**
   * Asigna una zona personal CONCRETA y libre a un aldeano Si el aldeano YA tiene ID de zona NO hace nada y
   * devolvemos false.
   *
   * @param citizen
   * @return true si ha podido
    ...
    for (int j = 0; j < Game.getWorld().getZones().size(); j++) {
      ...
      if (zmi.getType() == ZoneManagerItem.TYPE_PERSONAL) {
        if ((iZoneID == -1 || zone.getID() == iZoneID)
            && ((ZonePersonal) zone).getOwnerID() == -1) {
          // La tenemos
  ...
  public static void unAssignZone(Citizen citizen) {
    ...
    if (iID != 0) {
      ...
      for (int i = 0; i < Game.getWorld().getZones().size(); i++) {
        ...
        if (zone.getID() == iID
            && ZoneManager.getItem(zone.getIniHeader()).getType() == ZoneManagerItem.TYPE_PERSONAL) {
