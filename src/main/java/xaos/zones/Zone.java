...
public class Zone implements Externalizable {
  ...
  /**
   * Indica si se puede crear una Zone. Se le pasan los 2 puntos de las esquinas
   *
   * @param p3dIni
   * @param p3dEnd
   * @return
   */
  public static boolean areCellsAvailableForZone(
      ZoneManagerItem zmi,
      int x0,
      int y0,
      int x1,
    ...
    if (zoneExpand == null) {
      ...
      breakexpand : for (int x = xIni; x <= xEnd; x++) {
        for (int y = yIni; y <= yEnd; y++) {
      ...
      if (!bExpandOK) {
        ...
        // Si llega aquí es que una de las casillas toca la zona anterior, ahora tenemos que mirar
        // que no toque otras zonas
  ...
  /**
   * Elimina un punto de zone, comprueba que la zone no se haya quedado sin puntos
   *
   * @param p3d
   * @return true en caso de haber borrado o haber hecho inoperativo un item merger/splitter
   */
    ...
    if (cell.hasZone()) {
      ...
      if (zone != null) {
        ...
        if (zmi.getType() == ZoneManagerItem.TYPE_PERSONAL) {
          ...
          Citizen citizen;
          // Ańadimos la opción de cambiar/ańadir propietario
          SmartMenu smOwner = new SmartMenu(
              SmartMenu.TYPE_MENU, Messages.getString("Zone.1"), sm, null, null); // $NON-NLS-1$
              ...
              if (citizen.getCitizenData().getZoneID() != zone.getID()) {
                smOwner.addItem(
                    new SmartMenu(
                        ...
                        Integer.toString(zone.getID()),
                        Integer.toString(citizen.getID()),
                        null,
                        Color.ORANGE));
              } else {
                // Propietario, ańadimos linea de texto
                sm.addItem(
                    new SmartMenu(
                        SmartMenu.TYPE_TEXT,
                        Messages.getString("Zone.6")
                            + citizen.getCitizenData().getFullName()
                            + Messages.getString("Zone.7"),
                        null,
                        null,
                        null,
                        null,
          ...
          for (int i = 0; i < SoldierGroups.MAX_GROUPS; i++) {
            ...
            if (sgd.getZoneID() != zone.getID()) {
              ...
              // Éste es el grupo propietario, ańadimos una linea de texto
              sm.addItem(
                  new SmartMenu(
                      SmartMenu.TYPE_TEXT,
                      Messages.getString("Zone.8") + sgd.getName() + Messages.getString("Zone.9"),
                      null,
                      null,
                      null,
                      null,
      ...
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              Messages.getString("Zone.0") + zone,
              null,
              CommandPanel.COMMAND_EXPAND_ZONE,
              Integer.toString(cell.getZoneID()),
              null,
              null)); // $NON-NLS-1$
      ...
      sm.addItem(
          new SmartMenu(
              SmartMenu.TYPE_ITEM,
              Messages.getString("Zone.4") + zone,
              null,
  ...
  /**
   * Retorna una casilla libre de la zone o null si no la encuentra. Busca 25 veces a random, si así no encuentra una
   * casilla libre se recorre todos los points 1 por 1
   *
   * @param zone
   * @param iASZI
   * @return
  ...
  public static Point3DShort getFreeCellAtRandom(Zone zone, int iASZI) {
    ...
    while (iTrys < 25
        && (iASZI != World.getCell(zone.getPoints().get(iIndexPoint)).getAstarZoneID()
            || World.getCell(p3d).containsSpecificLiving(LivingEntity.TYPE_CITIZEN) != null
            || World.getCell(p3d).containsSpecificLiving(LivingEntity.TYPE_HERO) != null)) {
    ...
    if (iTrys == 25) {
      ...
      for (int i = 0; i < zone.getPoints().size(); i++) {
        ...
        if (iASZI == World.getCell(p3d).getAstarZoneID()
            && World.getCell(p3d).containsSpecificLiving(LivingEntity.TYPE_CITIZEN) == null
            && World.getCell(p3d).containsSpecificLiving(LivingEntity.TYPE_HERO) == null) {
          return p3d;
        }
  ...
  /**
   * Devuelve una casilla libre de la zone con una silla (o algo para sentarse), si no la encuentra devuelve null
   *
   * @param zone
   * @param iASZI
   * @return
    ...
    for (int i = 0; i < zone.getPoints().size(); i++) {
      ...
      if (iASZI == cell.getAstarZoneID()) {
        ...
        if (entity != null
            ...
            && ((Item) entity).isOperative()
            && ((Item) entity).isLocked()) {
          imi = ItemManager.getItem(entity.getIniHeader());
          if (imi.canBeUsedToSit()) {
            if (cell.containsSpecificLiving(LivingEntity.TYPE_CITIZEN) == null
                && cell.containsSpecificLiving(LivingEntity.TYPE_HERO) == null) {
              alSitItems.add(p3d);
            }
          }
        }
  ...
  /**
   * Devuelve una casilla libre de la zone con una cama, si no la encuentra devuelve null
   *
   * @param zone
   * @param iASZI
   * @return
                ...
                if (iIndexZ < 0) {
                  ...
                  if (cell.getAstarZoneID() == iASZI) {
                    ...
                    if (entity != null
                      ...
                      if (imi.canBeUsedToSleep()) {
                        if (cell.containsSpecificLiving(LivingEntity.TYPE_CITIZEN) == null
                            && cell.containsSpecificLiving(
                                LivingEntity.TYPE_HERO) == null) {
                          // Encontrada!
  ...
  public static boolean isCellAvailableForZone(int x, int y, int z, Zone expandZone) {
    ...
    // No miramos el digada ya que ya se mirará en el living-iscellAllowed, de paso así mirará si
    // hay base=true debajo
  ...
  private static boolean checkZoneNeighbors(
      ZoneManagerItem zmi,
      int xIni,
      int yIni,
      int xEnd,
  ...
  /**
   * Indica si la casilla pasada tiene una zona contenida en la lista de vecinos
   *
   * @param x
   * @param y
   * @param z
