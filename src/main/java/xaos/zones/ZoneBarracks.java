...
public class ZoneBarracks extends Zone implements Externalizable {
  ...
  /**
   * Asigna una zona cuartel libre a un grupo. Si el grupo YA tiene ID de zona NO hace nada y devolvemos false.
   *
   * @param sgd
  ...
  /**
   * Asigna una zona cuartel CONCRETA y libre a un grupo. Si el grupo YA tiene ID de zona NO hace nada y devolvemos
   * false.
   *
   * @param sgd
   * @param iZoneID
  ...
  public static boolean assignZone(SoldierGroupData sgd, int iZoneID) {
    ...
    for (int j = 0; j < Game.getWorld().getZones().size(); j++) {
      ...
      if (zmi.getType() == ZoneManagerItem.TYPE_BARRACKS) {
        if ((iZoneID == -1 || zone.getID() == iZoneID)
            && ((ZoneBarracks) zone).getGroupID() == -1) {
          // La tenemos
  ...
  /**
   * Libera una cuartel zone, se usa si se han petado todas las celdas de la zona, por ejemplo.
   *
   * @param sgd
   */
  public static void unAssignZone(SoldierGroupData sgd) {
    ...
    if (iID != 0) {
      ...
      for (int i = 0; i < Game.getWorld().getZones().size(); i++) {
        ...
        if (zone.getID() == iID
            && ZoneManager.getItem(zone.getIniHeader()).getType() == ZoneManagerItem.TYPE_BARRACKS) {
