...
public class ZoneManager {
  ...
  private static void loadItems() {
    ...
    // Cargar de fichero
    loadXMLZones(
        Towns.getPropertiesString("DATA_FOLDER") + "zones.xml", // $NON-NLS-2$
        true); // $NON-NLS-1$
    ...
    if (alMods != null && alMods.size() > 0) {
      for (int i = 0; i < alMods.size(); i++) {
        String sModActionsPath = fUserFolder.getAbsolutePath()
            ...
            + "zones.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
  ...
  private static void loadXMLZones(String sXMLName, boolean bLoadingMain) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo las zonas (ZoneManagerItem) a la hash
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bModChangingValues) {
            ...
            item.setMinWidth(
                UtilsXML.getChildValue(node.getChildNodes(), "minWidth")); // $NON-NLS-1$
            item.setMinHeight(
                UtilsXML.getChildValue(node.getChildNodes(), "minHeight")); // $NON-NLS-1$

            item.setNeighbors(
                UtilsXML.getChildValues(node.getChildNodes(), "neighbor")); // $NON-NLS-1$
          }
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("ZoneManager.0") + " [" + e.toString() + "]",
          "ZoneManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
}
