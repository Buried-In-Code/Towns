  ...
  public static void loadItems() {
    ...
    while (iterator.hasNext()) {
      ...
      if (priorityID == null) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ActionManagerItem.4") + sAux + "]",
            "ActionManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        Game.exit();
      }
      ...
      if (priorityID != null) {
        if (ActionPriorityManager.getItem(priorityID) == null) {
          Log.log(
              Log.LEVEL_ERROR,
              Messages.getString("ActionManagerItem.2") + sAux + "] [" + priorityID + "]",
              "ActionManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          Game.exit();
        }
      }
    }
  ...
  private static void loadXMLActions(String sPath, boolean bLoadingMain) {
    ...
    try {
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (bLoadingMain && bExists) {
            Log.log(
                Log.LEVEL_ERROR,
                Messages.getString("ActionManager.1") + sID + "]",
                "ActionManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setPriorityID(
                UtilsXML.getChildValue(node.getChildNodes(), "priorityID")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setKillsSource(
                UtilsXML.getChildValue(node.getChildNodes(), "killsSource")); // $NON-NLS-1$
          }
          ...
          if (bModChangingValues) {
            ...
            item.setInverted(
                UtilsXML.getChildValue(node.getChildNodes(), "inverted")); // $NON-NLS-1$
          }
      ...
      if (sID != null) {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ActionManager.0")
                + sPath
                + "] ["
                + sID
                + "]["
                + e.getMessage()
                + "]",
            "ActionManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
      } else {
        Log.log(
            Log.LEVEL_ERROR,
            Messages.getString("ActionManager.0") + sPath + "] [" + e.getMessage() + "]",
            "ActionManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      }
      Game.exit();
    }
  ...
  private static ArrayList<QueueItem> readQueue(NodeList list) throws Exception {
    ...
    try {
      for (int i = 0; i < list.getLength(); i++) {
        node = list.item(i);
        if (node.getNodeType() == Node.ELEMENT_NODE
            && node.getNodeName().equals("queue")
            && node.getChildNodes().item(0) != null) { // $NON-NLS-1$
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
              ...
              if (node.getAttributes() != null) {
                if (node.getAttributes().getNamedItem("useSource") != null) {
                  if (node.getAttributes().getNamedItem("useSource").getNodeValue() != null
                      && node.getAttributes()
                          .getNamedItem("useSource")
                          .getNodeValue()
      ...
      throw new Exception(
          Messages.getString("ActionManager.2")
              + e.getMessage()
              + "]"); //$NON-NLS-1$ //$NON-NLS-2$
    }
