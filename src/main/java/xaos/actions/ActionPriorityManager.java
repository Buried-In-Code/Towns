...
public class ActionPriorityManager {
  ...
  private static HashMap<String, Integer> prioritiesValues; // Prioridad numérica de cada "item" (se corresponde con

  // la posición en la
  // lista, se usa para performance)
  ...
  public static SmartMenu createJobGroupPanelMenu(int iGroupID) {
    ...
    // Ańadimos 1 linea por cada priority (para poner/quitar)
  ...
  public static void load(ObjectInputStream ois) throws Exception {
    ...
    if (itemList != null
        && prioritiesList != null
        && prioritiesValues != null
        && prioritiesList.size() == prioritiesValues.size()) {
    ...
    for (int i = 0; i < menuProfessions.getItems().size(); i++) {
      if (menuProfessions.getItems().get(i).getCommand() != null) {
        if (menuProfessions
            ...
            .get(i)
            .getCommand()
            .equals(CommandPanel.COMMAND_JOB_GROUP_ENABLE_ITEM)) {
          if (!cgd.containsDeniedJob(
              UtilsIniHeaders.getIntIniHeader(menuProfessions.getItems().get(i).getParameter2()))) {
            menuProfessions
                .getItems()
                .get(i)
                .setCommand(CommandPanel.COMMAND_JOB_GROUP_DISABLE_ITEM);
          }
        } else if (menuProfessions
            .getItems()
            .get(i)
            .getCommand()
            .equals(CommandPanel.COMMAND_JOB_GROUP_DISABLE_ITEM)) {
          if (cgd.containsDeniedJob(
              UtilsIniHeaders.getIntIniHeader(menuProfessions.getItems().get(i).getParameter2()))) {
            menuProfessions
                .getItems()
                .get(i)
                .setCommand(CommandPanel.COMMAND_JOB_GROUP_ENABLE_ITEM);
          }
        }
      }
    }
    ...
    for (int i = 0; i < menuProfessions.getItems().size(); i++) {
      if (menuProfessions.getItems().get(i).getCommand() != null) {
        if (menuProfessions
            ...
            .get(i)
            .getCommand()
            .equals(CommandPanel.COMMAND_PROFESSIONS_ENABLE_ITEM)) {
          if (!cit.getCitizenData()
              .containsDeniedJob(menuProfessions.getItems().get(i).getParameter2())) {
            menuProfessions
                .getItems()
                .get(i)
                .setCommand(CommandPanel.COMMAND_PROFESSIONS_DISABLE_ITEM);
          }
        } else if (menuProfessions
            .getItems()
            .get(i)
            .getCommand()
            .equals(CommandPanel.COMMAND_PROFESSIONS_DISABLE_ITEM)) {
          if (cit.getCitizenData()
              .containsDeniedJob(menuProfessions.getItems().get(i).getParameter2())) {
            menuProfessions
                .getItems()
                .get(i)
                .setCommand(CommandPanel.COMMAND_PROFESSIONS_ENABLE_ITEM);
          }
        }
      }
    }
  ...
  private static void loadXMLPriorityActions(String sXMLName) {
    try {
      ...
      // Lo recorremos entero y vamos ańadiendo a la hash
      ...
      Log.log(
          Log.LEVEL_ERROR,
          Messages.getString("ActionManager.0") + sXMLName + "] [" + e.toString() + "]",
          "ActionPriorityManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
      Game.exit();
    }
  }
}
